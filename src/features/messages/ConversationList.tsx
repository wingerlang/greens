...
export function ConversationList() {
    ...
    const filteredConversations = conversations.filter(c => {
        // Search
        // (Simple search on title for now)
        ...
        if (success) {
            if (targetId) {
                ...
                setActiveConversationId(targetId);
                 ...
                 // Logic for toggling showHidden?
                 // Actually, maybe we require password to TOGGLE showHidden to true?
                 // Current logic: simple toggle. But requirement said "lösenordsskydda dem".
                 // Let's assume locking individual chats is the main "protection".
                 // But hiding usually implies privacy.
                 // Let's protect the "Show Hidden" toggle too if currently false?
                 // For now, let's keep it simple: Show Hidden is a toggle. Accessing the specific hidden chat might be locked if it also has isLocked.
                 // But wait, if I hide it, I probably don't want it seen without password.
                 // So "Show Hidden" should probably prompt password if transitioning from false -> true.
            }
            setModalOpen(false);
            setTargetId(null);
        } else {
    ...
    const toggleShowHidden = () => {
        if (!showHidden) {
            ...
            setModalOpen(true);
            // We need to know we are unlocking "showHidden"
            // Let's add a mode to the modal logic or just infer.
            // If targetId is null, it means we are unlocking Global Hidden Mode.
        } else {
            setShowHidden(false);
        }
    };
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-0.5">
                                            <span className="font-semibold text-sm text-slate-900 dark:text-white truncate pr-2 flex items-center gap-1">
                                                {title}
