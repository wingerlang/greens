...
function getMaxLiftData(sessions: StrengthWorkout[], matchers: string[]): { weight: number; workoutId?: string } {
    let max = 0;
...
import { calculateCooperScore, calculateStrengthScore, calculateWeightliftingScore, calculateHyroxScore, calculateSinclair, calculateCyclingScore } from './beastCalculators.ts';
...
export function calculateBeastStats(
    ...
    return {
        ...
        totalScore: Math.round((cooperScore + strengthScore + wlScore + hyroxScore + cyclingScore) / 5)
    };
}
