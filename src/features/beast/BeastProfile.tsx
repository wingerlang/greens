      ...
      <div className="relative overflow-hidden rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl p-8 md:p-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-16">
          ...
          <div className="relative group">
            ...
            {/* Glow effect */}
            <div className="absolute inset-0 rounded-full bg-indigo-500/20 blur-xl -z-10 group-hover:bg-indigo-500/30 transition-all duration-700">
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <div className="space-y-2 mt-4">
              ...
              <StatRow
                label="Wilks"
                value={stats.strength.wilks.toFixed(1)}
                unit="pts"
              />
              ...
              <StatRow
                ...
                subValue={`${
                  (stats.strength.squat / user.weight).toFixed(2)
                }xBW`}
                onClick={() =>
                  handleNavigate("workout", stats.strength.squatWorkoutId)}
              />
              <StatRow
                label="Bench"
                value={`${stats.strength.bench}kg`}
                subValue={`${
                  (stats.strength.bench / user.weight).toFixed(2)
                }xBW`}
                onClick={() =>
                  handleNavigate("workout", stats.strength.benchWorkoutId)}
              />
              <StatRow
                label="Deadlift"
                value={`${stats.strength.deadlift}kg`}
                subValue={`${
                  (stats.strength.deadlift / user.weight).toFixed(2)
                }xBW`}
                onClick={() =>
                  handleNavigate("workout", stats.strength.deadliftWorkoutId)}
              />
            </div>
          </StatCard>
          ...
          >
            <div className="space-y-2 mt-4">
              <StatRow
                label="Total"
                value={`${stats.weightlifting.total}kg`}
                bold
              />
              <StatRow
                label="Sinclair"
                value={stats.weightlifting.sinclair.toFixed(1)}
                unit="pts"
              />
              <div className="h-px bg-slate-800 my-2"></div>
              <StatRow
                ...
                subValue={`${
                  (stats.weightlifting.snatch / user.weight).toFixed(2)
                }xBW`}
                onClick={() => handleNavigate(
                  "workout",
                  stats.weightlifting.snatchWorkoutId,
                )}
              />
              <StatRow
                label="Clean & Jerk"
                ...
                subValue={`${
                  (stats.weightlifting.cleanJerk / user.weight).toFixed(2)
                }xBW`}
                onClick={() =>
                  handleNavigate(
                    "workout",
                    stats.weightlifting.cleanJerkWorkoutId,
                  )}
              />
            </div>
          </StatCard>
          ...
          >
            <div className="space-y-3 mt-4">
              <StatRow
                ...
                bold
                onClick={() =>
                  handleNavigate("activity", stats.cycling.sourceActivityId)}
              />
              <StatRow
                label="Relative"
                value={`${stats.cycling.relativeFtp.toFixed(2)} W/kg`}
                fontMono
              />
              {stats.cycling.date && (
                <div
          ...
          >
            <div className="space-y-3 mt-4">
              <StatRow
                ...
                bold
                onClick={() =>
                  handleNavigate("activity", stats.cooper.sourceActivityId)}
              />
              <StatRow
                label="Avg Pace"
                value={`${
                  (12 / (stats.cooper.distance / 1000)).toFixed(2)
                } min/km`}
                fontMono
              />
              {stats.cooper.date && (
          ...
          >
            <div className="space-y-3 mt-4">
              <StatRow
                ...
                bold
                onClick={() =>
                  handleNavigate("activity", stats.hyrox.sourceActivityId)}
              />
              {stats.hyrox.date && (
                <div
...
function StatCard(
  { title, score, icon, children, borderColor, bgGradient }: any,
) {
  return (
    <div
      className={`relative overflow-hidden rounded-2xl bg-slate-900 border ${borderColor} p-6 flex flex-col h-full group transition-all hover:border-opacity-50 shadow-lg hover:shadow-indigo-500/10`}
    >
      {/* Background Gradient */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${bgGradient} opacity-50 group-hover:opacity-70 transition-opacity`}
      >
      </div>
      ...
      <div className="relative z-10 h-full flex flex-col">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex flex-col items-end">
            <span className="text-3xl font-black text-white tracking-tighter group-hover:scale-105 transition-transform">
              {score}
            </span>
...
function StatRow(
  { label, value, subValue, unit, bold, fontMono, onClick }: any,
) {
  return (
    <div
      className={`flex justify-between items-center text-sm py-1 transition-colors group/row ${
        onClick ? "cursor-pointer hover:bg-white/5 px-1 -mx-1 rounded" : ""
      }`}
      onClick={onClick}
    >
      ...
      <div className="flex items-baseline gap-1">
        <span
          className={`text-slate-200 group-hover/row:text-white transition-colors ${
            bold
              ? "font-bold text-base"
              : fontMono
              ? "font-mono text-xs"
