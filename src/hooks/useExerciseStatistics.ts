...
export function useExerciseStatistics(
  ...
  return useMemo(() => {
    ...
    const allExercises: ExerciseStats[] = Array.from(exerciseMap.values()).map(
      (stats) => ({
        name: stats.name,
        ...
        daysSinceLastTraining: Math.floor(
          (now.getTime() - new Date(stats.lastTrainedDate).getTime()) /
            (1000 * 60 * 60 * 24),
        ),
 ...
 */
export function formatExerciseRecency(
  daysSince: number,
  ...
  if (daysSince === 0) {
    ...
    return {
      text: `${Math.round(daysSince / 7)} vecka sedan`,
