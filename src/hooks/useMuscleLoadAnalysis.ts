...
export const useMuscleLoadAnalysis = (
  muscleId: string | null,
  targetExerciseId: string | null,
  intensityThreshold: number = 0.7,
) => {
  ...
  const stats = useMemo(() => {
    ...
    if (targetExerciseId) {
      ...
      relevantExercises = [
        ...allExercises.filter((e) => e.primaryMuscles.includes(muscleId)).map(
          (e) => ({ def: e, role: "primary" as const }),
        ),
        ...allExercises.filter((e) => e.secondaryMuscles.includes(muscleId))
          .map((e) => ({ def: e, role: "secondary" as const })),
      ];
    }

    // Helper to normalize names for matching
    const normalize = (str: string) =>
      str.toLowerCase().replace(/[^a-zåäö0-9]/g, "");

    // Helper to match a history entry to a relevant exercise with fuzzy matching
    const findMatch = (
      entry: any,
    ): { match?: typeof relevantExercises[0]; matchReason?: string } => {
      ...
      for (const re of relevantExercises) {
        ...
        // 1. Direct ID Match
        if (
          entryId && (exId === entryId || normalizedExId === normalizedEntryId)
        ) {
        ...
        if (normalizedEntryName && normalizedEntryName.length >= 3) {
          ...
          if (
            normalizedExId.includes(normalizedEntryName) ||
            normalizedExNameEn.includes(normalizedEntryName)
          ) {
            return {
              match: re,
              matchReason: `Fuzzy: "${entryName}" ⊂ "${exNameEn}"`,
            };
          }
          // "deadliftbarbell" contains "deadlift"
          if (
            normalizedEntryName.includes(normalizedExId) ||
            normalizedEntryName.includes(normalizedExNameEn)
          ) {
            return {
              match: re,
              matchReason: `FuzzyReverse: "${exNameEn}" ⊂ "${entryName}"`,
            };
          }
        }
        ...
        if (normalizedEntryId && normalizedEntryId.length >= 3) {
          if (
            normalizedExId.includes(normalizedEntryId) ||
            normalizedEntryId.includes(normalizedExId)
          ) {
    ...
    const actual1rmByDate: Record<
      string,
      { weight: number; exerciseName: string }
    > = {};

    // Track max weight per date for bar chart
    const maxWeightByDate: Record<
      string,
      { weight: number; originalWeight: number; exerciseName: string }
    > = {};

    // Track matched exercises for debugging
    const matchedExercisesMap: Record<
      string,
      {
        original: string;
        matchedTo: string;
    ...
    const workoutDetailsByDate: Record<
      string,
      Array<{
        exerciseName: string;
        exerciseId: string;
    ...
    const historySorted = (workoutHistory as any[]).sort((a, b) =>
      new Date(a.date).getTime() - new Date(b.date).getTime()
    );

    // Pre-calculate 1RM history for all relevant exercises to optimize intensity calculation
    const e1rmHistoryAcrossExercises: Record<
      string,
      { date: number; e1rm: number }[]
    > = {};
    historySorted.forEach((workout) => {
      ...
      workout.exercises?.forEach((e: any) => {
        ...
        if (matchResult.match) {
          ...
          if (!e1rmHistoryAcrossExercises[id]) {
            e1rmHistoryAcrossExercises[id] = [];
          }
          e.sets?.forEach((s: any) => {
            if (s.weight && s.reps) {
              const e1rmValue = s.weight * (1 + (s.reps / 30));
              e1rmHistoryAcrossExercises[id].push({
                date: workoutDate,
                e1rm: e1rmValue,
              });
            }
    ...
    historySorted.forEach((workout) => {
      ...
      workout.exercises?.forEach((exerciseEntry: any) => {
        ...
        const originalName = exerciseEntry.exerciseName ||
          exerciseEntry.exerciseId || "unknown";
        ...
        exerciseEntry.sets.forEach((set: WorkoutSet, setIndex: number) => {
          ...
          const setKey = `${
            workoutId || dateStr
          }-${matched.def.id}-${setIndex}-${weight}-${reps}`;
          ...
          const isPrimaryExercise = matched.role === "primary" ||
            matched.role === "target";
          ...
          if (isPrimaryExercise) {
            ...
            if (reps === 1) {
              if (
                !actual1rmByDate[dateStr] ||
                weight > actual1rmByDate[dateStr].weight
              ) {
                actual1rmByDate[dateStr] = {
                  weight,
                  exerciseName: matched.def.name_en,
          ...
          if (
            ...
            weightedMax > maxWeightByDate[dateStr].weight
          ) {
            maxWeightByDate[dateStr] = {
              weight: weightedMax,
          ...
          if (isHard) {
            hardVolumeByDate[dateStr] = (hardVolumeByDate[dateStr] || 0) +
              volume;
            totalHardTonnage += volume;
          } else {
            lightVolumeByDate[dateStr] = (lightVolumeByDate[dateStr] || 0) +
              volume;
          }
    ...
    const sortedDataDates = Array.from(
      new Set([
