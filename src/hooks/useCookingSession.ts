...
export function useCookingSession(): UseCookingSessionReturn {
  ...
  useEffect(() => {
    if (session) {
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ session, steps, ingredients }),
      );
    } else {
  ...
  const scaledIngredients: ScaledIngredient[] = ingredients.map(
    (ing, index) => {
      ...
      const isExcluded = customizations?.excludedIndices.includes(index) ||
        false;
      const customMultiplier = customizations?.multipliers[index] || 1;
      const effectiveMultiplier = isExcluded
        ? 0
        : portionMultiplier * customMultiplier;

      return {
        ...ing,
        scaledAmount: isExcluded
          ? "0"
          : scaleAmount(ing.amount, effectiveMultiplier),
        isExcluded,
        customMultiplier,
      };
    },
