...
export function useTrainingSummary(startDate: string, endDate: string) {
  ...
  const stats = useMemo((): SummaryStats => {
    ...
    filteredActivities.forEach((a: UniversalActivity) => {
      ...
      if (
        workoutType.includes("race") || subType.includes("race") ||
        (a as any).type?.toLowerCase() === "race"
      ) {
        raceCount++;
      }
    ...
    const exerciseStatsMap = new Map<
      string,
      { count: number; sets: number; reps: number }
    >();
    ...
    filteredStrengthSessions.forEach((s) => {
      ...
      s.exercises.forEach((e) => {
        ...
        const stats = exerciseStatsMap.get(e.exerciseName) ||
          { count: 0, sets: 0, reps: 0 };
        stats.count++;
    ...
    const maxScores = [...filteredActivities]
      .map((a: UniversalActivity) => {
        ...
        return {
          activity: a,
          score: calculatePerformanceScore(mappedActivity as any),
        };
      })
