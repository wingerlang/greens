...
import { generateId, getISODate, PlannedActivity } from "../models/types.ts";
...
export function useWeeklyPlanner() {
  ...
  const generateDraft = useCallback(() => {
    ...
    if (remainingSessions > 0) {
      const avgEasyDist = Math.max(
        3,
        Math.round(remainingVol / remainingSessions * 10) / 10,
      );
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    if (activeItem && activeItem.date !== overId) {
      ...
      setDraftActivities((prev) =>
        prev.map((a) => a.id === active.id ? { ...a, date: overId } : a)
      );
    }
  };
