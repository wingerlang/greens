...
import { calculateRecipeEstimate, type SynergyResult } from '../utils/ingredientParser.ts';
...
export function useSmartPlanner(recipes: Recipe[], foodItems: FoodItem[]) {
    ...
    const analyzePlan = useCallback((plan: WeeklyPlan): PlanAnalysis => {
        ...
        WEEKDAYS.forEach(day => {
            ...
            Object.entries(dayMeals).forEach(([_, planned]) => {
                ...
                // Seasonality
                if (recipe.seasons?.includes(currentSeason as any) || estimate.tags.includes('seasonal-house')) {
                    seasonalCount++;
                    currentDayAnalysis.tags.push('seasonal');
                }

                // Budget
                if (recipe.priceCategory === 'budget' || estimate.tags.includes('budget-win')) {
                    budgetCount++;
            ...
            } else if (dayProteinCats.has('legume') && !dayProteinCats.has('grain') && !currentDayAnalysis.isComplete) {
                currentDayAnalysis.missingComplement = 'grain';
                currentDayAnalysis.tips.push('Lägg till ett spannmål (t.ex. ris) för att göra proteinet fullvärdigt! 🌾');
            } else if (dayProteinCats.has('grain') && !dayProteinCats.has('legume') && !currentDayAnalysis.isComplete) {
                currentDayAnalysis.missingComplement = 'legume';
                currentDayAnalysis.tips.push('Lägg till en baljväxt (t.ex. bönor) för att göra proteinet fullvärdigt! 🥜');
            }
        ...
        // Generate tips
        if (proteinPoints < 50) {
            tips.push('Testa att kombinera bönor med råris eller quinoa för fullvärdigt protein. 🥜+🌾');
            criticalIssues.push('Låg proteinkvalitet på flera dagar');
        }
        if (varietyPoints < 60) {
            tips.push('Variera dina proteinkällor mer! Lägg till lite tofu, nötter eller frön i planen.');
        }
        if (seasonalityPoints < 40) {
            tips.push(`Just nu är det ${currentSeason} – använd mer rotfrukter och kål för att äta i säsong! ❄️`);
        }
        if (budgetPoints < 50) {
            tips.push('Tips: Lägg till fler linsgrytor eller soppor för att få ner veckokostnaden. 💰');
        }
     ...
     */
    const getOptimizationSuggestion = useCallback((plan: WeeklyPlan, day?: Weekday, meal?: MealType) => {
        const analysis = analyzePlan(plan);
        const targetDay = day || (Object.keys(analysis.dayAnalysis).find(d => !analysis.dayAnalysis[d as Weekday].isComplete) as Weekday);
