...
import { Recipe, Weekday, MealType, WeeklyPlan, WEEKDAYS } from '../models/types.ts';
...
export function useSmartSuggestions(recipes: Recipe[], weeklyPlans: WeeklyPlan[]) {
    ...
    const isSuitableForMealType = useCallback((recipe: Recipe, mealType: MealType): boolean => {
    ...
    const getSuggestions = useCallback((day: Weekday, mealType: MealType): RecipeSuggestion[] => {
        ...
        recipes.forEach(recipe => {
            ...
            if (day === 'friday') {
                const fridayHistory = history.filter((h: HistoryItem) => h.day === 'friday');
                const recipeFrequency = fridayHistory.filter((h: HistoryItem) => h.recipeId === recipe.id).length;
            ...
            const recipeHistory = history.filter((h: HistoryItem) => h.recipeId === recipe.id);
            const lastEaten = recipeHistory.length > 0
                ? Math.max(...recipeHistory.map((h: HistoryItem) => new Date(h.date).getTime()))
                : 0;
