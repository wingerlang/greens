...
export function useTimer(): UseTimerReturn {
  ...
  // Generate unique ID
  const generateId = () =>
    `timer_${Date.now()}_${Math.random().toString(36).slice(2)}`;

  // Add new timer
  const addTimer = useCallback(
    (label: string, minutes: number, stepIndex: number): string => {
      const id = generateId();
  ...
  const removeTimer = useCallback((id: string) => {
    setTimers((prev: Timer[]) =>
      prev.filter((timer: Timer) => timer.id !== id)
    );
  }, []);

  // Get timer for specific step
  const getTimerForStep = useCallback(
    (stepIndex: number): Timer | undefined => {
