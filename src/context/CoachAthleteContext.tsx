...
export function CoachAthleteProvider(
  { children, userId, userName }: CoachAthleteProviderProps,
) {
  ...
  const inviteAthlete = useCallback(
    async (email: string, name: string): Promise<CoachAthleteRelation> => {
  ...
  const acceptInvitation = useCallback((relationId: string) => {
    setRelations((prev) =>
      prev.map((r) =>
        r.id === relationId
          ? { ...r, status: "active", updatedAt: new Date().toISOString() }
          : r
      )
    );
  }, []);
  ...
  const removeAthlete = useCallback((relationId: string) => {
    setRelations((prev) =>
      prev.map((r) =>
        r.id === relationId
          ? { ...r, status: "removed", updatedAt: new Date().toISOString() }
          : r
      )
    );
  }, []);
  ...
  const getMyAthletes = useCallback(() => {
    return relations.filter((r) =>
      r.coachId === userId && r.status === "active"
    );
  }, [relations, userId]);

  const getMyCoach = useCallback(() => {
    return relations.find((r) =>
      r.athleteId === userId && r.status === "active"
    );
  }, [relations, userId]);
  ...
  const replyToComment = useCallback(
    (parentId: string, content: string): Comment => {
  ...
  const getCommentsFor = useCallback(
    (targetType: Comment["targetType"], targetId: string): Comment[] => {
  ...
  const sharePlan = useCallback(
    (planId: string, athleteIds: string[]): SharedPlan => {
...
export function useCoachAthlete(): CoachAthleteContextType {
  ...
  if (!context) {
    throw new Error(
      "useCoachAthlete must be used within a CoachAthleteProvider",
    );
  }
  return context;
}
