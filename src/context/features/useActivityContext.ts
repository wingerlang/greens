...
export function useActivityContext(
  ...
  const [coachConfig, setCoachConfig] = useState<CoachConfig | undefined>(
    undefined,
  );
  const [plannedActivities, setPlannedActivities] = useState<PlannedActivity[]>(
    [],
  );
  const [universalActivities, setUniversalActivities] = useState<
    UniversalActivity[]
  >([]);
  ...
  const addStrengthSession = useCallback(
    (session: Omit<StrengthWorkout, "id">): StrengthWorkout => {
      ...
      emitFeedEvent(
        ...
        newSession.name || "Styrkepass slutfört",
        {
          ...
          setCount: (newSession.exercises || []).reduce(
        ...
        [
          {
            label: "Volym",
            value: Math.round(totalVol / 1000),
            unit: "t",
            icon: "🏋️",
          },
          {
            label: "Övningar",
            value: (newSession.exercises || []).length,
            icon: "📋",
          },
        ],
        `${(newSession.exercises || []).length} övningar • ${
  ...
  const updateStrengthSession = useCallback(
    (id: string, updates: Partial<StrengthWorkout>): void => {
      setStrengthSessions((prev) => {
        ...
        if (updated) {
          ...
          fetch(`/api/activities/${id}?date=${dateParam}`, {
            ...
            console.error("Failed to persist strength session update:", e)
          );
        }
        return next;
      });
  ...
  const deleteStrengthSession = useCallback((id: string): void => {
    ...
    if (sessionToDelete) {
      storageService.deleteStrengthSession(id).catch((e) =>
        console.error("Failed to delete strength session", e)
      );
    }
  ...
  const addExercise = useCallback(
    (data: Omit<ExerciseEntry, "id" | "createdAt">): ExerciseEntry => {
      const newEntry: ExerciseEntry = {
        ...
        calorieBreakdown: data.calorieBreakdown ||
          (data.caloriesBurned > 0
            ? `Källhänvisning: Manuellt inlägg\nBeräkning: Baserad på angiven intensitet (${data.intensity}) och längd (${data.durationMinutes} min).`
            : undefined),
      };
      setExerciseEntries((prev) => [...prev, newEntry]);

      skipAutoSave.current = true;
      storageService.saveExerciseEntry(newEntry).catch((e) =>
      ...
      const typeLabel = (data.type.charAt(0).toUpperCase() + data.type.slice(1))
        .replace("Strength", "Styrka").replace("Walking", "Promenad").replace(
          "Running",
          "Löpning",
        ).replace("Cycling", "Cykling");
  ...
  const updateExercise = useCallback(
    (id: string, updates: Partial<ExerciseEntry>) => {
      ...
      if (!existing) {
        ...
        if (strSession) {
          updateStrengthSession(id, {
            ...
            excludeFromStats: updates.excludeFromStats !== undefined
              ? updates.excludeFromStats
              : strSession.excludeFromStats,
          });
          return;
        }
        ...
        if (uniActivity) {
              ...
              if (ua.id === id) {
                return {
                  ...
                  plan: {
                    ...
                    activityType: updates.type || ua.plan?.activityType ||
                      ua.performance?.activityType || "other",
                    distanceKm: updates.distance !== undefined
                      ? updates.distance
                      : (ua.plan?.distanceKm || 0),
                  },
                  performance: {
                    ...ua.performance,
                    ...
                    durationMinutes: updates.durationMinutes !== undefined
                      ...
                      : ua.performance?.durationMinutes,
                    ...
                    distanceKm: updates.distance !== undefined
                      ? updates.distance
                      : ua.performance?.distanceKm,
                    excludeFromStats: updates.excludeFromStats !== undefined
                      ? updates.excludeFromStats
                      : ua.performance?.excludeFromStats,
                  },
          ...
          fetch(`/api/activities/${id}?date=${dateParam}`, {
            ...
            console.error("Failed to persist virtual activity update:", e)
          );
          return;
        }
      ...
      if (
        (updates.intensity || updates.durationMinutes ||
          updates.caloriesBurned) && !updates.calorieBreakdown
      ) {
        updated.calorieBreakdown =
            ...
            return {
              ...
              plan: updates.title
                ? {
                  ...ua.plan,
                  title: updates.title,
                  activityType: ua.plan?.activityType ||
                    ua.performance?.activityType || "other",
                  distanceKm: ua.plan?.distanceKm ||
                    ua.performance?.distanceKm || 0,
                }
                : ua.plan,
              performance: {
                ...ua.performance,
      ...
      if (existing.date && updated.date && existing.date !== updated.date) {
        ...
        storageService.deleteExerciseEntry(id, existing.date).catch((e) =>
          console.error("Failed to delete old exercise", e)
        );
        storageService.saveExerciseEntry(updated).catch((e) =>
  ...
  const deleteExercise = useCallback((id: string) => {
    ...
    if (entryToDelete) {
      storageService.deleteExerciseEntry(id, entryToDelete.date).catch((e) =>
    ...
    if (activityToDelete) {
      storageService.deleteUniversalActivity(id, activityToDelete.date).catch(
        (e) => console.error("Failed to delete universal activity", e),
      );
    }
  }, [storageService]);
  ...
  const unifiedActivities = useMemo(() => {
    ...
    const normalizedLocal = exerciseEntries.map((e) => ({
      ...e,
      source: "manual",
    }));
    ...
    rawStrengthEntries.forEach((se) => {
      ...
      if (!seenCombined.has(contentKey)) {
        ...
        console.log(
          `🔍 Combined duplicate strength session detected: ${se.title} (${se.tonnage}kg, ${se.durationMinutes}min)`,
        );
      }
    });
    ...
    const stravaStrengthByDate = new Map<
      string,
      typeof normalizedServer[0][]
    >();
    ...
    strengthEntries.forEach((se) => {
      ...
      if (match) {
        ...
        const universalMatch = universalActivities.find((u) =>
          u.id === match!.id
        );
  ...
  useEffect(() => {
    if (
      !isLoaded || plannedActivities.length === 0 ||
    ...
    const updatedPlanned = plannedActivities.map((planned) => {
      ...
      const candidates = unifiedActivities
        ...
        .map((actual) => {
          const sameDate =
            actual.date.split("T")[0] === planned.date.split("T")[0];
          ...
          const isRunMatch = pType === "RUN" &&
            (aType === "running" || aType === "walking" || aType === "other");
          const isStrengthMatch = pType === "STRENGTH" && aType === "strength";
          const isBikeMatch = pType === "BIKE" && aType === "cycling";
          const isHyroxMatch = pType === "HYROX" &&
            (aType === "running" || aType === "strength" || aType === "other");

          if (
            !isRunMatch && !isStrengthMatch && !isBikeMatch && !isHyroxMatch
          ) {
            return {
              actual,
              score: 0,
              reason: `Inkompatibel typ: ${pType} vs ${aType}`,
            };
          }
          ...
          const plannedDuration = planned.estimatedDistance
            ? planned.estimatedDistance * 6
            : 45;
          ...
          if (planned.startTime && actual.date.includes("T")) {
            ...
            if (actualTime && actualTime.includes(":")) {
              ...
              const plannedMinutes = (plannedHM[0] || 0) * 60 +
                (plannedHM[1] || 0);
              const actualMinutes = (actualHM[0] || 0) * 60 +
                (actualHM[1] || 0);
              ...
              if (timeDiffMinutes <= 240) { // Within 4 hours
                ...
                score += bonus;
                reasons.push(
                  `Tidsnära (+${bonus}p, diff: ${timeDiffMinutes}m)`,
                );
              }
            } else {
      ...
      if (bestMatch && bestMatch.score >= 60) {
        ...
        console.log(
          `[DataContext] Auto-matched: "${planned.title}" -> "${bestMatch.actual.type}" (Score: ${bestMatch.score})`,
        );
        return {
          ...planned,
          status: "COMPLETED" as const,
          completedDate: bestMatch.actual.date,
          actualDistance: bestMatch.actual.distance ||
            planned.estimatedDistance,
        ...
        if (
          planned.reconciliation?.bestCandidateId !== bestMatch.actual.id ||
          planned.reconciliation?.score !== bestMatch.score
        ) {
          hasChanges = true;
          console.log(
            `[DataContext] candidate for "${planned.title}": ${bestMatch.score}% - ${bestMatch.reason}`,
          );
          return {
            ...planned,
            reconciliation: {
    ...
    if (hasChanges) {
      ...
      console.log(
        `[DataContext] Auto-reconciliation found ${newlyChanged.length} changes.`,
      );
      ...
      newlyChanged.forEach((p) => {
        skipAutoSave.current = true;
        storageService.savePlannedActivity(p).catch((e) =>
          console.error("Failed to persist reconciliation update:", e)
        );
      });
    }
  ...
  const addCompetition = useCallback(
    (data: Omit<Competition, "id" | "createdAt">): Competition => {
  ...
  const calculateParticipantPoints = useCallback(
    (compId: string, userId: string, date: string): number => {
  ...
  const addTrainingCycle = useCallback(
    (data: Omit<TrainingCycle, "id">): TrainingCycle => {
  ...
  const updateTrainingCycle = useCallback(
    (id: string, updates: Partial<TrainingCycle>) => {
      setTrainingCycles((prev) =>
        prev.map((c) => c.id === id ? { ...c, ...updates } : c)
      );
    },
    [],
  ...
  const addGoal = useCallback(
    (data: Omit<PerformanceGoal, "id" | "createdAt">): PerformanceGoal => {
      ...
      // Persist Granularly
      skipAutoSave.current = true;
      storageService.saveGoal(newGoal).catch((e) =>
  ...
  const updateGoal = useCallback(
    (id: string, updates: Partial<PerformanceGoal>) => {
      setPerformanceGoals((prev) => {
        const next = prev.map((g) => g.id === id ? { ...g, ...updates } : g);
        const updatedGoal = next.find((g) => g.id === id);
        if (updatedGoal) {
          skipAutoSave.current = true;
          storageService.saveGoal(updatedGoal).catch((e) =>
  ...
  const deleteGoal = useCallback((id: string) => {
    ...
    skipAutoSave.current = true;
    storageService.deleteGoal(id).catch((e) =>
  ...
  const addTrainingPeriod = useCallback(
    (
      data: Omit<TrainingPeriod, "id" | "createdAt" | "updatedAt">,
    ): TrainingPeriod => {
      ...
      skipAutoSave.current = true;
      storageService.savePeriod(newPeriod).catch((e) =>
  ...
  const updateTrainingPeriod = useCallback(
    (id: string, updates: Partial<TrainingPeriod>) => {
      setTrainingPeriods((prev) => {
        ...
        if (updatedPeriod) {
          skipAutoSave.current = true;
          storageService.savePeriod(updatedPeriod).catch((e) =>
  ...
  const deleteTrainingPeriod = useCallback((id: string) => {
    ...
    skipAutoSave.current = true;
    storageService.deletePeriod(id).catch((e) =>
    ...
    setPerformanceGoals((prev) =>
      prev.map((g) => {
        if (g.periodId === id) {
          ...
          storageService.saveGoal(updated).catch((e) =>
            console.error("Failed to unlink goal", e)
          );
          return updated;
        }
        return g;
  ...
  const updateCoachConfig = useCallback((updates: Partial<CoachConfig>) => {
    setCoachConfig((prev) =>
      prev ? { ...prev, ...updates } : updates as CoachConfig
    );
  }, []);

  const generateCoachPlanAction = useCallback(
    (stravaHistory: StravaActivity[], configOverride?: CoachConfig) => {
  ...
  const deletePlannedActivity = useCallback((id: string) => {
    ...
    skipAutoSave.current = true;
    storageService.deletePlannedActivity(id).catch((e) =>
      console.error("Failed to delete planned activity", e)
    );
  }, [skipAutoSave]);

  const updatePlannedActivity = useCallback(
    (id: string, updates: Partial<PlannedActivity>) => {
      setPlannedActivities((prev) => {
        const next = prev.map((a) => a.id === id ? { ...a, ...updates } : a);
        const updated = next.find((a) => a.id === id);
        if (updated) {
          skipAutoSave.current = true;
          storageService.savePlannedActivity(updated).catch((e) =>
            console.error("Failed to update planned activity", e)
          );
        }
        return next;
      });
  ...
  const savePlannedActivities = useCallback(
    (newActivities: PlannedActivity[]) => {
      setPlannedActivities((prev) => {
        const ids = new Set(newActivities.map((a) => a.id));
        const filtered = prev.filter((a) => !ids.has(a.id));

        const next = [...filtered, ...newActivities].sort((a, b) =>
          new Date(a.date).getTime() - new Date(b.date).getTime()
        );

        // Sync to API
        skipAutoSave.current = true;
        storageService.savePlannedActivities(newActivities).catch((e) =>
          console.error("Failed to save planned activities", e)
        );
  ...
  const completePlannedActivity = useCallback(
    (
      activityId: string,
      actualDist?: number,
      actualTime?: number,
      ...
      setPlannedActivities((prev) => {
        ...
        if (completed && original?.status !== "COMPLETED") {
          skipAutoSave.current = true;
          storageService.savePlannedActivity(completed).catch((e) =>
            console.error("Failed to save completed plan", e)
          );
          ...
          addExercise({
            ...
            durationMinutes: Math.round(
              (actualTime || (completed.estimatedDistance * 300)) / 60,
            ), // fallback to 5min/km
            intensity: feedback === "HARD" || feedback === "TOO_HARD"
              ? "high"
              : "moderate",
            ...
            notes: `Coached Session: ${completed.title}. Feedback: ${
              feedback || "None"
            }`,
          });
        }
  ...
  const addCoachGoal = useCallback(
    (goalData: Omit<CoachGoal, "id" | "createdAt" | "isActive">) => {
      ...
      setCoachConfig((prev) =>
        prev ? { ...prev, goals: [...(prev.goals || []), newGoal] } : {
