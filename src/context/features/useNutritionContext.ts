...
import {
  ...
  calculateRecipeNutrition as calculateRecipeNutritionUtil,
} from "../../utils/nutrition/calculations.ts";
...
export function useNutritionContext(
  ...
  const [pantryQuantities, setPantryQuantitiesState] = useState<
  ...
  const setPantryQuantity = useCallback(
  ...
  const getPantryQuantity = useCallback(
    (itemName: string): { quantity: number; unit: string } | undefined => {
  ...
  const addFoodItem = useCallback((data: FoodItemFormData): FoodItem => {
    ...
    storageService.createFoodItem(newItem).then((serverItem) => {
      if (serverItem) {
        setFoodItems((prev) =>
          prev.map((i) => i.id === newItem.id ? serverItem : i)
        );
      }
  ...
  const updateFoodItem = useCallback(
    (id: string, data: Partial<FoodItemFormData>): void => {
          ...
          storageService.updateFoodItem(updatedItem).then((serverItem) => {
            if (serverItem) {
              setFoodItems((current) =>
                current.map((i) => i.id === id ? serverItem : i)
              );
            }
          }).catch((e) => console.error("Failed to sync food:", e));
  ...
  const deleteFoodItem = useCallback((id: string): void => {
    ...
    setFoodItems((prev: FoodItem[]) =>
      prev.filter((item: FoodItem) => item.id !== id)
    );
    skipAutoSave.current = true;
    storageService.deleteFoodItem(id).catch((e) =>
  ...
  const addRecipe = useCallback((data: RecipeFormData): Recipe => {
    ...
    skipAutoSave.current = true;
    storageService.saveRecipe(newRecipe).catch((e) =>
  ...
  const updateRecipe = useCallback(
    (id: string, data: Partial<RecipeFormData>): void => {
      ...
      skipAutoSave.current = true;
      storageService.saveRecipe(updated).catch((e) =>
  ...
  const deleteRecipe = useCallback((id: string): void => {
    ...
    setRecipes((prev: Recipe[]) =>
      prev.filter((recipe: Recipe) => recipe.id !== id)
    );
    skipAutoSave.current = true;
    storageService.deleteRecipe(id).catch((e) =>
  ...
  const calculateRecipeNutrition = useCallback(
    (recipe: Recipe): NutritionSummary => {
      return calculateRecipeNutritionUtil(recipe, foodItems);
    },
    [foodItems],
  );

  const getRecipeWithNutrition = useCallback(
    (id: string): RecipeWithNutrition | undefined => {
      const recipe = recipes.find((r) => r.id === id);
  ...
  const addMealEntry = useCallback((data: MealEntryFormData): MealEntry => {
    ...
    newEntry.items.forEach((item) => {
      const { nutrition, caffeine, alcoholUnits } = calculateMealItemNutrition(
        item,
        recipes,
        foodItems,
      );
    ...
    if (totalAlcoholUnits > 0 || totalCaffeine > 0) {
      ...
      const newAlcohol = (currentVitals.alcohol || 0) +
        Math.round(totalAlcoholUnits * 10) / 10;
      const newCaffeine = (currentVitals.caffeine || 0) +
      ...
      console.log(
        `[Vitals] Added ${
          totalAlcoholUnits.toFixed(1)
        } units alcohol and ${totalCaffeine}mg caffeine for ${date}`,
      );
    }
    ...
    emitFeedEvent(
      "NUTRITION_MEAL",
      `Loggade ${
        newEntry.mealType.charAt(0).toUpperCase() + newEntry.mealType.slice(1)
      }`,
      {
        type: "NUTRITION_MEAL",
      ...
      [{
        label: "Energi",
        value: Math.round(totalCals),
        unit: "kcal",
        icon: "🔥",
      }],
    );

    logAction("CREATE", "meal_entry", newEntry.id, undefined, {
      mealType: newEntry.mealType,
      date: newEntry.date,
    });
  ...
  const updateMealEntry = useCallback(
    (id: string, data: Partial<MealEntry>): void => {
        ...
        const next = prev.map((entry: MealEntry) => {
          if (entry.id === id) {
            ...
            const isDifferent = Object.entries(data).some(([key, value]) => {
              ...
              if (key === "items") {
                return JSON.stringify(entry.items) !== JSON.stringify(value);
              }
      ...
      if (entryToUpdate && hasChanges) {
        skipAutoSave.current = true;
        storageService.updateMealEntry(entryToUpdate).catch((e) =>
          console.error("Failed to update meal", e)
        );
        logAction("UPDATE", "meal_entry", id, undefined, {
          date: (entryToUpdate as MealEntry).date,
        });
      }
    },
    [logAction, skipAutoSave],
  ...
  const deleteMealEntry = useCallback((id: string): void => {
    ...
    if (entryToDelete) {
      storageService.deleteMealEntry(id, entryToDelete.date).catch((e) =>
        console.error("Failed to delete meal", e)
      );
      logAction("DELETE", "meal_entry", id, undefined, {
    ...
    (date: string): NutritionSummary => {
      ...
      for (const entry of entries) {
        for (const item of entry.items) {
          const { nutrition: baseNutrition } = calculateMealItemNutrition(
            item,
            recipes,
            foodItems,
          );
          ...
          summary.vitaminB12 = (summary.vitaminB12 || 0) +
            (nutrition.vitaminB12 || 0);
          summary.vitaminC = (summary.vitaminC || 0) +
            (nutrition.vitaminC || 0);
          summary.vitaminA = (summary.vitaminA || 0) +
  ...
  const getWeeklyPlan = useCallback(
    (weekStartDate: string): WeeklyPlan | undefined => {
  ...
  const saveWeeklyPlan = useCallback(
    (weekStartDate: string, meals: WeeklyPlan["meals"]): void => {
      const now = new Date().toISOString();
      const existingIndex = weeklyPlans.findIndex((p: WeeklyPlan) =>
      ...
      skipAutoSave.current = true;
      storageService.saveWeeklyPlan(newPlan).catch((e) =>
  ...
  const getPlannedMealsForDate = useCallback(
  ...
  const updateQuickMeal = useCallback(
    (
      id: string,
      updates: Partial<Omit<QuickMeal, "id" | "userId" | "createdAt">>,
    ) => {
      setQuickMeals((prev) =>
        prev.map((m) => {
          if (m.id !== id) return m;
