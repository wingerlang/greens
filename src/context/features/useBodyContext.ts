...
export function useBodyContext(
  { currentUser, logAction, emitFeedEvent, skipAutoSave }: UseBodyContextProps,
) {
  ...
  const addWeightEntry = useCallback(
    (
      ...
      if (existingEntry) {
        ...
        setWeightEntries((prev) => {
          const next = prev.map((w) =>
            w.id === existingEntry.id ? updatedEntry : w
          );
      ...
      logAction("CREATE", "weight_entry", newEntry.id, undefined, {
        weight,
        date,
      });
  ...
  const bulkAddWeightEntries = useCallback(
    (entries: Partial<WeightEntry>[]) => {
    ...
    (
      ...
      setWeightEntries((prev) => {
        ...
        if (updated) {
          ...
          storageService.updateWeightEntry(updated).catch((e) =>
  ...
  const deleteWeightEntry = useCallback((id: string) => {
    setWeightEntries((prev) => {
      ...
      if (entry) {
        ...
        storageService.deleteWeightEntry(id, entry.date).catch((e) =>
  ...
  const addInjuryLog = useCallback(
    (data: Omit<InjuryLog, "id" | "createdAt" | "updatedAt">) => {
  ...
  const updateInjuryLog = useCallback(
    (id: string, updates: Partial<InjuryLog>) => {
      setInjuryLogs((prev) =>
        prev.map((log) =>
          log.id === id
  ...
  const addRecoveryMetric = useCallback(
    (metric: Omit<RecoveryMetric, "id">) => {
    ...
    (entry: Omit<BodyMeasurementEntry, "id" | "createdAt">) => {
      ...
      setWeightEntries((prev) => {
        ...
        if (existing) {
          ...
          if (entry.type === "thigh_left" || entry.type === "thigh_right") {
            updates.thigh = entry.value;
          }
          ...
          if (Object.keys(updates).length > 0) {
            ...
            storageService.updateWeightEntry(updatedWeight).catch((e) =>
              console.error("Failed to sync weight measurement:", e)
            );
      ...
      // Persist
      storageService.saveBodyMeasurement?.(newEntry).catch((e) =>
  ...
  const updateBodyMeasurement = useCallback(
    (id: string, updates: Partial<BodyMeasurementEntry>) => {
  ...
  const deleteBodyMeasurement = useCallback((id: string) => {
    ...
    storageService.deleteBodyMeasurement?.(id).catch((e) =>
