...
interface DataContextType {
  ...
  getPlannedMealsForDate: (
    date: string,
  ) => { mealType: MealType; meal: PlannedMeal }[];
  ...
  updateStrengthSession: (
    id: string,
    updates: Partial<StrengthWorkout>,
  ) => void;
  ...
  generateCoachPlan: (
    ...
    configOverride?: CoachConfig,
  ) => void;
  deletePlannedActivity: (id: string) => void;
  updatePlannedActivity: (
    id: string,
    updates: Partial<PlannedActivity>,
  ) => void;
  ...
  addInjuryLog: (
    log: Omit<InjuryLog, "id" | "createdAt" | "updatedAt">,
  ) => InjuryLog;
...
export function DataProvider({ children }: DataProviderProps) {
  ...
  const refreshData = useCallback(async () => {
    ...
    if (token) {
      ...
      try {
        ...
        const [userPayload, mePayload, planData, strengthData, quickMealsData] =
          await Promise.all([
            safeFetch<{ users: User[] }>("/api/users", {
            ...
            safeFetch<{ activities: PlannedActivity[] }>(
              "/api/planned-activities",
              { headers: { "Authorization": `Bearer ${token}` } },
            ),
            safeFetch<{ workouts: StrengthWorkout[] }>(
              "/api/strength/workouts",
              { headers: { "Authorization": `Bearer ${token}` } },
            ),
            safeFetch<QuickMeal[]>("/api/quick-meals", {
              headers: { "Authorization": `Bearer ${token}` },
            }),
          ]);
        ...
        // 1. Handle Users
        if (
          userPayload && userPayload.users && Array.isArray(userPayload.users)
        ) {
          console.log(
            "[DataContext] Loaded real users list:",
            userPayload.users.map((u) => u.username),
          );
        ...
        if (mePayload && mePayload.user) {
          console.log(
            "[DataContext] Resolved current user:",
        ...
        if (
          planData && planData.activities && Array.isArray(planData.activities)
        ) {
          console.log(
            "[DataContext] Loaded planned activities globally:",
            planData.activities.length,
          );
          const newActivities = planData.activities;
          const existing = data.plannedActivities || [];
          const newIds = new Set(
            newActivities.map((a: PlannedActivity) => a.id),
          );
          const merged = [
        ...
        if (
          strengthData && strengthData.workouts &&
          Array.isArray(strengthData.workouts)
        ) {
          console.log(
            "[DataContext] Loaded strength workouts globally:",
            strengthData.workouts.length,
          );
        ...
        if (quickMealsData && Array.isArray(quickMealsData)) {
          console.log(
            "[DataContext] Loaded quick meals:",
            quickMealsData.length,
          );
        ...
        if (e instanceof Error && e.name === "AbortError") {
          console.log(
            "[DataContext] Request aborted (expected during re-renders)",
          );
        } else {
    ...
    if (data.performanceGoals) {
      const migrated = (data.performanceGoals || []).map((
        g: PerformanceGoal,
      ) => ({
        ...g,
    ...
    if (data.universalActivities) {
      setUniversalActivities(data.universalActivities || []);
    }
          ...
          if (date.includes("/") || date.includes(".")) {
            ...
            if (parts.length === 3) {
              if (parts[2].length === 4) { // DD/MM/YYYY
                date = `${parts[2]}-${parts[1].padStart(2, "0")}-${
                  parts[0].padStart(2, "0")
                }`;
              } else if (parts[0].length === 4) { // YYYY/MM/DD
                date = `${parts[0]}-${parts[1].padStart(2, "0")}-${
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      if (shouldSkipApi) {
        console.log(
          "Optimizing auto-save: Skipping API sync for atomic update",
        );
  ...
  const calculateTrainingStreak = useCallback(
    (referenceDate?: string, type?: string): number => {
      return calculateTrainingStreakUtil(exerciseEntries, referenceDate, type);
    },
    [exerciseEntries],
  );

  const calculateWeeklyTrainingStreak = useCallback(
  ...
  const calculateCalorieGoalStreak = useCallback(
    (referenceDate?: string): number => {
      if (!currentUser?.settings) return 0;
      return calculateCalorieGoalStreakUtil(
        calculateDailyNutrition,
  ...
  const calculateExerciseCalories = useCallback(
    (
      ...
      return calculateExerciseCaloriesUtil(
        type,
        duration,
        intensity,
        getLatestWeight(),
