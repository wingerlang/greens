...
export function CookingModeProvider({ children }: CookingModeProviderProps) {
    ...
    const handleStartCooking = (recipe: Recipe, portions: number, customizations?: IngredientCustomizations) => {
    ...
    const handleEndSession = useCallback(() => {
        if (session) {
            ...
            const mealType = hour < 10 ? 'breakfast' : hour < 14 ? 'lunch' : hour < 18 ? 'dinner' : 'snack';
