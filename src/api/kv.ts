...
const baseKv =
  await (globalThis.Deno
    ? Deno.openKv("./greens.db")
    : (globalThis as any).Deno.openKv("./greens.db"));
      ...
      return function (this: any, ...args: any[]) {
        ...
        if (promise instanceof Promise) {
          return promise.then((result: any) => {
            ...
            const details = prop === "list"
              ? { count: "stream" }
              : (prop === "set" ? args[1] : undefined);
            addDebugLog({
              type: "kv",
    ...
    get(target, prop, receiver) {
      ...
      if (typeof original === "function") {
        ...
        return function (this: any, ...args: any[]) {
          ...
          if (
            ["set", "delete", "check", "sum", "min", "max"].includes(
              String(prop),
            )
          ) {
            addDebugLog({
