...
export async function getExerciseMappings(
  ...
  const mappings: Record<string, MuscleGroup> = {
    ...INITIAL_EXERCISE_MAPPINGS,
  };
  ...
  const iter = kv.list<MuscleGroup>({
    prefix: [...EXERCISE_MAP_KEY_PREFIX, userId],
  });
 ...
 */
export async function deleteExerciseMapping(
  userId: string,
  exerciseName: string,
): Promise<void> {
  const normalized = normalizeExerciseName(exerciseName);
