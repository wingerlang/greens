import { DebugContext, debugStorage } from "../utils/debugContext.ts";
...
function truncateForKV(
  ...
  if (
    result.responseBody && typeof result.responseBody === "string" &&
    result.responseBody.length > 1000
  ) {
    const originalLen = result.responseBody.length;
    result.responseBody = `[Truncated, original: ${originalLen} bytes] ${
  ...
  if (
    size > MAX_KV_SIZE && Array.isArray(result.logs) && result.logs.length > 0
  ) {
    const logsLen = result.logs.length;
    const truncatedLogs = (result.logs as unknown[]).slice(0, 10);
    truncatedLogs.push({
      message: `[Truncated ${logsLen - 10} additional logs]`,
      timestamp: Date.now(),
    });
  ...
  if (size > MAX_KV_SIZE && result.payload) {
    if (typeof result.payload === "string" && result.payload.length > 500) {
      result.payload =
        `[Truncated payload, original: ${result.payload.length} bytes]`;
    ...
    try {
      ...
      if (["POST", "PUT", "PATCH"].includes(req.method) && req.body) {
        try {
          ...
          payload = `[Failed to read payload: ${
      ...
      try {
        ...
        await kv.set(["system", "debug", requestId], truncatedData, {
          expireIn: 10 * 60 * 1000,
        });
      } catch (e) {
