...
const STRAVA_CLIENT_ID = globalThis.Deno
  ? Deno.env.get("STRAVA_CLIENT_ID") || ""
  : process.env.STRAVA_CLIENT_ID || "";
// @ts-ignore: Deno is polyfilled
const STRAVA_CLIENT_SECRET = globalThis.Deno
  ? Deno.env.get("STRAVA_CLIENT_SECRET") || ""
  : process.env.STRAVA_CLIENT_SECRET || "";
// @ts-ignore: Deno is polyfilled
const STRAVA_REDIRECT_URI = globalThis.Deno
 ...
 */
export function calculateStravaCalories(
  activity: StravaActivity,
  userSettings?: UserSettings,
): { calories: number; breakdown: string } {
  ...
  if (hr && userSettings && userSettings.weight && userSettings.birthYear) {
    ...
    if (gender === "male") {
      ...
      formula =
        "Keytel et al. (Male): -55.0969 + (0.6309 * HR) + (0.1988 * W) + (0.2017 * A)";
    } else {
      kjPerMin = -20.4022 + (0.4472 * hr) - (0.1263 * weight) + (0.0740 * age);
      formula =
        "Keytel et al. (Female): -20.4022 + (0.4472 * HR) - (0.1263 * W) + (0.0740 * A)";
    }
    ...
    if (Math.abs(originalKcalPerMin - kcalPerMin) > 0.1) {
      breakdown += `\n\nNotering: Begränsat från ${
        originalKcalPerMin.toFixed(1)
      } till ${
        kcalPerMin.toFixed(1)
      } kcal/min (rimliga gränser för ${type}: ${limits.min}-${limits.max} kcal/min).`;
    }
  ...
  if (type === "strength") {
    kcalPerMin = 4;
    reason = "Schablon för styrketräning (utan pulsdata)";
  } else if (type === "walking") {
    kcalPerMin = 3;
    reason = "Schablon för rask promenad";
  } else if (type === "running") {
    kcalPerMin = 8;
    reason = "Konservativ schablon för löpning (utan pulsdata)";
  } else if (type === "cycling") {
    kcalPerMin = 6;
    reason = "Schablon för cykling (utan pulsdata)";
  } else if (type === "swimming") {
    kcalPerMin = 7;
    reason = "Schablon för simning";
  } else if (type === "yoga") {
    kcalPerMin = 2.5;
    reason = "Schablon för yoga";
  }
  ...
  return {
    calories: total,
    breakdown:
      `Källhänvisning: App Schablon (Konservativ)\nAnledning: Saknar Strava-kalorier och pulsdata.\nBeräkning: ${
        durationMin.toFixed(1)
      } min × ${kcalPerMin} kcal/min (${reason}).`,
  };
}
 ...
 */
export function mapStravaActivityToExercise(
  activity: StravaActivity,
  userSettings?: UserSettings,
) {
  ...
  return {
    ...
    durationMinutes:
      (userSettings?.stravaTimePreference === "elapsed"
        ? activity.elapsed_time
        : (activity.moving_time || activity.elapsed_time)) / 60,
...
export function mapStravaToPerformance(
  activity: StravaActivity,
  userSettings?: UserSettings,
): ActivityPerformanceSection {
  const calorieData = calculateStravaCalories(activity, userSettings);
 ...
 */
export function createUniversalFromStrava(
  activity: StravaActivity,
  userId: string,
  userSettings?: UserSettings,
): UniversalActivity {
