 ...
 */
export async function saveRacePlan(
  userId: string,
  plan: Omit<RacePlan, "id" | "createdAt" | "updatedAt" | "userId"> & {
    id?: string;
  },
): Promise<RacePlan> {
 ...
 * Get a specific plan
 */
export async function getRacePlan(
  userId: string,
  planId: string,
 ...
 * Delete a race plan.
 */
export async function deleteRacePlan(
  userId: string,
  planId: string,
): Promise<void> {
 ...
 */
export async function handleRacePlanRoutes(
  req: Request,
  url: URL,
  headers: Headers,
): Promise<Response> {
