...
export async function handlePeriodRoutes(req: Request, url: URL, headers: Headers): Promise<Response> {
    ...
    if (method === "POST") {
        try {
            ...
            if (!period.id) return new Response(JSON.stringify({ error: "Missing ID" }), { status: 400, headers });
