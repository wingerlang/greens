...
export async function handleRecipeRoutes(req: Request, url: URL, headers: Headers): Promise<Response> {
    ...
    if (method === "GET") {
        ...
        if (lastPart !== "recipes") {
            ...
            if (!recipe) return new Response(JSON.stringify({ error: "Recipe not found" }), { status: 404, headers });
    ...
    if (method === "POST") {
        try {
            ...
            if (!recipe.id) return new Response(JSON.stringify({ error: "ID required" }), { status: 400, headers });

            await recipeRepo.saveRecipe(userId, recipe);
            return new Response(JSON.stringify({ success: true, recipe }), { headers });
        } catch (e) {
    ...
    if (method === "PUT") {
        try {
            ...
            if (!existing) return new Response(JSON.stringify({ error: "Recipe not found" }), { status: 404, headers });

            const updated = { ...existing, ...updates };
            await recipeRepo.saveRecipe(userId, updated);

            return new Response(JSON.stringify({ success: true, recipe: updated }), { headers });
        } catch (e) {
