...
export async function handleRecalculateCaloriesRoutes(
  req: Request,
  url: URL,
  headers: Headers,
): Promise<Response> {
    ...
    try {
      ...
      for (const activity of activities) {
        ...
        const stravaLikeActivity = {
          ...
          sport_type: mapActivityType(
            activity.performance.activityType || "running",
          ),
          start_date: activity.date,
          start_date_local: activity.date,
          elapsed_time: (activity.performance.elapsedTimeSeconds ||
            (activity.performance.durationMinutes || 60) * 60),
        ...
        const calorieResult = calculateStravaCalories(
          stravaLikeActivity as any,
          userSettings as any,
        );
        ...
        if (
          Math.abs(
            (activity.performance.calories || 0) - calorieResult.calories,
          ) > 1
        ) {
      ...
      console.log(
        `[RecalculateCalories] User ${userId}: Updated ${updatedCount}, Skipped ${skippedStravaCalories} (already have Strava calories)`,
      );
