...
import { getErrorLogs, logError } from "../utils/logger.ts";
import { safeStringify } from "../utils/jsonUtils.ts";
import { kv } from "../kv.ts";

export async function handleAdminKvRoutes(
  req: Request,
  url: URL,
  headers: Headers,
): Promise<Response> {
  ...
  if (!ctx || !hasRole(ctx, "admin")) {
    return new Response(
      safeStringify({ error: "Forbidden: Admin access required" }),
      { status: 403, headers },
    );
  }
  ...
  try {
    ...
    if (url.pathname === "/api/admin/backup" && req.method === "GET") {
      ...
      backupHeaders.set(
        "Content-Disposition",
        `attachment; filename="backup-${new Date().toISOString()}.jsonl"`,
      );
    ...
    if (url.pathname === "/api/admin/kv/stats" && req.method === "GET") {
      ...
      return new Response(safeStringify({ stats: currentStats, history }), {
        headers,
      });
    }
    ...
    if (url.pathname === "/api/admin/kv/entries" && req.method === "POST") {
      ...
      if (!Array.isArray(prefix)) {
        return new Response(
          JSON.stringify({ error: "Prefix must be an array" }),
          { status: 400, headers },
        );
      }
