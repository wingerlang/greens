...
export async function handleAdminRoutes(
  ...
  if (url.pathname === "/api/admin/metrics") {
    ...
    const avgResponseTime = responseTimes.length > 0
      ? responseTimes.reduce((acc, curr) => acc + curr.value, 0) /
  ...
  if (
    ...
    url.pathname.endsWith("/role") && req.method === "PATCH"
  ) {
    try {
  ...
  if (
    url.pathname.startsWith("/api/admin/users/") &&
    !url.pathname.endsWith("/role") && req.method === "DELETE"
  ) {
    try {
      ...
      console.log(
        `[Admin] Deleted user ${user.username} (${userId}) and all their data`,
      );

      return new Response(
        JSON.stringify({ success: true, deleted: user.username }),
        { headers },
      );
    } catch (e) {
