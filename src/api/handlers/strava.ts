...
export async function handleStravaRoutes(
  req: Request,
  url: URL,
  headers: Headers,
): Promise<Response> {
    ...
    try {
      ...
      if (error) {
        return Response.redirect(
          new URL("/profile?strava_error=" + error, origin).toString(),
          302,
        );
      }
      if (!code) {
        return new Response(
          JSON.stringify({ error: "No authorization code" }),
          { status: 400, headers },
        );
      }
      ...
      if (!tokens) {
        return Response.redirect(
          new URL("/profile?strava_error=token_exchange_failed", origin)
            .toString(),
          302,
        );
      }
      ...
      return Response.redirect(
        new URL("/profile?strava_connected=true", origin).toString(),
        302,
      );
    } catch (err) {
  ...
  if (url.pathname === "/api/strava/auth" && method === "GET") {
    ...
    return new Response(
      JSON.stringify({ authUrl: strava.getStravaAuthUrl(state) }),
      { headers },
    );
  }
  ...
  if (url.pathname === "/api/strava/status" && method === "GET") {
    ...
    if (Date.now() > stravaTokens.expiresAt) {
      const refreshed = await strava.refreshStravaToken(
        stravaTokens.refreshToken,
      );
      if (refreshed) {
        ...
        return new Response(
          JSON.stringify({ connected: false, error: "Token expired" }),
          { headers },
        );
      }
    }
    ...
    const stats = stravaTokens.athleteId
      ? await strava.getStravaAthleteStats(stravaTokens.athleteId, accessToken)
      : null;
      ...
      JSON.stringify({
        ...
        stats: stats
          ? {
            ...
            ytdDistance: Math.round(
              (stats.ytd_run_totals.distance + stats.ytd_ride_totals.distance) /
                1000,
            ),
          }
          : null,
  ...
  if (url.pathname === "/api/strava/scan" && method === "POST") {
    try {
      ...
      const report = await reconciliationService.scanStravaActivities(
        session.userId,
        accessToken,
        { fromDate },
      );
  ...
  if (url.pathname === "/api/strava/import" && method === "POST") {
    try {
      ...
      if (!activities || !Array.isArray(activities)) {
        return new Response(
          JSON.stringify({ error: "Invalid activities payload" }),
          { status: 400, headers },
        );
      }

      const result = await reconciliationService.syncActivities(
        session.userId,
        activities,
        { forceUpdate },
      );
  ...
  if (url.pathname === "/api/strava/sync" && method === "POST") {
    try {
      ...
      const lastSyncDate = (!fullSync && stravaTokens.lastSync)
        ? new Date(stravaTokens.lastSync).getTime() / 1000
        : undefined;
      ...
      const result = await reconciliationService.reconcileStravaActivities(
        session.userId,
        activities,
      );
  ...
  if (url.pathname === "/api/strava/migrate-start-times" && method === "POST") {
    try {
      ...
      const { activityRepo } = await import(
