import { getExerciseMappings, saveExerciseMapping } from "../db/exercise-mapper.ts";
...
export async function handleExerciseMapperRoutes(req: Request, url: URL, headers: Headers): Promise<Response> {
    ...
    if (req.method === "GET") {
        try {
            ...
            return new Response(JSON.stringify({ error: "Failed to fetch mappings" }), { status: 500, headers });
        }
    }
    ...
    if (req.method === "POST") {
        try {
            ...
            if (!exerciseName || !muscleGroup) {
                return new Response(JSON.stringify({ error: "Missing exerciseName or muscleGroup" }), { status: 400, headers });
            }

            await saveExerciseMapping(userId, exerciseName, muscleGroup as MuscleGroup);

            return new Response(JSON.stringify({ success: true }), { headers });

        } catch (error) {
            console.error("Error saving exercise mapping:", error);
            return new Response(JSON.stringify({ error: "Failed to save mapping" }), { status: 500, headers });
        }
    }
