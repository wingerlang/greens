...
import type { FeedEventCategory, FeedEventType } from "../../models/feedTypes.ts";
 ...
 */
export async function handleFeedRoutes(req: Request, url: URL, headers: Headers): Promise<Response> {
    ...
    try {
        ...
        if (url.pathname.match(/^\/api\/feed\/user\/[^/]+$/) && method === "GET") {
            ...
            return new Response(JSON.stringify({ events: visibleEvents }), { headers });
        }

        // ============================================
        // GET /api/feed/preferences/:targetUserId - Get subscription settings
        // ============================================
        if (url.pathname.match(/^\/api\/feed\/preferences\/[^/]+$/) && method === "GET") {
            const targetUserId = url.pathname.split('/').pop()!;

            const preference = await FeedRepository.getFollowPreference(userId, targetUserId);
        ...
        // ============================================
        if (url.pathname.match(/^\/api\/feed\/preferences\/[^/]+$/) && method === "PUT") {
            const targetUserId = url.pathname.split('/').pop()!;
        ...
        if (url.pathname === "/api/feed/events" && method === "POST") {
            ...
            const { type, title, summary, payload, visibility, timestamp, metrics } = body;
        ...
        // DELETE /api/feed/events/:eventId - Delete an event
        // ============================================
        if (url.pathname.match(/^\/api\/feed\/events\/[^/]+$/) && method === "DELETE") {
        ...
        // ============================================
        if (url.pathname.startsWith("/api/feed/events/") && url.pathname.endsWith("/visibility") && method === "PATCH") {
            const eventId = url.pathname.split("/")[4];
            const body = await req.json();
            const { visibility } = body;

            if (!visibility) {
                return new Response(JSON.stringify({ error: "Visibility is required" }), { status: 400, headers });
            }

            const success = await FeedRepository.updateEventVisibility(userId, eventId, visibility);
            if (!success) {
                return new Response(JSON.stringify({ error: "Event not found or unauthorized" }), { status: 404, headers });
            }
