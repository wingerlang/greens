...
async function granularReset(
  ...
  if (type === "meals") {
    ...
    if (data.vitals) {
      data.vitals = data.vitals.map((v: any) => ({
        ...v,
...
export async function handleUserRoutes(
  ...
  if (url.pathname === "/api/user/sessions") {
    if (method === "GET") {
      ...
      return new Response(JSON.stringify({ sessions: clientSessions }), {
        headers,
      });
    }
    if (method === "DELETE") {
  ...
  if (url.pathname.startsWith("/api/u/") && method === "GET") {
    ...
    try {
      ...
      let id = idEntry.value ||
        (await kv.get(["users_by_username", handle])).value;

      // Self-Healing Fallback: If index fails, scan users (slow path)
      if (!id) {
        console.log(
          `⚠️ Index miss for ${handle}, attempting slow scan repair...`,
        );
        ...
        if (match) {
          console.log(
            `✅ Found user ${match.username} via scan. Repairing index...`,
          );
          id = match.id;
  ...
  if (url.pathname === "/api/user/weight" && method === "GET") {
    try {
      const data = await getUserData(session.userId);
      return new Response(
        JSON.stringify({ history: data?.weightEntries || [] }),
        { headers },
      );
    } catch (e) {
      return new Response(JSON.stringify({ error: (e as Error).message }), {
    ...
    try {
      ...
      if (updates.handle) {
        ...
        if (user && updates.handle !== user.handle) {
          const existingId =
            (await kv.get(["users_by_handle", updates.handle])).value;
  ...
  if (url.pathname === "/api/users" && method === "GET") {
    try {
      ...
      return new Response(JSON.stringify({ users: communityUsers }), {
        headers,
      });
    } catch (e) {
      return new Response(JSON.stringify({ error: (e as Error).message }), {
  ...
  if (url.pathname === "/api/user/prs" && method === "POST") {
    try {
      ...
      if (!category || !time) {
        return new Response(
          JSON.stringify({ error: "Missing category or time" }),
          { status: 400, headers },
        );
      }
      const pr = {
  ...
  if (url.pathname === "/api/user/prs/detect" && method === "GET") {
    try {
      ...
      const iter = kv.list<UniversalActivity>({
        prefix: ["activities", targetUserId],
      });
      for await (const entry of iter) {
      ...
      const runs = activities.filter((a) =>
        a.performance?.activityType === "running" && a.status === "COMPLETED"
      );
  ...
  if (url.pathname === "/api/user/privacy" && method === "PATCH") {
    try {
      ...
      return new Response(
        JSON.stringify({ success: true, privacy: user.privacy }),
        { headers },
      );
    } catch (e) {
      return new Response(JSON.stringify({ error: (e as Error).message }), {
  ...
  if (
    url.pathname.startsWith("/api/u/") && url.pathname.endsWith("/stats") &&
    method === "GET"
  ) {
    ...
    try {
      ...
      const privacy = user.privacy ||
        { isPublic: true, sharing: { training: "FRIENDS" } };
      ...
      if (!isMe) {
        if (privacy.isPublic === false) {
          return new Response(JSON.stringify({ error: "Private profile" }), {
            status: 403,
            headers,
          });
        }
        if (privacy.sharing?.training === "PRIVATE") {
          return new Response(
            JSON.stringify({ stats: null, privacy: "private" }),
            { headers },
          );
        }
      }
      ...
      for await (const entry of iter) {
        ...
        if (
          date >= thirtyDaysAgo && date <= now && act.status === "COMPLETED"
        ) {
          activities.push(act); // Keep if needed, or just aggregate
          count++;
          if (act.performance?.distanceKm) {
            totalDistance += act.performance.distanceKm;
          }
          if (act.performance?.durationMinutes) {
            totalDuration += act.performance.durationMinutes;
          }
        }
      }
