...
export async function handleExerciseEntryRoutes(
  req: Request,
  url: URL,
  headers: Headers,
): Promise<Response> {
  ...
  if (method === "GET") {
    ...
    const entries = await exerciseEntryRepo.getEntriesInRange(
      userId,
      start,
      end,
    );
  ...
  if (method === "POST") {
    try {
      ...
      if (!entry.id || !entry.date) {
        return new Response(JSON.stringify({ error: "ID and Date required" }), {
          status: 400,
          headers,
        });
      }
  ...
  if (method === "PUT") {
    try {
      ...
      if (!updates.date) {
        return new Response(
          JSON.stringify({ error: "Date required for update" }),
          { status: 400, headers },
        );
      }
      ...
      const existingList = await exerciseEntryRepo.getEntriesByDate(
        userId,
        updates.date,
      );
