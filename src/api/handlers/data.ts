...
export async function handleDataRoutes(
  ...
  if (url.pathname.startsWith("/api/meals/") && method === "PUT") {
    ...
    if (!updates.date) {
      return new Response(
        JSON.stringify({ error: "Date is required for updates" }),
        { status: 400, headers },
      );
    }
  ...
  if (url.pathname === "/api/weight" && method === "POST") {
    ...
    if (typeof entry.weight !== "number" || !entry.date || !entry.id) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Invalid weight entry: weight(number), date(string), id(string) required",
        }),
        { status: 400, headers },
      );
    }
  ...
  if (url.pathname.startsWith("/api/foods/") && method === "PUT") {
    ...
    // Handle Image Moving
    if (
      updatedItem.imageUrl && updatedItem.imageUrl.startsWith("uploads/temp/")
    ) {
      ...
      try {
        ...
        if (
          existing.imageUrl && existing.imageUrl !== updatedItem.imageUrl &&
