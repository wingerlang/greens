...
export async function fetchAllStrengthWorkouts(): Promise<StrengthWorkout[]> {
  ...
  console.log(
    `[Stats] Fetching strength workouts for ${users.length} users...`,
  );

  // Optimization: Parallel fetch
  const workoutPromises = users.map((user) =>
    strengthRepo.getAllWorkouts(user.id)
  );
...
export async function fetchAllActivities(): Promise<UniversalActivity[]> {
  ...
  const activityPromises = users.map((user) =>
    activityRepo.getActivitiesByDateRange(user.id, "2000-01-01", "2099-12-31")
  );
...
export function calculateGlobalStats(
  ...
  stats.averages.distancePerUserPerMonth = Math.round(
    stats.global.totalDistanceKm / totalUsers / lifespanMonths,
  );
  stats.averages.tonnagePerUserPerMonth = Math.round(
    stats.global.totalTonnage / totalUsers / lifespanMonths,
  );
  ...
  stats.averages.workoutsPerUserPerMonth = parseFloat(
    (stats.global.totalWorkouts / totalUsers / lifespanMonths).toFixed(1),
  );

  // Session duration: Average per workout, guard against 0 workouts
  stats.averages.sessionDurationMinutes = Math.round(
    stats.global.totalDurationMinutes / Math.max(1, stats.global.totalWorkouts),
  );
  ...
  filteredWorkouts.forEach((w) => {
    w.exercises.forEach((we) => {
      ...
      const sessionTonnage = we.sets.reduce(
  ...
  filteredActivities.forEach((a) => {
    if (
      a.status === "COMPLETED" && a.performance?.distanceKm &&
