...
export class KvInspectorService {
  ...
  async getUserStorageUsage(): Promise<
    { users: UserStorageUsage[]; orphans: OrphanedData[] }
  > {
    ...
    for await (const entry of kv.list({ prefix: [] })) {
      ...
      // Skip system keys
      if (
        rootPrefix === "system" || rootPrefix === "idx_activities_source" ||
        rootPrefix.startsWith("stats")
      ) continue;
      ...
      if (userId) {
        if (userMap.has(userId)) {
          ...
          usage.prefixes[rootPrefix] = (usage.prefixes[rootPrefix] || 0) +
            entrySize;
        } else {
          // Orphaned data
          orphans.push({
    ...
    return {
      users: Array.from(usageMap.values()).sort((a, b) =>
  ...
  async listKeys(
  ...
  ): Promise<
    {
      ...
      keys: { key: unknown[]; size: number }[];
    }
  > {
    const iter = kv.list({ prefix }, { limit: 1000 });
    const subPrefixMap = new Map<
      string,
    ...
    for await (const entry of iter) {
      ...
      if (remainingKey.length === 1) {
        ...
        const existing = subPrefixMap.get(folderName) ||
          { name: folderName, count: 0, size: 0 };
        existing.count++;
    ...
    return {
      subPrefixes: Array.from(subPrefixMap.values()).sort((a, b) =>
