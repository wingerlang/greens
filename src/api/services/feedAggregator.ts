...
function bundleHydrationEvents(
    ...
    const bundled: FeedEvent = {
        ...
        summary: `${related.length} gånger under ${config.hydrationWindowHours} timmar`,
...
function bundleMealEvents(
    ...
    const bundled: FeedEvent = {
        ...
        metrics: [
            { label: 'Kalorier', value: Math.round(totalCalories), unit: 'kcal', icon: '🔥' },
            { label: 'Protein', value: Math.round(totalProtein), unit: 'g', icon: '💪' },
 ...
 */
export function groupEventsByDay(events: FeedEvent[]): Map<string, FeedEvent[]> {
