...
export class ReconciliationService {
  ...
  async scanStravaActivities(
    ...
    localActivities.forEach((a) => {
      if (
        ...
        a.performance.source.externalId
      ) {
    ...
    if (options.fromDate) {
      fetchOptions.after = Math.floor(
        new Date(options.fromDate).getTime() / 1000,
      );
    }
    ...
    for (const s of stravaActivities) {
      ...
      if (!existing) {
        ...
        if (Math.abs(stravaDurationMin - existingDuration) > 0.1) {
          changes.push(
            `Duration: ${existingDuration.toFixed(1)} -> ${
              stravaDurationMin.toFixed(1)
            } min`,
          );
        }
  ...
  async syncActivities(
    ...
    for (const stravaActivity of activitiesToSync) {
      try {
        ...
        const existing = await activityRepo.getActivityByExternalId(
          userId,
          "strava",
          externalId,
        );
        ...
        if (!existing) {
          ...
          const candidates = await activityRepo.getActivitiesByDateRange(
            userId,
            dateISO,
            dateISO,
          );
          const planMatch = this.findBestMatch(
            stravaActivity,
            candidates.filter((c) => c.status === "PLANNED"),
          );
          ...
          if (planMatch) {
            ...
            const newActivity = createUniversalFromStrava(
              stravaActivity,
              userId,
              user?.settings,
            );
          ...
          const freshPerformance = mapStravaToPerformance(
            stravaActivity,
            user?.settings,
          );
  ...
  async reconcileStravaActivities(
    ...
    const report = await this.scanStravaActivities(userId, "", {
      fromDate: stravaActivities[0]?.start_date,
    }); // Roughly
    ...
    for (const stravaActivity of stravaActivities) {
      ...
      const dayActivities = await activityRepo.getActivitiesByDateRange(
        userId,
        dateISO,
        dateISO,
      );
      const candidates = dayActivities.filter((a) =>
        a.status === "PLANNED" && !a.performance
      );
  ...
  private findBestMatch(
    ...
    const scored = candidates.map((c) => {
      ...
      if (
        planType === "running" &&
        (stravaType === "run" || stravaType === "trailrun")
      ) score += 10;
      else if (
        planType === "cycling" &&
        (stravaType === "ride" || stravaType === "virtualride")
      ) score += 10;
   ...
   * Merge logic
   */
  private async mergeActivity(
    target: UniversalActivity,
   ...
   */
  private async emitStravaFeedEvent(
    userId: string,
    ...
    const typeLabel = activity.type.replace("Run", "Löpning").replace(
      "Ride",
      "Cykling",
    ).replace("Walk", "Promenad");
    const distanceKm = activity.distance
      ? (Math.round(activity.distance / 10) / 100)
      : 0;
    const durationMinFormatted =
      ((activity.moving_time || activity.elapsed_time) / 60).toFixed(1);
    ...
    await FeedRepository.createEvent({
      ...
      metrics: [
        ...
        {
          label: "Energi",
          value: Math.round(calculateStravaCalories(activity, userSettings)),
          unit: "kcal",
          icon: "🔥",
        },
      ],
