...
export class CodeAnalysisService {
  ...
  async getDocumentationFiles(): Promise<
  ...
  private isExcluded(path: string, userExcludes: string[]): boolean {
    ...
    if (
      DEFAULT_EXCLUDES.some((ex) =>
        normalized.includes(ex) || normalized.split("/").includes(ex)
      )
    ) return true;
    // Check user excludes
  ...
  async getProjectStats(excludedPaths: string[] = []): Promise<ProjectStats> {
    ...
    if (complexityData.length > 0) {
      stats.complexityStats = {
        mostComplexFiles: complexityData.sort((a, b) => b.maxDepth - a.maxDepth)
          .slice(0, 20),
        averageDepth: complexityData.reduce((acc, curr) =>
          acc + curr.maxDepth, 0) / complexityData.length,
      };
    }
  ...
  private async walkAndCount(
    path: string,
    stats: ProjectStats,
    excludedPaths: string[],
    complexityData: { file: string; maxDepth: number }[],
  ) {
    try {
      for await (const entry of Deno.readDir(path)) {
        ...
        if (entry.isDirectory) {
          ...
          try {
            ...
            stats.linesByExtension[ext] = (stats.linesByExtension[ext] || 0) +
              lineCount;
            ...
            if (["ts", "tsx", "js", "jsx"].includes(ext)) {
              ...
              for (const line of lines) {
                ...
                if (
                  !trim || trim.startsWith("//") || trim.startsWith("/*") ||
                  trim.startsWith("*")
                ) continue;
      ...
      for await (const entry of Deno.readDir(path)) {
        ...
        if (entry.isDirectory) {
          ...
          try {
            ...
            size = stat.size;
            if (
              ["ts", "tsx", "js", "jsx", "css", "json", "md", "html"].includes(
                ext,
              )
            ) {
              const content = await Deno.readTextFile(fullPath);
  ...
  private async gatherFiles(
    path: string,
    list: { path: string; content: string; lines: number; name: string }[],
    excludedPaths: string[] = [],
  ) {
    try {
      for await (const entry of Deno.readDir(path)) {
  ...
  async analyzeFunctions(
    excludedPaths: string[] = [],
  ): Promise<DuplicateFunction[]> {
    const files: { path: string; content: string }[] = [];
    await this.gatherFiles(this.rootDir, files as any, excludedPaths);

    const functions: FunctionInfo[] = [];
    const IGNORE_PATTERNS =
      /^(use|set|get|handle|render|on|is|has|create|update|delete|fetch|load|moment|clsx|cn|z)$/i;
    const IGNORE_EXACT = [
      "useEffect",
      "useState",
      "useCallback",
    ...
    for (const file of files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const matchConst = line.match(
          /const\s+([a-zA-Z0-9_]+)\s*=\s*(async\s*)?(\([^)]*\)|[a-zA-Z0-9_]+)\s*=>/,
        );
        const matchMethod = line.match(
        ...
        if (
          name && name.length > 3 && !IGNORE_EXACT.includes(name) &&
          !IGNORE_PATTERNS.test(name)
        ) {
    ...
    const unique = duplicates.filter((d, index, self) =>
      index ===
        self.findIndex((t) =>
          (t.nameA === d.nameA && t.nameB === d.nameB) ||
          (t.nameA === d.nameB && t.nameB === d.nameA)
        )
    );
  ...
  async findSimilarFiles(
    excludedPaths: string[] = [],
  ): Promise<SimilarFilePair[]> {
    const files: { path: string; content: string }[] = [];
    await this.gatherFiles(this.rootDir, files as any, excludedPaths);

    const fileTerms = new Map<string, Set<string>>();
    for (const file of files) {
      const tokens = file.content.split(/[^a-zA-Z0-9_]+/).filter((t) =>
        t.length > 5 &&
        !/^(import|export|const|from|return|function|class|interface|async|await)$/
          .test(t)
      );
          ...
          if (line.includes("*/")) {
            ...
            if (text) {
              ...
              if (
                ...
                text.toLowerCase().includes("fixme")
              ) type = "todo";
  ...
  async getDependencies(): Promise<
    { name: string; version: string; type: "prod" | "dev" }[]
  > {
    ...
    try {
      ...
      if (pkg.devDependencies) {
        Object.entries(pkg.devDependencies).forEach(([k, v]) =>
          deps.push({ name: k, version: String(v), type: "dev" })
        );
      }
...
${
      issues.map((issue) =>
        `- [${issue.severity.toUpperCase()}] **${issue.type}**: ${issue.message} (${issue.file}) ${
          issue.relatedFile ? `-> ${issue.relatedFile}` : ""
        }`
      ).join("\n")
    }
...
## Large Files (>300 LOC)
${
      issues.filter((i) => i.type === "large_file").map((i) => `- ${i.file}`)
        .join("\n")
    }
