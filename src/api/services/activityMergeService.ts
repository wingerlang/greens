...
export function createMergedActivity(
  ...
  // Determine Name
  const stravaSource = activities.find((a) =>
    a.performance?.source?.source === "strava" || a.plan?.source === "strava" ||
    (a as any).source === "strava"
  );
  const baseName = stravaSource
    ? (stravaSource.plan?.title || stravaSource.performance?.notes ||
      "Strava Activity")
 ...
 */
export function getMergedActivityTitle(
  activities: UniversalActivity[],
): string {
...
export function mergeStrengthWorkouts(
  ...
  for (const workout of workouts) {
    for (const exercise of workout.exercises || []) {
      ...
      if (exerciseMap.has(normalizedName)) {
        ...
        existing.totalVolume = (existing.totalVolume || 0) +
          (exercise.totalVolume || 0);

        // Update top set if this one is heavier
        if (
          exercise.topSet &&
          (!existing.topSet || exercise.topSet.weight > existing.topSet.weight)
        ) {
  ...
  const isGeneric = (n?: string) =>
    !n || n.startsWith("Workout 20") || n === "Styrkepass";
  ...
  const mergedWorkout: StrengthWorkout = {
    ...
    notes: `Sammanslagen från ${workouts.length} pass: ${
      workouts.map((w) => w.name || w.sourceWorkoutName || "Okänt").join(", ")
    }`,
