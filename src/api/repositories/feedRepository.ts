...
const CATEGORY_TO_PRIVACY_KEY: Record<
  FeedEventCategory,
  keyof UserPrivacy["sharing"]
> = {
  "TRAINING": "training",
  "NUTRITION": "nutrition",
...
export class FeedRepository {
  ...
  static async createEvent(
    event: Omit<FeedEvent, "id" | "createdAt">,
  ): Promise<FeedEvent> {
  ...
  static async getUserEvents(
    ...
    for await (const entry of iter) {
      ...
      if (categories && categories.length > 0) {
        const eventCategory =
          EVENT_TYPE_TO_CATEGORY[event.type as FeedEventType];
  ...
  static async getMainFeed(
    ...
    for (const targetId of followingUserIds) {
      ...
      const subscribedCategories = preference?.subscribedCategories ||
        ["TRAINING"];
   ...
   */
  static async setFollowPreference(
    preference: Omit<FollowPreference, "id" | "createdAt" | "updatedAt">,
  ): Promise<FollowPreference> {
    const existing = await this.getFollowPreference(
      preference.followerId,
      preference.targetUserId,
    );
   ...
   */
  static async deleteFollowPreference(
    followerId: string,
    targetUserId: string,
  ): Promise<void> {
   ...
   */
  static async getAllFollowPreferences(
    followerId: string,
  ): Promise<FollowPreference[]> {
    const iter = db.list<FollowPreference>({
      prefix: ["follow_preferences", followerId],
    });
   ...
   */
  static async createDefaultFollowPreference(
    followerId: string,
    targetUserId: string,
  ): Promise<FollowPreference> {
  ...
  static async getEventCounts(
    userId: string,
  ): Promise<Record<FeedEventType, number>> {
   ...
   */
  static async updateEventVisibility(
    userId: string,
    eventId: string,
