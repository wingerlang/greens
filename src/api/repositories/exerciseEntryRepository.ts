...
export class ExerciseEntryRepository {
    ...
    async saveEntry(userId: string, entry: ExerciseEntry): Promise<void> {
        ...
        if (!entry.date || !entry.id) throw new Error("Entry must have date and id");
        await kv.set(["exercise_entries", userId, entry.date, entry.id], entry);
    }

    async getEntriesByDate(userId: string, date: string): Promise<ExerciseEntry[]> {
        const iter = kv.list<ExerciseEntry>({ prefix: ["exercise_entries", userId, date] });
    ...
    async getEntriesInRange(userId: string, startDate: string, endDate: string): Promise<ExerciseEntry[]> {
