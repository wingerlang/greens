...
export const analyticsRepository = {
  ...
  async getStats(daysBack = 30): Promise<AnalyticsStats> {
    ...
    const fetchViews = async () => {
      for await (
        const entry of kv.list<PageView>({ prefix: [KEY_PREFIX.PAGE_VIEW] }, {
          reverse: true,
        })
      ) {
    ...
    const fetchEvents = async () => {
      for await (
        const entry of kv.list<InteractionEvent>(
    ...
    const popularInteractions = Array.from(eventMap.entries()).map((
      [label, count],
    ) => ({
      label,
      count,
    ...
    const categorizePath = (path: string) => {
      if (path.startsWith("/calories") || path.startsWith("/database")) {
        return "Kalorier";
      }
      if (path.startsWith("/exercise") || path.startsWith("/strength")) {
        return "Träning";
      }
      if (path.startsWith("/planera") || path.startsWith("/planning")) {
        return "Planering";
      }
      if (
        path.startsWith("/profile") || path.startsWith("/social") ||
        path.startsWith("/settings")
      ) return "Profil & Social";
    ...
    const sessionDepth = pageViews.length > 0
      ? Number((events.length / pageViews.length).toFixed(2))
      : 0;
  ...
  async getEventsFiltered(
    { userId, type, daysBack = 7, limit = 100 }: {
      userId?: string;
      type?: string;
      daysBack?: number;
      limit?: number;
    ...
    for await (
      const entry of kv.list<InteractionEvent>({ prefix: [KEY_PREFIX.EVENT] }, {
        reverse: true,
      })
    ) {
      const e = entry.value;
  ...
  async getSessions(daysBack = 7): Promise<any[]> {
    ...
    for await (
      ...
      if (view.timestamp < session.startTime) {
        session.startTime = view.timestamp;
      }
   ...
   */
  async getUserActivityStats(daysBack = 30): Promise<
    Array<{
      userId: string;
      pageViews: number;
      events: number;
   ...
   */
  async getDailyActivity(daysBack = 30): Promise<
    Array<{
      date: string;
      pageViews: number;
      events: number;
  ...
  async getRetentionStats(daysBack = 30): Promise<any> {
    ...
    for await (
      ...
      if (
        !userFirstSeen.has(v.userId) ||
        v.timestamp < userFirstSeen.get(v.userId)!
      ) {
    ...
    userFirstSeen.forEach((firstTimestamp, userId) => {
      ...
      if (!cohorts.has(firstDateStr)) {
        cohorts.set(firstDateStr, {
          total: 0,
          retained: new Array(14).fill(0),
        });
      }
  ...
  async getAppDataStats(daysBack = 30): Promise<any> {
    ...
    for await (const entry of kv.list<any>({ prefix: ["activities"] })) {
      ...
      if (act.date >= cutoffStr && act.status === "COMPLETED") {
        cardioWorkoutCount++;
        if (act.type) {
          exerciseCounts.set(act.type, (exerciseCounts.get(act.type) || 0) + 1);
        }
        if (act.performance?.distance) {
          totalDistance += act.performance.distance;
        }
      }
    }
    ...
    return {
      nutrition: {
        topFoods,
        avgDailyCalories: mealCount > 0
          ? Math.round(totalCals / (mealCount / 3))
          : 0, // Approx 3 meals/day
  ...
  async getErrorStats(daysBack = 7): Promise<any> {
    ...
    const errors = new Map<
      string,
      { count: number; lastSeen: string; paths: Set<string> }
    >();
    ...
    for await (
      ...
      const existing = errors.get(e.label) ||
        { count: 0, lastSeen: e.timestamp, paths: new Set<string>() };
      existing.count++;
