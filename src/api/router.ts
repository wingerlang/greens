...
async function internalRouter(
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/auth")) {
      ...
      url.pathname.startsWith("/api/upload-temp") ||
      url.pathname.startsWith("/api/parse-image")
    ) {
  ...
  await logMetric("response_time", duration, {
    path: url.pathname,
    method,
    status: String(response.status),
  });
  await logMetric("request_count", 1, {
    path: url.pathname,
    method,
    status: String(response.status),
  });
