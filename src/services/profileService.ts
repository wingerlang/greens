...
export const profileService = {
   ...
   */
  async updatePrivacy(
    updates: Partial<ProfileData["privacy"]>,
  ): Promise<ProfileData["privacy"] | null> {
  ...
  async deletePR(category: string): Promise<boolean> {
    const res = await fetch(
      `${API_BASE}/user/prs/${encodeURIComponent(category)}`,
      {
        method: "DELETE",
        headers: getHeaders(),
      },
  ...
  async revokeSession(sessionToken: string): Promise<boolean> {
    const res = await fetch(
      `${API_BASE}/user/sessions/${encodeURIComponent(sessionToken)}`,
      {
        method: "DELETE",
        headers: getHeaders(),
      },
