...
export function calculateRollingLoad(
  ...
  return activities
    .filter((a) => {
      ...
      return actTime >= startTime && actTime <= new Date(endDate).getTime() &&
        a.status === "COMPLETED";
    })
...
export function generateFatigueTrends(
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const acuteLoad = calculateRollingLoad(
      activities,
      feedbackEntries,
      dateStr,
      ACUTE_WINDOW_DAYS,
    );
    const chronicLoad = calculateRollingLoad(
      activities,
      feedbackEntries,
      dateStr,
      CHRONIC_WINDOW_DAYS,
...
export function generateAdaptationSuggestions(
  ...
  const recentFeedback = feedbackEntries
    .filter((f) =>
      new Date(f.date).getTime() > now.getTime() - 14 * 24 * 60 * 60 * 1000
    )
  ...
  const consecutiveHard = recentFeedback
    ...
    .filter((f) =>
      f.perceivedDifficulty === "HARD" || f.perceivedDifficulty === "TOO_HARD"
    );
  ...
  if (consecutiveHard.length >= INJURY_RISK_CONSECUTIVE_HARD) {
    suggestions.push({
      ...
      severity: "warning",
      message:
        `Du har rapporterat ${consecutiveHard.length} tunga pass i rad. Överväg att lägga in extra vila.`,
  ...
  const avgFatigue =
    recentFeedback.reduce((sum, f) => sum + calculateFatigueScore(f), 0) /
  ...
  if (avgFatigue > HIGH_FATIGUE_THRESHOLD) {
    suggestions.push({
      ...
      type: "reduce_volume",
      severity: "critical",
      message: `Din genomsnittliga trötthet (${
        Math.round(avgFatigue)
      }%) är hög. Risk för överträning.`,
  ...
  if (recentInjuries.length > 0) {
    const locations = [
      ...new Set(recentInjuries.map((f) => f.injuryLocation).filter(Boolean)),
    ];
    suggestions.push({
      id: generateId(),
      type: "injury_risk",
      severity: "critical",
      message: `Du har rapporterat skada/smärta (${
        locations.join(", ") || "ospecificerad"
      }). Prioritera återhämtning.`,
      suggestedAction:
        "Överväg att vila helt eller byt till cross-training utan belastning.",
      createdAt: now.toISOString(),
    });
  }
  ...
  const poorSleepCount =
    recentFeedback.filter((f) => f.sleepQuality && f.sleepQuality <= 2).length;
  if (poorSleepCount >= 3) {
    suggestions.push({
      id: generateId(),
      type: "add_recovery",
      severity: "warning",
      message:
        `Du har rapporterat dålig sömn ${poorSleepCount} gånger senaste veckan. Sömn är kritisk för återhämtning.`,
      suggestedAction:
        "Prioritera 7-9 timmars sömn och undvik hårda pass vid sömnbrist.",
      createdAt: now.toISOString(),
    });
  }
  ...
  if (latestTSB < OVERTRAINING_TSB_THRESHOLD) {
    suggestions.push({
      ...
      severity: "critical",
      message: `Din träningsbalans (TSB: ${
        Math.round(latestTSB)
      }) indikerar stor belastning. Återhämtningsvecka rekommenderas.`,
...
export function calculateVolumeAdjustment(
  ...
  const avgTSB =
    recentTrends.reduce((sum, t) => sum + t.trainingStressBalance, 0) /
    recentTrends.length;
