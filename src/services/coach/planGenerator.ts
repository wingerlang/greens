...
const QUALITY_SESSION_TEMPLATES: WorkoutTemplate[] = [
    {
        ...
        createStructure: (vol, paces) => ({
            warmupKm: vol > 8 ? 3 : 2,
            mainSet: [{ reps: Math.max(1, Math.floor(vol)), distKm: 1, pace: formatPace(paces.interval), restMin: 2 }],
    ...
    },
    {
        ...
        description: 'Längre intervaller i tröskeltempo för att flytta mjölksyra-tröskeln.',
        createStructure: (vol, paces) => ({
            warmupKm: vol > 10 ? 3 : 2,
            mainSet: [{ reps: Math.max(2, Math.floor(vol / 2)), distKm: 2, pace: formatPace(paces.threshold), restMin: 1.5 }],
    ...
    },
    {
        ...
        createStructure: (vol, paces) => ({
            warmupKm: 3,
            mainSet: [{ reps: 8, distKm: 0.1, pace: formatPace(paces.repetition), restMin: 1 }],
            cooldownKm: 1
        })
    },
    {
        ...
        createStructure: (vol, paces) => ({
            warmupKm: 3,
            mainSet: [
                { reps: 1, distKm: vol / 2, pace: formatPace(paces.marathon || paces.easy * 0.9), restMin: 0 },
                { reps: 1, distKm: vol / 2, pace: formatPace(paces.threshold), restMin: 0 }
            ],
            cooldownKm: 2
        })
    }
...
const SCIENTIFIC_BENEFITS: Record<string, string> = {
    'LONG_RUN': 'Ökar mitokondriell densitet och kapillärisering för extrem uthållighet.',
    'INTERVALS': 'Höjer VO2 Max och din förmåga att arbeta under hög syreskuld.',
    'TEMPO': 'Flyttar din mjölksyratröskel så att du kan hålla högre fart längre.',
    'EASY': 'Främjar återhämtning och bygger den aeroba basen utan onödigt slitage.',
    'RECOVERY': 'Aktiv återhämtning som ökar blodgenomströmningen till trötta muskler.',
...
export function generateTrainingPlan(
    ...
    if (config.userProfile.currentForm) {
        currentVdot = calculateVDOT(config.userProfile.currentForm.distanceKm, config.userProfile.currentForm.timeSeconds);
    } else if (config.userProfile.recentRaceTime) {
        currentVdot = calculateVDOT(config.userProfile.recentRaceTime.distance, config.userProfile.recentRaceTime.timeSeconds);
    } else if (history.length > 0) {
        const runs = history.filter(a => a.type === 'Run');
        if (runs.length > 0) {
            const best = runs.sort((a, b) => (b.distance / b.moving_time) - (a.distance / a.moving_time))[0];
    ...
    const targetVdot = calculateTargetVDOT(activeGoal.type, activeGoal.targetTimeSeconds);
    ...
    const weeksUntilGoal = Math.ceil((targetDate.getTime() - today.getTime()) / (7 * 24 * 60 * 60 * 1000));
    const feasibility = assessGoalFeasibility(predictedVdot, targetVdot, weeksUntilGoal);

    // 5. Volume Analysis
    const recentWeeklyVolume = calculateRecentWeeklyVolume(history);
    let currentVolume = Math.max(25, Math.min(config.preferences.weeklyVolumeKm, recentWeeklyVolume * 1.2));
    ...
    for (let w = 0; w < weeksUntilGoal; w++) {
        ...
        const weeklyActivities = generateWeek(w, phase, currentVolume, config, paces, maxHr, restingHr, activeGoal.id, fatigueFactor, recentHardSessions, isVolumePR);
        plans.push(...weeklyActivities);

        if (phase !== 'TAPER' && phase !== 'RECOVERY') {
            currentVolume = Math.round(Math.min(currentVolume * 1.05, config.preferences.weeklyVolumeKm * 1.5) * 10) / 10;
        }
    }

    // Mark longest run in plan
    const longest = [...plans].sort((a, b) => (b.estimatedDistance || 0) - (a.estimatedDistance || 0))[0];
    if (longest) longest.isLongestInPlan = true;

    return plans;
}

function calculateTargetVDOT(goalType: string, targetTimeSecs?: number): number {
    if (!targetTimeSecs) return 40;
    // Map time to VDOT based on distance
    const dist = goalType === 'MARATHON' ? 42.195 : goalType === 'HALF_MARATHON' ? 21.097 : goalType === '10K' ? 10 : 5;
...
function calculateRecentWeeklyVolume(history: StravaActivity[]): number {
    ...
    const relevant = history.filter(a => new Date(a.start_date) > fourWeeksAgo && a.type === 'Run');
...
function generateWeek(
    ...
    const otherDays = trainingDays.filter(d => d !== longRunDay && d !== (phase !== 'BASE' && intervalIsTrainingDay ? intervalDay : -1));
    let remainingVol = Math.max(0, weeklyVolume - longDist - qualitySessionVol);

    const easyPerDay = otherDays.length > 0 ? Math.max(4, remainingVol / otherDays.length) : 0;
    ...
    trainingDays.forEach(day => {
        ...
        activityDate.setDate(activityDate.getDate() + (weekNum * 7) + ((day - activityDate.getDay() + 7) % 7));
        const dateStr = getISODate(activityDate);

        if (day === longRunDay) {
            activities.push(createActivity(dateStr, 'LONG_RUN', longDist * fatigueFactor, paces.easy, 2, maxHr, restingHr, goalId, isVolumePR));
        } else if (day === intervalDay && phase !== 'BASE') {
            activities.push(createQualitySession(dateStr, phase, qualitySessionVol * fatigueFactor, paces, maxHr, restingHr, goalId));
        } else {
            const isRecovery = fatigueFactor < 1.0 || (recentHardSessions.length >= 3 && activities.length % 2 === 0);
            activities.push(createActivity(dateStr, isRecovery ? 'RECOVERY' : 'EASY', easyPerDay * (isRecovery ? 0.7 : 1.0) * fatigueFactor, paces.easy, 1, maxHr, restingHr, goalId));
        }
    });
...
function createActivity(
    ...
    return {
        ...
        type: 'RUN',
        category,
        title: `${Math.round(dist)}km ${category === 'LONG_RUN' ? 'Långpass' : 'Distans'}`,
        description: `Spring i bekvämt tempo (${paceStr} min/km). Håll pulsen i zon ${hrZone}.`,
        scientificBenefit: SCIENTIFIC_BENEFITS[category],
        isVolumePR: isVolumePR,
        structure: {
            warmupKm: 0,
            mainSet: [{ reps: 1, distKm: Math.max(0.1, Math.round(dist * 10) / 10), pace: paceStr, restMin: 0 }],
            cooldownKm: 0
        },
        targetPace: paceStr,
        targetHrZone: hrZone,
...
function createQualitySession(
    ...
    return {
        ...
        targetPace: formatPace(template.category === 'TEMPO' ? paces.threshold : paces.interval),
        targetHrZone: 4,
