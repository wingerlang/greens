import { SleepSession, ActivitySource, UniversalActivity } from '../models/types.ts';
...
export const garminService = {
    async syncSleepData(startDate: string, endDate: string): Promise<SleepSession[]> {
        console.log(`[Garmin] Syncing sleep from ${startDate} to ${endDate}...`);
        // Simulate API delay
        await new Promise(resolve => setTimeout(resolve, 800));
        return MOCK_GARMIN_SLEEP.filter(s => s.date >= startDate && s.date <= endDate);
    },

    async syncActivities(startDate: string, endDate: string): Promise<UniversalActivity[]> {
        console.log(`[Garmin] Syncing activities from ${startDate} to ${endDate}...`);
        return [];
    }
};
