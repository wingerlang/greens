...
export class LocalStorageService implements StorageService {
  async load(): Promise<AppData> {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (cloudData && Object.keys(cloudData).length > 2) {
            ...
            if (localStored) {
              ...
              if (
                ...
                console.warn(
                  "[Storage] Cloud has no recipes but local does. Merging.",
                );
                cloudData.recipes = localData.recipes;
                if (!cloudData.foodItems || cloudData.foodItems.length < 5) {
                  cloudData.foodItems = localData.foodItems;
                }
              }
              ...
              if (localGoals.length > 0) {
                ...
                localGoals.forEach((localG) => {
                  if (!cloudIds.has(localG.id)) {
                    ...
                    console.log(
                      `[Storage] Preserving local-only goal: ${localG.name} (${localG.id})`,
                    );
          ...
          notificationService.notify(
            "error",
            "Kunde inte ladda data från servern (API Error)",
          );
        }
      } catch (e) {
        ...
        notificationService.notify(
          "error",
          "Kunde inte ladda data från servern (Nätverksfel)",
        );
      }
    }
    ...
    if (!data) {
      ...
      if (!data.foodItems || data.foodItems.length < SAMPLE_FOOD_ITEMS.length) {
        ...
        const newItems = SAMPLE_FOOD_ITEMS.filter((f) =>
      ...
      if (!data.recipes || data.recipes.length < 10) {
        ...
        const missingRecipes = SAMPLE_RECIPES.filter((r) =>
      ...
      if (token && ENABLE_CLOUD_SYNC) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            notificationService.notify(
              "error",
              "Kunde inte spara data till servern",
            );
          }
        } catch (e) {
  ...
  async addWeightEntry(entry: WeightEntry): Promise<void> {
    ...
    if (token && ENABLE_CLOUD_SYNC) {
      try {
        ...
        notificationService.notify(
          "error",
          "Kunde inte spara vikt till servern",
        );
      }
    }
  }
  ...
  async saveRecipe(recipe: any): Promise<any> {
    ...
    if (token && ENABLE_CLOUD_SYNC) {
      try {
        ...
        // Or:
        const url = recipe.createdAt === recipe.updatedAt
          ? "/api/recipes"
          : `/api/recipes/${recipe.id}`;
        const fetchMethod = recipe.createdAt === recipe.updatedAt
          ? "POST"
          : "PUT";
  ...
  async saveExerciseEntry(entry: any): Promise<any> {
    ...
    if (token && ENABLE_CLOUD_SYNC) {
      try {
        ...
        const url = entry.createdAt === entry.updatedAt
          ? "/api/exercise-entries"
          : `/api/exercise-entries/${entry.id}`;
        const fetchMethod = entry.createdAt === entry.updatedAt
          ? "POST"
          : "PUT";
    ...
    if (token && ENABLE_CLOUD_SYNC) {
      try {
        ...
        if (res.ok) {
          ...
          notificationService.notify(
            "error",
            "Kunde inte spara mått till servern",
          );
        }
      } catch (e) {
  ...
  async deleteBodyMeasurement(id: string): Promise<void> {
    ...
    data.bodyMeasurements =
      data.bodyMeasurements?.filter((m: any) => m.id !== id) || [];
    ...
    if (token && ENABLE_CLOUD_SYNC) {
      try {
        ...
        if (res.ok) {
          ...
          notificationService.notify(
            "error",
            "Kunde inte ta bort mått från servern",
          );
        }
      } catch (e) {
        ...
        notificationService.notify(
          "error",
          "Nätverksfel vid borttagning av mått",
        );
      }
    }
  }
