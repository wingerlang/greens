...
class BackupServiceImpl {
  ...
  async getSnapshots(trackId?: string): Promise<BackupSnapshot[]> {
    const snapshots = await apiRequest<BackupSnapshot[]>(
  ...
  async createSnapshot(
    ...
    const autoDescription = description ||
      this.generateAutoDescription(entityCounts, trigger);
    ...
    const result = await apiRequest<
      { success: boolean; snapshot: BackupSnapshot }
    >(
      "/api/backup/snapshots",
      {
        method: "POST",
  ...
  private generateAutoDescription(
    ...
    if (counts.strengthSessions > 0) {
      parts.push(`${counts.strengthSessions} styrkepass`);
    }
  ...
  async restore(options: RestoreOptions): Promise<RestoreResult> {
    ...
    if (options.createBackupFirst !== false) {
      try {
        const preBackup = await this.createSnapshot(
          "PRE_RESTORE",
          "Före återställning",
        );
    ...
    try {
      if (options.mode === "FULL") {
        ...
        return {
          ...
          restoredCategories: Object.keys(
            snapshot.entityCounts,
          ) as (keyof BackupEntityCounts)[],
        ...
        for (const category of categories) {
          ...
          if (
            dataKey &&
            (snapshotData as Record<string, unknown>)[dataKey] !== undefined
          ) {
