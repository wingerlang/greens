...
function getEntityLabel(
  ...
  if (typeof entity.mealType === "string") {
    return `${entity.mealType} (${entity.date || "okänt datum"})`;
  }
  if (typeof entity.type === "string") {
    return `${entity.type} (${entity.date || ""})`;
  }
...
export async function compareSnapshots(
  ...
  const fromData = await backupService.getSnapshotData(fromId) as Record<
    string,
    unknown
  >;
  const toData = await backupService.getSnapshotData(toId) as Record<
    string,
    unknown
  >;
  ...
  for (const [category, dataKey] of Object.entries(CATEGORY_DATA_KEYS)) {
    ...
    if (cat === "vitals") {
      ...
      if (
        catChanges.added > 0 || catChanges.removed > 0 ||
        catChanges.modified > 0
      ) {
...
export function getDiffStats(diff: BackupDiff): {
  ...
  const categories = Object.entries(diff.summary.byCategory).map((
    [cat, stats],
  ) => ({
    name: cat,
  ...
  return {
    total: diff.summary.totalAdded + diff.summary.totalRemoved +
