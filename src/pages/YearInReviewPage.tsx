...
export function YearInReviewPage() {
  ...
  const yearlyGoals = useMemo(() => {
    ...
    const startOfYear = new Date(Math.min(...selectedYears), 0, 1)
      .toISOString();
  ...
  const legacyActivities = useMemo(() => {
    return unifiedActivities
      ...
      .filter((
        a,
      ): a is import("../models/types").ExerciseEntry & { source: string } =>
        a !== null
      );
  }, [unifiedActivities]);

  // Data for goal calculations
  const { weightEntries = [], mealEntries = [], foodItems = [], recipes = [] } =
    useData();
  ...
  const stats = useMemo(() => {
    ...
    const typeMap = new Map<
      string,
      { count: number; time: number; dist: number }
    >();
      ...
      const score = calculatePerformanceScore(
        {
          ...
          distance: dist,
          type: a.performance?.activityType,
        } as any,
        [],
      );
    ...
    const activePercentage = totalDaysInPeriod > 0
      ? (activeDays.size / totalDaysInPeriod) * 100
      : 0;
    ...
    let bestLift = {
      weight: 0,
      exercise: "",
      activity: null as UniversalActivity | null,
    };
    ...
    yearlyStrengthSessions.forEach((s) => {
      ...
      if (
        !maxVolumeSession ||
        (s.totalVolume || 0) > (maxVolumeSession.totalVolume || 0)
      ) {
        maxVolumeSession = s;
      }
    });
    ...
    const allRuns = [...yearlyActivities]
      .filter((a: UniversalActivity) => {
        ...
        return runningTypes.some((rt) =>
          type.includes(rt) || rt.includes(type)
        );
      })
      .sort((a: UniversalActivity, b: UniversalActivity) =>
        (b.performance?.distanceKm || 0) - (a.performance?.distanceKm || 0)
      );
    ...
    const fastestRuns = [...yearlyActivities]
      .filter((a: UniversalActivity) =>
        ...
        (a.performance?.distanceKm || 0) > 5
      )
      .sort((a: UniversalActivity, b: UniversalActivity) => {
        const paceA = (a.performance?.durationMinutes || 0) /
          (a.performance?.distanceKm || 1);
        const paceB = (b.performance?.durationMinutes || 0) /
          (b.performance?.distanceKm || 1);
        return paceA - paceB;
      })
      .slice(0, 10);
    ...
    const maxScores = [...yearlyActivities]
      .map((a: UniversalActivity) => {
        ...
        return {
          activity: a,
          score: calculatePerformanceScore(mappedActivity, []),
        };
      })
      ...
      s.exercises.forEach((e) => {
        ...
        e.sets.forEach((set) => {
          if ((set.weight || 0) === maxWeight && maxWeight > 0) {
            setsAtMaxWeight++;
            if ((set.reps || 0) > maxRepsAtMaxWeight) {
              maxRepsAtMaxWeight = set.reps || 0;
            }
            if ((set.distance || 0) > maxDistanceAtMaxWeight) {
              maxDistanceAtMaxWeight = set.distance || 0;
            }
          }
        });
        if (maxWeight > 0) {
          ...
          if (!existing || maxWeight > existing.weight) {
            const isPB = strengthPBs.some((pb) =>
              ...
              (pb.type === "1rm" || pb.type === "reps" || pb.type === "3rm" ||
                pb.type === "5rm" || pb.type === "10rm")
            );

            liftMap.set(key, {
              weight: maxWeight,
              reps: maxRepsAtMaxWeight,
              distance: maxDistanceAtMaxWeight > 0
                ? maxDistanceAtMaxWeight
                : undefined,
              exercise: e.exerciseName,
              date: s.date,
              id: s.id,
    ...
    const topLifts = Array.from(liftMap.values()).sort((a, b) =>
      b.weight - a.weight
    ).slice(0, 10);
    ...
    yearlyActivities.forEach((a: UniversalActivity) => {
      ...
      const existing = dayTotals.get(date) ||
        { date, totalMinutes: 0, activities: [] as any[] };
      ...
      existing.activities.push({
        ...
        minutes,
        id: a.id,
        name: a.plan?.title || a.performance?.notes ||
          a.performance?.activityType || "Aktivitet",
        distance: a.performance?.distanceKm || 0,
        time: timeStr,
      });
  ...
  const monthlyData = useMemo(() => {
    const months = Array(12).fill(0).map((_, i) => ({
      name: new Date(2024, i, 1).toLocaleString("sv-SE", { month: "short" })
        .replace(".", ""), // Generic year
      dist: 0,
      time: 0,
      cals: 0,
      count: 0,
  ...
  const smoothedPaceData = useMemo(() => {
    ...
    const intervalDays = paceInterval === "1d"
      ? 1
      : paceInterval === "1w"
      ? 7
      : paceInterval === "2w"
    ...
    const buckets = new Map<
      number,
      { totalTime: number; totalDist: number; firstDate: Date }
    >();
    ...
    yearlyActivities.forEach((a: UniversalActivity) => {
      if (
        ...
        const curr = buckets.get(bucketIndex) ||
          { totalTime: 0, totalDist: 0, firstDate: d };
    ...
    const result = Array.from({ length: maxBuckets }, (_, i) => {
      ...
      if (!data || data.totalDist === 0) {
        return { bucket: i, pace: null, date: null };
      }
      return {
        bucket: i,
  ...
  const smoothedDurationData = useMemo(() => {
    ...
    const minYear = Math.min(...selectedYears);
    const startDate = new Date(minYear, 0, 1);
    const intervalDays = durationInterval === "1d"
      ? 1
      : durationInterval === "1w"
      ? 7
      : durationInterval === "2w"
    ...
    const buckets = new Map<
      number,
      { totalTime: number; count: number; firstDate: Date }
    >();
    ...
    yearlyActivities.forEach((a: UniversalActivity) => {
      ...
      const curr = buckets.get(bucketIndex) ||
        { totalTime: 0, count: 0, firstDate: d };
      buckets.set(bucketIndex, {
        totalTime: curr.totalTime + (a.performance?.durationMinutes || 0),
        count: curr.count + 1,
        firstDate: curr.firstDate < d ? curr.firstDate : d,
    ...
    const result = Array.from({ length: maxBuckets }, (_, i) => {
      ...
      if (!data || data.count === 0) {
        return { bucket: i, avgDuration: 0, date: null };
      }
      return {
        bucket: i,
    ...
    selectedYears.forEach((year) => {
      ...
      yearlyActivities.forEach((a: UniversalActivity) => {
        ...
        const existing = activityMap.get(date) ||
          { minutes: 0, cardio: 0, strength: 0, count: 0, labels: [] };
        ...
        existing.labels.push(
          `${mins}m ${a.performance?.activityType || "Aktivitet"}`,
        );
      ...
      <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 space-y-12 animate-in fade-in duration-700">
        ...
        <header className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/5 pb-6">
          <div>
            ...
            <p className="text-slate-400 uppercase tracking-widest font-bold mt-2">
          ...
          <div className="flex gap-2 bg-slate-900 border border-white/10 rounded-lg p-1 overflow-x-auto max-w-full">
            {availableYears.map((y) => (
              <button
                ...
                className={`px-4 py-2 rounded-md font-bold text-sm transition-all whitespace-nowrap ${
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5 relative overflow-hidden group hover:border-emerald-500/30 transition-all">
            ...
            <p className="text-3xl font-black text-white">
              {Math.round(stats.totalDist).toLocaleString()}{" "}
              <span className="text-lg text-emerald-400">km</span>
            </p>
          </div>
          <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5 relative overflow-hidden group hover:border-cyan-500/30 transition-all">
            ...
            <p className="text-3xl font-black text-white">
              {Math.round(stats.totalTime / 60).toLocaleString()}{" "}
              <span className="text-lg text-cyan-400">h</span>
            </p>
          </div>
          <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5 relative overflow-hidden group hover:border-purple-500/30 transition-all">
            ...
            <p className="text-3xl font-black text-white">
              {Math.round(stats.totalTonnage / 1000).toLocaleString()}{" "}
              <span className="text-lg text-purple-400">ton</span>
            </p>
          </div>
          ...
          <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/5 relative overflow-hidden group hover:border-indigo-500/30 transition-all">
            ...
            <p className="text-3xl font-black text-white">
              {stats.totalSessions}{" "}
              <span className="text-lg text-indigo-400">st</span>
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {yearlyGoals.map((goal) => {
                ...
                const isFailed = !isCompleted &&
                  ...
                  goal.period !== "daily" && goal.period !== "weekly"; // Simple fail check
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs font-bold">
                        <span>
                          {Math.round(progress.current)} {goal.targets[0]?.unit}
                        </span>
          ...
          <div className="bg-slate-900/30 border border-white/5 p-6 rounded-3xl flex flex-col items-center justify-center">
            ...
            <div className="w-full h-64 relative">
                ...
                <PieChart>
                  ...
                  >
                    {stats.types.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={(COLORS as any)[entry.name.toLowerCase()] ||
                          COLORS.other}
                      />
                    ))}
                  </Pie>
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                ...
                <span className="block text-[10px] text-white uppercase tracking-widest">
                  Timmar
                </span>
              </div>
            </div>
            <div className="w-full space-y-2 mt-4">
              {stats.types.sort((a, b) => b.count - a.count).slice(0, 4).map(
                (t) => (
                  <div
                    key={t.name}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{
                          backgroundColor:
                            (COLORS as any)[t.name.toLowerCase()] ||
                            COLORS.other,
                        }}
                      >
                      </div>
                    ...
                    <span className="font-bold text-white">
                      {t.count}{" "}
                      ({Math.round(t.count / stats.totalSessions * 100)}%)
                    </span>
                  </div>
                ),
              )}
        ...
        <div className="bg-slate-900/30 border border-white/5 p-6 rounded-3xl">
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            {(() => {
              const peak = hourlyData.reduce(
                (max, h) => h.frequency > max.frequency ? h : max,
                hourlyData[0],
              );
              // Morgon: 04:00-10:59 (index 4-10)
              const totalMorning = hourlyData.slice(4, 11).reduce(
                (sum, h) => sum + h.frequency,
                0,
              );
              // Lunch: 11:00-13:30 (index 11-13, partial 14)
              const totalLunch = hourlyData.slice(11, 14).reduce(
                (sum, h) => sum + h.frequency,
                0,
              );
              // Eftermiddag: 13:30-17:00 (index 14-16, partial 13)
              const totalAfternoon = hourlyData.slice(14, 17).reduce(
                (sum, h) => sum + h.frequency,
                0,
              );
              // Kväll: 17:00-23:30 (index 17-23)
              const totalEvening = hourlyData.slice(17, 24).reduce(
                (sum, h) => sum + h.frequency,
                0,
              );
              // Natt: 23:30-04:00 (index 0-3)
              const totalNight = hourlyData.slice(0, 4).reduce(
                (sum, h) => sum + h.frequency,
                0,
              );
              return (
          ...
          {selectedHour !== null && (
            <div className="mt-8 animate-in fade-in slide-in-from-top-4 duration-300">
              <h4 className="text-lg font-bold mb-4 flex items-center gap-2">
                <span
                  className="w-3 h-3 rounded-full"
                  style={{ backgroundColor: getPeriodColor(selectedHour) }}
                >
                </span>
                Aktiviteter kl {selectedHour.toString().padStart(2, "0")}:00 -
                {" "}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {unifiedActivities
                  .map((a) => {
                    ...
                    const resolved = (a as any)._mergeData?.universalActivity ||
                      a;
                  ...
                  .filter(({ resolved }) => {
                    ...
                    const hasExplicitTime =
                      !!resolved.performance?.startTimeLocal &&
                      !resolved.performance.startTimeLocal.endsWith(
                        "T00:00:00.000Z",
                      ) &&
                      !resolved.performance.startTimeLocal.endsWith(
                        "T00:00:00Z",
                    ...
                    const d = new Date(
                      resolved.performance?.startTimeLocal || resolved.date,
                    );
                  ...
                  .sort((a, b) =>
                    new Date(b.resolved.date).getTime() -
                    new Date(a.resolved.date).getTime()
                  )
                  .map(({ original: a, resolved }) => (
                    ...
                    >
                      <div className="flex flex-col overflow-hidden">
                        <div className="flex items-center gap-2">
                          {(resolved as any).source === "strava" ||
                            (resolved.performance?.source?.source ===
                                "strava") && (
                                <span className="text-[10px] bg-[#FC4C02]/20 text-[#FC4C02] px-1 rounded font-bold">
                                  Strava
                                </span>
                              )}
                          {(resolved as any).source === "garmin" && (
                            <span className="text-[10px] bg-[#000000]/20 text-[#555] px-1 rounded font-bold bg-white/10">
                              Garmin
                            </span>
                          )}
                          {(resolved as any).type === "planned" && (
                            <span className="text-[10px] bg-slate-700 text-slate-300 px-1 rounded font-bold">
                              Planned
                            </span>
                          )}
                          <span className="text-xs font-bold text-slate-300 truncate group-hover:text-white transition-colors">
                            {
                              ...
                              resolved.plan?.title ||
                              ((resolved as any).performance?.activityType ===
                                  "strength"
                                ? "Styrkepass"
                                : (resolved as any).performance
                        ...
                        {resolved.performance?.distanceKm
                          ? (
                            <span className="text-sm font-black text-white">
                              {resolved.performance.distanceKm.toFixed(1)}{" "}
                          ...
                          : (
                            <span className="text-sm font-black text-white">
                              {resolved.performance?.durationMinutes ||
                                (resolved as any).durationMinutes || 0}{" "}
                              <span className="text-[10px] text-slate-500 font-normal">
                                min
                              </span>
                            </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-slate-900/30 border border-white/5 p-6 rounded-3xl">
              ...
              <div className="w-full">
                <WeeklyVolumeChart
                  workouts={strengthWorkoutsForChart}
            ...
            <div className="bg-slate-900/30 border border-white/5 p-6 rounded-3xl">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                <span>🏃</span> {selectedYears.length > 1
                  ? "Distans per månad (Löpning)"
                  : "Distans per vecka (Löpning)"}
              </h3>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={smoothedPaceData}>
                    ...
                    <YAxis
                      ...
                      tickFormatter={(val) =>
                        `${Math.floor(val)}:${
                          Math.round((val % 1) * 60).toString().padStart(2, "0")
                        }`}
                    />
                    <Tooltip
                      contentStyle={{
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {stats.longestRaces.length > 0 && (
                <div className="bg-slate-900/50 border border-rose-500/10 rounded-3xl p-6 flex flex-col space-y-4">
                  <p className="text-rose-400 text-sm font-bold uppercase tracking-widest text-center mb-2">
                    -- Längsta Tävlingar --
                  </p>
                  <div className="space-y-3">
                    {stats.longestRaces.slice(
                      0,
                      expandedCards["races"] ? 10 : 5,
                    ).map((a: UniversalActivity, i: number) => (
                      <div
                        key={i}
                        className={`cursor-pointer group flex items-center justify-between transition-all hover:scale-[1.02] ${
                          i === 0
                            ? "bg-rose-500/10 p-3 rounded-2xl border border-rose-500/20 shadow-lg shadow-rose-500/5"
                            : "py-1"
                        }`}
                        onClick={() => setSelectedActivity(a)}
                      >
                        <div className="flex flex-col flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            {(() => {
                              const type = a.performance?.activityType ||
                                "other";
                              const typeInfo = EXERCISE_TYPES.find((t) =>
                                t.type === type
                          ...
                          <div className="flex items-baseline gap-2 mt-0.5">
                            ...
                            >
                              {a.performance?.distanceKm?.toFixed(1)}{" "}
                              <span className="text-xs text-slate-500">km</span>
                            </span>
                            <span className="text-[10px] text-slate-500 font-bold uppercase">
                              • {formatSwedishDate(a.date)}
                            </span>
                            {a.performance?.durationMinutes &&
                              a.performance?.distanceKm && (
                              <span className="text-[10px] text-rose-400 font-bold">
                                • {formatPace(
                                  (a.performance.durationMinutes * 60) /
                                    a.performance.distanceKm,
                                ).replace("/km", "")} min/km
                              </span>
                            )}
                            {new Date(a.date).getFullYear() ===
                              ...
                              <span className="ml-1 text-[10px] text-rose-300 animate-pulse bg-rose-500/10 px-1 rounded border border-rose-500/20">
                                ✨ I år
                              </span>
                            )}
                          </div>
                        ...
                        <span
                          className={`text-xs font-black p-2 rounded-full ${
                            i === 0
                  ...
                  {stats.longestRaces.length > 5 && (
                    ...
                    >
                      {expandedCards["races"]
                        ? "▲ Visa mindre"
                        : `▼ Visa ${stats.longestRaces.length - 5} till`}
                    </button>
                  )}
                </div>
              )}
              ...
              <div className="bg-slate-900/50 border border-emerald-500/10 rounded-3xl p-6 flex flex-col space-y-4">
                <p className="text-emerald-400 text-sm font-bold uppercase tracking-widest text-center mb-2">
                  -- Längsta Träningspass --
                </p>
                <div className="space-y-3">
                  {stats.longestTrainingRuns.slice(
                    0,
                    expandedCards["training"] ? 10 : 5,
                  ).map((a: UniversalActivity, i: number) => (
                    <div
                      key={i}
                      className={`cursor-pointer group flex items-center justify-between transition-all hover:scale-[1.02] ${
                        i === 0
                          ? "bg-emerald-500/10 p-3 rounded-2xl border border-emerald-500/20 shadow-lg shadow-emerald-500/5"
                          : "py-1"
                      }`}
                      onClick={() => setSelectedActivity(a)}
                    >
                      <div className="flex flex-col flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          {(() => {
                            const type = a.performance?.activityType || "other";
                            const typeInfo =
                              EXERCISE_TYPES.find((t) => t.type === type) ||
                              EXERCISE_TYPES.find((t) => t.type === "other");
                            return (
                              <span className="text-[8px] font-black uppercase px-1.5 py-0.5 bg-slate-800 text-slate-500 rounded border border-white/5 tracking-tighter shrink-0 flex items-center gap-1">
                                <span>{typeInfo?.icon}</span>
                                {typeInfo?.label}
                          ...
                          <span
                            ...
                            title={a.plan?.title || a.performance?.notes ||
                              a.performance?.activityType || "Löpning"}
                          >
                            {a.plan?.title || a.performance?.notes ||
                              a.performance?.activityType || "Löpning"}
                          </span>
                        ...
                        <div className="flex items-baseline gap-2 mt-0.5">
                          ...
                          {new Date(a.date).getFullYear() ===
                              new Date().getFullYear() && (
                            <span className="ml-1 text-[10px] text-emerald-300 animate-pulse bg-emerald-500/10 px-1 rounded border border-emerald-500/20">
                              ✨ I år
                            </span>
                          )}
                        </div>
                ...
                {stats.longestTrainingRuns.length > 5 && (
                  ...
                  >
                    {expandedCards["training"]
                      ? "▲ Visa mindre"
                      : `▼ Visa ${
                        Math.min(5, stats.longestTrainingRuns.length - 5)
                      } till`}
                  </button>
                )}
              </div>
              ...
              <div className="bg-slate-900/50 border border-cyan-500/10 rounded-3xl p-6 flex flex-col space-y-4">
                <p className="text-cyan-400 text-sm font-bold uppercase tracking-widest text-center mb-2">
                  -- Snabbaste Tempo --
                </p>
                <div className="space-y-3">
                  {stats.fastestRuns.slice(0, expandedCards["fastest"] ? 10 : 5)
                    .map((a, i) => (
                      <div
                        key={i}
                        className={`cursor-pointer group flex items-center justify-between transition-all hover:scale-[1.02] ${
                          i === 0
                            ? "bg-cyan-500/10 p-3 rounded-2xl border border-cyan-500/20 shadow-lg shadow-cyan-500/5"
                            : "py-1"
                        }`}
                        onClick={() => setSelectedActivity(a)}
                      >
                          ...
                          <div className="flex items-baseline gap-2 mt-0.5">
                            ...
                            <span
                              className={`text-[10px] font-bold ${
                                i === 0 ? "text-cyan-300" : "text-cyan-400"
                              }`}
                            >
                            ...
                            {new Date(a.date).getFullYear() ===
                                new Date().getFullYear() && (
                              <span className="ml-1 text-[10px] text-cyan-300 animate-pulse bg-cyan-500/10 px-1 rounded border border-cyan-500/20">
                                ✨ I år
                              </span>
                            )}
                          </div>
                ...
                {stats.fastestRuns.length > 5 && (
                  ...
                  >
                    {expandedCards["fastest"]
                      ? "▲ Visa mindre"
                      : `▼ Visa ${
                        Math.min(5, stats.fastestRuns.length - 5)
                      } till`}
                  </button>
                )}
              </div>
              ...
              <div className="bg-slate-900/50 border border-purple-500/10 rounded-3xl p-6 flex flex-col space-y-4">
                <p className="text-purple-400 text-sm font-bold uppercase tracking-widest text-center mb-2">
                  -- Tyngsta Lyft --
                </p>
                <div className="space-y-3">
                  {stats.topLifts.slice(0, expandedCards["lifts"] ? 10 : 5).map(
                    (l, i) => (
                      <div
                        key={i}
                        className={`cursor-pointer group flex items-center justify-between transition-all hover:scale-[1.02] ${
                          i === 0
                            ? "bg-purple-500/10 p-3 rounded-2xl border border-purple-500/20 shadow-lg shadow-purple-500/5"
                            : "py-1"
                        }`}
                        onClick={() => {
                          const session = yearlyStrengthSessions.find((s) =>
                            s.id === l.id
                          );
                          if (session) {
                            setSelectedActivity(
                              session as any as UniversalActivity,
                            );
                          }
                        }}
                      >
                        <div className="flex flex-col">
                          ...
                          >
                            ...
                            <span className="text-xs text-purple-400 ml-1">
                              × {l.distance && (l.reps <= 1)
                                ...
                                : l.reps}
                              {l.setsAtMaxWeight > 1 &&
                                ` [${l.setsAtMaxWeight} set]`}
                            </span>
                            {l.isPB && (
                              <span className="ml-2 text-[10px] bg-amber-500 text-slate-900 px-1 rounded font-black align-middle shadow-sm shadow-amber-500/50">
                                🏆 PB
                              </span>
                            )}
                          </span>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-slate-600 font-bold uppercase">
                              {formatSwedishDate(l.date)}
                            </span>
                            <span className="text-[10px] text-slate-400/60 font-medium italic truncate">
                              ({getRelativeTime(l.date)})
                            </span>
                ...
                {stats.topLifts.length > 5 && (
                  ...
                  >
                    {expandedCards["lifts"]
                      ? "▲ Visa mindre"
                      : `▼ Visa ${Math.min(5, stats.topLifts.length - 5)} till`}
                  </button>
                )}
              </div>
              ...
              <div className="bg-slate-900/50 border border-fuchsia-500/10 rounded-3xl p-6 flex flex-col space-y-4">
                <p className="text-fuchsia-400 text-sm font-bold uppercase tracking-widest text-center mb-2">
                  -- Största Volympass --
                </p>
                <div className="space-y-3">
                  {stats.topVolumeSessions.slice(
                    0,
                    expandedCards["volume"] ? 10 : 5,
                  ).map((s, i) => (
                    <div
                      key={i}
                      className={`cursor-pointer group flex items-center justify-between transition-all hover:scale-[1.02] ${
                        i === 0
                          ? "bg-fuchsia-500/10 p-3 rounded-2xl border border-fuchsia-500/20 shadow-lg shadow-fuchsia-500/5"
                          : "py-1"
                      }`}
                      onClick={() =>
                        setSelectedActivity(s as any as UniversalActivity)}
                    >
                      <div className="flex flex-col flex-1 min-w-0">
                        <span
                          className={`font-black ${
                        ...
                        <div className="flex flex-wrap gap-1 mt-1 mb-1">
                          {s.exercises.slice(0, 3).map((e, idx) => (
                            <span
                              key={idx}
                              className="text-[9px] px-1 bg-slate-800 text-slate-400 rounded border border-white/5 truncate max-w-[80px]"
                            >
                              {e.exerciseName}
                            </span>
                          ))}
                          {s.exercises.length > 3 && (
                            <span className="text-[9px] text-slate-600">
                              +{s.exercises.length - 3} till
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] text-slate-500 font-bold uppercase">
                            {formatSwedishDate(s.date)}
                          </span>
                          <span className="text-[10px] text-slate-400/60 font-medium italic">
                            ({getRelativeTime(s.date)})
                          </span>
                          {new Date(s.date).getFullYear() ===
                              new Date().getFullYear() && (
                            <span className="text-[10px] text-fuchsia-300 animate-pulse">
                              ✨ I år
                            </span>
                          )}
                        </div>
                ...
                {stats.topVolumeSessions.length > 5 && (
                  ...
                  >
                    {expandedCards["volume"]
                      ? "▲ Visa mindre"
                      : `▼ Visa ${
                        Math.min(5, stats.topVolumeSessions.length - 5)
                      } till`}
                  </button>
                )}
              </div>
          ...
          {stats.biggestTrainingDays.length > 0 && (() => {
            ...
            const displayedDays = stats.biggestTrainingDays.slice(
              0,
              expandedCards["days"] ? 15 : 6,
            );
            const totals = displayedDays.reduce(
              (
                acc: { running: number; cycling: number; time: number },
                day: any,
              ) => {
              ...
              <div className="mt-6 bg-gradient-to-br from-slate-900/80 via-slate-900/60 to-amber-950/20 border border-amber-500/20 rounded-2xl p-5 flex flex-col space-y-4 shadow-2xl shadow-amber-500/5">
                ...
                <div className="flex items-center justify-center gap-6 text-xs">
                  ...
                  <div className="flex items-center gap-1.5 text-amber-400/80">
                    <span>⏱️</span>
                    <span className="font-black text-sm">
                      {Math.floor(totals.time / 60)}h{" "}
                      {Math.round(totals.time % 60)}m
                    </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {displayedDays.map((day: any, i: number) => {
                    ...
                    // Day-specific totals
                    const dayTotals = day.activities.reduce(
                      (
                        acc: { running: number; cycling: number; time: number },
                        act: any,
                      ) => {
                        if (act.type === "running") {
                          acc.running += act.distance || 0;
                        }
                        if (act.type === "cycling") {
                          acc.cycling += act.distance || 0;
                        }
                        acc.time += act.minutes || 0;
                        return acc;
                      },
                          ...
                          <div className="flex flex-col gap-0.5">
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-baseline gap-2">
                                ...
                                >
                                  ...
                                  {Math.round(day.totalMinutes % 60)}m
                                </span>
                            ...
                            <span
                              className={`text-[10px] font-bold uppercase tracking-wider ${
                                isTop3 ? "text-amber-400/70" : "text-slate-500"
                              }`}
                            >
                          ...
                          {day.activities.map((act: any, idx: number) => {
                            ...
                            return (
                              <div
                                key={idx}
                                onClick={() => {
                                  const fullAct = unifiedActivities.find((u) =>
                                    u.id === act.id
                                  ) || yearlyActivities.find((ya) =>
                                    ya.id === act.id
                                  );
                                  if (fullAct) {
                                    setSelectedActivity(
                                      fullAct as UniversalActivity,
                                    );
                                  }
                                }}
                              ...
                              >
                                <span
                                  className={`text-xs font-mono px-1.5 py-0.5 rounded ${colors.text} ${colors.bg}`}
                                >
                                  {act.time || "—"}
                                </span>
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                  {act.distance > 0 && (
                                    <span
                                      className={`text-xs font-black px-1.5 py-0.5 rounded ${colors.bg} ${colors.text}`}
                                    >
                ...
                {stats.biggestTrainingDays.length > 6 && (
                  ...
                  >
                    ...
                    {expandedCards["days"]
                      ? "Visa färre"
                      : `Visa ${
                        Math.min(9, stats.biggestTrainingDays.length - 6)
                      } till`}
                  </button>
                )}
              </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-2 gap-4">
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-400">
                ...
                <div
                  ...
                  title="Längsta paus mellan två träningspass under perioden"
                >
                  <span>🔥</span>{" "}
                  <span>
                    Paus:{" "}
            ...
            {isFlowMode
              ? (
                <div className="bg-slate-900/40 border border-white/10 rounded-3xl overflow-hidden relative w-full mb-8">
                  <div className="flex flex-wrap justify-center gap-0 select-none">
                    {[...selectedYears].sort((a, b) => b - a).map(
                      (year, yearIdx) => {
                        const isEven = yearIdx % 2 === 0;
                        // Calculate year info for focus mode
                        const yearActivities = yearlyActivities.filter((a) =>
                          new Date(a.date).getFullYear() === year
                        );
                        const yearStrength = yearlyStrengthSessions.filter(
                          (s) => new Date(s.date).getFullYear() === year,
                        );
                        const sessionsInYear = yearActivities.length +
                          yearStrength.length;

                        const yearGrid = yearlyGrids[year] || [];
                        const activeInYear = yearGrid.filter((d) =>
                          d.minutes > 0
                        ).length;
                        ...
                        if (isThisYear) {
                          const start = new Date(year, 0, 1);
                          const now = new Date();
                          elapsedInYear = Math.floor(
                            (now.getTime() - start.getTime()) /
                              (1000 * 60 * 60 * 24),
                          ) + 1;
                        }

                        const percInYear = elapsedInYear > 0
                          ? (activeInYear / elapsedInYear) * 100
                          : 0;
                        ...
                        yearGrid.filter((d) => d.minutes > 0).forEach((d) => {
                          ...
                          if (lastDateYear) {
                            const diff = Math.ceil(
                              (curr.getTime() - lastDateYear.getTime()) /
                                (1000 * 60 * 60 * 24),
                            );
                        ...
                        return (
                          <div
                            key={year}
                            onClick={() =>
                              setFocusedYear(isFocused ? null : year)}
                            className={`flex-1 min-w-[300px] md:min-w-0 md:basis-1/2 flex items-stretch border-white/5 cursor-pointer transition-all duration-300 ${
                              isEven
                                ? "md:border-r justify-end"
                                : "justify-start"
                            } ${yearIdx >= 2 ? "border-t" : ""} ${
                              isFocused
                                ? "bg-amber-500/5 ring-inset ring-1 ring-amber-500/30"
                                : "hover:bg-white/[0.02]"
                            } group`}
                          >
                            {/* Left Label (Column 1) */}
                            {isEven && (
                              <div
                                className={`flex flex-col items-center justify-center py-1 px-2 border-r border-white/5 transition-colors shrink-0 ${
                                  isFocused
                                    ? "bg-amber-500/20"
                              ...
                              >
                                <span
                                  className={`text-[10px] font-black [writing-mode:vertical-lr] rotate-180 py-2 ${
                                    isFocused
                                      ? "text-amber-400"
                                      : "text-slate-500 group-hover:text-amber-500/50"
                                  }`}
                                >
                                  {year}
                                </span>
                            ...
                            <div className="flex flex-col p-1 gap-1">
                              <div className="flex gap-[1px] bg-white/[0.01]">
                                {Array.from({ length: 53 }).reverse().map(
                                  (_, weekIndexOrigin) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        {Array.from({ length: 7 }).map(
                                          (_, dayIndex) => {
                                            const dayData = yearlyGrids[year]
                                            ...
                                            if (dayData.minutes > 0) {
                                              if (isTriple) {
                                                intensityClass =
                                                  "bg-gradient-to-br from-amber-400 to-rose-500 shadow-lg shadow-amber-500/20";
                                              } else if (isDouble) {
                                                intensityClass =
                                                  "bg-gradient-to-br from-emerald-500 to-purple-600 shadow-md shadow-emerald-500/10";
                                ...
                                <div className="flex gap-4">
                                  <div className="flex flex-col">
                                    <span className="text-[10px] font-bold text-white uppercase tracking-tighter">
                                      {activeInYear} / {elapsedInYear}{" "}
                                      {isThisYear ? `(av ${totalInYear})` : ""}
                                      {" "}
                                      aktiva
                                    </span>
                                    <span className="text-[8px] text-slate-500 uppercase font-black">
                                      {sessionsInYear} pass totalt
                                    </span>
                                  </div>
                            ...
                            {!isEven && (
                              <div
                                className={`flex flex-col items-center justify-center py-1 px-2 border-l border-white/5 transition-colors shrink-0 ${
                                  isFocused
                                    ? "bg-amber-500/20"
                                    : "bg-white/[0.03] group-hover:bg-amber-500/10"
                                }`}
              ...
              : (
                [...selectedYears].sort((a, b) => b - a).map((year) => (
                  <div
                    key={year}
                    className="bg-slate-900/30 border border-white/5 p-6 rounded-3xl overflow-hidden"
                  >
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="text-[10px] text-slate-600 font-bold uppercase">
                        {yearlyGrids[year]?.filter((d: any) => d.minutes > 0)
                          ?.length || 0} aktiva dagar
                      </span>
                    </div>
                      ...
                      {Array.from({ length: 53 }).map((_, weekIndex) => (
                        ...
                        >
                          {Array.from({ length: 7 }).map((_, dayIndex) => {
                            ...
                            const isDouble = dayData.cardio > 0 &&
                              dayData.strength > 0;
                            const isTriple = dayData.count >= 3;
                            ...
                            if (dayData.minutes > 0) {
                              if (isTriple) {
                                intensityClass =
                                  "bg-gradient-to-br from-amber-400 to-rose-500 shadow-lg";
                              } else if (isDouble) {
                                intensityClass =
                                  "bg-gradient-to-br from-emerald-500 to-purple-600 shadow-md";
                              } else if (dayData.cardio > 0) {
                                intensityClass = dayData.minutes < 30
                                  ? "bg-emerald-900"
                                  : dayData.minutes < 60
                                  ? "bg-emerald-700"
                                  : dayData.minutes < 90
        ...
        {selectedActivity && (
          <ActivityDetailModal
            activity={(() => {
              ...
              if (
                !(selectedActivity as any).performance &&
                (selectedActivity as any).type
              ) {
              ...
              return mapUniversalToLegacyEntry(selectedActivity) || {
                ...
                durationMinutes:
                  (selectedActivity.performance?.durationMinutes || 0),
                intensity: "moderate",
                caloriesBurned: (selectedActivity.performance?.calories || 0),
                source:
                  (selectedActivity.performance?.source?.source || "generated"),
                title: selectedActivity.plan?.title ||
                  (selectedActivity as any).title ||
                  (selectedActivity as any).name ||
                  (selectedActivity.performance?.activityType
                    ? `${
                      selectedActivity.performance.activityType.charAt(0)
                        .toUpperCase()
                    }${selectedActivity.performance.activityType.slice(1)}`
                    : undefined) ||
