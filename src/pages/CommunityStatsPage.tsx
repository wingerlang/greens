...
import {
  CommunityStats,
  statisticsService,
} from "../services/statisticsService.ts";
...
export function CommunityStatsPage() {
  ...
  function StatCard(
    ...
    return (
      <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex flex-col items-center text-center hover:bg-slate-800/50 transition-colors shrink-0 min-w-[160px]">
        ...
        <div
          className={`text-3xl lg:text-4xl font-black ${color} mb-1 tracking-tight`}
        >
          {value}
          <span className="text-lg text-gray-500 ml-1 font-bold">{unit}</span>
        </div>
  ...
  // Global Filters
  const [timeFilter, setTimeFilter] = useState<"all" | "12m" | "2026" | "1m">(
    "all",
  );
  const [genderFilter, setGenderFilter] = useState<
    "all" | "male" | "female" | "unknown"
  >("all"); // Visual placebo for now if backend doesn't support
  const [relativeFilter, setRelativeFilter] = useState<"absolute" | "weight">(
    "absolute",
  );
  const [rmMode, setRmMode] = useState<"1rm" | "1erm">("1erm"); // 1RM (actual) vs 1eRM (estimated)
  const [exerciseSortMode, setExerciseSortMode] = useState<
    "popular" | "strongest" | "weakest"
  >("popular");
  ...
  const selectedExerciseData = selectedExercise
    ? stats.strength.exercises[selectedExercise]
    : null;
  ...
  const lifetimeDistance = filteredActivities.reduce(
    (acc, e) => acc + (e.distance || 0),
    0,
  );
  const lifetimeHours =
    filteredActivities.reduce((acc, e) => acc + (e.durationMinutes || 0), 0) /
    60;

  // Tonnage: Check relative filter
  let rawTonnage =
    filteredActivities.reduce((acc, e) => acc + (e.tonnage || 0), 0) +
    filteredStrength.reduce((acc, s) => acc + (s.totalVolume || 0), 0);
  if (relativeFilter === "weight") rawTonnage = rawTonnage / userWeight;
  const lifetimeTonnage = rawTonnage;

  const lifetimeCalories = filteredActivities.reduce(
    (acc, e) => acc + (e.caloriesBurned || 0),
    0,
  );

  // Calculate time span in filter for proper per-month averages
  const allDates = filteredActivities.map((e) => new Date(e.date).getTime())
    .sort((a, b) => a - b);
  const firstLogDate = allDates.length > 0 ? allDates[0] : Date.now();
  const lastLogDate = allDates.length > 0
    ? allDates[allDates.length - 1]
    : Date.now();

  // Weeks in filter for weekly average
  const weeksInFilter = Math.max(
    1,
    (Date.now() - firstLogDate) / (1000 * 60 * 60 * 24 * 7),
  );
  ...
  const monthsInFilter = Math.max(
    1,
    (lastLogDate - firstLogDate) / (1000 * 60 * 60 * 24 * 30.44),
  ...
  let totalFilteredTonnage = filteredStrength.reduce(
    (acc, s) => acc + (s.totalVolume || 0),
    0,
  );
  if (relativeFilter === "weight") {
    totalFilteredTonnage = totalFilteredTonnage / userWeight;
  }
  ...
  if (relativeFilter === "weight") {
    currentMonthTonnage = currentMonthTonnage / userWeight;
  }
  ...
  const getMonthlyTrendData = () => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      result.push({
        ...
        distance: monthActivities.reduce(
          (acc, e) => acc + (e.distance || 0),
          0,
        ),
  ...
  const userAvgDuration = lifetimeSessions > 0
    ? (lifetimeHours * 60) / lifetimeSessions
    : 0;
  ...
  const getUserAvgTonnage = (exerciseName: string) => {
    ...
    const totalTonnage = matches.reduce((acc, s) => {
      ...
      let tonnage = ex?.sets.reduce((setAcc, set) =>
        setAcc + (set.weight * set.reps), 0) || 0;
      if (relativeFilter === "weight") {
        tonnage = tonnage / userWeight;
      }
      return acc + tonnage;
    }, 0);
  ...
  const getUserCardioPBWithDetails = (
    distanceKm: number,
  ): CardioPBDetails | null => {
    const tolerance = getToleranceForDistance(distanceKm);
    const matches = filteredActivities.filter((e) =>
      e.distance && Math.abs(e.distance - distanceKm) <= tolerance &&
      e.durationMinutes > 0
    );
  ...
  const calculateRadarData = () => {
    ...
    const scoreFrequency = Math.min(
      100,
      (userWorkoutsPerMonthTotal /
        (stats.averages.workoutsPerUserPerMonth || 1)) * 50,
    );

    const userTotalTonnage = strengthSessions.reduce(
      (acc, s) => acc + (s.totalVolume || 0),
      0,
    );
    const scoreVolume = Math.min(
      100,
      (userTotalTonnage / (stats.averages.tonnagePerUserPerMonth * 10 || 1)) *
        50,
    ); // Scale by 10 for "all time" vs community monthly avg
    ...
    strengthSessions.forEach((s) =>
      s.exercises.forEach((e) =>
        uniqueExercises.add(e.exerciseName.toLowerCase())
      )
    );
    ...
    return [
      ...
      {
        subject: "Kondition",
        A: Math.round(scoreCardio),
        B: 50,
        fullMark: 100,
      },
      {
        subject: "Frekvens",
        A: Math.round(scoreFrequency),
        B: 50,
        fullMark: 100,
      },
      {
        subject: "Mångsidighet",
        A: Math.round(scoreVariety),
        B: 50,
        fullMark: 100,
      },
    ];
    ...
    return (
      <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          ...
          />
          <StatCard
            label={relativeFilter === "weight"
              ? "Volym / Kroppsvikt"
              : "Total Volym"}
            value={formatTons(lifetimeTonnage)}
            unit=""
            icon="🏋️‍♂️"
            color="text-rose-400"
          />
          ...
          <StatCard
            label="Totala Reps"
            value={strengthSessions.reduce((acc: number, s) =>
              acc + s.exercises.reduce((exAcc: number, ex) =>
                exAcc +
                ex.sets.reduce((setAcc: number, set) =>
                  setAcc + (set.reps || 0), 0), 0), 0).toLocaleString()}
            unit="st"
            icon="🔢"
            color="text-lime-400"
          />
          <StatCard
            label="Höjdmeter"
            value={unifiedActivities.reduce(
              (acc, e) =>
                acc + (e.elevationGain || 0),
              0,
            ).toLocaleString()}
            unit="m"
            icon="⛰️"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-1 bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <span className="bg-emerald-500/20 text-emerald-500 p-2 rounded-lg text-xs">
                AI Profile
              </span>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 h-full">
              ...
              <div className="space-y-10">
                <ComparisonBar
                  ...
                  maxValue={Math.max(
                    stats.averages.workoutsPerUserPerMonth * 2,
                    userWorkoutsPerMonthTotal,
                  )}
                  color="#f472b6"
                  explanation={`Din data (${
                    ...
                    Math.round(monthsInFilter)
                  } mån\n• Snitt: ${
                    userWorkoutsPerMonthTotal.toFixed(1)
                  } pass/mån\n\nCommunity-snitt: ${
                    stats.averages.workoutsPerUserPerMonth.toFixed(1)
                  } pass/mån`}
                />
                <ComparisonBar
                  ...
                  color="#60a5fa"
                  explanation={`Din data:\n• Total tid: ${
                    Math.round(lifetimeHours * 60)
                  } min\n• Antal pass: ${lifetimeSessions} st\n• Snitt: ${
                    Math.round(userAvgDuration)
                ...
                <ComparisonBar
                  ...
                  maxValue={Math.max(
                    stats.averages.tonnagePerUserPerMonth * 1.5,
                    monthlyTonnage,
                    1000,
                  ) / 1000}
                  color="#a855f7"
                  explanation={`Din volym denna månad:\n• Styrkepass: ${
                    strengthSessions.filter((s) => {
                      const d = new Date(s.date);
                      return d.getMonth() === currentMonth &&
                        d.getFullYear() === currentYear;
                    }).length
                  } st\n• Total volym: ${
                    formatTons(monthlyTonnage)
                  }\n\nCommunity-snitt: ${
                    formatTons(stats.averages.tonnagePerUserPerMonth)
                  }/mån`}
                />
              </div>
            </div>
            ...
            <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
              <h3 className="text-xl font-bold text-white mb-4">
                Underlag (Senaste 5 passen)
              </h3>
              <div className="space-y-2">
                {unifiedActivities.slice(0, 5).map((act) => (
                  <div
                    key={act.id}
                    className="flex justify-between items-center p-3 bg-slate-800/30 rounded-lg text-sm"
                  >
                    <div>
                      ...
                      <div className="text-gray-500 text-xs">
                        {new Date(act.date).toLocaleDateString("sv-SE")} •{" "}
                        {act.source}
                      </div>
                    </div>
                ...
                <Link
                  to="/activity-log"
                  className="block text-center text-emerald-500 text-xs font-bold pt-2 hover:underline"
                >
                  Se hela din logg →
                </Link>
              </div>
  ...
  const StrengthTabContents = () => {
    ...
    const selectedExerciseData = (selectedExercise &&
      stats.strength.exercises[selectedExercise.toLowerCase()]) || null;
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex text-xs gap-1">
                {([{ key: "popular", label: "🔥" }, {
                  key: "strongest",
                  label: "💪",
                }, { key: "weakest", label: "📈" }] as const).map((mode) => (
                  <button
                    key={mode.key}
                    onClick={() => setExerciseSortMode(mode.key)}
                    className={`px-2 py-1 rounded transition-all ${
                      exerciseSortMode === mode.key
                        ? "bg-emerald-600 text-white"
                        : "text-gray-500 hover:text-white"
                    }`}
                    title={mode.key === "popular"
                      ? "Mest populär"
                      : mode.key === "strongest"
                      ? "Starkast vs snitt"
                      : "Svagast vs snitt"}
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              <div className="space-y-2">
                {filteredExerciseStats.map((ex) => {
                  const pb = ex.userPB;
                  const diff = ex.avg1RM > 0
                    ? (((pb - ex.avg1RM) / ex.avg1RM) * 100)
                    : 0;
                  // Slugify for navigation
                  const slugifiedName = ex.name.toLowerCase().replace(
                    /[^a-z0-9]+/g,
                    "-",
                  ).replace(/(^-|-$)/g, "");
                    ...
                    <div key={ex.name} className="relative group">
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors uppercase tracking-tight">
                              {ex.name}
                            </div>
                            <div className="flex gap-2 mt-1">
                              <div className="text-[10px] text-gray-500 uppercase font-bold">
                                {ex.athleteCount} atleter
                              </div>
                              <div className="text-[10px] text-emerald-500 uppercase font-black tracking-widest">
                                {ex.userSessions} pass
                              </div>
                            </div>
                          </div>
                          <div className="text-right">
                            <div className="text-sm font-black text-emerald-400">
                              {pb ? `${formatNumber(pb)} kg` : "--"}
                            </div>
                            {pb > 0 && ex.avg1RM > 0 && (
                              <div
                ...
                <>
                  ...
                  <div className="relative z-10">
                    <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter mb-8 flex items-baseline gap-3">
                      {selectedExerciseData.name}
                      <span className="text-sm font-bold text-gray-500 not-italic tracking-normal">
                        {selectedExerciseData.athleteCount} atleter kör denna
                      </span>
                    </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
                      <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-800/50 relative group/pb">
                        ...
                        <div className="text-2xl font-black text-emerald-400 flex items-center gap-2 cursor-help">
                          ...
                          <span className="text-emerald-400 p-1 bg-emerald-400/10 rounded text-[10px] animate-pulse">
                            PB
                          </span>
                        </div>
                        <PBHoverCard
                          pb={getUserPBWithDetails(selectedExerciseData.name)}
                          rmMode={rmMode}
                        />
                      </div>
                      ...
                      <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-800/50">
                        ...
                        <div className="text-2xl font-black text-white">
                          {Math.round(
                            getUserAvgSets(selectedExerciseData.name),
                          )}
                          <span className="text-xs text-gray-500 font-bold ml-1 uppercase">
                            set
                          </span>
                        </div>
                        {(() => {
                          const userSets = getUserAvgSets(
                            selectedExerciseData.name,
                          );
                          ...
                          return (
                            <div
                              className={`text-[10px] mt-1 font-black uppercase ${
                                diff > 0 ? "text-emerald-500" : "text-amber-500"
                              }`}
                            >
                              {diff > 0 ? "+" : ""}
                              {Math.round(diff)}% vs snitt ({commSets})
                            </div>
                          );
                        })()}
                      </div>
                    ...
                    <div className="space-y-8">
                      <ComparisonBar
                        ...
                        color="#10b981"
                        pbDetails={getUserPBWithDetails(
                          selectedExerciseData.name,
                        )}
                        rmMode={rmMode}
                      />
                      <ComparisonBar
                        ...
                        maxValue={Math.max(
                          selectedExerciseData.avgTonnage * 2,
                          getUserAvgTonnage(selectedExerciseData.name),
                        )}
                        color="#a855f7"
                      />
                    </div>
                    ...
                    <div className="mt-12 grid grid-cols-3 gap-4 border-t border-slate-700/50 pt-8">
                      <div className="text-center">
                        ...
                        <div className="text-xl font-black text-white leading-none">
                          {formatTons(
                            strengthSessions.reduce((acc: number, s) =>
                              acc + s.exercises.filter((ex) =>
                                ex.exerciseName.toLowerCase() ===
                                  selectedExerciseData.name.toLowerCase()
                              ).reduce((exAcc: number, ex) =>
                                exAcc +
                                ex.sets.reduce(
                                  (setAcc: number, set) =>
                                    setAcc + (set.weight * set.reps),
                                  0,
                                ), 0), 0),
                          )}
                        </div>
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-xl font-black text-white leading-none">
                          {formatNumber(
                            strengthSessions.reduce((acc: number, s) =>
                              acc +
                              s.exercises.filter((ex) =>
                                ex.exerciseName.toLowerCase() ===
                                  selectedExerciseData.name.toLowerCase()
                              ).reduce(
                                (exAcc: number, ex) => exAcc + ex.sets.length,
                                0,
                              ), 0),
                          )}{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(["5k", "10k", "21k"] as const).map((dist) => {
            ...
            return (
              <div
                key={dist}
                className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 relative group/card hover:border-slate-600 transition-all"
              >
                <div className="absolute top-0 right-0 p-4 opacity-10 font-black text-6xl group-hover/card:scale-110 transition-transform">
                  {dist}
                </div>
                ...
                <div className="space-y-4 relative z-10">
                  <div className="flex justify-between items-end border-b border-slate-800 pb-2">
                    ...
                    <span className="text-xl font-bold text-white">
                      {formatDuration(dStats.avgTimeSeconds)}
                    </span>
                  </div>
                  <div className="flex justify-between items-end border-b border-slate-800 pb-2">
                    <span className="text-gray-400 text-sm">Snabbast</span>
                    <span className="text-xl font-bold text-amber-400">
                      {formatDuration(dStats.fastestTimeSeconds)}
                    </span>
                  </div>
                  <div className="pt-4">
                            ...
                            <div className="space-y-2">
                              <div className="flex justify-between">
                                ...
                                <span className="text-white font-bold">
                                  {new Date(cardioPB.date).toLocaleDateString(
                                    "sv-SE",
                                  )}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                ...
                                <span className="text-white font-bold">
                                  {cardioPB.actualDistance.toFixed(2)} km
                                </span>
                              </div>
          ...
          <div className="grid grid-cols-6 gap-2">
            {[
              ...
              const count = filteredActivities.filter((e) =>
                e.distance && e.distance >= bucket.min &&
                e.distance < bucket.max
              ).length;
              const maxOverallBucket = Math.max(
                ...[0, 3, 5, 10, 15, 21].map((min, idx, arr) =>
                  filteredActivities.filter((e) =>
                    e.distance && e.distance >= min &&
                    e.distance < (arr[idx + 1] || 999)
                  ).length
                ),
                1,
              );
