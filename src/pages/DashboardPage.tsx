...
import { DailyVitals, getISODate } from "../models/types.ts";
...
import {
  ...
  DashboardSleepCard,
  DashboardWaterCard,
} from "../features/dashboard/components/QuickLogCards.tsx";
...
export function DashboardPage() {
  ...
  const measurementsByDate = useMemo(() => {
    ...
    (bodyMeasurements || []).forEach(
      (m: { date: string; type: string; value: number }) => {
        if (!map[m.date]) {
          map[m.date] = {};
        }
        if (m.type === "waist") {
  ...
  const latestWaist = useMemo(() => {
    for (const date of allUniqueDatesDesc) {
      ...
      const bEntry = (bodyMeasurements || []).find((m) =>
        m.date === date && m.type === "waist"
      );
  ...
  const earliestWeightInRange = weightTrendEntries.length > 0
    ? weightTrendEntries[0].weight
    : 0;
  const latestWeightInRange = weightTrendEntries.length > 0
    ? weightTrendEntries[weightTrendEntries.length - 1].weight
    : 0;
  ...
  const proteinRatio = latestWeightVal > 0
    ? (proteinCurrent / latestWeightVal)
    : 0;
  const targetProteinRatio = latestWeightVal > 0
    ? (proteinTarget / latestWeightVal)
    : 0;

  // Training Analysis
  const todaysPlan = plannedActivities.find((p) =>
    p.date.split("T")[0] === selectedDate
  );
  const completedTraining = unifiedActivities.filter((e) =>
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-slate-950 p-4 md:p-12 font-sans text-slate-900 dark:text-white animate-in fade-in duration-500 transition-colors relative overflow-x-hidden">
      ...
      {/* Sticky Date Header */}
      <div
        className={`fixed top-16 left-0 right-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm transition-all ${
          selectedDate !== today ? "py-2" : "py-2"
        }`}
      >
        <div className="max-w-5xl mx-auto px-4 flex items-center justify-center gap-4">
          ...
          <div
            ...
            className={`font-bold text-sm cursor-pointer px-3 py-1 rounded-lg transition-all ${
              selectedDate !== today
          ...
          >
            {selectedDate === today
              ? "Idag"
              : selectedDate === getISODate(new Date(Date.now() - 86400000))
              ? "Igår"
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="group relative">
                <h1
                  ...
                  } font-bold tracking-tight text-slate-900 dark:text-white cursor-pointer`}
                  onClick={() => setSelectedDate(today)}
                >
                  {selectedDate === today ? "Idag" : selectedDate ===
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              title={vitals.completed
                ? "Markera som ej avslutad"
                : "Markera som avslutad"}
            >
              <CheckCircle
                size={16}
                className={vitals.completed
            ...
            {!vitals.completed && (
              <button
                ...
                title={vitals.incomplete
                  ? "Markera som fullständig"
                  : "Markera som ofullständig"}
              >
                <AlertCircle
                  size={16}
                  className={vitals.incomplete ? "animate-pulse" : ""}
                />
                <span className="text-xs font-black uppercase tracking-wider">
                  {vitals.incomplete
                    ? "Ofullständig dag"
                    : "Markera ofullständig"}
                </span>
              </button>
            )}
            <button
        ...
        >
          ...
          <div className="md:col-span-12 flex flex-col gap-4 mb-4">
            <DailySummaryCard
              ...
              trainingMinutes={completedTraining.reduce(
                (sum, act) => sum + act.durationMinutes,
                0,
              )}
              burnedCalories={burned}
              measurementsCount={0}
              weighInDone={unifiedHistory.some((w) =>
                w.date === today && w.weight
              )}
          ...
          {cardOrder.map((card) => {
            ...
            if (card.id === "sleep") {
              return (
                <DashboardSleepCard
                  ...
                  tempValue={tempValue}
                  onCardClick={() =>
                    handleCardClick("sleep", vitals.sleep || 0)}
            ...
            if (card.id === "alcohol") {
              ...
              const isWeekendLimit = dayOfWeek === 0 || dayOfWeek === 5 ||
                dayOfWeek === 6;
              const alcLimit =
                settings.dailyAlcoholLimitWeekend !== undefined &&
                  settings.dailyAlcoholLimitWeekday !== undefined
                  ? (isWeekendLimit
                    ? settings.dailyAlcoholLimitWeekend
                    : settings.dailyAlcoholLimitWeekday)
                  : undefined;
            ...
            if (card.id === "water") {
              return (
                <DashboardWaterCard
                  ...
                  tempValue={tempValue}
                  onCardClick={() =>
                    handleCardClick("water", vitals.water || 0)}
                  onValueChange={setTempValue}
            ...
            if (card.id === "caffeine") {
              return (
                <DashboardCaffeineCard
                  ...
                  tempValue={tempValue}
                  onCardClick={() =>
                    handleCardClick("caffeine", vitals.caffeine || 0)}
                  onValueChange={setTempValue}
          ...
          <div
            className={`md:col-span-12 ${
              density === "compact" ? "p-3 rounded-2xl" : "p-6 rounded-[2rem]"
            } bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col justify-center`}
          >
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex gap-1 justify-between">
                  {Array.from({ length: 14 }).map((_, i) => (
                    <div
                      key={i}
                      className={`${
                        density === "compact" ? "h-4" : "h-6"
                      } flex-1 rounded-sm flex items-center justify-center transition-all border ${
                        i < streakDays % 14
                          ? "bg-slate-900 dark:bg-emerald-600 border-slate-800 dark:border-emerald-500 text-white"
                          : "bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-200 dark:text-slate-800"
                      }`}
                    >
                      {i < streakDays % 14
              ...
              <div className="flex-[1.5] grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <div className="text-xl font-black text-indigo-500 dark:text-indigo-400">
                    + Pass
                  </div>
                </div>
