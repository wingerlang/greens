...
import {
  ...
  extractBrand,
  extractFromJSONLD,
  extractPackagingWeight,
  parseNutritionText,
} from "../utils/nutrition/index.ts";
...
export function DatabasePage({ headless = false }: { headless?: boolean }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    for (const item of foodItems) {
      ...
      if (nameLower === query || brandLower === query) {
        ...
        (brandLower && brandLower.includes(query)) || descLower.includes(query)
      ) {
  ...
  useEffect(() => {
    ...
    const urlSearch = searchParams.get("search");
    if (
      urlSearch && filteredItems.length === 1 &&
      filteredItems[0].name.toLowerCase() === urlSearch.toLowerCase()
    ) {
  ...
  const stats = useMemo(() => ({
    ...
    incompleteFoods: foodItems.filter((f) =>
      f.calories === 0 || !f.category || f.category === "other"
    ).length,
    missingMicros: foodItems.filter((f) =>
      !f.iron && !f.zinc && !f.vitaminB12
    ).length,
  }), [foodItems, recipes]);
  ...
  const databaseStatistics = useMemo(() => {
    ...
    const catMap: Record<
      string,
      { calories: number; protein: number; count: number }
    > = {};
    ...
    const diversityValue = totalLoggedTotal > 0
      ? (uniqueItemsCount / totalLoggedTotal).toFixed(2)
      : "0";
  ...
  const myContentData = useMemo(() => {
    ...
    mealEntries.forEach((entry) => {
      entry.items.forEach((item) => {
        if (item.type === "estimate" && item.estimateDetails) {
          const key =
            `${item.estimateDetails.name}-${item.estimateDetails.caloriesAvg}`;
    ...
    const estimations = Array.from(estimateMap.values()).sort((a, b) =>
      b.count - a.count
    );
  ...
  const applyParsedData = (parsed: any) => {
    const knownBrands = Array.from(
      new Set(foodItems.map((f) => f.brand).filter(Boolean)),
    ) as string[];
    const brand = parsed.brand || extractBrand(parsed.text || "", knownBrands);
    const packageWeight = parsed.packageWeight ||
      extractPackagingWeight(parsed.text || "");
    ...
    setFormData((prev) => ({
      ...
      defaultPortionGrams: parsed.defaultPortionGrams !== undefined
        ? parsed.defaultPortionGrams
        : prev.defaultPortionGrams,
    }));
  };
  ...
  const handleTextPaste = async (text: string) => {
    ...
    if (urlMatch) {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error("Kunde inte hämta sidan. Kontrollera URL:en.");
        }
        ...
        const finalResults = {
          ...
          name: cleanProductName(data.title, data.h1) || results.name ||
            textResults.name,
          text: data.text,
        };
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      setParseError(
        err instanceof Error ? err.message : "Kunde inte läsa bilden.",
      );
    } finally {
      setIsParsing(false);
    }
  ...
  return (
    <div className="database-page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          label="Ofullständiga"
          value={stats.incompleteFoods}
          icon="🔴"
          color="text-red-400"
        />
        <StatCard
          label="Saknar Mikros"
          value={stats.missingMicros}
          icon="🟡"
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div className="px-6 py-4 bg-slate-900/50 border border-slate-800 rounded-2xl flex-1 min-w-[200px]">
              ...
              <div className="text-2xl font-black text-emerald-400">
                {myContentData.myQuickMeals.length}
              </div>
            </div>
            <div className="px-6 py-4 bg-slate-900/50 border border-slate-800 rounded-2xl flex-1 min-w-[200px]">
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">
              ...
              <div className="text-2xl font-black text-amber-400">
                {myContentData.estimations.length}
              </div>
            </div>
          </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {myContentData.estimations.map((est, idx) => (
                  <div
                    key={idx}
                    className="bg-slate-900 border border-slate-800 p-4 rounded-2xl hover:border-slate-600 transition-colors group"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div className="font-bold text-slate-200 group-hover:text-emerald-400 transition-colors">
                        {est.name}
                      </div>
                      <div className="px-2 py-0.5 bg-slate-800 rounded text-[9px] font-black text-slate-500">
                        x{est.count}
                      </div>
                    </div>
                    <div className="flex gap-4 text-sm font-mono">
                      <div className="text-emerald-400">
                        {est.caloriesAvg} kcal
                      </div>
                      {est.protein && (
                        <div className="text-blue-400">{est.protein}g P</div>
                      )}
                      <div className="text-slate-500 text-[10px] self-center ml-auto italic">
                        Senast {est.lastUsed}
                      </div>
                    </div>
                  </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {myContentData.myQuickMeals.map((qm) => (
                  <div
                    key={qm.id}
                    className="bg-slate-900 border border-slate-800 p-4 rounded-2xl hover:border-slate-600 transition-colors group relative overflow-hidden"
                  >
                    <div className="absolute top-0 right-0 p-2 bg-emerald-500/10 text-emerald-400 text-[8px] font-black uppercase tracking-tighter">
                      Quick Meal
                    </div>
                    <div className="font-bold text-slate-200 mb-3 group-hover:text-emerald-400 transition-colors">
                      {qm.name}
                    </div>
                          ...
                          <span>
                            {item.type === "foodItem"
                              ? foodItems.find((f) => f.id === item.referenceId)
                                ?.name
                              : item.estimateDetails?.name || "Okänd"}
                          </span>
              ...
              : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {myContentData.myFoods.map((item) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="font-bold text-slate-200 group-hover:text-emerald-400 transition-colors truncate pr-2">
                          {item.name}
                        </div>
                        <div className="font-mono text-emerald-400 font-bold text-sm whitespace-nowrap">
                          {item.calories}
                        </div>
                      </div>
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl">
              ...
              <div className="text-2xl font-black text-emerald-400">
                {databaseActions.filter((a) => a.actionType === "CREATE")
                  .length}
              </div>
            </div>
            <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl">
              ...
              <div className="text-2xl font-black text-blue-400">
                {databaseActions.filter((a) => a.actionType === "UPDATE")
                  .length}
              </div>
            </div>
            <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl">
              ...
              <div className="text-2xl font-black text-rose-400">
                {databaseActions.filter((a) => a.actionType === "DELETE")
                  .length}
              </div>
            </div>
          </div>
              ...
              {databaseActions.length === 0
                ? (
                  <div className="p-12 text-center text-slate-500">
                    ...
                    Inga ändringar loggade ännu. Prova att lägga till eller
                    redigera en råvara!
                  </div>
                )
                : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-slate-500 flex items-center gap-2">
                          <span className="px-1.5 py-0.5 bg-slate-800 rounded text-[9px] uppercase font-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-3xl">
                ...
                <div className="text-xl font-black text-white">
                  {databaseStatistics.summary.mostLoggedItem}
                </div>
              </div>
              <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-3xl">
                <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">
                ...
                <div className="text-xl font-black text-emerald-400">
                  {databaseStatistics.summary.topCategory}
                </div>
              </div>
              <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-3xl">
                <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">
                ...
                <div className="text-xl font-black text-white">
                  {databaseStatistics.summary.diversityValue}{" "}
                  <span className="text-xs text-slate-500">unika/totalt</span>
                </div>
              </div>
            </div>
              ...
              <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-3xl flex flex-col min-h-[400px]">
                ...
                <div className="flex-1">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={databaseStatistics.topItems}
                      layout="vertical"
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        dataKey="value"
                      >
                        {databaseStatistics.categoryStats.map((
                          entry,
                          index,
                        ) => (
                          <Cell
          ...
          <>
            <div className="flex flex-col md:flex-row gap-4 mb-6 sticky top-0 z-20 bg-slate-950/80 backdrop-blur-md py-4 -mx-4 px-4 md:mx-0 md:px-0">
              ...
              <select
                ...
                value={sourceFilter}
                onChange={(e) =>
                  setSourceFilter(e.target.value as "all" | "user")}
              >
              ...
              ? (
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                  ...
                  <div className="grid grid-cols-[auto,1fr,auto,auto,auto,auto,auto,auto,auto,auto,auto,auto,auto,auto] gap-4 p-4 border-b border-slate-800 bg-slate-900/80 text-[10px] font-bold text-slate-500 uppercase tracking-wider items-center">
                    ...
                    >
                      Kcal {sortConfig?.key === "calories" &&
                        (sortConfig.direction === "asc" ? "↑" : "↓")}
                    </div>
                    <div
                      className="text-right hidden sm:block cursor-pointer hover:text-white transition-colors"
                      onClick={() => handleSort("protein")}
                    >
                      Prot {sortConfig?.key === "protein" &&
                        (sortConfig.direction === "asc" ? "↑" : "↓")}
                    </div>
                    <div
                      className="text-right hidden sm:block cursor-pointer hover:text-white transition-colors"
                    ...
                    >
                      Loggningar {sortConfig?.key === "frequency" &&
                        (sortConfig.direction === "asc" ? "↑" : "↓")}
                    </div>
                    <div
                      className="hidden xl:block cursor-pointer hover:text-white transition-colors"
                      onClick={() => handleSort("creator")}
                    >
                      Skapad av {sortConfig?.key === "creator" &&
                        (sortConfig.direction === "asc" ? "↑" : "↓")}
                    </div>
                    <div
                      className="hidden 2xl:block cursor-pointer hover:text-white transition-colors"
                      onClick={() => handleSort("date")}
                    >
                      Datum {sortConfig?.key === "date" &&
                        (sortConfig.direction === "asc" ? "↑" : "↓")}
                    </div>
                    ...
                    <AnimatePresence initial={false}>
                      {filteredItems.map((item: FoodItem) => (
                        ...
                        >
                          ...
                          >
                            <div className="font-bold text-slate-200 group-hover:text-emerald-400 transition-colors truncate">
                              {item.name}
                            </div>
                            {item.brand && (
                          ...
                          <div className="text-center hidden lg:block">
                            <select
                              ...
                              value={item.unit}
                              onChange={(e) =>
                                handleCellBlur(item, "unit", e.target.value)}
                            >
        ...
        >
          ...
          >
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-800 sticky top-0 bg-slate-900 z-10">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  <kbd className="px-1.5 py-0.5 bg-slate-800 rounded text-[10px] font-mono">
                    ESC
                  </kbd>{" "}
                  stäng •{" "}
                  <kbd className="px-1.5 py-0.5 bg-slate-800 rounded text-[10px] font-mono">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div
                className={`bg-emerald-500/5 rounded-2xl p-4 border relative overflow-hidden transition-all ${
                  isDragging
              ...
              >
                <label className="block text-[10px] font-bold uppercase tracking-wider text-emerald-500/70 mb-2 flex items-center gap-2">
                  ...
                  {isParsing && (
                    <div className="flex items-center gap-1.5 ml-auto">
                      ...
                      <span className="text-[9px] normal-case font-medium text-emerald-500/80">
                        Analyserar...
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex-1 relative">
                    <textarea
                      className={`w-full bg-slate-900/50 border ${
                        parseError ? "border-red-500/30" : "border-slate-700/50"
                      } rounded-xl px-4 py-3 text-sm text-slate-300 placeholder:text-slate-600 focus:outline-none focus:border-emerald-500/50 min-h-[80px] resize-none transition-all`}
                ...
                <p className="text-[9px] text-slate-500 mt-2 italic">
                  Stödjer text, URL och bilder (OCR). Klistra in bild direkt
                  (Ctrl+V) eller dra & släpp!
                </p>
                ...
                {isDragging && (
                  <div className="absolute inset-0 bg-emerald-500/20 backdrop-blur-sm flex items-center justify-center border-2 border-emerald-500 border-dashed rounded-2xl pointer-events-none">
                    <span className="text-emerald-300 font-bold animate-bounce">
                      Släpp bilden här!
                    </span>
                  </div>
                )}
                ...
                {editingItem && (
                  <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3">
                    ...
                    <p className="text-[10px] text-slate-500 mt-2">
                      Detta namn visas i din logg och sökning istället för
                      originalnamnet.
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-slate-500 font-bold">
                        G
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            defaultPortionGrams: Number(e.target.value),
                          })}
                        placeholder="t.ex. 35"
                        className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 pr-12"
                      />
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const calories = Math.round(
                          protein * 4 + formData.carbs * 4 + formData.fat * 9,
                        );
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const calories = Math.round(
                          formData.protein * 4 + carbs * 4 + formData.fat * 9,
                        );
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const calories = Math.round(
                          formData.protein * 4 + formData.carbs * 4 + fat * 9,
                        );
