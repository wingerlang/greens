    ...
    <GoalCard
      ...
      progress={progress
        ? {
          ...
          percentage: (!isNaN(progress.percentage) ? progress.percentage : 0),
        }
    ...
    <div className="min-h-screen bg-[#141419] text-white pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-8">
        ...
        <div className="bg-gradient-to-br from-emerald-900/20 to-black border border-emerald-500/20 rounded-2xl p-6 relative overflow-hidden">
          ...
          {/* Background Deco */}
          <Target
            className="absolute -right-4 -bottom-4 text-emerald-500/5 rotate-12"
            size={150}
          />
        </div>
        ...
        <div className="p-6 bg-[#1e1e24] rounded-2xl border border-white/5">
          ...
          <p className="text-sm text-white/50">
            Fortsätt logga dina aktiviteter för att se hur du ligger till mot
            din planering för {period.focusType}-fokus.
          </p>
        </div>
        ...
        <button
          onClick={async () => {
            if (
              confirm(
                "Är du säker på att du vill avsluta och ta bort denna period? Målen kommer ligga kvar men utan period-koppling.",
              )
            ) {
