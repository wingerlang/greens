  ...
  const fetchAllData = async () => {
    ...
    try {
      if (activeTab === "overview") {
        const [statsRes, usersRes, omniboxRes, dailyRes, eventsRes] =
          await Promise.all([
            ...
            fetch(
              `/api/usage/events?days=${daysBack}&limit=50${
        ...
        const [statsData, usersData, omniboxData, dailyData, eventsData] =
          await Promise.all([
            statsRes.json(),
            usersRes.json(),
            omniboxRes.json(),
  ...
  if (loading && !stats && activeTab === "overview") {
    return (
      <div className="p-8 text-slate-500 flex items-center gap-2">
        <RefreshCw className="animate-spin" size={16} /> Laddar statistik...
      </div>
    );
  }
                  ...
                  <div className="h-8 flex items-end gap-0.5">
                    {pulse.map((v, i) => (
                      <div
                        ...
                        style={{
                          height: `${
                            Math.min(100, (v / (Math.max(...pulse) || 1)) * 100)
                          }%`,
                        }}
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                <div className="h-[250px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={Object.entries(stats?.moduleStats || {}).map((
                          [name, value],
                        ) => ({ name, value }))}
                        innerRadius={60}
                        outerRadius={80}
                        ...
                        dataKey="value"
                      >
                        {Object.entries(stats?.moduleStats || {}).map((
                          _entry,
                          index,
                        ) => (
                          <Cell
              ...
              <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-black text-emerald-500">
                        {stats?.conversionStats?.meals.planned
                          ? Math.round(
                            (stats.conversionStats.meals.logged /
                              stats.conversionStats.meals.planned) * 100,
                          )
                          : 0}%
                      </span>
                    </div>
                        ...
                        style={{
                          width: `${
                            stats?.conversionStats
                              ? (stats.conversionStats.meals.logged /
                                (stats.conversionStats.meals.planned || 1)) *
                                100
                              : 0
                          }%`,
                        }}
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-slate-500">
                      <span>
                        {stats?.conversionStats?.meals.logged} Loggade
                      </span>
                      <span>
                        {stats?.conversionStats?.meals.planned} Planerade
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-black text-blue-500">
                        {stats?.conversionStats?.training.planned
                          ? Math.round(
                            (stats.conversionStats.training.completed /
                              stats.conversionStats.training.planned) * 100,
                          )
                          : 0}%
                      </span>
                    </div>
                      ...
                      <div
                        ...
                        style={{
                          width: `${
                            stats?.conversionStats
                              ? (stats.conversionStats.training.completed /
                                (stats.conversionStats.training.planned || 1)) *
                                100
                              : 0
                          }%`,
                        }}
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-slate-500">
                      <span>
                        {stats?.conversionStats?.training.completed} Genomförda
                      </span>
                      <span>
                        {stats?.conversionStats?.training.planned} Planerade
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 p-4 bg-slate-800/30 border border-slate-800 rounded-xl flex items-start gap-3">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed font-medium">
                    Konverteringsgraden mäter hur väl dina användare följer sina
                    planer. En hög siffra indikerar att appens planeringstjänst
                    skapar disciplin och värde.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                  <p className="text-[10px] text-amber-500 font-bold leading-relaxed">
                    Mäter medeltid från öppnad modal till färdig loggning. Låga
                    värden indikerar hög effektivitet och låg friktion.
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {users.map((u) => (
                        <tr
                          ...
                          onClick={() =>
                            setSelectedUserId(
                              u.userId === selectedUserId ? null : u.userId,
                            )}
                        ...
                        >
                          <td
                            className="px-3 py-2 font-mono text-xs text-slate-300 truncate max-w-[120px]"
                            title={u.userId}
                          >
                            {u.userId.slice(0, 8)}...
                          </td>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                  ...
                  <div className="space-y-2">
                    {omnibox.topSearches.slice(0, 5).map((s, i) => (
                      <div
                        key={i}
                        className="flex justify-between text-xs text-slate-400 border-b border-slate-800 pb-1"
                      >
                        <span>{s.query}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={appDataStats?.nutrition?.topFoods || []}
                      layout="vertical"
                    >
                      <XAxis type="number" hide />
                      <YAxis
                ...
                <div className="mt-6 flex gap-4">
                  <div className="flex-1 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                    ...
                    <div className="text-2xl font-black text-white">
                      {appDataStats?.nutrition?.avgDailyCalories || 0} kcal
                    </div>
                  </div>
                  <div className="flex-1 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                    <div className="text-sm font-black text-slate-500 uppercase">
                    ...
                    <div className="text-2xl font-black text-white">
                      {appDataStats?.nutrition?.totalMealsLogged || 0} st
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={appDataStats?.training?.topExercises || []}
                      layout="vertical"
                    >
                      <XAxis type="number" hide />
                      <YAxis
                ...
                <div className="mt-6 grid grid-cols-2 gap-4">
                  <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700">
                    ...
                    <div className="text-lg font-black text-blue-400">
                      {appDataStats?.training?.totalDistance || 0} km
                    </div>
                  </div>
                  <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700">
                    <div className="text-[10px] font-black text-slate-500 uppercase">
                    ...
                    <div className="text-lg font-black text-amber-500">
                      {appDataStats?.training?.totalTonnage || 0} kg
                    </div>
                  </div>
                  <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700">
                    <div className="text-[10px] font-black text-slate-500 uppercase">
                    ...
                    <div className="text-lg font-black text-emerald-400">
                      {appDataStats?.training?.cardioWorkoutCount || 0}
                    </div>
                  </div>
                  <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700">
                    <div className="text-[10px] font-black text-slate-500 uppercase">
                    ...
                    <div className="text-lg font-black text-purple-400">
                      {appDataStats?.training?.strengthWorkoutCount || 0}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-800">
                  {errorStats.map((err, i) => (
                    ...
                    >
                      <td className="px-4 py-4 font-bold text-red-400 max-w-md break-words">
                        {err.message}
                      </td>
                  ...
                  {errorStats.length === 0 && (
                    <tr>
                      <td
                        colSpan={4}
                        className="px-4 py-20 text-center text-slate-500 font-bold"
                      >
                    ...
                    {sessions.map((s) => (
                      <tr key={s.sessionId} className="hover:bg-slate-800/30">
                          ...
                          <div className="flex items-center gap-1 opacity-70">
                            {s.pathFlow.slice(0, 3).map((path, i) => (
                              <React.Fragment key={i}>
                                ...
                                <span
                                  className="text-xs bg-slate-800 px-1.5 py-0.5 rounded truncate max-w-[100px]"
                                  title={path}
                                >
                                  {path}
                                </span>
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={() =>
                              setPlayingSessionId(s.sessionId)}
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 overflow-x-auto">
      ...
      <table className="w-full text-[10px] border-collapse">
        <thead>
          <tr>
            <th className="p-2 text-left bg-slate-800/50 sticky left-0 z-10 w-24">
              Datum
            </th>
            <th className="p-2 text-center bg-slate-800/50">Antal</th>
            {new Array(14).fill(0).map((_, i) => (
              <th key={i} className="p-2 text-center bg-slate-800/20 w-8">
                D{i + 1}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-slate-800">
          {data.map((row) => (
            <tr key={row.date} className="hover:bg-slate-800/30">
              <td className="p-2 font-bold text-slate-300 sticky left-0 bg-slate-900 z-10">
                {row.date}
              </td>
              <td className="p-2 text-center font-bold text-white bg-slate-800/30">
                {row.total}
              </td>
              {row.retained.map((count: number, i: number) => {
                ...
                return (
                  <td
                    key={i}
                    className={`p-2 text-center border border-slate-800/50 transition-colors ${bgColor} ${
                      percent > 50 ? "text-white" : "text-slate-400"
                    }`}
              ...
              <div key={i} className="flex items-center gap-4 group">
                <div className="flex-1 bg-slate-800/50 p-3 rounded-xl border border-slate-700 flex items-center justify-between group-hover:border-blue-500/50 transition-colors">
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    />
                    <span className="text-xs font-mono text-blue-400 font-bold truncate max-w-[150px]">
                      {to}
                    </span>
                  </div>
              ...
              <div key={i} className="flex flex-col gap-1">
                ...
                <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        (item.count / (exitStats[0]?.count || 1)) * 100
                      }%`,
                    }}
                  />
                </div>
                    ...
                    {currentEvent._type === "view"
                      ...
                      ? (
                        <>
                          <div className="relative">
                            <MousePointer2
                              size={64}
                              className="text-red-500 mb-4 animate-ping absolute top-0 left-0 opacity-50"
                            />
                            <MousePointer2
                              size={64}
                          ...
                          <h3 className="text-2xl font-black text-red-500 uppercase tracking-widest">
                            Rage Click
                          </h3>
                          ...
                          <div className="mt-4 px-4 py-2 bg-red-500/10 border border-red-500/50 rounded text-red-300 text-xs">
                            Användaren klickade upprepade gånger på{" "}
                            {currentEvent.target}
                          </div>
                        </>
                      )
                      ...
                      ? (
                        <>
                          ...
                          <p className="text-slate-300 font-mono mt-2 text-sm">
                            {currentEvent.label}
                          </p>
                      ...
                      : (
                        <>
                          ...
                          <p className="text-pink-400 font-mono mt-2">
                            {currentEvent.target}: {currentEvent.label}
                          </p>
                          {currentEvent.metadata && (
...
function HeatmapView({ events }: { events: InteractionEvent[] }) {
  ...
  const clicks = events.filter((e) =>
    (e.type === "click" || e.type === "rage_click") && e.coordinates
  );
  ...
  const filteredClicks = selectedPath === "all"
    ? clicks
    : clicks.filter((e) => e.path === selectedPath);
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
      ...
      <div className="relative w-full aspect-video bg-slate-100 dark:bg-slate-950 rounded-xl overflow-hidden border border-slate-800 shadow-inner">
        ...
        {filteredClicks.map((click, i) => (
          <div
            key={i}
            className={`absolute w-6 h-6 rounded-full blur-md transform -translate-x-1/2 -translate-y-1/2 transition-opacity duration-500 ${
              click.type === "rage_click"
                ? "bg-red-500 opacity-80 z-20 w-8 h-8"
                : "bg-orange-500 opacity-40 hover:opacity-100 z-10"
            }`}
            style={{
...
function FrictionView({ events }: { events: InteractionEvent[] }) {
  ...
  const stats = React.useMemo(() => {
    ...
    const slowPages = events.filter((e) =>
      e.type === "other" && (e.metadata?.duration > 1000)
    ).length;
    ...
    return [
      {
        subject: "Rage Clicks",
        A: Math.min(100, rageClicks * 10),
        fullMark: 100,
      },
      { subject: "Errors", A: Math.min(100, errors * 20), fullMark: 100 },
      {
        subject: "Dead Clicks",
        A: Math.min(100, deadClicks * 5),
        fullMark: 100,
      },
      {
        subject: "Slow Pages",
        A: Math.min(100, slowPages * 10),
        fullMark: 100,
      },
        ...
        <div className="space-y-6">
          <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
            ...
            <p className="text-sm text-slate-400">
              {stats.sort((a, b) => b.A - a.A)[0].subject}{" "}
              är det största problemet just nu. Detta indikerar att användare
              upplever frustration eller tekniska hinder.
            </p>
          </div>
...
function FunnelView({ events }: { events: InteractionEvent[] }) {
  ...
  const funnelData = React.useMemo(() => {
    ...
    const step1Users = new Set(
      events.filter((e) => e.path === "/database").map((e) => e.userId),
    );
    ...
    const step2Users = new Set(
      events.filter((e) =>
        e.path === "/database" &&
        (e.label.includes("Add") || e.target === "button")
      ).map((e) => e.userId),
    ); // Looser match for demo

    // 3. Submitted (mock)
    const step3Users = new Set(
      events.filter((e) => e.type === "submit" || (e.label === "Spara")).map(
        (e) => e.userId,
      ),
    );
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
      ...
      <div className="grid grid-cols-4 gap-4 mt-8">
        {funnelData.map((step, i) => (
          <div
            key={step.name}
            className="bg-slate-800 p-4 rounded-xl text-center relative overflow-hidden"
          >
            <div
