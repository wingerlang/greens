...
export default function ExerciseDatabasePage() {
    ...
    const [activeTab, setActiveTab] = useState<'exercises' | 'unmapped'>('exercises');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            <div className="md:flex md:items-center md:justify-between">
                ...
                <div className="flex gap-2 mt-4 md:mt-0">
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'exercises' ? 'bg-emerald-500 text-slate-900' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                    >
                        Övningar
                    </button>
                    <button
                        onClick={() => setActiveTab('unmapped')}
                        className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'unmapped' ? 'bg-amber-500 text-slate-900' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                    >
            ...
            {activeTab === 'exercises' ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 bg-slate-900/50 border border-white/5 rounded-2xl p-6 max-h-[80vh] overflow-y-auto">
                        <h3 className="text-lg font-bold text-white mb-4">Muscle Hierarchy</h3>
                ...
                <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
                    <h3 className="text-xl font-bold text-white mb-4">Övningar som saknar mappning</h3>
                    <p className="text-slate-400 text-sm mb-6">Dessa namn har dykt upp i träningspass men matchar ingen känd övning (eller alias) i databasen.</p>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-12 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 text-emerald-400">
                            ...
                            <p className="text-sm opacity-80">Inga okända övningar hittades.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {unmappedExercises.map(name => (
                                <div key={name} className="bg-slate-950 border border-white/10 p-4 rounded-xl flex flex-col justify-between group hover:border-amber-500/50 transition-all">
            ...
            {mappingTarget && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" onClick={() => setMappingTarget(null)} />
                    <div className="relative bg-slate-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-lg p-6">
                        <h3 className="text-xl font-bold text-white mb-4">Mappa "{mappingTarget}"</h3>
                        <p className="text-sm text-slate-400 mb-6">Välj vilken övning i databasen som motsvarar <strong>{mappingTarget}</strong>. Detta lägger till namnet som ett alias.</p>

                        <div className="mb-6">
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Välj Övning</label>
                            <select
                            ...
                            >
                                ...
                                {exercises.sort((a,b) => a.name_sv.localeCompare(b.name_sv)).map(ex => (
                                    <option key={ex.id} value={ex.id}>{ex.name_sv} / {ex.name_en}</option>
                                ))}
                            </select>
                        </div>
