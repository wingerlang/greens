...
export function PublicProfilePage() {
  ...
  const [stats, setStats] = useState<
    { distance: number; duration: number; count: number } | null
  >(null);
      ...
      .then(async (user) => {
        ...
        if (user) {
          ...
          if (currentUser) {
            ...
            const isFollowing = await socialService.checkIsFollowing(
              currentUser.id,
              user.id,
            );
  ...
  if (loading) {
    return (
      <div className="p-8 text-center text-slate-500">Wait for it... 🏃‍♂️</div>
    );
  }
  if (!profile) {
    return (
      <div className="p-8 text-center text-rose-500">
        Användaren hittades inte. 🤷‍♂️
      </div>
    );
  }
  ...
  const isMe = currentUser?.id === profile.id ||
    currentUser?.email === profile.email; // Fallback check
    ...
    return (
      <div className="max-w-2xl mx-auto mt-10 p-8 content-card text-center">
        ...
        <p className="text-slate-500 mt-2">
          Du måste följa @{profile.handle} för att se deras aktivitet.
        </p>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 fade-in">
      ...
      <div className="content-card relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-r from-emerald-900/40 to-slate-900/40">
        </div>
              ...
              ? (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white font-bold">
                      {Math.round(stats.duration / 60)}h {stats.duration % 60}m
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
            ...
            <div className="content-card">
              ...
              <div className="text-center">
                <div className="text-3xl font-black text-white">
                  72.5 <span className="text-sm text-slate-500">kg</span>
                </div>
            ...
            : (
              <div className="text-center py-12 bg-slate-900/40 rounded-3xl border border-dashed border-white/10">
                <p className="text-sm text-slate-500">
                  Inga offentliga aktiviteter ännu.
                </p>
              </div>
            )}
        </div>
