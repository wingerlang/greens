      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Vår samling av kvalitetssäkrade övningar.
          </p>
        </div>
        <Link
        ...
        : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((ex) => (
              <div
                key={ex.id}
                className="bg-slate-900/80 border border-white/5 p-5 rounded-2xl hover:border-emerald-500/30 transition-all group relative overflow-hidden"
              >
                ...
                <div className="flex flex-wrap gap-1.5 mt-auto">
                  {ex.primaryMuscles.map((m) => (
                    <span
                      key={m}
                      className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 text-[10px] font-bold uppercase border border-emerald-500/20"
                    >
                      {m}
                    </span>
                  ))}
                  {ex.secondaryMuscles.slice(0, 2).map((m) => (
                    <span
                      key={m}
                      className="px-2 py-0.5 rounded-md bg-slate-800 text-slate-400 text-[10px] font-bold uppercase border border-white/5"
                    >
                      {m}
                    </span>
                  ))}
