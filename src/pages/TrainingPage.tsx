...
import { WEEKDAY_LABELS } from "../models/types.ts";
...
import {
  EXERCISE_TYPES,
  INTENSITIES,
} from "../components/training/ExerciseModal.tsx";
...
export function TrainingPage() {
  ...
  const { tab, subTab, id } = useParams<
    { tab?: string; subTab?: string; id?: string }
  >();

  // URL State Management
  const currentTab = useMemo(() => {
    if (!tab) return "overview";
    return (["overview", "styrka", "kondition", "races"].includes(tab)
      ? tab
      : "overview") as "overview" | "styrka" | "kondition" | "races";
  }, [tab]);
  ...
  const [selectedCycle, setSelectedCycle] = useState<TrainingCycle | null>(
    null,
  );
  ...
  const handleCreateCycleAfter = (cycle: TrainingCycle) => {
    ...
    const nextStart =
      new Date(endDate.getTime() + 24 * 60 * 60 * 1000).toISOString().split(
        "T",
      )[0];
    // Guess next goal?
    const nextGoal = cycle.goal === "deff"
      ? "Bulk"
      : cycle.goal === "bulk"
      ? "Deff"
      : "Mål";
      ...
      entriesByDate[date].forEach((entry) => {
        entry.items.forEach((item) => {
          ...
          if (item.type === "foodItem") {
            ...
            if (food) {
              const base =
                (food.unit === "g" || food.unit === "ml" || food.unit === "l" ||
                    food.unit === "kg")
                  ? 100
                  : 1;
            ...
            if (recipe) {
              ...
              const kcalPerServing = recipe.servings > 0
                ? recipeTotalKcal / recipe.servings
                : recipeTotalKcal;
    ...
    switch (preset) {
      case "all":
        ...
        if (legacyExerciseEntries.length > 0) {
          const sorted = [...legacyExerciseEntries].sort((a, b) =>
  ...
  const exerciseEntries = useMemo(() => {
    ...
    const strength = (strengthSessions || []).map((w): ExerciseEntry => ({
      ...
      durationMinutes: w.duration ||
        (w.exercises.length * 4) + (w.totalSets * 1.5) || 45,
      intensity: "high",
    ...
    const uniqueLegacy = (legacyExerciseEntries || []).filter((e) =>
      !serverIds.has(e.id)
    );
      ...
      {currentTab === "overview" && (
        <>
          ...
          <TrainingOverview
            ...
            year={filterStartDate
              ? new Date(filterStartDate).getFullYear()
              : new Date().getFullYear()}
            isFiltered={true}
            onExerciseClick={handleEditExercise}
            periodLabel={activePreset === "all"
              ? "Total Volym"
              : activePreset === "ytd"
              ? `Årsvolym ${new Date().getFullYear()}`
              : activePreset === "prev"
              ? `Årsvolym ${new Date().getFullYear() - 1}`
              : activePreset === "ttm"
              ? "Trailing 12 Months"
              ...
              ? (
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 to-slate-950 border border-white/10 p-6 shadow-2xl group">
                  <div className="absolute top-0 right-0 p-4 opacity-10 text-[100px] leading-none select-none grayscale group-hover:grayscale-0 transition-all duration-500">
                    {activeCycle.goal === "deff"
                      ? "🔥"
                      : activeCycle.goal === "bulk"
                      ? "💪"
                      : "⚖️"}
                  </div>
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={() => {
                          if (
                            confirm(
                              "Är du säker på att du vill avsluta denna period?",
                            )
                          ) {
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-400">
                            Definiera ditt mål och följ din utveckling över tid.
                          </p>
                        </div>
                        <button
                          onClick={() => setIsCycleCreatorOpen(true)}
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-slate-400 text-xs mt-1">
                    Vikt, kalorier och träningsbelastning
                  </p>
                </div>
                <div className="flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setVisibleMetrics((prev) => ({
                            ...prev,
                            [m.id]: !prev[m.id as keyof typeof prev],
                          }))}
                  ...
                  <div className="flex bg-slate-900 rounded-lg p-1 border border-white/5">
                    <button
                      onClick={() => setZoomLevel((z) => Math.min(12, z + 2))}
                      className="px-3 hover:bg-white/10 rounded text-slate-400 font-bold"
                    >
                      -
                    </button>
                    <span className="px-2 flex items-center text-[10px] font-mono text-slate-500">
                      {zoomLevel}m
                    </span>
                    <button
                      onClick={() => setZoomLevel((z) => Math.max(2, z - 2))}
                      className="px-3 hover:bg-white/10 rounded text-slate-400 font-bold"
                    >
                      +
                    </button>
              ...
              {dailyExercises.length > 0
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm">
                              {EXERCISE_TYPES.find((t) => t.type === ex.type)
                                ?.label}
                            </span>
                            <span
                              className={`text-[10px] font-bold uppercase ${
                                INTENSITIES.find((i) =>
                                  i.value === ex.intensity
                                )?.color
                              }`}
                            >
                              {INTENSITIES.find((i) => i.value === ex.intensity)
                                ?.label}
                            </span>
                          </div>
                          <div className="text-xs text-slate-500">
                            {formatActivityDuration(ex.durationMinutes)} •{" "}
                            {ex.notes || "Inga anteckningar"}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                ...
                : (
                  <div className="text-center py-8 text-slate-600 italic text-sm">
                    Ingen träning loggad för {selectedDate}
                  </div>
                )}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              ...
              <div className="p-4 rounded-2xl bg-slate-900/50 border border-white/5">
                ...
                <div className="text-2xl font-black text-rose-400">
                  {Math.round(
                    filteredExerciseEntries.reduce(
                      (sum, e) => sum + (e.tonnage || 0),
                      0,
                    ) / 1000,
                  )}{" "}
                  <span className="text-sm text-slate-500 font-bold">ton</span>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-900/50 border border-white/5">
                ...
                <div className="text-2xl font-black text-sky-400">
                  {Math.round(
                    filteredExerciseEntries.reduce(
                      (sum, e) => sum + (e.distance || 0),
                      0,
                    ),
                  )}{" "}
                  <span className="text-sm text-slate-500 font-bold">km</span>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-900/50 border border-white/5">
                ...
                <div className="text-2xl font-black text-emerald-400">
                  {Math.round(
                    filteredExerciseEntries.length > 0
                      ? filteredExerciseEntries.reduce(
                        (sum, e) => sum + e.durationMinutes,
                        0,
                      ) / filteredExerciseEntries.length
                      : 0,
                  )}{" "}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span className="text-xl">
                            {EXERCISE_TYPES.find((t) => t.type === type)?.icon}
                          </span>
                          <span className="text-xs font-bold capitalize text-slate-300">
                            {EXERCISE_TYPES.find((t) => t.type === type)?.label}
                          </span>
                        </div>
                  ...
                  {exerciseEntries.some((e) => e.tonnage) && (
                    <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/20 text-center mb-4">
                      ...
                      <p className="text-[10px] text-slate-400 mt-1">
                        Du lyfte {Math.round(Math.max(
                          0,
                          ...exerciseEntries.filter((e) => e.tonnage).map((e) =>
                            e.tonnage || 0
                          ),
                        ))} kg som mest i ett pass.
                      </p>
                    </div>
                  )}
                  {exerciseEntries.some((e) => e.distance) && (
                    <div className="p-6 rounded-2xl bg-gradient-to-br from-sky-500/10 to-transparent border border-sky-500/20 text-center">
                      <span className="text-4xl">👟</span>
                      <h5 className="text-sm font-bold text-sky-400 mt-2">
                        Distanstopp
                      </h5>
                      <p className="text-[10px] text-slate-400 mt-1">
                        Längsta passet var {Math.max(
                          0,
                          ...exerciseEntries.filter((e) => e.distance).map(
                            (e) => e.distance || 0,
                          ),
                        )} km.
                      </p>
            ...
            {performanceGoals.length > 0
              ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {performanceGoals.map((goal) => {
                    ...
                    let current = 0;
                    if (
                      goal.type === "frequency" && goal.targets[0]?.exerciseType
                    ) {
                    ...
                    const target = goal.targets[0]?.count ||
                      goal.targets[0]?.value || 1;
              ...
              : (
                <div className="text-center py-12 text-slate-600">
                  ...
                  <p className="text-sm">
                    Inga mål ännu. Skapa ett för att börja spåra din framgång!
                  </p>
                </div>
              )}
          </div>
