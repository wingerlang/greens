...
export const MuscleOverviewPage: React.FC = () => {
    ...
    const getExercisesForMuscle = (muscleId: string) => {
        const primary = exercises.filter(e => e.primaryMuscles.includes(muscleId));
        const secondary = exercises.filter(e => e.secondaryMuscles.includes(muscleId));
    ...
    return (
        <div className="space-y-6 pb-20">
            <header>
                ...
                <p className="text-slate-400">Utforska övningar baserat på muskelgrupper.</p>
            </header>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {muscleHierarchy.categories.map((category) => (
                    ...
                    >
                        ...
                        <span className="text-xs opacity-60">{category.groups.length} grupper</span>
                    </button>
                ))}
            </div>
                ...
                {selectedCategory && (
                    ...
                    >
                        {muscleHierarchy.categories
                            ...
                            ?.groups.map((group) => (
                                <div key={group.id} className="bg-slate-900 border border-white/5 rounded-2xl overflow-hidden">
                                    <button
                                        onClick={() => setExpandedGroup(expandedGroup === group.id ? null : group.id)}
                                        className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors"
                                    >
                                        <span className="font-bold text-slate-200">{group.name}</span>
                                        <span className={`text-slate-500 transition-transform duration-200 ${expandedGroup === group.id ? 'rotate-180' : ''}`}>▼</span>
                                    </button>
                    ...
                    >
                        <div className="min-h-screen md:min-h-0 p-6 md:p-0">
                            ...
                            <div className="max-w-4xl mx-auto md:max-w-none space-y-6">
                                ...
                                {(() => {
                                    const { primary, secondary } = getExercisesForMuscle(selectedMuscle);
                                    ...
                                    return (
                                        <div className="grid md:grid-cols-2 gap-8">
                                            ...
                                            <div className="space-y-4">
                                                <h3 className="text-sm font-bold text-emerald-400 uppercase tracking-wider">Primärt Fokus ({primary.length})</h3>
                                                <div className="grid gap-2">
                                                    {primary.map(exercise => (
                                                        <div key={exercise.id} className="p-3 bg-slate-900 border border-emerald-500/20 rounded-xl flex items-center justify-between group hover:border-emerald-500/50 transition-colors">
                                                            <span className="text-slate-200 font-medium">{exercise.name_sv}</span>
                                                            <Link
                                                    ...
                                                    {primary.length === 0 && <p className="text-slate-600 italic">Inga övningar.</p>}
                                                </div>
                                            </div>

                                            {/* Secondary Focus */}
                                            <div className="space-y-4">
                                                <h3 className="text-sm font-bold text-blue-400 uppercase tracking-wider">Sekundärt Fokus ({secondary.length})</h3>
                                                <div className="grid gap-2">
                                                    {secondary.map(exercise => (
                                                        <div key={exercise.id} className="p-3 bg-slate-900 border border-blue-500/10 rounded-xl flex items-center justify-between group hover:border-blue-500/30 transition-colors">
                                                            <span className="text-slate-200 font-medium">{exercise.name_sv}</span>
                                                            <Link
                                                                to={`/training/load?muscle=${selectedMuscle}&exercise=${exercise.id}`}
                                                    ...
                                                    {secondary.length === 0 && <p className="text-slate-600 italic">Inga övningar.</p>}
                                                </div>
                                            </div>
                                        </div>
                                    );
