...
export function AthleteListPage() {
  ...
  const pendingInvitations = relations.filter((r) =>
    r.athleteId === "me" && r.status === "pending"
  );
  ...
  const RelationCard = (
    { relation, isCoach }: { relation: CoachAthleteRelation; isCoach: boolean },
  ) => (
    <div
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${
            isCoach ? "bg-amber-500/20" : "bg-indigo-500/20"
          }`}
        >
          {isCoach ? "👨‍🏫" : "🏃"}
        </div>
        <div>
          <div className="text-sm font-black text-white">
            {isCoach ? relation.coachName : relation.athleteName}
          </div>
      ...
      <div className="flex gap-2">
        {relation.status === "pending" && !isCoach && (
          <>
            <button
              onClick={() => acceptInvitation(relation.id)}
              className="px-3 py-1.5 bg-emerald-500 text-white text-[9px] font-black uppercase rounded-lg hover:bg-emerald-400"
            >
              Acceptera
            </button>
            <button
              onClick={() => declineInvitation(relation.id)}
              className="px-3 py-1.5 bg-slate-700 text-slate-400 text-[9px] font-black uppercase rounded-lg hover:bg-slate-600"
            >
              Neka
            </button>
          </>
        ...
        {relation.status === "active" && isCoach && (
          <button
            onClick={() => removeAthlete(relation.id)}
            className="px-3 py-1.5 bg-rose-500/10 text-rose-400 text-[9px] font-black uppercase rounded-lg hover:bg-rose-500/20"
          >
            Ta bort
          </button>
        )}
        ...
        <section className="glass-card p-5 bg-gradient-to-br from-rose-500/5 to-transparent border-rose-500/10">
          ...
          <div className="space-y-3">
            {pendingInvitations.map((r) => (
              <RelationCard key={r.id} relation={r} isCoach={true} />
            ))}
          </div>
        </section>
      )}
        ...
        <section className="glass-card p-5">
          ...
          {myAthletes.length === 0
            ...
            : (
              <div className="space-y-3">
                {myAthletes.map((r) => (
                  <RelationCard key={r.id} relation={r} isCoach={false} />
                ))}
              </div>
            )}
        </section>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="p-4 bg-slate-900/40 rounded-xl text-center">
            <div className="text-2xl font-black text-amber-400">
              {relations.filter((r) => r.status === "pending").length}
            </div>
            <div className="text-[9px] text-slate-500 uppercase font-bold">
              Väntande
            </div>
