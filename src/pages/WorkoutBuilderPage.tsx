...
import { BODY_PARTS, MUSCLE_MAP } from "../data/muscleMap.ts";
...
export function WorkoutBuilderPage() {
  ...
  const [activeTab, setActiveTab] = useState<"BUILD" | "ANALYZE" | "COMPARE">(
    "BUILD",
  );
  ...
  // IMPORT LOGIC
  useEffect(() => {
    const fromActivityId = searchParams.get("fromActivity") ||
    ...
    if (fromActivityId) {
      ...
      if (activity) {
        ...
        let title = isRun
          ? `Pass: ${activity.distance}km`
          : `${activity.type} Session`;
        if (activity.subType && activity.subType !== "default") {
          const sub = activity.subType.charAt(0).toUpperCase() +
        ...
        const imported: WorkoutDefinition = {
          ...
          subCategory:
            isRun && activity.subType && activity.subType !== "default"
              ? (activity.subType.charAt(0).toUpperCase() +
                activity.subType.slice(1))
              : undefined,
          description: activity.notes ||
            `Importerat från aktivitet den ${activity.date}`,
          exercises: isRun
            ? [
              { id: generateId(), title: "Uppvärmning", exercises: [] },
              {
                ...
                exercises: [{
                  id: generateId(),
                  exerciseId: "run",
                  name: activity.subType === "interval"
                    ? "Intervaller"
                    : "Löpning",
                  sets: 1,
      ...
      const strengthSession = strengthSessions.find((s) =>
        s.id === fromActivityId
      );
      if (strengthSession) {
        ...
        const imported: WorkoutDefinition = {
          ...
          category: "STRENGTH",
          subCategory: strengthSession.title?.toLowerCase().includes("push")
            ? "Push"
            : strengthSession.title?.toLowerCase().includes("pull")
            ? "Pull"
            : strengthSession.title?.toLowerCase().includes("ben")
            ? "Ben"
            : undefined,
  ...
  const [activeSectionId, setActiveSectionId] = useState<string>(
    workout.exercises?.[0]?.id || INITIAL_WORKOUT.exercises![0].id,
  );

  const handleSubCategoryChange = (sub: string) => {
    let updatedWorkout = { ...workout, subCategory: sub };
    const totalEx =
      workout.exercises?.reduce((sum, s) => sum + s.exercises.length, 0) || 0;
    if (totalEx <= 1) {
      if (workout.category === "RUNNING") {
        const needsKm = ["Långpass", "Intervall", "Tempo", "Backe"].includes(
          sub,
        );
        ...
        updatedWorkout.exercises = [
          ...
          {
            id: generateId(),
            title: sub === "Intervall" ? "Intervaller" : "Huvuddel",
            exercises: [{
              id: generateId(),
              exerciseId: "run-main",
              name: sub === "Intervall" ? "Intervaller" : "Löpning",
              sets: 1,
              reps: sub === "Långpass" ? "15-20 km" : "5-10 km",
              weight: sub === "Tempo" ? "Progressiv" : "RPE 5",
              rest: 0,
            }],
          },
          {
  ...
  const getProgressionTarget = (exerciseName: string) => {
    ...
    const sortedSessions = [...strengthSessions].sort((a: any, b: any) =>
      b.date.localeCompare(a.date)
    );
    const relevantSessions = sortedSessions.filter((s: any) =>
      s.exercises.some((e: any) =>
        (e.exerciseName || e.name).toLowerCase() === exerciseName.toLowerCase()
      )
    );
    ...
    const lastEx = (lastSession.exercises as any[]).find((e) =>
      (e.exerciseName || e.name).toLowerCase() === exerciseName.toLowerCase()
    );
    if (!lastEx || (!lastEx.sets && (lastEx as any).weight === undefined)) {
      return null;
    }
    ...
    for (const session of relevantSessions) {
      const ex = (session.exercises as any[]).find((e) =>
        (e.exerciseName || e.name).toLowerCase() === exerciseName.toLowerCase()
      );
      if (!ex) continue;
      ...
      if (Array.isArray(ex.sets)) {
        sessionMax = ex.sets.reduce(
          (max: number, s: any) =>
            Math.max(max, calculateEstimated1RM(s.weight, s.reps)),
          0,
        );
      } else {
        sessionMax = calculateEstimated1RM(
          (ex as any).weight || 0,
          (ex as any).reps || 0,
        );
      }
    ...
    const bestSet = lastEx.sets.reduce(
      (best: StrengthSet, current: StrengthSet) => {
  ...
  const applyTarget = (
    ...
    const eIdx = newW.exercises[sIdx].exercises.findIndex((x: any) =>
      x.id === exerciseId
    );
    if (eIdx === -1) return;

    newW.exercises[sIdx].exercises[eIdx].weight = typeof weight === "number"
      ? `${weight}kg`
      : weight;
      ...
      <div className="h-20 border-b border-white/5 bg-slate-900/20 flex items-center justify-between px-8 backdrop-blur-2xl">
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <label className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] mb-1">
              Passets titel
            </label>
            <input
              type="text"
          ...
          <div className="max-w-4xl mx-auto space-y-12 pb-48">
            ...
            <div className="bg-slate-900/40 border border-white/10 rounded-[2.5rem] p-10 grid grid-cols-3 gap-10 relative overflow-hidden backdrop-blur-xl">
              <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none select-none">
                <span className="text-9xl text-white font-black italic tracking-tighter">
                  GENUS
                </span>
              </div>
              ...
              <div className="col-span-3 flex flex-wrap gap-2 pt-2 border-t border-white/5">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest self-center mr-4">
                  Tränar:
                </span>
                {workout.targetedMuscles && workout.targetedMuscles.length > 0
                  ? (
                    workout.targetedMuscles.map((m) => (
                      <span
                        key={m}
                        className="px-4 py-1.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-[10px] font-black rounded-full uppercase tracking-wider"
                      >
                        {m}
                      </span>
                    ))
                  ...
                  : (
                    <span className="text-[10px] text-slate-600 italic">
                      Lägg till övningar för att se muskelgrupper...
                    </span>
                  )}
              </div>
            </div>
            ...
            <div className="space-y-8">
              {workout.exercises?.map((section, idx) => (
                <div
                  ...
                  className={`group transition-all duration-500 rounded-[2.5rem] p-8 border ${
                    activeSectionId === section.id
                      ? "border-indigo-500/40 bg-indigo-500/5 shadow-3xl shadow-indigo-500/10"
                ...
                >
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const newEx = workout.exercises!.filter((s) =>
                          s.id !== section.id
                        );
                        update("exercises", newEx);
                      }}
                  ...
                  <div className="space-y-3">
                    {section.exercises.map((ex, i) => (
                      <div
                        key={ex.id}
                        className="flex items-center gap-6 bg-slate-950/40 border border-white/5 p-6 rounded-3xl hover:border-white/20 transition-all hover:translate-x-1 group/ex"
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-8">
                            <div className="flex flex-col">
                              <span className="text-[9px] font-black text-slate-600 uppercase tracking-[0.1em] mb-1">
                                Set
                              </span>
                              <input
                                className="bg-transparent font-mono text-indigo-400 font-bold outline-none"
                                defaultValue={ex.sets}
                                onBlur={(e) => {
                                  const sIdx = workout.exercises!.findIndex(
                                    (s) => s.id === section.id,
                                  );
                                  const eIdx = section.exercises.findIndex(
                                    (x) => x.id === ex.id,
                                  );
                                  const newW = JSON.parse(
                                    JSON.stringify(workout),
                                  );
                                  newW.exercises[sIdx].exercises[eIdx].sets =
                            ...
                            <div className="flex flex-col">
                              ...
                              <input
                                ...
                                onBlur={(e) => {
                                  ...
                                  newW.exercises[sIdx].exercises[eIdx].reps =
                                    e.target.value;
                                  setWorkout(newW);
                                }}
                              />
                            ...
                            <div className="flex flex-col">
                              ...
                              <input
                                ...
                                onBlur={(e) => {
                                  ...
                                  newW.exercises[sIdx].exercises[eIdx].weight =
                                    e.target.value;
                                  setWorkout(newW);
                                }}
                              />
                            ...
                            return (
                              <div className="mt-3 space-y-3">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <button
                                    onClick={() =>
                                      applyTarget(
                                        section.id,
                                        ex.id,
                                        target.powerTarget.weight,
                                        target.powerTarget.reps,
                                      )}
                                    ...
                                    title="Öka vikt (+2.5%)"
                                  >
                                    ⚡ Power: {target.powerTarget.weight}kg x
                                    {" "}
                                    {target.powerTarget.reps}
                                  </button>
                                  <button
                                    onClick={() =>
                                      applyTarget(
                                        section.id,
                                        ex.id,
                                        target.volumeTarget.weight,
                                        target.volumeTarget.reps,
                                      )}
                                  ...
                                  >
                                    📈 Volym: {target.volumeTarget.weight}kg x
                                    {" "}
                                    {target.volumeTarget.reps}
                                  </button>
                                </div>
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover/ex:opacity-100 p-2 text-slate-600 hover:text-rose-500 transition-all"
                        >
                          ✕
                        </button>
                      </div>
                    ...
                    {section.exercises.length === 0 && (
                      <div className="py-12 text-center border-2 border-dashed border-white/5 rounded-[2.5rem] bg-slate-900/10">
                        ...
                        <button className="text-sm text-indigo-400 font-black uppercase tracking-widest hover:text-indigo-300 transition-colors">
                          + Sök i övningsarkivet
                        </button>
                      </div>
                    )}
                  </div>
          ...
          <div className="p-6 border-b border-white/5 flex gap-2">
            {[
              ...
              <button
                ...
                className={`flex-1 py-3 text-[11px] font-black uppercase tracking-widest rounded-2xl transition-all ${
                  activeTab === t.id
