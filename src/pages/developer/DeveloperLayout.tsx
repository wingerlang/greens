      ...
      <div className="space-y-4">
        <div>
          <h3 className="text-sm font-medium text-slate-400 mb-2">
            Excluded Folders (Analysis & Explorer)
          </h3>
          <div className="space-y-2">
            {commonFolders.map((folder) => (
              <label
                key={folder}
                className="flex items-center gap-2 p-2 rounded bg-slate-800 cursor-pointer hover:bg-slate-700"
              >
                <input
                  type="checkbox"
        ...
        <div className="pt-4 border-t border-slate-700">
          ...
          <div className="mt-2 space-y-1">
            {excludedFolders.filter((f) => !commonFolders.includes(f)).map(
              (f) => (
                <div
                  key={f}
                  className="flex items-center justify-between p-2 rounded bg-slate-800 text-sm"
                >
                  <span className="font-mono text-slate-300">{f}</span>
                  <button
                    onClick={() => toggleExclusion(f)}
