...
import { CheckSquare, AlertCircle, FileText, ChevronRight, Filter } from 'lucide-react';
...
export function DeveloperManagement() {
    ...
    useEffect(() => {
        ...
        .then(data => {
            ...
            const todos = (data.comments || []).filter((c: Comment) => c.type === 'todo');
            setComments(todos);
        })
        .catch(console.error)
    ...
    if (loading) return <div className="p-8 text-slate-400 animate-pulse">Scanning for TODOs...</div>;
                    ...
                    <div className="flex bg-slate-700 rounded-md p-0.5">
                        {['all', 'todo', 'fixme'].map((f) => (
                            <button
                                ...
                                className={`px-2 py-1 text-xs font-medium rounded transition-colors uppercase ${filter === f ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}
                            >
                                {f}
                            </button>
                        ))}
                ...
                <div className="flex-1 overflow-auto divide-y divide-slate-800">
                    {filteredComments.map((comment, i) => (
                        <div
                            ...
                            className={`p-4 cursor-pointer hover:bg-slate-800/50 transition-colors ${selectedComment === comment ? 'bg-indigo-900/20 border-l-2 border-indigo-500' : 'border-l-2 border-transparent'}`}
                        >
                            <div className="flex items-start justify-between gap-2 mb-1">
                                <span className={`text-xs font-bold px-1.5 py-0.5 rounded uppercase ${comment.text.toLowerCase().includes('fixme') ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400'}`}>
                                    {comment.text.toLowerCase().includes('fixme') ? 'FIXME' : 'TODO'}
                                </span>
                                <span className="text-xs text-slate-500 font-mono truncate max-w-[120px]" title={comment.file}>
                    ...
                    <>
                        <div className="p-4 border-b border-slate-700 bg-slate-800">
                             <div className="flex items-center gap-2 text-sm text-slate-400 mb-2">
                                ...
                                <span className="text-slate-600">L{selectedComment.line}</span>
                             </div>
                        ...
                        <div className="flex-1 bg-slate-950 p-4 overflow-auto">
                            <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Code Context</h4>
                                ...
                                <SyntaxHighlighter
                                    ...
                                    lineProps={(lineNumber) => {
                                        return lineNumber === selectedComment.line
                                            ? { style: { display: 'block', backgroundColor: 'rgba(255, 180, 0, 0.1)' } }
                                            : {};
                                    }}
                                >
                            ...
                            <div className="mt-6 flex justify-end">
                                <button
                                    onClick={() => {/* Integration with issue tracker could go here */}}
