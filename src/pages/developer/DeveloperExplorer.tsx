...
export function DeveloperExplorer() {
  const { token } = useAuth();
  const { excludedFolders, setExcludedFolders, refreshTrigger } =
    useDeveloper();
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    fetch(
      `/api/developer/search?q=${encodeURIComponent(searchQuery)}&${query}`,
      {
        headers: { "Authorization": `Bearer ${token}` },
      },
    )
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          } else {setExcludedFolders(
                              excludedFolders.filter((f) => f !== folder),
                            );}
                        }}
                ...
                {searchResults.map((res, i) => (
                  <div key={i} className="space-y-1">
                    ...
                    {res.matches.slice(0, 3).map((m, j) => (
                      <div
                        key={j}
                        className="text-xs text-slate-400 pl-4 truncate opacity-70"
                      >
                        L{m.line}: {m.content}
                      </div>
                    ))}
                ...
                : (
                  <div className="flex flex-col h-full">
                    <div className="flex items-center bg-slate-800 border-b border-slate-700 p-2 text-xs font-semibold text-slate-300 sticky top-0">
                      <div
                        className="flex-1 px-2 cursor-pointer hover:text-white flex items-center gap-1"
                        onClick={() => toggleSort("name")}
                      >
                        File <SortIcon field="name" />
                      </div>
                      <div
                        className="w-16 px-2 text-right cursor-pointer hover:text-white flex items-center justify-end gap-1"
                    ...
                    <div className="overflow-auto flex-1 p-0">
                      {flatFiles.map((file) => (
                        <div
                          ...
                          className={`flex items-center p-2 border-b border-slate-800 cursor-pointer hover:bg-slate-800 ${
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden">
          <File
            size={14}
            className={selectedPath === node.path
              ? "text-indigo-400"
              : "text-slate-600 group-hover:text-slate-500"}
          />
          <span className="truncate">{node.name}</span>
        </div>
        <span className="text-xs text-slate-600 ml-2 whitespace-nowrap">
          {node.lines} L
        </span>
      </div>
    );
