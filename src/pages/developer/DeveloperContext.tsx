...
export function DeveloperProvider({ children }: { children: React.ReactNode }) {
    ...
    const isExcluded = (path: string) => {
        return excludedFolders.some(ex => path === ex || path.startsWith(ex + '/') || path.includes('/' + ex + '/'));
    };

    const triggerRefresh = () => setRefreshTrigger(prev => prev + 1);

    return (
        <DeveloperContext.Provider value={{ excludedFolders, toggleExclusion, isExcluded, refreshTrigger, triggerRefresh }}>
            {children}
