...
import { LineChart, Line, BarChart, Bar, ScatterChart, Scatter, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend, ZAxis } from 'recharts';
import { Camera, RefreshCw, GitCommit, Layers, Box, AlertOctagon, Filter, ChevronDown, ChevronRight, Hash } from 'lucide-react';
...
export function DeveloperDeepAnalysis() {
    ...
    // View State
    const [selectedSnapshot, setSelectedSnapshot] = useState<DeveloperSnapshot | null>(null);
    ...
    const takeSnapshot = async () => {
        ...
        try {
            const excludesList = customExcludes.split(',').map(s => s.trim()).filter(s => s);
    ...
    if (loading) return <div className="p-8 text-slate-400 animate-pulse">Loading Deep Analysis...</div>;
        ...
        <div className="space-y-6 max-w-[1600px] mx-auto pb-12">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">Deep System Analysis & Evolution</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        ...
                        >
                            {snapshotting ? <RefreshCw size={16} className="animate-spin" /> : <Camera size={16} />}
                ...
                {showControls && (
                    <div className="mt-4 pt-4 border-t border-slate-800 animate-in fade-in slide-in-from-top-2">
                        ...
                        <p className="text-xs text-slate-500 mt-2">
                            Comma separated. These rules will be applied to the new snapshot and stored in history.
                        </p>
                    </div>
                )}
            </div>
            ...
            {history.length > 0 && (
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                    {history.map(h => (
                        ...
                        >
                            <span className={`font-semibold ${selectedSnapshot?.timestamp === h.timestamp ? 'text-indigo-400' : 'text-slate-300'}`}>
                            ...
                            <div className="mt-2 flex gap-2">
                                <span className="text-slate-400">{h.stats.totalFiles} files</span>
                            </div>
                        </button>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <MetricCard
                            ...
                            value={currentStats?.complexityStats?.averageDepth.toFixed(1) || "N/A"}
                            icon={<AlertOctagon size={16} className="text-amber-400" />}
                            trend={evolutionData.length > 1 ? (evolutionData[evolutionData.length-1].complexity - evolutionData[evolutionData.length-2].complexity).toFixed(1) : undefined}
                        />
                        <MetricCard
                            title="Dependencies"
                     ...
                     <div className="bg-slate-800 p-5 rounded-xl border border-slate-700">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {currentStats?.excludedRules && currentStats.excludedRules.length > 0 ? (
                                currentStats.excludedRules.map((rule, i) => (
                                    <span key={i} className="px-2 py-1 bg-slate-900 border border-slate-700 rounded text-xs text-slate-400 font-mono">
                                        {rule}
                                    </span>
                                ))
                            ) : (
                                <span className="text-xs text-slate-500 italic">No custom exclusions recorded.</span>
                            )}
                        </div>
                     </div>
                     ...
                     <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 h-[300px]">
                        <h3 className="text-sm font-semibold text-slate-300 mb-4">System Evolution</h3>
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={evolutionData}>
                                ...
                                <Legend />
                                <Line yAxisId="left" type="monotone" dataKey="files" name="Files" stroke="#10b981" strokeWidth={2} dot={false} />
                                <Line yAxisId="right" type="monotone" dataKey="complexity" name="Complexity" stroke="#f59e0b" strokeWidth={2} dot={false} />
                                <Line yAxisId="right" type="monotone" dataKey="dependencies" name="Deps" stroke="#3b82f6" strokeWidth={2} strokeDasharray="4 4" dot={false} />
                            </LineChart>
                        </ResponsiveContainer>
                     </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 h-[350px]">
                         <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center justify-between">
                            ...
                            <span className="text-xs text-slate-500 font-normal">Indent Levels</span>
                        </h3>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart layout="vertical" data={complexityData.slice(0, 10)}>
                                ...
                                <Bar dataKey="depth" name="Max Indentation" fill="#f59e0b" radius={[0, 4, 4, 0]} barSize={16} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 h-[400px] overflow-hidden flex flex-col">
                        <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
                             <GitCommit size={14} className="text-purple-400" /> Hotspots (Most Changed)
                        </h3>
                         <div className="flex-1 overflow-auto pr-2 custom-scrollbar">
                            <div className="space-y-2">
                                {hotspotData.map((file, i) => (
                                    <div key={i} className="flex items-center justify-between p-2 rounded hover:bg-slate-700/50 transition-colors group">
                                        <div className="min-w-0">
                                            <div className="text-xs font-mono text-slate-300 truncate" title={file.fullPath}>{file.name}</div>
                                            <div className="text-[10px] text-slate-500 truncate" title={file.fullPath}>{file.fullPath}</div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <div className="h-1.5 w-16 bg-slate-700 rounded-full overflow-hidden">
                                                <div className="h-full bg-purple-500" style={{ width: `${Math.min(100, (file.changes / (hotspotData[0]?.changes || 1)) * 100)}%` }} />
                                            </div>
                                            <span className="text-xs font-mono text-slate-400 w-6 text-right">{file.changes}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="space-y-6">
                    {/* New Files Trend */}
                    <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 h-[300px]">
                        <h3 className="text-sm font-semibold text-slate-300 mb-4">File Creation Velocity</h3>
                         <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={newFilesData}>
                                ...
                                <Bar dataKey="count" name="New Files" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 p-6 rounded-xl border border-indigo-500/20">
                        ...
                        <p className="text-sm text-slate-400 leading-relaxed">
                            This dashboard provides deep insights into codebase evolution.
                            Use snapshots to track how complexity and dependencies grow over time.
                            <br/><br/>
                            <strong>Hotspots</strong> identify files that change frequently—candidates for refactoring.
...
function MetricCard({ title, value, icon, subtitle, trend }: { title: string, value: string | number, icon: React.ReactNode, subtitle?: string, trend?: string | number }) {
    return (
        <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
            ...
            <div className="flex items-end gap-2">
                ...
                {trend && (
                    <span className={`text-xs mb-1 ${Number(trend) > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>
