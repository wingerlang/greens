...
import { AlertTriangle, Copy, Trash2, FileWarning, ArrowRight, ShieldCheck } from 'lucide-react';
...
export function DeveloperHealth() {
    ...
    if (loading) return <div className="p-8 text-slate-400 animate-pulse">Running health check...</div>;
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto pb-12">
            <header>
                ...
                <p className="text-slate-400">Automated analysis of technical debt, duplication, and unused code.</p>
            </header>
            ...
            <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
                <div className="p-4 border-b border-slate-700 bg-slate-800/50 flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">Files exceeding 300 lines</span>
                </div>
                <div className="divide-y divide-slate-700/50">
                    {largeFiles.length === 0 ? (
                        ...
                        largeFiles.slice(0, 10).map((issue, i) => (
                            <div key={i} className="p-4 hover:bg-slate-700/30 transition-colors flex items-center justify-between">
                                <span className="font-mono text-sm text-slate-300">{issue.file}</span>
                                <span className={`text-xs px-2 py-1 rounded-full border ${issue.severity === 'high' ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-amber-500/10 border-amber-500/50 text-amber-400'}`}>
            ...
            <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
                <div className="p-4 border-b border-slate-700 bg-slate-800/50 flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">Similar functions detected</span>
                </div>
                <div className="divide-y divide-slate-700/50">
                    {duplicates.length === 0 ? (
                        <div className="p-8 text-center text-slate-500 flex flex-col items-center gap-2">
                        ...
                        duplicates.map((dup, i) => (
                            <div key={i} className="p-4 hover:bg-slate-700/30 transition-colors space-y-2">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-sm text-white font-medium">
                                        <span>{dup.nameA}</span>
                                        {dup.nameA !== dup.nameB && <span className="text-slate-500">vs {dup.nameB}</span>}
                                    </div>
                                ...
                                <div className="flex items-center gap-4 text-xs text-slate-400 font-mono">
                                    <span className="truncate flex-1" title={dup.fileA}>{dup.fileA.split('/').slice(-2).join('/')}</span>
                                    <ArrowRight size={12} className="text-slate-600" />
                                    <span className="truncate flex-1 text-right" title={dup.fileB}>{dup.fileB.split('/').slice(-2).join('/')}</span>
                                </div>
                            </div>
                        ))
                    )}
            ...
            <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
                <div className="p-4 border-b border-slate-700 bg-slate-800/50 flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">Not imported by other files</span>
                </div>
                <div className="p-0">
                    {unusedFiles.length === 0 ? (
                        ...
                        <div className="max-h-[300px] overflow-auto">
                             {unusedFiles.map((file, i) => (
                                <div key={i} className="p-3 px-4 hover:bg-slate-700/30 border-b border-slate-700/50 flex items-center justify-between group">
...
function HealthCard({ title, value, icon, color }: { title: string, value: number, icon: React.ReactNode, color: string }) {
    return (
