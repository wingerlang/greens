...
const MAIN_EXERCISES = [
  ...
  { id: "bench", name: "Bänkpress", patterns: ["bänkpress", "bench", "bänk"] },
  {
    id: "deadlift",
    name: "Marklyft",
    patterns: ["marklyft", "deadlift", "mark"],
  },
  {
    id: "overhead",
    name: "Militärpress",
    patterns: ["militärpress", "overhead", "axlar"],
  },
...
export function MatchupPage() {
  const {
    users,
    currentUser,
    strengthSessions: mySessions,
    weightEntries: myWeightEntries,
  } = useData();
  ...
  const [progressionFromDate, setProgressionFromDate] = useState<string>(
    "2025-01-01",
  );
  const [progressionToDate, setProgressionToDate] = useState<string>(
  ...
  // --- Resolved Users ---
  const userA = useMemo(() => users.find((u) => u.id === userAId), [
    users,
    userAId,
  ]);
  const userB = useMemo(() => users.find((u) => u.id === userBId), [
    users,
    userBId,
  ]);
  ...
  const fetchSessions = async (uid: string) => {
    ...
    if (uid === currentUser?.id && mySessions && mySessions.length > 0) {
      return mySessions;
    }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [dataA, dataB, prA, prB, extA, extB] = await Promise.all([
        ...
        fetch(`/api/user/prs/detect?userId=${userAId}`, {
          headers: { "Authorization": `Bearer ${token}` },
        }),
        fetch(`/api/user/prs/detect?userId=${userBId}`, {
          headers: { "Authorization": `Bearer ${token}` },
        }),
      ]);
      setSessionsA(dataA);
  ...
  const filteredSessionsA = useMemo(
    () => sessionsA.filter((s) => new Date(s.date) >= filterDate),
    [sessionsA, filterDate],
  );
  const filteredSessionsB = useMemo(
    () => sessionsB.filter((s) => new Date(s.date) >= filterDate),
    [sessionsB, filterDate],
  );
  ...
  const get1RM = (sessions: any[], patterns: string[]) =>
    getExerciseStats(sessions, patterns).max1RM;
        ...
        if (name === normalizedTarget) {
          ...
          if (ex.sets && Array.isArray(ex.sets)) {
            ex.sets.forEach((set: any) => {
              ...
              const estimated = conservative1RM(weight, reps);
              const shouldReplace = estimated > estimated1RM ||
                (estimated >= estimated1RM * 0.95 &&
                  reps < estimatedBestSetReps);
              if (shouldReplace) {
  ...
  const powerStats = useMemo(() => {
    const weightA = userA?.settings?.weight ||
      (userAId === currentUser?.id ? (myWeightEntries[0]?.weight || 80) : 80);
    const weightB = userB?.settings?.weight || 85;

    // Enhanced 1RM getter that returns details
    const getBestSetForPatternsLocal = (
      sessions: any[],
      patterns: string[],
    ...
    const squatA = getBestSetForPatternsLocal(sessionsA, [
      "knäböj",
      "squat",
      "böj",
    ], [
    ...
    const benchA = getBestSetForPatternsLocal(sessionsA, [
      "bänkpress",
      "bench",
      "bänk",
    ], [
    ...
    const deadA = getBestSetForPatternsLocal(sessionsA, [
      "marklyft",
      "deadlift",
      "mark",
    ], [
    ...
    const squatB = getBestSetForPatternsLocal(sessionsB, [
      "knäböj",
      "squat",
      "böj",
    ], [
    ...
    const benchB = getBestSetForPatternsLocal(sessionsB, [
      "bänkpress",
      "bench",
      "bänk",
    ], [
    ...
    const deadB = getBestSetForPatternsLocal(sessionsB, [
      "marklyft",
      "deadlift",
      "mark",
    ], [
    ...
    const totalA = squatA.maxEstimated1RM + benchA.maxEstimated1RM +
      deadA.maxEstimated1RM;
    const totalB = squatB.maxEstimated1RM + benchB.maxEstimated1RM +
      deadB.maxEstimated1RM;

    const genderA =
      (userA?.settings?.gender === "female" ? "female" : "male") as
        | "male"
        | "female";
    const genderB =
      (userB?.settings?.gender === "female" ? "female" : "male") as
        | "male"
        | "female";
    ...
    return {
      totalA,
      totalB,
      pointsA: isNaN(pointsA)
        ? 0
        : Math.round(pointsA * (viewMode === "relative" ? 100 : 10)) /
          (viewMode === "relative" ? 100 : 10),
      pointsB: isNaN(pointsB)
        ? 0
        : Math.round(pointsB * (viewMode === "relative" ? 100 : 10)) /
          (viewMode === "relative" ? 100 : 10),
      weightA,
      weightB,
      breakdown: [
  ...
  const tableData = useMemo(() => {
    ...
    const getAvgSleep = (extra: any) => {
      if (!extra || !extra.sleepSessions || extra.sleepSessions.length === 0) {
        return 0;
      }
      const sessions = extra.sleepSessions.filter((s: any) =>
        new Date(s.date) >= filterDate
      );
    ...
    const scale = (
      ...
      if (statsMode === "points" && isVolume) {
        return calculateIPFPoints(weight, val, gender);
      }
      return val;
    };
    ...
    return [
      ...
      {
        label: "Total Vikt",
        a: format(scale(statsA.vol, weightA, genderA, true), true),
        b: format(scale(statsB.vol, weightB, genderB, true), true),
        unit: statsMode === "raw" ? "kg" : "pts",
        isRaw: true,
      },
      {
        label: "Total Tid",
      ...
      {
        label: "Vikt / Pass",
        a: format(scale(statsA.avgVol, weightA, genderA, true), true),
        b: format(scale(statsB.avgVol, weightB, genderB, true), true),
        unit: statsMode === "raw" ? "kg" : "pts",
        isRaw: true,
      },
      {
        label: "Set / Pass",
        a: statsA.avgSets.toFixed(1),
        b: statsB.avgSets.toFixed(1),
        unit: "st",
        isRaw: true,
      },
      {
        label: "Reps / Pass",
        a: Math.round(statsA.avgReps),
        b: Math.round(statsB.avgReps),
        unit: "st",
        decimals: 0,
      },
      {
        label: "Reps / Set",
        a: statsA.sets > 0 ? (statsA.reps / statsA.sets).toFixed(1) : "-",
        b: statsB.sets > 0 ? (statsB.reps / statsB.sets).toFixed(1) : "-",
        unit: "st",
        isRaw: true,
      },
      {
        label: "Övningar / Pass",
        a: statsA.avgEx.toFixed(1),
        b: statsB.avgEx.toFixed(1),
        unit: "st",
        isRaw: true,
      },
      {
        label: "Tid / Pass",
        a: Math.round(statsA.avgTime),
        b: Math.round(statsB.avgTime),
        unit: "min",
        decimals: 0,
      },
    ];
  ...
  const progressionData = useMemo(() => {
    ...
    const get1RMUpToDate = (
      ...
      sessions.filter((s) => new Date(s.date) <= upToDate).forEach(
        (session) => {
          if (!session.exercises) return;
          session.exercises.forEach((ex: any) => {
  ...
  const formatTime = (seconds: any) => {
    ...
    const rmins = mins % 60;
    if (hrs > 0) {
      return `${hrs}:${rmins.toString().padStart(2, "0")}:${
          ...
          <div className="flex items-center gap-3 flex-1 justify-end">
              ...
              <div className="relative group inline-flex items-center gap-1 justify-end">
                ...
                >
                  {users.map((u) => (
                    ...
                    >
                      {u.name} {u.id === currentUser?.id ? "(Mig)" : ""}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="relative">
              ...
              {userAId === currentUser?.id && (
                <div className="absolute -top-1 -right-1 bg-emerald-400 text-slate-900 text-[8px] font-black px-1 rounded shadow-sm">
                  ME
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-3 flex-1">
            <div className="relative">
              ...
              {userBId === currentUser?.id && (
                <div className="absolute -top-1 -left-1 bg-indigo-400 text-slate-900 text-[8px] font-black px-1 rounded shadow-sm">
                  ME
                </div>
              )}
            </div>
              ...
              {isSearchingB
                ? (
                  <div className="absolute left-0 top-0 z-[60] min-w-[200px] bg-slate-900 border border-white/10 rounded-xl p-2 shadow-2xl animate-in fade-in zoom-in duration-200">
                    <input
                      ...
                      onBlur={() =>
                        setTimeout(() => setIsSearchingB(false), 200)}
                    />
                    <div className="mt-2 max-h-[200px] overflow-y-auto">
                      {users.filter((u) => {
                        const s = userBSearch.toLowerCase();
                        return u.name.toLowerCase().includes(s) ||
                          (u.handle || u.username).toLowerCase().includes(
                            s.replace("@", ""),
                          );
                      }).map((u) => (
                        <button
                        ...
                        >
                          <div className="w-6 h-6 rounded bg-slate-800 flex items-center justify-center font-bold">
                            {u.name[0]}
                          </div>
                          <div className="flex flex-col">
                            <span className="font-bold group-hover/u:text-white">
                              {u.name}
                            </span>
                ...
                : (
                  <div
                    ...
                    onClick={() => setIsSearchingB(true)}
                  >
                    <div className="flex items-center gap-1 group">
                      <span className="text-lg sm:text-2xl font-black text-white hover:text-indigo-400 transition-colors uppercase decoration-dotted underline-offset-4 decoration-indigo-500/30 underline">
                        {userB?.name || "Välj..."}
                      </span>
                      <ChevronDown
                        size={16}
      ...
      <div className="max-w-6xl mx-auto space-y-8 px-4 sm:px-0">
        ...
        <section className="glass p-4 sm:p-6 rounded-[32px] border border-white/5 relative overflow-hidden group">
          ...
          <div className="absolute left-4 top-1/2 -translate-y-1/2 opacity-[0.08] group-hover:opacity-[0.25] transition-opacity duration-300 pointer-events-none">
            <svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke={userA?.settings?.gender === "female"
                ? "#ec4899"
                : "#3b82f6"}
              strokeWidth="0.8"
              className="group-hover:drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]"
            >
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-[0.08] group-hover:opacity-[0.25] transition-opacity duration-300 pointer-events-none">
            <svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke={userB?.settings?.gender === "female"
                ? "#ec4899"
                : "#3b82f6"}
              strokeWidth="0.8"
              className="group-hover:drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]"
            >
          ...
          <div className="grid grid-cols-3 gap-4 text-center items-center relative z-10">
            ...
            <div className="space-y-3">
              <div className="flex flex-col">
                <span className="text-xl sm:text-3xl font-black text-white">
                  {userA?.settings?.birthYear
                    ? (new Date().getFullYear() - userA.settings.birthYear)
                    : "?"}
                </span>
            ...
            <div className="space-y-3">
              <div className="flex flex-col text-right">
                <span className="text-xl sm:text-3xl font-black text-white">
                  {userB?.settings?.birthYear
                    ? (new Date().getFullYear() - userB.settings.birthYear)
                    : "?"}
                </span>
                <span className="text-[9px] text-slate-500 font-bold">ÅR</span>
              </div>
              <div className="flex flex-col text-right">
                <span className="text-xl sm:text-3xl font-black text-indigo-400">
                  {userB?.settings?.weight || "?"}
                </span>
                <span className="text-[9px] text-slate-500 font-bold">KG</span>
              </div>
              <div className="flex flex-col text-right">
                <span className="text-xl sm:text-3xl font-black text-white">
                  {userB?.settings?.height || "?"}
                </span>
                <span className="text-[9px] text-slate-500 font-bold">CM</span>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="group relative">
                  ...
                  <div className="absolute left-0 top-6 w-72 bg-slate-900 border border-white/10 rounded-xl p-4 text-xs text-slate-300 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                    ...
                    <p className="mb-2">
                      ...
                      Summan av bästa 1RM för bänkpress, knäböj och marklyft i
                      kg.
                    </p>
                    <p className="mb-2">
                      <span className="text-amber-400 font-bold">Relativ:</span>
                      {" "}
                      Total styrka delat med kroppsvikt. Visar hur stark du är i
                      förhållande till din storlek.
                    </p>
                    <p>
                      <span className="text-indigo-400 font-bold">
                        Poäng (IPF):
                      </span>{" "}
                      Internationella Styrkelyftförbundets formel som korrigerar
                      för både kön och kroppsvikt – möjliggör rättvis
                      jämförelse.
                    </p>
                  </div>
                </div>
              ...
              <div className="flex bg-slate-900 rounded-lg p-1">
                ...
                <button
                  ...
                  title="IPF-poäng (korrigerat för kön & vikt)"
                >
                  Poäng
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-900/50 border border-white/5 relative overflow-hidden">
                <span className="text-[10px] uppercase text-slate-500 font-bold block mb-1">
                  {userA?.name?.toUpperCase()} TOTAL
                </span>
                <div className="text-4xl font-black text-white">
                  {viewMode === "raw" ? powerStats.totalA : powerStats.pointsA}
                </div>
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-900/50 border border-white/5 relative overflow-hidden">
                <span className="text-[10px] uppercase text-slate-500 font-bold block mb-1">
                  {userB?.name?.toUpperCase()} TOTAL
                </span>
                <div className="text-4xl font-black text-slate-300">
                  {viewMode === "raw" ? powerStats.totalB : powerStats.pointsB}
                </div>
              ...
              <div className="flex flex-col gap-2">
                {powerStats.breakdown.map((item) => {
                  ...
                  const renderUserStats = (stats: any, color: string) => {
                    if (!stats.bestEstimatedSet && !stats.heaviestSet) {
                      return (
                    ...
                    const isSame = stats.bestEstimatedSet &&
                      stats.heaviestSet &&
                      stats.bestEstimatedSet.weight ===
                        stats.heaviestSet.weight &&
                    ...
                    const renderSet = (
                      ...
                      <div className="mb-1 last:mb-0">
                        ...
                        >
                          ...
                          <span
                            className={`text-xs font-bold text-${color}-400 font-mono`}
                          >
                            {set.weight}kg{" "}
                        ...
                        <div
                          className={`flex flex-col ${
                            color === "emerald" ? "items-end" : "items-start"
                          }`}
                        >
                          <span
                    ...
                    if (isSame) {
                      return renderSet(
                        stats.bestEstimatedSet,
                        stats.exactNameEstimated,
                        "BÄSTA SET",
                  ...
                  return (
                    <div
                      key={item.id}
                      className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 p-2 rounded-lg bg-white/[0.02] hover:bg-white/[0.04] transition-colors group"
                    >
                      {/* User A Side */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white tracking-tight">
                  Giganternas Kamp – Statistik
                </h3>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                  Djupanalys av träningsdata
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
              ...
              <div className="flex bg-slate-900 p-1 rounded-xl border border-white/5 shadow-2xl shadow-black/20">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-tight transition-all flex items-center gap-1.5 ${
                    statsMode === "raw"
                ...
                >
                  <Scale
                    size={12}
                    className={statsMode === "bw" ? "text-emerald-400" : ""}
                  />
                  Vikt-korrigerad
                </button>
                <button
                ...
                >
                  <Users
                    size={12}
                    className={statsMode === "points" ? "text-indigo-400" : ""}
                  />
                  Kön & Vikt (IPF)
                </button>
              </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/5">
                {tableData.filter((row) => {
                  ...
                  >
                    <td className="px-4 py-2 font-bold text-slate-300 text-[11px]">
                      {row.label}
                    </td>
        ...
        <section className="glass p-6 sm:p-8 rounded-3xl">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-3 bg-slate-900/50 p-3 rounded-xl border border-white/5">
              <div className="flex flex-col">
                <label className="text-[9px] text-slate-500 uppercase font-black mb-1">
                  Från
                </label>
                <input
                  type="date"
              ...
              <div className="flex gap-1 ml-2">
                <button
                  onClick={() => {
                    setProgressionFromDate("2025-01-01");
                    setProgressionToDate(
                      new Date().toISOString().split("T")[0],
                    );
                  }}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/[0.02] text-[10px] font-black text-slate-500 uppercase tracking-widest">
                  ...
                  <th className="px-4 py-3 border-b border-white/5 text-center">
                    Flest PB
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {progressionData.filter((row) =>
                  row.currentA > 0 || row.currentB > 0
                ).map((row, i) => (
                  <tr
                    key={i}
                    className="hover:bg-white/[0.02] transition-colors group"
                  >
                    <td className="px-4 py-3 font-bold text-slate-300 text-[11px]">
                      <Link
                        to={`/strength/${encodeURIComponent(row.name)}`}
                        className="hover:text-emerald-400 transition-colors"
                      >
                        {row.name}
                      </Link>
                      <span className="block text-[9px] text-slate-600 font-normal">
                        Nu: {row.currentA}kg / {row.currentB}kg
                      </span>
                    </td>
        ...
        <section className="glass p-6 sm:p-8 rounded-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              const valA = formatTime(
                prsA.find((p) => p.category === pb.id)?.time,
              );
              const valB = formatTime(
                prsB.find((p) => p.category === pb.id)?.time,
              );
                ...
                <Link
                  ...
                  to={valA !== "-"
                    ? `/activities?activityId=${
                      prsA.find((p) => p.category === pb.id)?.activityId
                    }`
                    : "#"}
        ...
        <section>
          <div className="flex flex-col gap-4 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {/* Sort */}
              <div className="flex items-center gap-1 bg-slate-900/50 p-1 rounded-lg border border-white/5">
                <span className="text-[9px] text-slate-500 uppercase font-black px-2">
                  Sortera:
                </span>
                {[
                  { key: "alpha", label: "A-Ö" },
                  ...
                  {
                    key: "a-wins",
                    label: `${userA?.name?.split(" ")[0] || "A"} ⬆`,
                  },
                  {
                    key: "b-wins",
                    label: `${userB?.name?.split(" ")[0] || "B"} ⬆`,
                  },
              ...
              <div className="flex items-center gap-1 bg-slate-900/50 p-1 rounded-lg border border-white/5">
                ...
                {[
                  ...
                  {
                    key: "a-wins",
                    label: `${userA?.name?.split(" ")[0] || "A"} vinner`,
                  },
                  {
                    key: "b-wins",
                    label: `${userB?.name?.split(" ")[0] || "B"} vinner`,
                  },
                  { key: "tie", label: "Lika" },
                ].map((opt) => (
                  <button
            ...
            ? (
              <div className="text-center py-12 text-slate-500">
                <p className="text-lg font-bold mb-2">
                  Inga gemensamma övningar
                </p>
                <p className="text-xs">
                  Ni har inte gjort samma övningar ännu.
                </p>
              </div>
            )
            : (() => {
              ...
              const filteredData = exerciseData.filter((d) => {
                ...
                if (h2hFilter === "a-wins") {
                  return d.statsA.max1RM > d.statsB.max1RM;
                }
                if (h2hFilter === "b-wins") {
                  return d.statsB.max1RM >
                    d.statsA.max1RM;
                }
                if (h2hFilter === "tie") {
                  return d.statsA.max1RM ===
                    d.statsB.max1RM;
                }
                return true;
              });
                        ...
                        >
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between text-[10px] font-black text-slate-500 uppercase">
                              {(() => {
                                const urlA =
                                  statsA.bestSet && statsA.bestSet.sessionId
                                    ? `/strength/${
                                      encodeURIComponent(exName)
                                    }?sessionId=${statsA.bestSet.sessionId}`
                                    : `/strength/${encodeURIComponent(exName)}`;

                                const urlB =
                                  statsB.bestSet && statsB.bestSet.sessionId
                                    ? `/strength/${
                                      encodeURIComponent(exName)
                                    }?sessionId=${statsB.bestSet.sessionId}`
                                    : `/strength/${encodeURIComponent(exName)}`;
                                ...
                                return (
                                  <>
                                    ...
                                    >
                                      ...
                                      {rmMode === "actual" &&
                                        statsA.actual1RM === 0 &&
                                        statsA.estimated1RM > 0 && (
                                        <span className="block text-[7px] text-slate-600 italic">
                                          inga singelyft - visar e1RM
                                        </span>
                                      )}
                                      {rmMode === "actual" &&
                                        statsA.actual1RM > 0 &&
                                        statsA.estimated1RM !==
                                          statsA.actual1RM &&
                                        (
                                          <span className="block text-[7px] text-slate-600">
                                            e1RM: {statsA.estimated1RM}kg
                                          </span>
                                        )}
                                      {rmMode === "estimated" &&
                                        statsA.actual1RM > 0 && (
                                        <span className="block text-[7px] text-slate-600">
                                          1RM: {statsA.actual1RM}kg
                                        </span>
                                      )}
                                    </Link>
                                    <span className="text-[9px]">
                                      {rmMode === "actual" ? "1RM" : "e1RM"}
                                    </span>
                                    <Link
                                      to={urlB}
                                    ...
                                    >
                                      ...
                                      {rmMode === "actual" &&
                                        ...
                                        (
                                          <span className="block text-[7px] text-slate-600">
                                            e1RM: {statsB.estimated1RM}kg
                                          </span>
                                        )}
                                      {rmMode === "estimated" &&
                                        statsB.actual1RM > 0 && (
                                        <span className="block text-[7px] text-slate-600">
                                          1RM: {statsB.actual1RM}kg
                                        </span>
                                      )}
                                    </Link>
                                  </>
                            ...
                            <div className="flex justify-between text-[9px] text-slate-500 pt-1">
                              <span>
                                {(statsA.totalVolume / 1000).toFixed(1)}t vol
                              </span>
                              <span>{statsA.count} vs {statsB.count} pass</span>
                              <span>
                                {(statsB.totalVolume / 1000).toFixed(1)}t vol
                              </span>
                            </div>
                          </div>
                        </div>
        ...
        <section className="glass p-8 rounded-3xl flex flex-col justify-center items-center text-center border border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent">
          ...
          <p className="text-slate-400 max-w-lg mx-auto">
            Baserat på totalvolym och grensegrar har{" "}
            {powerStats.pointsA > powerStats.pointsB
              ? userA?.name
              : userB?.name} initiativet just nu.
          </p>
        </section>
