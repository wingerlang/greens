...
import {
  GoalProgressRing,
  MiniProgressRing,
} from "../components/goals/GoalProgressRing";
...
export function GoalsPage() {
  ...
  const viewingGoal = useMemo(
    () =>
      viewingGoalId
        ? performanceGoals.find((g) => g.id === viewingGoalId) || null
        : null,
    ...
    goals.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "deadline":
          ...
          break;
        case "category":
          comparison = (a.goal.category || "training").localeCompare(
            b.goal.category || "training",
          );
          break;
        case "name":
  ...
  const handleSaveGoal = (
    goalData: Omit<PerformanceGoal, "id" | "createdAt">,
  ) => {
    if (editingGoal) {
  ...
  const formatProgress = (current: number, target: number, unit?: string) => {
    ...
    return `${formatNum(current)} / ${formatNum(target)}${
      unit ? ` ${unit}` : ""
    }`;
  };
  ...
  const { periodGoals, looseGoals } = useMemo(() => {
    if (selectedCategory !== "all") {
      return { periodGoals: [], looseGoals: filteredGoals };
    }
          ...
          <div className="goals-hero-text">
            ...
            {activePeriod
              ? (
                <button
                  onClick={() =>
                    navigate(`/training/period/${activePeriod.id}`)}
                ...
                >
                  ...
                  <div className="ml-auto text-emerald-400/50 group-hover:text-emerald-400">
                    →
                  </div>
                </button>
              )
      ...
      <div className="goals-category-tabs">
        ...
        {(Object.keys(CATEGORY_CONFIG) as GoalCategory[]).map((cat) => (
          <button
            ...
            style={{
              "--accent": CATEGORY_CONFIG[cat].color,
        ...
        >
          ...
          >
            ...
            <p className="text-slate-300">
              Är du säker på att du vill ta bort detta mål? All progress och
              historik försvinner.
            </p>
