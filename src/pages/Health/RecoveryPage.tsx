...
export function RecoveryPage() {
  ...
  const [editingLog, setEditingLog] = useState<InjuryLog | undefined>(
    undefined,
  );
    ...
    async function fetchHistory() {
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/strength/workouts?start=${startStr}&end=${endStr}`,
          {
  ...
  const activeInjuries = injuryLogs.filter((log) =>
    log.status === "active" || log.status === "recovering"
  );

  // Computed Load & Risks
  const acuteLoad = useMemo(() => calculateAcuteLoad(strengthWorkouts), [
    strengthWorkouts,
  ]);
  const risks = useMemo(() => analyzeInjuryRisk(acuteLoad, injuryLogs), [
    acuteLoad,
    injuryLogs,
  ]);
  ...
  const durabilityScore = useMemo(() => {
    ...
    const totalLoad = Object.values(acuteLoad).reduce((a, b) =>
      a + b.load, 0) || 0;
  ...
  const readinessColor = durabilityScore > 80
    ? "text-emerald-400"
    : durabilityScore > 50
    ? "text-amber-400"
    : "text-rose-400";
  const durabilityLabel = durabilityScore > 90
    ? "Oskadlig"
    : durabilityScore > 75
    ? "Hållbar"
    : durabilityScore > 50
  ...
  const suggestedRoutines = useMemo(() => {
    ...
    activeInjuries.forEach((injury) => {
      REHAB_ROUTINES.filter((r) => r.tags.includes(injury.bodyPart)).forEach(
        (r) => relevant.add(r.id),
      );
    });
    risks.forEach((risk) => {
      REHAB_ROUTINES.filter((r) => r.tags.includes(risk.part)).forEach((r) =>
        relevant.add(r.id)
      );
    });
  ...
  const displayRoutines = suggestedRoutines.length > 0
    ? suggestedRoutines
    : REHAB_ROUTINES.slice(0, 2);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-24">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-slate-400 max-w-2xl">
          Hantera återhämtning, logga skadekänningar och få rehab-förslag. Din
          kropp är ditt viktigaste verktyg.
        </p>
      </header>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="flex items-center justify-center py-4">
              <div className="text-center">
                <span
                  className={`text-6xl font-black ${readinessColor} tracking-tighter`}
                >
                  {durabilityScore}%
                </span>
                <p
                  className={`text-sm mt-2 font-bold uppercase ${readinessColor} opacity-80`}
                >
                  {durabilityLabel}
                </p>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-2 gap-4 text-center">
              <div>
                ...
                <span className="text-lg font-mono font-bold text-white">
                  {Object.values(acuteLoad).reduce((a, b) => a + b.load, 0)}
                </span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-slate-500 uppercase">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-xs text-slate-500 font-bold uppercase">
                Klicka på kartan för att logga
              </span>
            </div>
          ...
          <div className="bg-gradient-to-br from-slate-900/80 to-indigo-950/20 border border-indigo-500/20 rounded-2xl p-6">
            <header className="mb-6">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                🧠 AI Fysio{" "}
                <span className="text-indigo-400 text-xs px-2 py-0.5 bg-indigo-500/10 rounded uppercase tracking-wider">
                  BETA
                </span>
              </h2>
