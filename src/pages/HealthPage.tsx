...
} from "../utils/healthAggregator.ts";
...
export function HealthPage() {
  ...
  const {
    ...
    weightEntries,
    mealEntries,
    exerciseEntries: manualExerciseEntries,
  ...
  const { activities: fetchedUniversalActivities } = useUniversalActivities(
    365,
  );
    ...
    const strava = fetchedUniversalActivities
      ...
      .filter((e) => {
        ...
        if (t === "strength" || t === "WeightTraining") {
          const hasDedicatedWorkout = strength.some((s) =>
            s.date.split("T")[0] === e.date.split("T")[0]
          );
  ...
  const activeView = useMemo(() => {
    ...
    if (
      ["weight", "vikt", "sleep", "sömn", "measurements", "matt", "mått"]
        .includes(m)
    ) return "body";
  ...
  const filteredExerciseEntries = useMemo(() => {
    if (timeframe === "2024") {
      return unifiedExerciseEntries.filter((e) => e.date.startsWith("2024"));
    }
    if (timeframe === "2025") {
      return unifiedExerciseEntries.filter((e) => e.date.startsWith("2025"));
    }
  ...
  const filteredWeightEntries = useMemo(() => {
    if (selectedPeriod) {
      return weightEntries.filter((e) =>
        e.date >= selectedPeriod.startDate &&
        (!selectedPeriod.endDate || e.date <= selectedPeriod.endDate)
      );
    }
    if (timeframe === "2024") {
      return weightEntries.filter((e) => e.date.startsWith("2024"));
    }
    if (timeframe === "2025") {
      return weightEntries.filter((e) => e.date.startsWith("2025"));
    }
  ...
  const finalSnapshots = useMemo(() => {
    if (selectedPeriod) {
      return snapshots.filter((s) =>
        ...
        (!selectedPeriod.endDate || s.date <= selectedPeriod.endDate)
      );
    }
    if (timeframe === "2024") {
  ...
  const stats = useMemo(() => calculateHealthStats(finalSnapshots), [
    finalSnapshots,
  ]);

  const handleTabChange = (tab: string) => {
    const isSwedishPath = window.location.pathname.includes("/hälsa") ||
      window.location.pathname.includes("/halsa");
    ...
    <div className="health-page animate-in fade-in duration-500">
      ...
      <header className="sticky top-0 z-50 bg-[#0a0f18]/95 backdrop-blur-md border-b border-white/5 py-2 px-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex bg-slate-900/50 p-0.5 rounded-lg border border-white/5 overflow-x-auto">
          {(["7d", "30d", "3m", "6m", "year", "2025", "all"] as TimeFrame[])
            .map((tf) => (
              <button
                key={tf}
                onClick={() => {
        ...
        <div className="flex gap-1 overflow-x-auto">
          <button
            className={`px-3 py-1 text-[11px] font-bold uppercase tracking-wide rounded transition-all ${
              activeView === "overview"
      ...
      <main className="health-grid">
        ...
        {activeView === "recovery" && <RecoveryPage />}
      </main>
    </div>
  );
}
