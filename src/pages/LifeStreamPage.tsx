...
export function LifeStreamPage() {
  ...
  const [stats, setStats] = useState<
    { followingCount: number; trainingForm: number }
  >({
    followingCount: 0,
    trainingForm: 0,
  });
            ...
            ? (
              events.map((event) => (
                <FeedEventCard
                  key={event.id}
                  event={event}
                  userName={event.userId === currentUser?.id
                    ? "Du"
                    : `Användare ${event.userId.slice(0, 4)}`}
            ...
            : (
              <div className="text-center py-20 bg-slate-900/50 rounded-3xl border border-dashed border-white/10">
                ...
                <p className="text-sm text-slate-500 px-10">
                  Börja följa vänner eller logga dina egna aktiviteter för att
                  fylla flödet.
                </p>
                <button
