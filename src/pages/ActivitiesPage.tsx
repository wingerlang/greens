...
const ActivityRow = ({
  ...
  const isMergedActivity = (universalMatch?.mergeInfo?.isMerged === true &&
    (universalMatch?.mergeInfo?.originalActivityIds?.length || 0) > 0) ||
    (activity.source === "merged" && !!(activity as any)._mergeData);

  // Get title for merged activities
  let displayTitle = activity.title && activity.title !== "-"
    ? activity.title
    : "-";
  let mergedSources: {
    id: string;
  ...
  if (isMergedActivity) {
    if (universalMatch?.mergeInfo?.originalActivityIds) {
      ...
      mergedSources = universalMatch.mergeInfo.originalActivityIds.map(
        (oid) => {
          const match = universalActivities.find((u) => u.id === oid);
          const matchAny = match as any;
          const activitySource = match?.performance?.source?.source ||
            matchAny?.source || "unknown";

          return {
            id: oid,
            source: activitySource,
            title: match?.plan?.title || match?.performance?.notes ||
              matchAny?.title || "Unknown Activity",
            type: (matchAny?.type as string) ||
              (match?.performance?.activityType as string) || "unknown",
          };
        },
      );
    ...
    if (
      displayTitle === "-" || displayTitle === "Styrkepass" ||
      displayTitle === "Strava Activity"
    ) {
      const stravaSource = mergedSources.find((s) => s.source === "strava");
      const strengthSource = mergedSources.find((s) =>
        s.source === "strength" || s.source === "strengthlog" ||
        s.source === "hevy"
      );
      ...
      if (
        ...
        ![
          ...
          "-",
          "Strava Activity",
        ].includes(stravaSource.title)
      ) {
        displayTitle = stravaSource.title;
      } else if (
        strengthSource?.title && strengthSource.title !== "Styrkepass" &&
        strengthSource.title !== "-"
      ) {
      ...
      >
        ...
        <td className="w-8 px-1">
          <button
            ...
            className={`opacity-0 group-hover:opacity-100 transition-all w-5 h-5 rounded flex items-center justify-center text-xs ${
              isSelectedForMerge
                ? "opacity-100 bg-indigo-500 text-white"
                : "bg-slate-800 hover:bg-slate-700 text-slate-400"
            }`}
            title={isSelectedForMerge
              ? "Ta bort från merge"
              : "Markera för merge (Shift+klick för flera)"}
          >
            {isSelectedForMerge ? "✓" : "+"}
          </button>
        </td>
        ...
        <td className="px-3 py-2">
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center gap-2">
              <span
                className="text-white font-bold text-xs truncate max-w-[180px]"
                title={displayTitle}
              >
                {displayTitle}
              </span>
        ...
        <td className="px-3 py-2">
          <div className="flex items-center gap-1.5">
            <span className="capitalize text-white font-bold text-xs">
              {activity.type}
            </span>
            {/* Score badge inline */}
            {score > 0 && (
            ...
            {activity.subType === "interval" && (
              <span
                className="text-[8px] uppercase font-bold bg-red-500/20 text-red-400 px-1 rounded"
                title="Intervallpass"
              >
                ⚡
              </span>
            ...
            {activity.subType === "long-run" && (
              <span
                className="text-[8px] uppercase font-bold bg-blue-500/20 text-blue-400 px-1 rounded"
                title="Långpass"
              >
                🏃
              </span>
            ...
            {activity.subType === "race" && (
              <span
                className="text-[8px] uppercase font-bold bg-amber-500/20 text-amber-400 px-1 rounded"
                title="Tävling"
              >
                🏆
              </span>
        ...
        <td className="px-3 py-2">
          {(() => {
            if (isMergedActivity) {
                    ...
                    ? (
                      <>
                        ...
                        <div className="flex items-center -space-x-1 opacity-50 group-hover/source:opacity-100 transition-opacity">
                          {mergedSources.some((s) => s.source === "strava") && (
                            <span title="Strava" className="text-xs">🔥</span>
                          )}
                          {mergedSources.some((s) =>
                            s.source === "strength" ||
                            s.source === "strengthlog"
                          ) && (
                            <span title="StrengthLog" className="text-xs">
                              💪
                            </span>
                          )}
                          {mergedSources.some((s) => s.source === "hevy") && (
                            <span title="Hevy" className="text-xs">🏋️</span>
                          )}
                          {mergedSources.some((s) => s.source === "garmin") && (
                            <span title="Garmin" className="text-xs">⌚</span>
                          )}
                        </div>
                        <span className="text-[10px] text-slate-600 group-hover/source:text-slate-400">
                          ▶
                        </span>
                      </>
                    )
              ...
              return (
                <span className="text-[#FC4C02] font-bold text-[9px] uppercase">
                  🔥 Strava
                </span>
              );
            } else if (
              activity.source === "strength" ||
              activity.source === "strengthlog"
            ) {
              return (
                <span className="text-purple-400 font-bold text-[9px] uppercase">
                  💪 StrengthLog
                </span>
              );
            } else if (activity.source === "hevy") {
              return (
                <span className="text-blue-400 font-bold text-[9px] uppercase">
                  🏋️ Hevy
                </span>
              );
            } else {
        ...
        <td className="px-3 py-2">
          <div className="flex flex-col text-[10px] font-mono text-slate-300 leading-tight">
            ...
            {pace && <span className="text-indigo-400">{pace}/km</span>}
            {activity.tonnage && (
              <span className="text-purple-400">
                {(activity.tonnage / 1000).toFixed(1)}t
              </span>
            )}
          </div>
        </td>
        <td
          className="px-3 py-2 text-[10px] italic opacity-50 truncate max-w-[80px]"
          title={activity.notes || ""}
        >
          {activity.notes
            ? (activity.notes.length > 20
              ? activity.notes.slice(0, 20) + "…"
              : activity.notes)
            : "-"}
        </td>
      </tr>
            ...
            <div className="flex flex-col gap-3 pl-8 border-l-2 border-indigo-500/30">
              ...
              <div className="space-y-2">
                {mergedSources.map((source) => (
                  <div
                    key={source.id}
                    className="flex items-center justify-between bg-slate-950 p-2 rounded-lg border border-white/5"
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const fullActivity = universalActivities.find((u) =>
                          u.id === source.id
                        );
                        if (fullActivity) {
                          ...
                          let mappedType: "run" | "strength" | "other" =
                            "other";
                          const uType = perf?.activityType ||
                            plan?.activityType;
                          if (
                            uType === "running" || (uType as any) === "run"
                          ) mappedType = "run";
                          else if (uType === "strength") {
                            mappedType = "strength";
                          }
                          ...
                          const mapped: ExerciseEntry & { source: string } = {
                            ...
                            title: plan?.title || (fullActivity as any).name ||
                              "Unknown",
                            notes: perf?.notes,
...
export function ActivitiesPage() {
  const {
    unifiedActivities: allActivities,
    universalActivities,
    strengthSessions,
    addStrengthSession,
  ...
  const [selectedActivity, setSelectedActivity] = useState<
    (ExerciseEntry & { source: string }) | null
  >(null);
  ...
  const handleSetSelectedActivity = (
    activity: (ExerciseEntry & { source: string }) | null,
  ) => {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(
      allActivities.map((a) => new Date(a.date).getFullYear()),
    );
  ...
  const handleMergeActivities = async () => {
    ...
    try {
      ...
      const selectedStrengthWorkouts = (strengthSessions as StrengthWorkout[])
        .filter((w) => selectedForMerge.has(w.id));

      // If all selected are strength workouts, merge them client-side
      if (
        selectedStrengthWorkouts.length >= 2 &&
        selectedStrengthWorkouts.length === selectedIds.length
      ) {
        // This is a strength-only merge - handle client-side
        const tempUserId = "local-user"; // Client-side merge doesn't need real userId
        const mergedWorkout = mergeStrengthWorkouts(
          selectedStrengthWorkouts,
          tempUserId,
        );
        ...
        alert(
          `Sammanslog ${selectedStrengthWorkouts.length} styrkepass!\n\n` +
      ...
      const activitiesToMerge = universalActivities.filter((u) =>
        selectedForMerge.has(u.id)
      );
      ...
      if (activitiesToMerge.length < 2) {
        ...
        if (selectedStrengthWorkouts.length > 0) {
          alert(
            "Blandade aktivitetstyper kan inte slås ihop. Välj bara styrkepass eller bara andra aktiviteter.",
          );
          setIsMerging(false);
          return;
        }
  ...
  const selectedActivitiesForMerge = processedActivities.filter((a) =>
    selectedForMerge.has(a.id)
  );
        ...
        <div className="flex justify-between items-end flex-wrap gap-4">
          <div>
            ...
            <p className="text-slate-400">
              ...
              av <span className="text-slate-500">{allActivities.length}</span>
              {" "}
              aktiviteter.
            </p>
          </div>
            ...
            <div className="flex-1 space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      const { filters, remainingText } = parseSmartQuery(
                        searchQuery,
                      );
                      if (filters.length > 0) {
                        setActiveSmartFilters((prev) => {
                          const existing = new Set(prev.map((f) =>
                            f.originalQuery
                          ));
                          const newFilters = filters.filter((f) =>
                            !existing.has(f.originalQuery)
                          );
          ...
          {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4 border-t border-white/5 animate-in slide-in-from-top-2 duration-200">
              ...
              <div className="space-y-2">
                ...
                <select
                  value={availableYears.includes(parseInt(datePreset))
                    ? datePreset
                    : "custom"}
      ...
      <div className="bg-slate-900/50 border border-white/5 rounded-3xl overflow-hidden shadow-xl">
        <table className="w-full text-left text-sm text-slate-400">
          ...
          <tbody className="divide-y divide-white/5">
            {processedActivities.slice(0, page * ITEMS_PER_PAGE).map((
              activity,
              i,
            ) => (
              <ActivityRow
        ...
        {processedActivities.length > page * ITEMS_PER_PAGE && (
          <div className="p-4 border-t border-white/5 flex justify-center bg-slate-900/50">
            ...
            >
              Visa fler aktiviteter ({processedActivities.length -
                page * ITEMS_PER_PAGE} kvar)
            </button>
          </div>
        )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Total: {selectedActivitiesForMerge.reduce((s, a) =>
                    s + (a.distance || 0), 0).toFixed(1)} km,
                  {formatDuration(selectedActivitiesForMerge.reduce((s, a) =>
                    s + (a.durationMinutes * 60 || 0), 0))}
                </p>
              </div>
            </div>
