...
export function ProfilePage() {
  const { settings, updateSettings, toggleMealVisibility, theme, toggleTheme } =
    useSettings();
  const {
    users,
    currentUser,
  ...
  const [expandedQuickMeal, setExpandedQuickMeal] = useState<string | null>(
    null,
  );
  ...
  const [saveStatus, setSaveStatus] = useState<
    { field: string; status: "saving" | "success" | "error" } | null
  >(null);
  ...
  const updateProfile = <K extends keyof ProfileData>(
    ...
    if (
      ["name", "bio", "location", "handle", "avatarUrl", "website"].includes(
        field as string,
      )
    ) {
  ...
  const availableUsers = users.filter((u) =>
    u.id !== currentUser?.id && !categoryOverrides[u.id]
  );
  ...
  const content = (displayProfile: ProfileData) => (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 pb-24">
      ...
      {saveStatus && (
        <div
          className={`fixed bottom-8 right-8 z-[100] px-5 py-3 rounded-2xl font-bold text-sm shadow-2xl flex items-center gap-3 border
                    ${
            saveStatus.status === "saving"
              ? "bg-slate-800 text-slate-300 border-white/10 animate-toast-in"
              : saveStatus.status === "success"
              ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30 animate-toast-in"
        ...
        >
          ...
          <div>
            ...
            <div className="text-[10px] opacity-60 font-medium uppercase tracking-wider mt-1">
              {saveStatus.field.charAt(0).toUpperCase() +
                saveStatus.field.slice(1)}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="px-6 space-y-8 max-w-4xl mx-auto">
                  ...
                  isPreviewMode
                    ? (
                      <p className="text-slate-300 whitespace-pre-wrap">
                        {displayProfile.bio || "Ingen biografi."}
                      </p>
                    )
                    : (
                      <InlineTextArea
              ...
              : (
                <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-12 text-center">
                  ...
                  <p className="text-slate-400">
                    Denna sektion är dold för besökaren.
                  </p>
                </div>
              )}
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <section className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
              <h3 className="text-lg font-bold text-white mb-2">
                Mina Träningsintressen
              </h3>
              <p className="text-sm text-slate-400 mb-6">
                Aktivera de träningsformer du vill se i appen.
              </p>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  ...
                  const isActive = settings.trainingInterests
                    ?.[type.id as "running" | "strength" | "hyrox"] ?? true;
                  return (
                    <button
                      key={type.id}
                      onClick={() => {
                        const current = settings.trainingInterests ||
                          { running: true, strength: true, hyrox: true };
                        updateSettings({
                          trainingInterests: {
                            ...current,
                            [type.id]: !isActive,
              ...
              <section className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
                ...
                <div className="space-y-4 max-w-md">
                  <DataField
                    ...
                    suffix="km"
                    onChange={(v: string) =>
                      updateSettings({ longRunThreshold: Number(v) })}
                  />
                  <p className="text-xs text-slate-500 mt-2">
                    Pass som överstiger denna distans klassas automatiskt som
                    långpass.
                  </p>
                </div>
              </section>
        ...
        {activeTab === "goals" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <section
              className={`bg-slate-900/50 border border-white/5 rounded-2xl p-6 relative overflow-hidden ${
                activePeriod ? "border-amber-500/30" : ""
              }`}
            >
              {activePeriod && (
              ...
              <div className="grid md:grid-cols-4 gap-3">
                <DataField
                  label="Kalorier"
                  value={activePeriod?.nutritionGoal?.calories.toString() ||
                    settings.dailyCalorieGoal?.toString() || ""}
                  type="number"
                  suffix="kcal"
                  readonly={!!activePeriod}
                  onChange={(v: string) =>
                    updateSettings({ dailyCalorieGoal: Number(v) })}
                />
                <DataField
                  label="Protein"
                  value={activePeriod?.nutritionGoal?.protein?.toString() ||
                    settings.dailyProteinGoal?.toString() || ""}
                  type="number"
                  suffix="g"
                  readonly={!!activePeriod}
                  onChange={(v: string) =>
                    updateSettings({ dailyProteinGoal: Number(v) })}
                />
                <DataField
                  label="Kolhydrater"
                  value={activePeriod?.nutritionGoal?.carbs?.toString() ||
                    settings.dailyCarbsGoal?.toString() || ""}
                  type="number"
                  suffix="g"
                  readonly={!!activePeriod}
                  onChange={(v: string) =>
                    updateSettings({ dailyCarbsGoal: Number(v) })}
                />
                <DataField
                  label="Fett"
                  value={activePeriod?.nutritionGoal?.fat?.toString() ||
                    settings.dailyFatGoal?.toString() || ""}
                  type="number"
                  suffix="g"
                  readonly={!!activePeriod}
                  onChange={(v: string) =>
                    updateSettings({ dailyFatGoal: Number(v) })}
                />
              </div>

              {activePeriod && (
                <p className="text-xs text-amber-500/70 mt-4 italic">
                  Dessa mål hanteras automatiskt av din aktiva träningsperiod.
                  Ändra i perioden för att uppdatera.
                </p>
              )}
            </section>
            ...
            <section className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                <DataField
                  ...
                  suffix="h"
                  onChange={(v: string) =>
                    updateSettings({ dailySleepGoal: Number(v) })}
                />
                <DataField
                  label="Vatten"
                  ...
                  suffix="glas"
                  onChange={(v: string) =>
                    updateSettings({ dailyWaterGoal: Number(v) } as any)}
                />
                <DataField
                  label="Steg"
                  ...
                  suffix=""
                  onChange={(v: string) =>
                    updateSettings({ dailyStepGoal: Number(v) } as any)}
                />
                <DataField
                  label="Koffein Max"
                  value={(settings as any).dailyCaffeineMax?.toString() ||
                    "400"}
                  type="number"
                  suffix="mg"
                  onChange={(v: string) =>
                    updateSettings({ dailyCaffeineMax: Number(v) } as any)}
                />
                <DataField
                  ...
                  suffix="min"
                  onChange={(v: string) =>
                    updateSettings({ dailyTrainingGoal: Number(v) } as any)}
                />
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-slate-400 text-xs">
                      Aktivera timer och snabbval vid 08:00
                    </div>
                  </div>
                  <button
                    onClick={() =>
                      updateSettings({
                        noccoOClockEnabled: !settings.noccoOClockEnabled,
                      })}
                  ...
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${
                        settings.noccoOClockEnabled ? "left-7" : "left-1"
                      }`}
                    />
                  </button>
                </div>
                  ...
                  <button
                    onClick={() => {
                      const showFeedback = (text: string, color: string) => {
                        ...
                        fb.className =
                          `fixed top-10 left-1/2 -translate-x-1/2 z-[110] px-6 py-3 rounded-full bg-${color}-500 text-white font-bold shadow-xl animate-in slide-in-from-top-4`;
                        fb.innerText = text;
                      ...
                      testModal.className =
                        "fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-500";
                      testModal.innerHTML = `
                      ...
                      testModal.querySelector("#test-nocco-btn")
                        ?.addEventListener("click", () => {
                          showFeedback("+180mg Koffein loggat! 🚀", "blue");
                          closeModal();
                        });
                      testModal.querySelector("#test-coffee-btn")
                        ?.addEventListener("click", () => {
                          showFeedback("+80mg Koffein loggat! ☕", "amber");
                          closeModal();
                        });
                      testModal.querySelector("#nocco-test-close")
        ...
        {activeTab === "privacy" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <section className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
              ...
              <p className="text-sm text-slate-400 mb-6">
                Ge specifika personer tillgång till kategorier som annars är
                privata.
              ...
              <div className="space-y-4">
                {Object.entries(categoryOverrides).length === 0
                  ...
                  : (
                    Object.entries(categoryOverrides).map(
                      ([userId, overrides]) => {
                                ...
                                ([key, { label, icon }]) => {
                                  ...
                                  return (
                                    <button
                                      key={key}
                                      onClick={() =>
                                        toggleCategoryOverride(userId, key)}
            ...
            <section className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
              ...
              <div className="space-y-4">
                    ...
                    {ALL_MEALS.map((meal) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                          settings.visibleMeals.includes(meal)
                ...
                <DataField
                  ...
                  onChange={(v: any) =>
                    updateProfile("weekStartsOn", v === "sunday" ? 0 : 1)}
                />
                <DataField
                  label="Enheter"
                ...
                ? (
                  <div className="text-center py-8 text-slate-500">
                    ...
                    <p className="text-xs mt-1">
                      Spara en måltid som snabbval via /kalorier.
                    </p>
                  </div>
                )
                : (
                  <div className="space-y-2 max-h-[500px] overflow-y-auto">
                    {quickMeals.map((qm) => {
                      ...
                      const itemDetails = qm.items.map((item, idx) => {
                        if (item.type === "recipe") {
                          ...
                          return {
                            ...
                            calculatedCalories: Math.round(
                              caloriesPer100 * item.servings / 100,
                            ),
                            icon: "🥕",
                            unit: "g",
                          };
                      ...
                      const totalCalories = itemDetails.reduce(
                        (sum, i) => sum + i.calculatedCalories,
                        0,
                      );
                      ...
                      return (
                        <div
                          key={qm.id}
                          className="bg-slate-800/50 rounded-xl border border-white/5 hover:border-emerald-500/30 transition-all overflow-hidden"
                        >
                          {/* Header - clickable to expand */}
                          <div
                            className="flex items-center justify-between p-4 cursor-pointer hover:bg-white/5 transition-colors"
                            onClick={() =>
                              setExpandedQuickMeal(isExpanded ? null : qm.id)}
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (
                                    confirm(`Ta bort snabbval "${qm.name}"?`)
                                  ) {
                            ...
                            <div className="border-t border-white/5 bg-slate-950/50 p-4 space-y-2 animate-in slide-in-from-top-2 duration-200">
                              {isEditing && (
                                <div className="mb-3 p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg text-xs text-amber-400">
                                  ✏️ Redigeringsläge - ändra gram/portioner
                                  direkt. Klicka ✏️ igen för att avsluta.
                                </div>
                              )}
                                      ...
                                      <div className="flex items-center gap-2">
                                        <input
                                          ...
                                          onChange={(e) => {
                                            ...
                                            updatedItems[item.idx].servings =
                                              newValue;
                                            updateQuickMeal?.(qm.id, {
                                              items: updatedItems,
                                            });
                                          }}
                                        ...
                                        <button
                                          onClick={() => {
                                            const updatedItems = qm.items
                                              .filter((_, i) => i !== item.idx);
                                            if (updatedItems.length === 0) {
                                              if (
                                                confirm(
                                                  "Ta bort hela snabbvalet?",
                                                )
                                              ) {
                                                deleteQuickMeal(qm.id);
                                              }
              ...
              <div className="p-4 bg-white/5 rounded-xl mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Ladda ner all din data som JSON.
                  </p>
                </div>
