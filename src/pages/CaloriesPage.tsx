...
import { formatActivityDuration, normalizeText } from "../utils/formatters.ts";
...
export function CaloriesPage() {
  ...
  const [mealType, setMealType] = useState<MealType>(
    getDefaultMealType(urlDate || getISODate()),
  );
  ...
  // Vitals local state
  const currentVitals = useMemo(() => getVitalsForDate(selectedDate), [
    getVitalsForDate,
    selectedDate,
  ]);
  ...
  const getItemName = (item: MealItem): string => {
    if (item.type === "estimate") {
      return (item.estimateDetails?.uncertaintyEmoji
        ? `${item.estimateDetails.uncertaintyEmoji} `
        : "") + (item.estimateDetails?.name || "Estimering");
    }
    if (item.type === "recipe") {
  ...
  const getItemCalories = (item: MealItem): number => {
    ...
    if (item.type === "recipe") {
      ...
      if (recipe?.ingredientsText) {
        ...
        return Math.round(
          (estimate.calories / recipe.servings) * item.servings,
        );
      }
      const recipeWithNutrition = getRecipeWithNutrition(item.referenceId);
      if (recipeWithNutrition) {
        return Math.round(
          recipeWithNutrition.nutritionPerServing.calories * item.servings,
        );
      }
    } else {
      ...
      if (food) {
        ...
        if (item.loggedAsCooked) {
          const yieldFactor = item.effectiveYieldFactor || food.yieldFactor ||
            2.3;
          if (yieldFactor > 1) {
  ...
  const unloggedPlannedMeals = useMemo(() => {
    return plannedMeals.filter(
      (pm: { mealType: MealType; meal: PlannedMeal }) => {
        const alreadyLogged = dailyEntries.some((entry: MealEntry) =>
          entry.mealType === pm.mealType &&
          entry.items.some((item: MealItem) =>
            item.referenceId === pm.meal.recipeId
          )
        );
  ...
  return (
    <div className="calories-page">
      ...
      <div
        className={`fixed top-16 left-0 right-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm py-2 transition-all`}
      >
        ...
        <div className="planned-meals-banner">
          <div className="banner-content">
            ...
            <div className="banner-text">
              <strong>
                {unloggedPlannedMeals.length}{" "}
                planerade måltid{unloggedPlannedMeals.length > 1 ? "er" : ""}
              </strong>
              <span className="banner-subtitle">
                {unloggedPlannedMeals.map(
                  (pm: { mealType: MealType; meal: PlannedMeal }) => {
                    const recipe = recipes.find((r) =>
                      r.id === pm.meal.recipeId
                    );
                    return recipe?.name || "Okänt recept";
                  },
                ).join(", ")}
        ...
        <div className="mx-4 mb-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-between animate-in slide-in-from-top duration-500">
          ...
          <div className="text-right">
            ...
            <div className="text-xs font-bold text-white">
              +{Math.round(goals.extraCalories * 0.15 / 4)}g Protein
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-center mb-4">
        <button
          ...
          className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-all flex items-center gap-2 ${
            userSettings.incompleteDays?.[selectedDate]
              ? "bg-amber-500/20 border-amber-500/40 text-amber-400"
              : "bg-transparent border-slate-700 text-slate-500 hover:border-slate-500"
          }`}
        >
          {userSettings.incompleteDays?.[selectedDate]
            ? "⚠️ Dagen markerad som inkomplett"
            : "Markera dag som inkomplett"}
        </button>
      </div>
      ...
      <div className="mx-4 mt-6 p-4 bg-slate-900/50 border border-white/5 rounded-2xl grid grid-cols-3 gap-4">
        ...
        >
          <span className="text-[10px] font-black uppercase text-slate-500 mb-1">
            Vatten
          </span>
        ...
        <div
          className="flex flex-col items-center cursor-pointer hover:bg-white/5 rounded-xl transition-colors p-2"
          onClick={() =>
            handleCardClick("caffeine", currentVitals.caffeine || 0)}
        >
          <span className="text-[10px] font-black uppercase text-slate-500 mb-1">
            Koffein
          </span>
            ...
            {editing === "alcohol"
              ...
              : (
                <>
                  <span className="text-xl">🍷</span>
                  <span className="text-lg font-bold text-white">
                    {currentVitals.alcohol || 0}
                    <span className="text-[10px] text-slate-500 ml-0.5">e</span>
                  </span>
                </>
              )}
          </div>
      ...
      <MealTimeline
        ...
        onReplaceItem={(item, entryId) => {
          setMealType(
            dailyEntries.find((e) => e.id === entryId)?.mealType || "snack",
          );
          setIsFormOpen(true);
        }}
                ...
                >
                  <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500">
                        ...
                        {ex.intensity}
                        {(ex as any).source === "strava" && (
                          <span className="ml-2 text-[#FC4C02] font-bold">
                            ST
                          </span>
                        )}
                        {(ex as any).source === "manual" && (
                  ...
                  <span
                    className={`font-black ${
                      ex.caloriesBurned > 0 ? "text-rose-400" : "text-slate-400"
                    } ${
                      ex.calorieBreakdown
                        ? "cursor-help border-b border-rose-400/20 md:border-b-0"
                        : ""
                    }`}
