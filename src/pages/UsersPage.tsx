...
export function UsersPage() {
  ...
  const [sort, setSort] = useState<"relevance" | "name" | "newest">(
    "relevance",
  );
  ...
  const filteredUsers = useMemo(() => {
    ...
    return result.sort((a, b) => {
      ...
      return (a.name || a.handle || a.username).localeCompare(
        b.name || b.handle || b.username,
      );
    });
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto items-center">
          <div className="relative group min-w-[300px] w-full md:w-auto">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <span className="text-gray-500 group-focus-within:text-emerald-400 transition-colors">
                🔍
              </span>
            </div>
            <input
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-10 h-10 rounded-full overflow-hidden bg-slate-800 flex-shrink-0 ${
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-gray-600 group-hover:text-emerald-500 transition-colors">
                    →
                  </span>
                </div>
              </Link>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="relative">
                    <div
                      className={`w-16 h-16 rounded-2xl overflow-hidden bg-slate-800 ${
                        isMe
                    ...
                    {isAdmin && (
                      <div
                        className="absolute -bottom-2 -right-2 bg-slate-900 p-1 rounded-full text-lg"
                        title="Admin"
                      >
                        🛡️
                      </div>
                ...
                <div>
                  <h3 className="font-bold text-white text-lg truncate flex items-center gap-2">
                    {user.name || user.username}
                    {isAdmin && (
                      <span className="text-xs bg-rose-500/10 text-rose-500 px-1.5 py-0.5 rounded border border-rose-500/20">
                        ADMIN
                      </span>
                    )}
                  </h3>
                  <p className="text-emerald-500/80 text-sm font-medium mb-2">
                    @{handle}
                  </p>
