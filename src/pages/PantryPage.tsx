...
export function PantryPage() {
    ...
    const commonStaples = [
        'Salt', 'Peppar', 'Olivolja', 'Smör', 'Mjölk', 'Ägg',
        'Lök', 'Vitlök', 'Pasta', 'Ris', 'Krossade tomater',
        'Socker', 'Mjöl', 'Bakpulver', 'Soja', 'Buljong'
    ];
    ...
    const recipeMatches = useMemo(() => {
        ...
        return recipes.map(recipe => {
            if (!recipe.ingredientsText) return { recipe, matchPercentage: 0, missing: [] };
            ...
            ingredients.forEach(ing => {
                ...
                const hasItem = pantryItems.some(pItem =>
                    ingName.includes(pItem.toLowerCase()) || pItem.toLowerCase().includes(ingName)
                );
    ...
    const handleAddCustom = (e: React.FormEvent) => {
        ...
        if (searchTerm.trim()) {
            ...
            const item = searchTerm.trim().charAt(0).toUpperCase() + searchTerm.trim().slice(1);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr,400px] gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                        ...
                        <div className="mb-6">
                            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 px-1">Vanliga Basvaror</h3>
                                ...
                                {commonStaples.map(item => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${isOwned
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-3 px-1">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Ditt Skafferi ({pantryItems.length})</h3>
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-slate-900/50 border border-slate-800 rounded-2xl p-6 relative overflow-hidden h-full">
                        ...
                        <div className="space-y-3 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
                            {recipeMatches.length === 0 ? (
                                ...
                                recipeMatches.map(({ recipe, matchPercentage, missing }: { recipe: Recipe, matchPercentage: number, missing: string[] }) => (
                                    <div key={recipe.id} className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 hover:border-emerald-500/30 transition-all group cursor-pointer">
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-bold text-slate-200 group-hover:text-emerald-400 transition-colors">{recipe.name}</h3>
                                            <span className={`text-xs font-bold px-2 py-1 rounded-md ${matchPercentage === 100 ? 'bg-emerald-500 text-emerald-950' :
                                                matchPercentage >= 75 ? 'bg-emerald-500/20 text-emerald-400' :
                                                    matchPercentage >= 50 ? 'bg-yellow-500/20 text-yellow-400' :
                                        ...
                                        {missing.length > 0 && matchPercentage < 100 && (
                                            <div className="mt-2 text-xs">
                                                ...
                                                <span className="text-rose-400/80">
                                                    ...
                                                    {missing.length > 3 && ` +${missing.length - 3} till`}
                                                </span>
                                            </div>
                                        )}
                                    </div>
