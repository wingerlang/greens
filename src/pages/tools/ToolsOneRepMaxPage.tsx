...
import {
  getPersonalRecords,
  PERCENTAGE_MAP,
} from "../../utils/strengthStatistics.ts";
import {
  Check,
  ChevronRight,
  Copy,
...
export function ToolsOneRepMaxPage() {
  ...
  const smartPersonalRecords = useMemo(() => {
    ...
    STANDARD_REPS.forEach((targetRep) => {
      ...
      if (
        bestImplied.weight > 0 &&
        (!actual || bestImplied.weight > actual.weight)
      ) {
        inferred[targetRep] = {
          weight: bestImplied.weight, // Same weight, not calculated
          source: bestImplied,
          e1rm:
            calculateAverage1RM(bestImplied.weight, bestImplied.reps).average,
          inferred: true,
  ...
  useEffect(() => {
    if (selectedExercise) {
      ...
      const allRecords = [
        ...prValues.map((pr) => ({
          ...pr,
          e1rm: calculateAverage1RM(pr.weight, pr.reps).average,
          inferred: false,
        })),
        ...Object.entries(smartPersonalRecords).map(([reps, data]) => ({
          reps: Number(reps),
          ...data,
        })),
      ];
  ...
  const tableRows = useMemo(() => {
    ...
    const nonStandardReps = prValues.filter((pr) =>
      !STANDARD_REPS.includes(pr.reps)
    ).sort((a, b) => a.reps - b.reps);

    const addNonStandardRow = (pr: { reps: number; weight: number }) => {
      if (!rows.some((row) => row.reps === pr.reps)) {
        const formulaWeight = Math.round(
          maxResults.average / (1 + pr.reps / 30),
        );
    ...
    if (isExpanded) {
      ...
      if (highReps.length > 0) {
        ...
        if (selected.length < 4) {
          selected = [...selected, ...odds.slice(-(4 - selected.length))].sort((
            a,
            b,
          ) => a.reps - b.reps);
        }
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">
            Beräkna ditt max och se hur du ska lasta stången.
          </p>
        </div>
        {selectedExercise && (
          <button
            onClick={() => {
              const url = `${window.location.origin}/rm/${
                encodeURIComponent(selectedExercise)
              }`;
        ...
        <div className="bg-slate-900 border border-white/5 rounded-3xl p-6 overflow-hidden flex flex-col h-[600px]">
          <h2 className="text-xl font-bold text-white mb-4">
            {selectedExercise
              ? (
                <>
                  <Link
                    to={`/styrka/${encodeURIComponent(selectedExercise)}`}
          ...
          <div className="flex-1 overflow-auto custom-scrollbar -mx-2 px-2">
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-slate-900 z-10 shadow-lg shadow-slate-900/50">
                <tr className="text-[10px] text-slate-500 border-b border-white/5 uppercase tracking-wider">
                  ...
                  <th className="py-2 px-2 font-black text-right hidden sm:table-cell">
                    e1RM
                  </th>
              ...
              <tbody className="text-sm">
                {tableRows.map((row) => {
                  ...
                  if (selectedExercise || isMyRecord || isInferred) {
                    ...
                    const actualDiffVal = Math.round(e1rm) -
                      Math.round(maxResults.average);
                    const diffPct = maxResults.average
                      ? ((Math.round(e1rm) - Math.round(maxResults.average)) /
                        Math.round(maxResults.average)) * 100
                      : 0;
                  ...
                  return (
                    ...
                    >
                      <td
                        className={`px-2 text-right font-black ${
                          isInferred
                            ? "py-1 text-xs text-slate-400"
                            : "py-2 text-white group-hover:text-emerald-400 transition-colors"
                        }`}
                      >
                        {r}
                      </td>
                      ...
                      {/* PB Column */}
                      <td
                        className={`px-2 text-right font-mono flex items-center justify-end gap-1 ${
                          isInferred
                            ? "py-1 text-xs text-slate-400"
                            : "py-2 text-sm"
                        }`}
                      ...
                      <td
                        className={`px-2 text-right font-mono text-slate-500 hidden sm:table-cell ${
                          isInferred ? "py-1 text-xs" : "py-2 text-xs"
                        }`}
                      >
                        {Math.round(e1rm)}
                      </td>
            ...
            {(Object.values(personalRecords).length > 0 ||
              tableRows.length < Object.values(personalRecords).length + 7) && (
              <button
        ...
        <div className="bg-slate-900 border border-white/5 rounded-3xl p-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Visualisera hur du ska lasta stången
              </p>
            </div>

            <div className="flex items-center gap-4 bg-slate-950 p-2 rounded-xl border border-white/5">
              <div className="flex flex-col">
                <label className="text-[10px] uppercase font-bold text-slate-500 mb-1 px-2">
                  Målvikt
                </label>
                <input
                  type="number"
          ...
          <div className="flex flex-col justify-center items-center bg-slate-950 rounded-2xl p-8 py-24 border border-white/5 overflow-visible">
            ...
            <div className="flex items-center gap-0.5 mb-12 relative w-full justify-center max-w-4xl mx-auto">
              {/* Sleeve Cap */}
              <div className="h-8 w-4 bg-slate-400 rounded-l-sm border-r border-black/30 shadow-sm z-10">
              </div>
                ...
                {loadingResult.plates.length > 0
                  ? (
                    loadingResult.plates.flatMap((plate) =>
                      ...
                      <div
                        key={idx}
                        className="flex items-center -ml-0.5 first:ml-0 relative group"
                      >
                        {/* Plate */}
                        <div
                          className={`
              ...
              {/* Collar */}
              <div className="h-10 w-4 bg-slate-500 rounded-sm shadow-lg z-20 mx-0.5 border-l border-white/10">
              </div>
              ...
              >
                {/* Knurling Texture */}
                <div className="absolute inset-0 opacity-20 bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAYAAACp8Z5+AAAAIklEQVQIW2NkQAKrVq36zwjjgzj//v37zwjjgzj//v37zwQAOTZDf9jH5wQAAAAASUVORK5CYII=')]">
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-12 text-center">
              <div>
                ...
                <div className="text-4xl font-black text-white tracking-tighter">
                  {targetWeight}{" "}
                  <span className="text-lg text-slate-600 font-bold">kg</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-4xl font-black text-emerald-400 tracking-tighter">
                  {loadingResult.plates.reduce(
                    (acc, p) => acc + p.weight * p.count,
                    0,
                  )}{" "}
                  <span className="text-lg text-emerald-600/50 font-bold">
                    kg
                  </span>
                </div>
              </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Baserat på ditt e1RM:{" "}
                <span className="text-emerald-400 font-bold">
                  {maxResults.average} kg
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              const suggestedWeight =
                Math.round((maxResults.average * pct) / 100 / 2.5) * 2.5; // Round to nearest 2.5kg
              return (
                <button
                  key={label}
                  onClick={() => {
                ...
                >
                  <div className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1 group-hover:text-emerald-400 transition-colors">
                    {label}
                  </div>
                  <div className="text-2xl font-black text-white tracking-tight">
                    {suggestedWeight}{" "}
                    <span className="text-sm text-slate-600 font-bold">kg</span>
                  </div>
            ...
            {[
              {
                ...
                desc: "0 reps i reserv",
                pct: 100,
                color: "text-red-400 bg-red-500/10 border-red-500/20",
              },
              {
                rpe: 9,
                label: "Nästan max (RPE 9)",
                desc: "1 rep i reserv",
                pct: 96,
              ...
              },
              {
                rpe: 6,
                label: "Lätt (RPE 6)",
                desc: "4+ reps i reserv",
                pct: 84,
                color: "text-blue-400 bg-blue-500/10 border-blue-500/20",
              },
            ].map(({ rpe, label, desc, pct, color }) => {
              const rpeWeight =
                Math.round((maxResults.average * pct) / 100 / 2.5) * 2.5;
              return (
                <button
                  key={rpe}
                  onClick={() => {
