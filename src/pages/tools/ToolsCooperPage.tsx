...
export function ToolsCooperPage() {
  ...
  useEffect(() => {
    ...
    if (unifiedActivities && unifiedActivities.length > 0) {
      const runs = unifiedActivities.filter((a) =>
        a.type === "running" && (a.durationMinutes || 0) >= 10
      );
  ...
  const nextLevelThreshold = details?.nextLevel && standard
    ? (standard
      .levels[
        details.nextLevel.toLowerCase() as keyof typeof standard.levels
      ] || 0)
    : 0;
  ...
  const getProgressPercent = () => {
    ...
    if (distance < excellent) {
      return 60 + mapToSegment(distance, good, excellent);
    }
                    ...
                    <div className="pl-5 space-y-1">
                      <div className="text-white font-medium truncate">
                        {prefillSource.id
                          ? (
                            <Link
                              to={`/activity/${prefillSource.id}`}
                              className="hover:text-emerald-400 underline decoration-slate-700 hover:decoration-emerald-400 transition-all"
                            >
                              {prefillSource.name}
                            </Link>
                          )
          ...
          <div className="bg-slate-900 border border-white/5 rounded-3xl p-8 relative overflow-hidden">
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-br ${
                details
                  ? COOPER_LEVEL_COLORS[details.grade]
            ...
            <div className="relative z-10">
                ...
                <div>
                  ...
                  <div
                    className={`text-4xl md:text-6xl font-black ${
                      details
                        ? COOPER_LEVEL_TEXT_COLORS[details.grade]
                        : "text-white"
                    } drop-shadow-2xl`}
                  >
                    {details?.grade || "N/A"}
                    ...
                    ? (
                      <div
                        className={`text-2xl font-bold ${
                          COOPER_LEVEL_TEXT_COLORS[details.nextLevel]
                        }`}
                      >
                        {details.nextLevel}
                      </div>
              ...
              <div className="mb-10 select-none relative pt-6">
                <div className="h-6 bg-slate-950 rounded-full relative overflow-hidden flex shadow-inner shadow-black/50">
                  {/* Segments */}
                  {["Very Bad", "Bad", "Average", "Good", "Excellent"].map((
                    lvl,
                  ) => (
                    <div
                      key={lvl}
                      className={`flex-1 border-r border-slate-900/50 bg-gradient-to-r ${
                        COOPER_LEVEL_COLORS[lvl as CooperLevel]
                      } opacity-20 hover:opacity-30 transition-opacity`}
                    >
                    </div>
                  ))}
                    ...
                    {showPaceImprovement
                      ? (
                        <div className="text-lg text-white">
                          Öka tempot till{" "}
                          <strong className="text-emerald-400">
                            {formatPace(nextLevelPace)}
                          </strong>{" "}
                          /km
                          <span className="text-sm text-slate-500 ml-2">
                            (-{Math.round((paceImprovementRaw || 0) * 60)}s/km)
                          </span>
                        </div>
                      )
                      : (
          ...
          <div className="bg-slate-900 border border-white/5 rounded-3xl p-6">
            ...
            >
              <span className="font-bold text-white flex items-center gap-2">
                ...
                Tabell ({gender === "male" ? "Män" : "Kvinnor"}{" "}
                {standard?.ageMin}-{standard?.ageMax} år)
              </span>
              <span
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-950 text-slate-400 uppercase text-[10px] tracking-wider">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right font-bold hidden sm:table-cell">
                        Tempo
                      </th>
                  ...
                  <tbody className="divide-y divide-white/5 bg-slate-900/50">
                    {(["Excellent", "Good", "Average", "Bad"] as const).map(
                      (lvl) => {
                        const threshold = standard
                          .levels[
                            lvl.toLowerCase() as keyof typeof standard.levels
                          ];
                        const pace = convertTimeToPace(
                          threshold / 1000,
                          12 * 60,
                        );
                    ...
                    <tr
                      className={details?.grade === "Very Bad"
                        ? "bg-red-500/10"
                        : ""}
                    >
                      <td
                        className={`px-4 py-3 font-bold ${
                          COOPER_LEVEL_TEXT_COLORS["Very Bad"]
                        }`}
                      >
                        Very Bad
                      </td>
