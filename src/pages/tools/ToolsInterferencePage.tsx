...
import { getISODate, WEEKDAYS } from "../../models/types.ts";
import {
  Activity,
  AlertTriangle,
  ArrowRight,
...
export function ToolsInterferencePage() {
  ...
  const { timelineData, warnings, todayChart, patternAnalysis } = useMemo(
    () => {
      ...
      // Filter History
      const history = unifiedActivities.filter((a) =>
      ...
      const grouped: Record<
        string,
        { date: string; activities: any[]; warning?: ConflictWarning }
      > = {};
      ...
      return {
        timelineData: Object.values(grouped).sort((a, b) =>
  ...
  const formatDay = (iso: string) => {
    ...
    const diff = Math.ceil(
      (d.getTime() - new Date(today).getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  const getSignalBadge = (signal: SignalCategory) => {
    switch (signal) {
      case "MTOR":
        return (
          <div className="flex items-center gap-1 text-[10px] font-bold text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 px-2 py-0.5 rounded">
            <Dumbbell size={10} /> mTOR
          </div>
        );
      case "AMPK_HIGH":
        return (
          <div className="flex items-center gap-1 text-[10px] font-bold text-rose-600 bg-rose-100 dark:bg-rose-900/30 px-2 py-0.5 rounded">
            <Zap size={10} /> AMPK
          </div>
        );
      case "AMPK_LOW":
        return (
          <div className="flex items-center gap-1 text-[10px] font-bold text-sky-600 bg-sky-100 dark:bg-sky-900/30 px-2 py-0.5 rounded">
            <Activity size={10} /> Lätt
          </div>
        );
      case "HYBRID":
        return (
          <div className="flex items-center gap-1 text-[10px] font-bold text-purple-600 bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 rounded">
            <Zap size={10} /> Hybrid
          </div>
        );
      default:
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8 animate-fade-in text-slate-900 dark:text-slate-100">
      ...
      {todayChart.length > 0 && <SignalTimelineChart activities={todayChart} />}
      ...
      <div className="bg-indigo-900 text-indigo-100 rounded-3xl p-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500 rounded-full blur-[100px] opacity-20 -mr-16 -mt-16">
        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white">
              Vetenskapen bakom interferens
            </h2>
            <p className="leading-relaxed opacity-90">
              <strong>mTOR</strong> (styrka) och <strong>AMPK</strong>{" "}
              (kondition) är biologiska "strömbrytare" som styr din anpassning.
              Problemet: AMPK blockerar mTOR. Spring direkt efter styrka =
              mindre muskeltillväxt.
            </p>
            ...
            <div className="grid md:grid-cols-3 gap-4 pt-4">
              <div className="bg-indigo-950/50 p-4 rounded-xl border border-indigo-400/20">
                ...
                <p className="text-sm opacity-80">
                  6-24 timmar mellan pass för bäst anpassning.
                </p>
              </div>
              <div className="bg-indigo-950/50 p-4 rounded-xl border border-indigo-400/20">
                ...
                <p className="text-sm opacity-80">
                  Styrka på morgon, kondition på kväll. Minst 4h mellanrum.
                </p>
              </div>
              <div className="bg-indigo-950/50 p-4 rounded-xl border border-indigo-400/20">
                ...
                <p className="text-sm opacity-80">
                  Riskerar överträning. Endast för avancerade med god
                  återhämtning.
                </p>
              </div>
            </div>
  ...
  Object.entries(byDate).forEach(([date, acts]) => {
    ...
    if (acts.length >= 3) {
      results.push({
        ...
        description:
          `${acts.length} träningspass samma dag kan leda till överträning och bristande återhämtning.`,
        recommendation:
          "Överväg att flytta åtminstone ett pass till nästa dag.",
      });
    }
    ...
    if (strengthActs.length >= 2) {
      ...
      if (overlap.length > 0) {
        results.push({
          ...
          description: `Du tränar samma muskler (${
            overlap.join(", ")
          }) två gånger på ${formatDay(date)}.`,
          recommendation:
            "Dela upp i överkropp/underkropp eller separera med 48h.",
        });
      }
    }
    ...
    if (acts.length === 1 && signals.includes("MTOR")) {
      results.push({
        ...
        description: `Ensamt styrkepass på ${
          formatDay(date)
        } ger maximal mTOR-aktivering utan interferens.`,
      });
    }
  });
