...
import { Calculator, Utensils, Sparkles, Scale, Info, Check } from 'lucide-react';
        ...
        return (
            <div className="max-w-2xl mx-auto animate-in slide-in-from-bottom-4">
                ...
                <div className="bg-slate-900 border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
                    <NutritionWizard onSave={handleWizardSave} onCancel={() => setShowWizard(false)} />
                </div>
            </div>
        );
    }
        ...
        <div className="max-w-3xl mx-auto space-y-8 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-400 mt-2">Sätt dina näringsmål snabbt eller använd guiden för djupare analys.</p>
                </div>
                <button
                ...
                <div className="space-y-6">
                    <div className="bg-slate-900/50 border border-white/5 rounded-3xl p-6 space-y-4">
                        ...
                        <div className="relative">
                            ...
                            />
                            <div className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-600 font-bold">kcal</div>
                        </div>
                    </div>
                        ...
                        <div className="grid gap-2">
                            {Object.entries(diets).map(([key, diet]) => (
                                <button
                                    ...
                                    className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${dietType === key
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            <div className="font-bold text-sm tracking-tight">{diet.label}</div>
                                            <div className="text-[10px] opacity-60 font-medium">{diet.desc}</div>
                                        </div>
                                    </div>
                ...
                <div className="space-y-6">
                    <div className="bg-slate-900 border border-white/5 rounded-3xl p-8 sticky top-6">
                        <div className="text-center mb-8">
                            <div className="text-[10px] uppercase font-black text-slate-500 tracking-[0.2em] mb-2">Daglig Fördelning</div>
                            <div className="text-5xl font-black text-white tracking-widest">{calories}</div>
                            <div className="text-slate-600 font-bold uppercase text-[10px] mt-1 tracking-widest">kcal per dag</div>
                        </div>

                        <div className="space-y-4">
                            <MacroResultItem label="Protein" value={macros.protein} color="emerald" percentage={currentDiet.p} />
                            <MacroResultItem label="Kolhydrater" value={macros.carbs} color="blue" percentage={currentDiet.c} />
                            <MacroResultItem label="Fett" value={macros.fat} color="rose" percentage={currentDiet.f} />
                        </div>

                        <div className="mt-10 pt-8 border-t border-white/5">
                            <button
                                onClick={handleSave}
                                className={`w-full py-4 rounded-2xl font-black transition-all flex items-center justify-center gap-3 ${saveFeedback
                    ...
                    <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl flex gap-3">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed">
                            Målen sparas direkt på din profil och kommer att styra din Dashboard och dina veckoplaneringar.
                        </p>
                    </div>
                </div>
            </div>
...
function MacroResultItem({ label, value, color, percentage }: { label: string, value: number, color: string, percentage: number }) {
    const colorMap: any = {
        ...
        <div className="space-y-1.5">
            <div className="flex justify-between items-end">
                ...
                <div className="flex items-baseline gap-1">
                    <span className={`text-xl font-black ${colorMap[color].split(' ')[1]}`}>{value}</span>
                    <span className="text-[10px] text-slate-600 font-bold">g</span>
                </div>
            </div>
            <div className="h-1.5 w-full bg-slate-950 rounded-full overflow-hidden">
                <div
                    className={`h-full ${colorMap[color].split(' ')[0]} transition-all duration-700`}
