...
export function ToolsReplayPage() {
    ...
    const eventsWithPB = useMemo(() => {
        const sortedExercises = [...exerciseEntries].sort((a, b) => a.date.localeCompare(b.date));
    ...
    // Stats
    const stats = useMemo(() => {
        const events = historicalData.flatMap(d => d.events).filter(e => e.type !== 'weight');
        const totalDist = events.reduce((sum, e) => sum + (e.data.distance || 0), 0);
        ...
        const weightDiff = currentWeight && startWeight ? currentWeight - startWeight : 0;
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start justify-between">
                <div className="space-y-2">
                    ...
                    <p className="text-slate-400 text-sm max-w-md">
                        Återupplev året som gått. Välj period, luta dig tillbaka och se dina framsteg spelas upp.
                    </p>
                </div>
                        ...
                        <div className="flex-1 space-y-1">
                            ...
                            <div className="relative h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div
                                    ...
                                    style={{ width: `${Math.min(100, (getDaysDiff(startDate, currentDate) / getDaysDiff(startDate, endDate)) * 100)}%` }}
                                />
                            </div>
                        </div>

                        <div className="flex flex-col gap-1">
                            <span className="text-[9px] font-bold text-slate-500 uppercase">Hastighet</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6 lg:col-span-1 h-[600px] flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="p-4 rounded-2xl bg-slate-900 border border-white/5">
                            <div className="text-[10px] uppercase text-slate-500 font-bold mb-1">Nuvarande Vikt</div>
                            <div className="text-2xl font-black text-white">
                                {stats.currentWeight ? stats.currentWeight.toFixed(1) : '-'} <span className="text-sm font-bold text-slate-500">kg</span>
                            </div>
                            <div className={`text-xs font-bold ${stats.weightDiff <= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                                {stats.weightDiff > 0 ? '+' : ''}{stats.weightDiff.toFixed(1)} kg
                            </div>
                        </div>
                        <div className="p-4 rounded-2xl bg-slate-900 border border-white/5">
                            <div className="text-[10px] uppercase text-slate-500 font-bold mb-1">Total Volym</div>
                            <div className="text-2xl font-black text-white">
                                {Math.round(stats.totalTon / 1000)} <span className="text-sm font-bold text-slate-500">ton</span>
                            </div>
                        </div>
                        <div className="p-4 rounded-2xl bg-slate-900 border border-white/5">
                            <div className="text-[10px] uppercase text-slate-500 font-bold mb-1">Total Distans</div>
                            <div className="text-2xl font-black text-white">
                                {Math.round(stats.totalDist)} <span className="text-sm font-bold text-slate-500">km</span>
                            </div>
                        </div>
                        <div className="p-4 rounded-2xl bg-slate-900 border border-white/5">
                            <div className="text-[10px] uppercase text-slate-500 font-bold mb-1">Antal Pass</div>
                            <div className="text-2xl font-black text-white">
                                {stats.totalWorkouts}
                            </div>
                        </div>
                    ...
                    <div className="flex-1 bg-slate-900/50 rounded-3xl border border-white/5 p-4 overflow-hidden flex flex-col relative">
                        <h3 className="text-xs font-bold text-white uppercase tracking-wider mb-4 px-2">Händelseflöde</h3>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline">
                                                <h4 className={`text-xs font-black uppercase truncate ${event.isGold ? 'text-amber-400' : 'text-white'}`}>
                                                    {event.title}
                                                </h4>
                                                <span className="text-[9px] text-slate-500 font-mono">{event.date}</span>
                                            </div>
                                            <p className="text-[11px] text-slate-300 truncate">{event.subtitle}</p>
                                        </div>
                                        <div className="text-right">
                                            <div className={`text-xs font-black ${event.isGold ? 'text-amber-400' : 'text-white'}`}>
                                                {event.value}
                                            </div>
                                        </div>
                                    </div>
                ...
                <div className="lg:col-span-2 bg-slate-900 border border-white/5 rounded-3xl p-6 relative overflow-hidden h-[600px] flex flex-col">
                    ...
                    <div className="flex justify-between items-end mb-4 relative z-10">
                        <div>
                            <h2 className="text-xl font-bold text-white">Viktkurva & Milstolpar</h2>
                            <p className="text-xs text-slate-400">Visualisering av kroppsvikt i relation till träning</p>
                        </div>
                    </div>
...
export default ToolsReplayPage;
