...
export default function ToolsLoadAnalysisPage() {
  ...
  const availableExercises = useMemo(() => {
    ...
    rawData.forEach((session) => {
      session.exercises.forEach((ex: any) => {
        const norm = ex.exerciseName.toLowerCase().trim().replace(
          /[^a-z0-9\s]/g,
          "",
        ); // Naive norm
  ...
  // Insights
  const insight = useMemo(() => generateLoadInsights(analysisResult), [
    analysisResult,
  ]);
    ...
    <Layout title="Belastningsanalys">
      <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-400 max-w-xl">
              Analysera din "Effective Volume" – set som faktiskt räknas. Vi
              filtrerar bort uppvärmning och jämför din volym mot din
              styrkeutveckling.
            </p>
          </div>
          <button
          ...
          >
            {showAdmin ? "Dölj Admin" : "Hantera Övningar"}{" "}
            {unmapped.length > 0 && <span className="text-rose-500">•</span>}
          </button>
        </div>
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            >
              <option value="">
                {selectedMuscle
                  ? "Alla övningar för " +
                    MUSCLE_DISPLAY_NAMES[selectedMuscle as MuscleGroup]
                  : "Alla övningar"}
              </option>
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Staplar = Effektiva Set (&gt;75%), Linje = e1RM Trend
              </p>
            </div>
            {selectedExercise && (
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-900 border border-slate-800 rounded-3xl p-8 relative overflow-hidden">
          <div
            className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-${
              insight.color.replace("text-", "")
            } to-transparent opacity-5 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none`}
          >
          </div>
