...
export function ToolsRacePredictorPage() {
  ...
  const bestPerformance = useMemo(() => {
    ...
    return pbs.reduce((prev, current) =>
  ...
  if (!universalActivities) {
    return <div className="p-10 text-slate-400">Laddar...</div>;
  }
        ...
        ? (
          <>
            ...
            <div className="relative overflow-hidden bg-slate-900 border border-purple-500/30 rounded-3xl p-8 shadow-[0_0_50px_rgba(168,85,247,0.15)]">
              <div className="absolute top-0 right-0 p-40 bg-purple-600/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2">
              </div>
              ...
              <div className="relative z-10 grid md:grid-cols-2 gap-8 items-center">
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-400">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xl text-white font-bold">
                        {formatPace(bestPerformance.pace)}
                      </span>
                    </div>
                    <div className="w-px h-8 bg-white/10"></div>
                    <div className="flex flex-col">
                ...
                <div className="bg-slate-950/50 rounded-2xl p-6 border border-white/5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setImprovementFactor(Number(e.target.value))}
                  ...
                  <div className="mt-4 text-xs text-slate-400 leading-relaxed">
                    Dra i reglaget för att se hur dina tider skulle se ut om du
                    blev <strong>{Math.abs(improvementFactor)}%</strong>{" "}
                    {improvementFactor >= 0 ? "snabbare" : "långsammare"}.
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-950 text-slate-500 uppercase text-[10px] tracking-wider font-bold">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-right text-purple-300">
                          Prognos
                        </th>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {predictions.map((row) => {
                        const isActive =
                          row.label === bestPerformance.distanceLabel;
                        const hasPB = !!row.actualPB;

                        const diffSeconds = hasPB
                          ? row.actualPB!.timeSeconds - row.predictedSeconds
                          : 0;
                        const diffPercent = hasPB
                          ? (diffSeconds / row.predictedSeconds) * 100
                          : 0;
                        ...
                        return (
                          ...
                          >
                            <td className="px-6 py-4 font-bold text-white">
                              {row.label}
                              {isActive && (
                                <span className="ml-2 text-[10px] bg-purple-500 text-white px-1.5 py-0.5 rounded font-bold uppercase">
                                  Best
                                </span>
                              )}
                            </td>
                                ...
                                ? (
                                  <div className="flex flex-col items-end">
                                    <span className="text-white">
                                      {formatSeconds(
                                        Math.round(row.actualPB.timeSeconds),
                                      )}
                                    </span>
                                ...
                                ? (
                                  <div
                                    className={getPerformanceColor(
                                      row.actualPB!.timeSeconds,
                                      row.basePredictedSeconds,
                                    )}
                                  >
                                    {diffSeconds > 0 ? "+" : ""}
                                    {formatSeconds(
                                      Math.abs(Math.round(diffSeconds)),
                                    )}
                                    {
        ...
        : (
          <div className="p-12 text-center border border-dashed border-slate-700 rounded-3xl">
            ...
            <h3 className="text-xl font-bold text-white mb-2">
              Inga löp-data hittades
            </h3>
            <p className="text-slate-400">
              Logga några löppass med distanser som 5km, 10km osv för att se
              prognoser!
            </p>
          </div>
        )}
