...
const TOTAL_STANDARDS = {
  male: Array.from(
    { length: 5 },
    (_, i) =>
      STANDARDS.male.squat[i] + STANDARDS.male.bench[i] +
      STANDARDS.male.deadlift[i],
  ),
  female: Array.from(
    { length: 5 },
    (_, i) =>
      STANDARDS.female.squat[i] + STANDARDS.female.bench[i] +
      STANDARDS.female.deadlift[i],
  ),
};
...
export function ToolsStrengthStandardsPage() {
  ...
  useEffect(() => {
    ...
    if (
      settingsGender &&
      (settingsGender === "male" || settingsGender === "female")
    ) {
    ...
    (["squat", "bench", "deadlift", "ohp"] as const).forEach((key) => {
      ...
      if (maxERM > 0 || maxWeight > 0) {
        ...
        const valToSet = showEstimated
          ? Math.round(maxERM)
          : (maxWeight > 0 ? maxWeight : Math.round(maxERM));
  ...
  const getLiftStats = (key: keyof typeof lifts) => {
    ...
    return {
      ...
      percentToNext: Math.min(
        100,
        Math.max(0, (val / (std[levelIdx] * weight)) * 100),
      ), // Approximation
    };
  };
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="text-center md:text-left">
        ...
        <p className="text-slate-400 max-w-2xl">
          Analysera din styrka mot globala standarder. Vi har automatiskt hämtat
          dina bästa lyft från din historik.
        </p>
      </div>
              ...
              <div>
                ...
                <div className="flex gap-1 bg-slate-950 rounded-lg p-1">
                  <button
                    ...
                    className={`flex-1 rounded p-1 text-xs font-bold ${
                      gender === "male"
          ...
          <div className="bg-slate-900 border border-white/5 rounded-3xl p-6">
            ...
            <div className="space-y-4">
              {(["squat", "bench", "deadlift", "ohp"] as const).map((lift) => {
                const detected = showEstimated
                  ? lifts[lift].erm
                  : (lifts[lift].actual || lifts[lift].erm);
                const isModified = lifts[lift].weight !== detected &&
                  detected > 0;
                const liftLabel = lift === "ohp"
                  ? "Militärpress"
                  : lift === "bench"
                  ? "Bänkpress"
                  : lift === "squat"
                ...
                const sourceSet = showEstimated
                  ? lifts[lift].bestSetERM
                  : lifts[lift].bestSetWeight;
                  ...
                  <div key={lift} className="flex flex-col gap-1">
                      ...
                      <div className="flex-1">
                        {lifts[lift].exerciseName
                          ? (
                            <Link
                              to={`/styrka/${
                                slugify(lifts[lift].exerciseName)
                              }`}
                        ...
                        <div className="text-[10px] text-slate-500">
                          {lifts[lift].exerciseName
                            ? (
                              <span title={lifts[lift].exerciseName}>
                                {lifts[lift].exerciseName.substring(0, 20)}
                                {lifts[lift].exerciseName.length > 20
                                  ? "..."
                                  : ""}
                              </span>
                            )
                    ...
                    {sourceValid && (
                      <div className="text-[9px] text-slate-500 font-mono text-right border-t border-white/5 pt-1 mt-0.5">
                        Källa ({showEstimated ? "eRM" : "Max"}):{" "}
                        {sourceSet.weight}kg x {sourceSet.reps}{" "}
                        ({sourceSet.date})
                      </div>
                    )}
                  </div>
              ...
              return (
                <div className="bg-gradient-to-br from-indigo-500/10 to-blue-500/10 border border-indigo-500/20 rounded-3xl p-6 flex flex-col justify-center">
                  ...
                  <div className="text-white">
                    ...
                    <strong className="text-xl capitalize text-indigo-300">
                      {liftName}
                    </strong>
                    <span className="text-slate-400">med</span>
                    <strong className="text-xl text-white">
                      {Math.round(rec.diff)}kg
                    </strong>
                  </div>
            ...
            <div className="relative z-10 w-full md:w-auto">
              <div className="bg-slate-950/50 backdrop-blur-md border border-white/10 rounded-2xl p-6 text-center min-w-[200px]">
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className={`text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br ${
                        LEVEL_COLORS[totalLevelIdx]
                      } mb-2`}
                    >
                ...
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 w-3/4 rounded-full">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-4">
            {(["squat", "bench", "deadlift", "ohp"] as const).map((lift) => {
              ...
              const maxVal = Math.max(
                stats.val * 1.5,
                STANDARDS[gender][lift][4] * weight,
              );

              return (
                <div
                  key={lift}
                  className="bg-slate-900 border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-all group"
                >
                  <div className="flex justify-between items-end mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        {lifts[lift].exerciseName
                          ? (
                            <Link
                              ...
                              className="text-lg font-bold text-white capitalize hover:text-emerald-400 decoration-emerald-500/50 hover:underline transition-all"
                            >
                              {lift === "ohp"
                                ? "Militärpress"
                                : lift === "bench"
                        ...
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase bg-gradient-to-r ${stats.levelColor} text-white/90`}
                        >
                          {stats.levelLabel}
                        </span>
                      </div>
                      <div className="text-xs text-slate-400">
                        ...
                        <span className="text-emerald-400 font-bold">
                          {Math.min(99, Math.round(stats.ratio * 25))}%
                        </span>{" "}
                        av motionärer (est.)
                      </div>
                    </div>
                  ...
                  <div className="relative h-6 bg-slate-800 rounded-full w-full mb-2 flex">
                    ...
                    {LEVEL_COLORS.map((color, i) => {
                      ...
                      const prevReq = i === 0
                        ? 0
                        : STANDARDS[gender][lift][i - 1] * weight;
                      const nextReq = i < 5
                        ? STANDARDS[gender][lift][i] * weight
                        : maxVal * 1.2; // Extend last segment
                      ...
                      return (
                        ...
                        >
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/segment:block z-30 whitespace-nowrap">
                            ...
                            {/* Arrow */}
                            <div className="w-2 h-2 bg-slate-900 border-r border-b border-white/10 transform rotate-45 absolute -bottom-1 left-1/2 -translate-x-1/2">
                            </div>
                          </div>
                        </div>
                      );
                    ...
                    <div
                      ...
                      style={{
                        left: `${
                          Math.min(98, Math.max(1, (stats.val / maxVal) * 100))
                        }%`,
                      }}
                    >
                      {/* Marker Tooltip */}
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/marker:block z-40 whitespace-nowrap">
                        ...
                        <div className="w-3 h-3 bg-slate-950 border-r border-b border-white/20 transform rotate-45 absolute -bottom-1.5 left-1/2 -translate-x-1/2">
                        </div>
                      </div>
                    </div>
                    ...
                    {stats.eRM > stats.val && (
                      <div
                        ...
                        style={{
                          left: `${Math.min(100, (stats.eRM / maxVal) * 100)}%`,
                        }}
                    ...
                    <span>
                      {stats.nextLabel && (
                        <span className="text-emerald-500 font-bold">
                          {Math.max(0, stats.nextWeight - stats.val)}kg till
                          {" "}
                          {stats.nextLabel}
                        </span>
                      )}
