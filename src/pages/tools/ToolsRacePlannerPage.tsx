...
export function ToolsRacePlannerPage() {
  ...
  const [nutritionMode, setNutritionMode] = useState<"simple" | "advanced">(
    "simple",
  );
  ...
  // UI State
  const [activeTab, setActiveTab] = useState<"config" | "plan" | "logistics">(
    "config",
  );
  ...
  const runSimulation = () => {
    // 1. Weather
    const penalty = calculateWeatherPenaltyFactor(
    ...
    const caffeinePresetData = CAFFEINE_PRESETS.find((p) =>
      p.id === caffeinePreset
    );
    const preRaceCaffeine = includePreRaceCaffeine
      ? (caffeinePresetData?.preRaceMg || 0)
      : 0;
    ...
    const s = generateSplits(
      raceProfile.distanceKm,
      adjustedTargetSeconds,
      pacingStrategy.type as any,
    );
    setSplits(s);
    ...
    if (penalty > 1.05) {
      newWarnings.push(
        `Varning: Vädret beräknas sänka din prestation med ${
          Math.round((penalty - 1) * 100)
        }%. Sänk tempot!`,
      );
    }
    if (res.crashTime !== null) {
      newWarnings.push(
        `CRASH WARNING: Glykogendepåerna tar slut vid ${
          Math.floor(res.crashTime / 60)
        } minuter! Öka intaget.`,
      );
    }

    // Check hourly carbs
    const totalCarbs = intakeEvents.reduce(
      (acc, e) => acc + (e.product?.carbsG || 0) * e.amount,
      0,
    );
    ...
    if (gPerH > 90) {
      newWarnings.push(
        `Högt kolhydratsintag (${
          Math.round(gPerH)
        }g/h). Risk för magproblem om du inte tränat på det.`,
      );
    }
    if (gPerH < 30 && raceProfile.distanceKm > 21) {
      newWarnings.push(
        `Lågt kolhydratsintag (${
          Math.round(gPerH)
        }g/h). Rekommenderas 60-90g/h för ultra.`,
      );
    }
    ...
    if (hydration.hydrationStatus === "critical") {
      newWarnings.push(
        `⚠️ Kritisk dehydrering (${hydration.dehydrationPercent}%)! Öka vätskeintaget.`,
      );
    } else if (hydration.hydrationStatus === "warning") {
      newWarnings.push(
        `Vätskebalans: ${hydration.dehydrationPercent}% kroppsvikt förlust. Överväg mer dryck.`,
      );
    }
  ...
  const handleGenerateSimpleIntake = () => {
    const carbSource =
      CARB_SOURCE_PRESETS.find((p) => p.id === carbSourcePreset) ||
      CARB_SOURCE_PRESETS[0];
    const caffeinePresetData = CAFFEINE_PRESETS.find((p) =>
      p.id === caffeinePreset
    );
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {warnings.length > 0 && (
          <div className="mb-8 space-y-2">
            {warnings.map((w, i) => (
              <div
                key={i}
                className="bg-amber-500/10 border border-amber-500/20 text-amber-200 p-4 rounded-xl flex items-start gap-3"
              >
        ...
        {activeTab === "config" && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                ...
                <label className="text-xs text-slate-400 uppercase font-bold flex items-center gap-2">
                  ...
                  {suggestedSweatPresetId && (
                    <span className="text-[10px] text-emerald-400">
                      (Förslag: {SWEAT_RATE_PRESETS.find((p) =>
                        p.id === suggestedSweatPresetId
                      )?.label})
                    </span>
                  )}
                </label>
                ...
                <InputGroup label="Fuktighet" suffix="%">
                  <input
                    ...
                    onChange={(e) =>
                      setEnvironment({
                        ...environment,
                        humidityPercent: parseFloat(e.target.value),
                      })}
                  />
                </InputGroup>
              </div>
            ...
            <ConfigSection
              title="Energi & Nutrition"
              icon={<Zap size={18} />}
              badge={nutritionMode === "simple" ? "Enkel" : "Avancerad"}
            >
              {/* Mode Toggle */}
              <div className="flex gap-2 mb-4">
                <button
                  onClick={() =>
                    setNutritionMode("simple")}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <input
                          ...
                          value={carbsPerHour}
                          onChange={(e) =>
                            setCarbsPerHour(parseInt(e.target.value) || 60)}
                        />
                    ...
                    <div className="space-y-2">
                      ...
                      {carbSourcePreset === "auto" && (
                        <p className="text-[10px] text-emerald-400 mt-1">
                          Valt baserat på väder:{" "}
                          {suggestedCarbSource.gelPct}% Gel /{" "}
                          {suggestedCarbSource.drinkPct}% Dryck
                        </p>
                      )}
                    </div>
                        ...
                        <div className="space-y-2 mt-2">
                          <label className="flex items-center gap-2 text-sm">
                            <input
                              ...
                              onChange={(e) =>
                                setIncludePreRaceCaffeine(e.target.checked)}
                              className="rounded bg-slate-800 border-white/20"
                            />
                            <span className="text-slate-300">
                              Koffein före start (30 min)
                            </span>
                            <span className="text-emerald-400 text-xs">
                              +{CAFFEINE_PRESETS.find((p) =>
                                p.id === caffeinePreset
                              )?.preRaceMg}mg
                            </span>
                          </label>
                          <p className="text-[10px] text-slate-500">
                            Totalt: {includePreRaceCaffeine
                              ? CAFFEINE_PRESETS.find((p) =>
                                p.id === caffeinePreset
                              )?.preRaceMg
                              : 0}mg före +
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div>
                            <div className="text-xl font-black text-emerald-400">
                              {intakeEvents.reduce((s, e) =>
                                s + (e.product?.carbsG || 0), 0)}g
                            </div>
                  ...
                  <div>
                    ...
                    <div className="max-h-64 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                      {intakeEvents.map((evt, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2 bg-slate-800 p-2 rounded text-sm"
                        >
                          <input
                            type="number"
                            ...
                            onChange={(e) => {
                              ...
                              newEvents[idx].distanceKm = parseFloat(
                                e.target.value,
                              );
                              setIntakeEvents(newEvents);
                            }}
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              const prod = PRESET_PRODUCTS.find((p) =>
            ...
            <ConfigSection title="Dropbags" icon={<ShoppingBag size={18} />}>
              <div className="space-y-2">
                <p className="text-xs text-slate-400">
                  Ange vid vilka KM du har dropbags:
                </p>
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                <div className="flex items-center justify-center bg-slate-950/50 rounded p-1 border border-white/5 mb-1">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const m = Math.floor(
                        (raceProfile.targetTimeSeconds % 3600) / 60,
                      );
                      setRaceProfile({
                        ...raceProfile,
                        targetTimeSeconds: h * 3600 + m * 60,
                      });
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const h = Math.floor(
                        raceProfile.targetTimeSeconds / 3600,
                      );
              ...
              <div className="bg-slate-900 rounded-xl p-3 border border-white/5 flex flex-col justify-between">
                ...
                <div className="text-[10px] text-slate-500">
                  {formatTime(simResult.finishTime / raceProfile.distanceKm)
                    .substring(3, 8)}/km
                </div>
              </div>
              ...
              >
                ...
                <div
                  className={`text-xl font-black ${
                    simResult.crashTime ? "text-red-500" : "text-emerald-500"
                  }`}
                >
                  {simResult.crashTime
                    ? `${Math.floor(simResult.crashTime / 60)} min`
                    : "Nej"}
                </div>
                <div className="text-[10px] text-slate-500">
                  {simResult.crashTime
                    ? `~${
                      (simResult.crashTime / 60 / (simResult.finishTime / 60) *
                        raceProfile.distanceKm).toFixed(1)
                    } km`
                    : "Energi räcker"}
                </div>
              </div>
              ...
              {hydrationSummary && (
                <div
                  className={`rounded-xl p-3 border flex flex-col justify-between ${
                    hydrationSummary.hydrationStatus === "critical"
                      ? "bg-red-900/10 border-red-500/50"
                      : hydrationSummary.hydrationStatus === "warning"
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-4 text-[10px]">
                  ...
                  <span className="flex items-center gap-1">
                    <span
                      className="w-3 h-0.5 bg-blue-500 border-dashed border-t border-transparent"
                      style={{ borderTopStyle: "dashed", borderWidth: "1px" }}
                    >
                    </span>{" "}
                    Vätska
                  </span>
                  <span className="flex items-center gap-1">
                    <span
                      className="w-3 h-0.5 bg-amber-500 border-dashed border-t border-transparent"
                      style={{ borderTopStyle: "dashed", borderWidth: "1px" }}
                    >
                    </span>{" "}
                    Koffein
                  ...
                  <LineChart data={simResult.timeline}>
                    ...
                    <RechartsTooltip
                      ...
                      formatter={(value: number, name: string) => {
                        if (name.includes("Vätska")) {
                          return [`${value.toFixed(2)} L`, name];
                        }
                        if (name.includes("Koffein")) {
                    ...
                    <Line
                      ...
                      style={{
                        filter:
                          "drop-shadow(0px 0px 4px rgba(16, 185, 129, 0.5))",
                      }}
                    />
                    {/* Fluid Balance Line */}
                    <Line
                    ...
                    <ReferenceLine
                      ...
                      stroke="#f59e0b"
                      strokeDasharray="3 3"
                      label={{ value: "-2% BW", fill: "#f59e0b", fontSize: 10 }}
                    />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="bg-slate-900 border border-white/5 rounded-2xl p-4 flex flex-col">
                ...
                <ul className="space-y-3 text-xs text-slate-300 flex-1">
                  <li className="flex gap-2 items-start">
                    ...
                    <span>
                      <b>Carbo-load:</b>{" "}
                      Ät extra kolhydrater (8-10g/kg) dagen innan.
                    </span>
                  </li>
                  <li className="flex gap-2 items-start">
                    <span className="text-emerald-500 font-bold">✓</span>
                    <span>
                      <b>Frukost:</b> Lättsmält frukost 3-4h innan start.
                    </span>
                  </li>
                  {includePreRaceCaffeine
                    ? (
                      <li className="flex gap-2 items-start">
                        <span className="text-emerald-500 font-bold">✓</span>
                        <span>
                          <b>Koffein:</b> Inta {CAFFEINE_PRESETS.find((p) =>
                            p.id === caffeinePreset
                          )?.preRaceMg}mg koffein 30-45 min innan start.
                        </span>
                      </li>
                    )
                    : (
                      <li className="flex gap-2 items-start">
                        ...
                        <span className="text-slate-500">
                          Inget koffein innan start valt.
                        </span>
                      </li>
                    )}
                  <li className="flex gap-2 items-start">
                    <span className="text-emerald-500 font-bold">✓</span>
                    <span>
                      <b>Hydrering:</b> Drick 500ml vatten 2h innan start.
                    </span>
                  </li>
                </ul>
              </div>
              <div className="bg-slate-900 border border-white/5 rounded-2xl p-4 flex flex-col">
                ...
                <ul className="space-y-3 text-xs text-slate-300 flex-1">
                  <li className="flex gap-2 items-start">
                    ...
                    <span>
                      <b>Refill:</b>{" "}
                      1g kolhydrater/kg kroppsvikt direkt efter mål.
                    </span>
                  </li>
                  <li className="flex gap-2 items-start">
                    <span className="text-blue-500 font-bold">✓</span>
                    <span>
                      <b>Protein:</b>{" "}
                      Få i dig 20-30g protein för muskelreparation.
                    </span>
                  </li>
                  {hydrationSummary && hydrationSummary.netDeficitL > 0.5 && (
                    <li className="flex gap-2 items-start">
                      ...
                      <span>
                        <b>Vätska:</b> Drick ca{" "}
                        {(hydrationSummary.netDeficitL * 1.5).toFixed(1)}L
                        vätska under em/kvällen.
                      </span>
                    </li>
                  )}
                </ul>
              ...
              <div className="overflow-x-auto max-h-[400px] custom-scrollbar">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5 text-[11px]">
                    {splits.map((split, i) => {
                      const exactActions = intakeEvents.filter((e) =>
                        Math.abs(e.distanceKm - split.km) < 0.5
                      );
                      // Show every 5km OR if there is an action
                      if (
                        split.km % 5 !== 0 && exactActions.length === 0 &&
                        split.km !== Math.ceil(raceProfile.distanceKm)
                      ) return null;
                      ...
                      return (
                        <tr
                          ...
                          className={`hover:bg-white/5 transition-colors ${
                            isActionRow ? "bg-emerald-900/5" : ""
                          }`}
                        >
                          <td className="px-3 py-1.5 font-mono text-slate-400 border-r border-white/5">
                            {split.km}
                          </td>
                          ...
                          <td className="px-3 py-1.5">
                            {exactActions.map((a, ai) => (
                              <div
                                key={ai}
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-300 text-[10px] mr-2 shadow-sm"
                              >
                                {a.product?.type === "liquid"
                                  ? (
                                    <Droplet
                                      size={8}
            ...
            <div className="bg-slate-900 border border-white/5 rounded-2xl p-6 space-y-6">
              ...
              <div className="space-y-6">
                {logistics.map((leg, i) => (
                  <div
                    key={i}
                    className="bg-slate-950 rounded-xl p-4 border border-white/5"
                  >
                ...
                <ul className="space-y-3 text-sm text-slate-300">
                  ...
                  {(() => {
                    const finishDate = new Date(
                      `${raceProfile.date}T${raceProfile.startTime}`,
                    );
                    finishDate.setSeconds(
                      finishDate.getSeconds() + (simResult?.finishTime || 0),
                    );
                    const sunsetDate = new Date(
                      `${raceProfile.date}T${environment.sunsetTime}`,
                    );
                    ...
                    if (finishDate > sunsetDate) {
                      return (
                        <li className="flex items-start gap-3 text-amber-300">
                          ...
                          <div>
                            <strong>Pannlampa krävs!</strong>
                            <br />
                            Du går i mål ca{" "}
                            {finishDate.toTimeString().substring(0, 5)}, vilket
                            är efter solnedgång ({environment.sunsetTime}).
                          </div>
                        </li>
                      );
                    }
                  ...
                  {environment.temperatureC < 5 && (
                    <li className="flex items-start gap-3">
                      ...
                      <span>
                        Kallt väder ({environment.temperatureC}°C). Ta med
                        handskar och buff.
                      </span>
                    </li>
                  )}
                  {environment.temperatureC > 20 && (
                    <li className="flex items-start gap-3">
                      ...
                      <span>
                        Varmt väder! Is-bandana eller keps rekommenderas.
                      </span>
                    </li>
                  )}
                </ul>
              ...
              <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                  {savedPlans.map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center justify-between p-3 bg-slate-950 rounded hover:bg-slate-800 transition-colors cursor-pointer"
                      onClick={() => {
                    ...
                    >
                      ...
                      {selectedPlanId === p.id && (
                        <div className="text-emerald-400 text-xs">Aktiv</div>
                      )}
                    </div>
                  ))}
                  {savedPlans.length === 0 && (
