...
import { ArrowLeft, Target, Info, Dumbbell, TrendingUp } from 'lucide-react';
...
export function ToolsOlympicPage() {
    ...
    const efficiency = useMemo(() => {
        ...
        return {
            ...
            snatchStatus: snatchRatio < 60 ? 'Teknikbegränsad' : snatchRatio > 66 ? 'Styrkebegränsad' : 'Balanserad',
            cjStatus: cjRatio < 80 ? 'Teknikbegränsad' : cjRatio > 85 ? 'Styrkebegränsad' : 'Balanserad'
        };
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in pb-24">
            <div className="mb-8">
                <Link to="/tools" className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition-colors text-sm font-medium mb-4">
                    <ArrowLeft className="w-4 h-4" />
                    Tillbaka till verktyg
                </Link>
                <h1 className="text-3xl font-black text-white uppercase italic tracking-tighter">Olympisk Tyngdlyftning</h1>
                <p className="text-slate-400 mt-2">Beräkna din Sinclair-poäng och analysera din tekniska effektivitet mot IWF-standarder.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <div className="bg-slate-900 border border-white/5 rounded-3xl p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                         <div>
                            <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Kön</label>
                            <div className="flex bg-slate-950 p-1 rounded-xl border border-white/10">
                                <button
                                    onClick={() => setGender('male')}
                                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${gender === 'male' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
                                >
                                    MAN
                                </button>
                                <button
                                    onClick={() => setGender('female')}
                                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${gender === 'female' ? 'bg-pink-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
                                >
                                    KVINNA
                                </button>
                            </div>
                        ...
                        <div>
                            <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Kroppsvikt (kg)</label>
                            <input
                                type="number"
                                value={bodyWeight}
                    ...
                    <div className="space-y-4 pt-2">
                        <div>
                            <label className="flex justify-between text-[10px] uppercase font-bold text-slate-500 mb-1">
                                ...
                                {efficiency && <span className={efficiency.snatchRatio < 60 ? 'text-rose-400' : 'text-emerald-400'}>{efficiency.snatchRatio}% av Böj</span>}
                            </label>
                            <input
                                type="number"
                                value={snatch}
                        ...
                        <div>
                            <label className="flex justify-between text-[10px] uppercase font-bold text-slate-500 mb-1">
                                ...
                                {efficiency && <span className={efficiency.cjRatio < 80 ? 'text-rose-400' : 'text-emerald-400'}>{efficiency.cjRatio}% av Böj</span>}
                            </label>
                            <input
                                type="number"
                                value={cleanJerk}
                        ...
                        <div className="pt-4 border-t border-white/5">
                            <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Knäböj (För analys)</label>
                            <input
                                type="number"
                                value={squat}
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-indigo-950 border border-white/10 rounded-3xl p-8 text-center relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-8 opacity-5 text-9xl font-black">IWF</div>

                        <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-2">Sinclair Total</h3>
                        ...
                        <div className="inline-flex items-center gap-2 bg-white/5 px-4 py-2 rounded-full border border-white/5">
                            <span className="text-xs font-bold text-slate-300">Totalvikt: <span className="text-white">{total} kg</span></span>
                        </div>
                    </div>

                    {/* LEVELS / PROGRESS */}
                    <div className="bg-slate-900 border border-white/5 rounded-3xl p-6">
                        <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6">Nivåbedömning</h3>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                <div className="flex justify-between text-xs font-bold mb-2">
                                    ...
                                    <span className="text-slate-400">{Math.round((total / bodyWeight) * 100) / 100}x BW</span>
                                </div>
                                <div className="h-3 bg-slate-950 rounded-full overflow-hidden flex relative">
                                    ...
                                    <div
                                        ...
                                        style={{
                                            width: `${Math.min(100, (total / (bodyWeight * 2.5)) * 100)}%`, // Scale: 0 to 2.5x BW
            ...
            {efficiency && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <div className="bg-slate-900 border border-white/5 rounded-3xl p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="text-lg font-black text-white italic">Ryck-analys</h3>
                                <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${
                                    efficiency.snatchStatus === 'Balanserad' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
                                }`}>
                            ...
                            <div className="flex items-end gap-2 mb-4">
                                <span className="text-4xl font-black text-white">{efficiency.snatchRatio}%</span>
                                <span className="text-xs text-slate-400 font-bold mb-1">av Knäböj</span>
                            </div>

                            <p className="text-sm text-slate-400 leading-relaxed">
                                {efficiency.snatchStatus === 'Teknikbegränsad' && "Du har en stor styrkereserv i benen. Fokusera på teknik, rörlighet och explosivitet för att få ut mer av din styrka i rycket."}
                                {efficiency.snatchStatus === 'Styrkebegränsad' && "Din teknik är utmärkt! Du utnyttjar din benstyrka till max. För att öka i ryck måste du nu bli starkare i knäböj."}
                                {efficiency.snatchStatus === 'Balanserad' && "Du ligger inom det optimala intervallet (60-66%). Fortsätt träna både styrka och teknik parallellt."}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-900 border border-white/5 rounded-3xl p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="text-lg font-black text-white italic">Stöt-analys</h3>
                                <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${
                                    efficiency.cjStatus === 'Balanserad' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
                                }`}>
                                    {efficiency.cjStatus}
                                </span>
                            </div>
                            ...
                            <div className="flex items-end gap-2 mb-4">
                                <span className="text-4xl font-black text-white">{efficiency.cjRatio}%</span>
                                <span className="text-xs text-slate-400 font-bold mb-1">av Knäböj</span>
                            </div>

                            <p className="text-sm text-slate-400 leading-relaxed">
                                {efficiency.cjStatus === 'Teknikbegränsad' && "Din stöt släpar efter din benstyrka. Fokusera på vändningen (clean) och överstöten (jerk) tekniskt."}
                                {efficiency.cjStatus === 'Styrkebegränsad' && "Imponerande effektivitet! Du får ut nästan allt av din styrka. Prioritera tyngre knäböj för att driva upp totalen."}
                                {efficiency.cjStatus === 'Balanserad' && "Du ligger perfekt (80-85%). Din styrka och teknik utvecklas i takt."}
                            </p>
                        </div>
                    </div>
                </div>
