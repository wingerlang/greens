  ...
  const pbs = React.useMemo(() => {
    ...
    targets.forEach((dist) => {
      const match = exerciseEntries
        .filter((e) =>
          e.type === "running" && e.distance &&
          Math.abs(e.distance - dist) < (dist * 0.05)
        )
    ...
    <div className="space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-slate-400">
          Se hur lång tid olika distanser tar vid ett visst tempo.
        </p>
      </div>
          ...
          <div className="space-y-1 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
            {distances.map((dist) => {
              ...
              return (
                <div
                  key={dist.name}
                  className="flex justify-between items-center px-3 py-2 rounded-lg hover:bg-white/5 transition-colors"
                >
