...
import { PlannedActivity, UniversalActivity } from "../models/types.ts";
...
function mapUniversalToPlanned(u: UniversalActivity): PlannedActivity {
  return {
    ...
    category: u.plan?.activityCategory ||
      (u.performance?.source?.source === "strava" ? "EASY" : "EASY"),
    ...
    actualTimeSeconds: u.performance?.durationMinutes
      ? u.performance.durationMinutes * 60
      : undefined,
  };
}
...
export function CoachPage() {
  ...
  useEffect(() => {
    if (
      tabFromUrl &&
      ["plan", "summary", "setup", "analysis", "progress"].includes(tabFromUrl)
    ) {
      ...
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-8 max-w-7xl mx-auto">
        <div>
          ...
          <p className="text-slate-400 max-w-xl text-[11px] leading-relaxed opacity-80 font-medium">
            Din personliga, algoritmiska löpcoach. Jack Daniels VDOT,
            80/20-regeln & adaptiv progression.
          </p>
        </div>
        ...
        {!coachConfig && activeTab !== "setup"
          ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-20 h-20 mb-6 bg-emerald-500/10 rounded-full flex items-center justify-center text-4xl">
                🔭
              </div>
              <h2 className="text-xl font-black mb-3">
                Dags att bygga din plan
              </h2>
              <p className="text-slate-400 mb-6 max-w-sm text-sm">
                Berätta för coachen om din nuvarande form och mål.
              </p>
              <button
                onClick={() => handleTabChange("setup")}
          ...
          : (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              {activeTab === "plan" && (
                <CoachCalendar activities={displayActivities} />
              )}
              {activeTab === "summary" && (
                <CoachPlanSummary activities={displayActivities} />
              )}
