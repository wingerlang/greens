        ...
        <div className="flex-1">
          <div className="flex flex-wrap gap-2 mb-2">
            {routine.tags.map((tag) => (
              <span
                key={tag}
                className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-slate-800 text-slate-400"
              >
                {tag}
              </span>
            ))}
            {routine.condition && (
              <span
                className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${
                  routine.condition === "pain"
                    ? "bg-rose-500/20 text-rose-400"
                    : routine.condition === "tightness"
        ...
        <div className="text-right shrink-0">
          ...
          <button
            className={`p-2 rounded-full transition-transform duration-300 ${
              expanded ? "rotate-180 bg-slate-800" : ""
            }`}
          >
            👇
          </button>
        ...
        <div className="px-5 pb-5 animate-in slide-in-from-top-2 border-t border-slate-800/50 pt-4">
          ...
          <div className="space-y-3">
            {routine.exercises.map((ex, i) => (
              <div
                key={ex.id}
                className="flex gap-3 items-start bg-slate-950/50 p-3 rounded-lg border border-slate-800/50"
              >
