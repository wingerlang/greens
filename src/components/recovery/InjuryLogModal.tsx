...
import { BodyPart, InjuryType, PainLevel, InjuryLog, InjuryStatus } from '../../models/types.ts';
...
export function InjuryLogModal({ bodyPart, onClose, existingLog }: InjuryLogModalProps) {
    const { addInjuryLog, updateInjuryLog, deleteInjuryLog } = useData();

    // Form State
    const [severity, setSeverity] = useState<PainLevel>(existingLog?.severity || 5);
    const [type, setType] = useState<InjuryType>(existingLog?.type || 'soreness');
    const [status, setStatus] = useState<InjuryStatus>(existingLog?.status || 'active');
    const [side, setSide] = useState<'left' | 'right' | 'both' | 'center'>(existingLog?.side || 'right');
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <div className="flex justify-between items-end mb-2">
                                <label className="text-slate-400 text-sm font-bold uppercase">Smärtnivå</label>
                                <span className={`text-2xl font-black ${severity > 7 ? 'text-rose-500' : severity > 4 ? 'text-amber-500' : 'text-emerald-500'}`}>
                                    {severity}/10
                                </span>
                            </div>
                            <input
                                ...
                                max="10"
                                value={severity}
                                onChange={(e) => setSeverity(parseInt(e.target.value) as PainLevel)}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="text-slate-400 text-xs font-bold uppercase block mb-2">Typ</label>
                                <select
                                    value={type}
                            ...
                            <div>
                                <label className="text-slate-400 text-xs font-bold uppercase block mb-2">Status</label>
                                <select
                                    value={status}
                        ...
                        {/* Side Selector */}
                        <div>
                            <label className="text-slate-400 text-xs font-bold uppercase block mb-2">Sida</label>
                            <div className="flex bg-slate-800 rounded-lg p-1">
                                {['left', 'right', 'both', 'center'].map((s) => (
                                    <button
                                        key={s}
                                        onClick={() => setSide(s as any)}
                                        className={`flex-1 py-1.5 rounded-md text-xs font-bold capitalize transition-all ${side === s ? 'bg-indigo-500 text-white' : 'text-slate-400 hover:text-white'}`}
                                    >
                                        {s === 'left' ? 'Vänster' : s === 'right' ? 'Höger' : s === 'both' ? 'Båda' : 'Center'}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        {/* Notes */}
                        <div>
                            <label className="text-slate-400 text-xs font-bold uppercase block mb-2">Anteckningar</label>
                            <textarea
                                value={notes}
