...
export function StravaConnectionCard() {
  ...
  useEffect(() => {
    ...
    if (params.get("strava_error")) {
      setStatus({
        connected: false,
        error: params.get("strava_error") || "Connection failed",
      });
      window.history.replaceState({}, "", window.location.pathname);
    }
  }, [token]);
  ...
  const handleRecalculateCalories = async () => {
    if (
      !confirm(
        "Detta omberäknar kalorier för ALLA aktiviteter baserat på den nya formeln. Fortsätta?",
      )
    ) return;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400">
                ...
                {status.athlete.premium && (
                  <span className="ml-2 text-orange-400">★ Premium</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-950/30 rounded-xl p-4 border border-white/5">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex bg-slate-900 p-1 rounded-lg border border-white/5">
                <button
                  onClick={() =>
                    updateSettings({ stravaTimePreference: "moving" })}
                ...
                <button
                  onClick={() =>
                    updateSettings({ stravaTimePreference: "elapsed" })}
                  className={`px-3 py-1.5 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${
                    settings.stravaTimePreference === "elapsed"
                      ? "bg-orange-500 text-white shadow-lg shadow-orange-500/20"
                      : "text-slate-500 hover:text-slate-300"
                  }`}
                >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      ...
                      <p className="text-[9px] text-slate-500 mt-1 leading-relaxed">
                        Applicerar den senaste kalori-formeln på alla befintliga
                        aktiviteter. Bevarar merges och all annan data.
                      </p>
                    </div>
                    <button
      ...
      {!status?.connected && (
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-400">
            Koppla ditt Strava-konto för att automatiskt importera dina
            träningspass, löprundor, cykelturer och mer!
          </p>
