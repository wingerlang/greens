...
export function StravaActivityImportModal(
  { isOpen, onClose, initialRange }: StravaActivityImportModalProps,
) {
  const { token } = useAuth();
  ...
  const [step, setStep] = useState<
    "setup" | "scanning" | "review" | "importing" | "success"
  >("setup");
  ...
  const [selectedChanged, setSelectedChanged] = useState<Set<number>>(
    new Set(),
  );
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setStep("review");
      if (
        data.newActivities.length === 0 && data.changedActivities.length > 0
      ) {
  ...
  const handleImport = async () => {
    ...
    try {
      const newToImport = report.newActivities.filter((a) =>
        selectedNew.has(a.id)
      );
      const changedToImport = report.changedActivities.filter((a) =>
        selectedChanged.has(a.strava.id)
      ).map((x) => x.strava);
      ...
      setImportStats({
        created: newToImport.length,
        updated: changedToImport.length,
      });
      await refreshData();
      setStep("success");
  ...
  return (
    <div
      className="modal-overlay backdrop-blur-md bg-slate-950/80 fixed inset-0 z-50 flex items-center justify-center p-4"
      onClick={onClose}
    >
      <div
            ...
            <div className="space-y-6">
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-400 max-w-md mx-auto">
                  Vi hämtar din historik från Strava och jämför med din databas.
                  Inget sparas förrän du godkänner.
                </p>
              </div>
          ...
          {step === "scanning" && (
            <div className="flex flex-col items-center justify-center h-64 space-y-6">
              <div className="relative w-20 h-20">
                ...
                <div className="absolute inset-0 border-4 border-[#FC4C02] border-t-transparent rounded-full animate-spin">
                </div>
              </div>
              <div className="text-center">
                ...
                <p className="text-slate-400 text-sm mt-2">
                  Hämtar aktiviteter och jämför data.
                </p>
            ...
            <div className="flex flex-col h-full">
              <div className="flex items-center justify-between mb-6 bg-slate-950/50 p-4 rounded-xl border border-white/5">
                <div className="flex gap-4 text-sm">
                  <div>
                    <span className="text-slate-400">Totalt på Strava:</span>
                    {" "}
                  ...
                  <div>
                    <span className="text-slate-400">Matchade (OK):</span>{" "}
              ...
              <div className="flex-1 overflow-auto min-h-[300px]">
                {activeTab === "new"
                        ...
                        <div
                          ...
                          className={`flex items-center gap-4 p-3 rounded-lg border cursor-pointer ${
                            selectedNew.has(a.id)
                              ...
                              : "bg-slate-950/30 border-white/5 hover:bg-slate-800"
                          }`}
                        >
                          <div
                            ...
                            <div className="text-xs text-slate-400 flex flex-wrap gap-x-3 gap-y-1 mt-0.5">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                <span className="text-slate-500">
                                  {(a.elapsed_time / 60).toFixed(1)} min
                                </span>
                              </span>
                            </div>
                          </div>
                          ...
                          <div
                            onClick={(e) => {
                              ...
                              const next = {
                                ...report,
                                newActivities: report.newActivities.map((na) =>
                                  na.id === a.id
                                    ? {
                                      ...na,
                                      excludeFromStats: !na.excludeFromStats,
                                    }
                                    : na
                                ),
                              };
                  ...
                  : (
                    <div className="space-y-2">
                      {report.changedActivities.length === 0 && (
                        <div className="text-slate-500 text-center py-10">
                          Inga ändringar hittades.
                        </div>
                      )}
                      {report.changedActivities.map((
                        { strava: s, changes },
                      ) => (
                        <div
                          key={s.id}
                          onClick={() => toggleChanged(s.id)}
                          className={`flex items-start gap-4 p-3 rounded-lg border cursor-pointer ${
                            selectedChanged.has(s.id)
                              ? "bg-amber-500/10 border-amber-500/30"
                              : "bg-slate-950/30 border-white/5 hover:bg-slate-800"
                          }`}
                        >
                          <div
                            className={`w-5 h-5 mt-1 rounded flex items-center justify-center border ${
                              selectedChanged.has(s.id)
                            ...
                            <div className="text-xs text-slate-400 flex flex-wrap gap-x-3 gap-y-1 mb-2">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                <span className="text-amber-400 font-medium">
                                  {(s.moving_time / 60).toFixed(1)}
                                </span>
                                <span className="text-slate-600">/</span>
                                <span className="text-slate-500">
                                  {(s.elapsed_time / 60).toFixed(1)} min
                                </span>
                              </span>
                            </div>
                          ...
                          <div
                            onClick={(e) => {
                              ...
                              const next = {
                                ...report,
                                changedActivities: report.changedActivities.map(
                                  (ca) =>
                                    ca.strava.id === s.id
                                      ? {
                                        ...ca,
        ...
        {step === "review" && (
          <div className="p-4 bg-slate-950 border-t border-white/5 flex gap-4">
            <button
              ...
              className="px-6 py-3 rounded-xl bg-slate-800 text-slate-300 font-bold hover:bg-slate-700"
            >
              Backa
            </button>
