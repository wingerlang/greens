    ...
    <section className="mt-8">
      ...
      <div className="space-y-3">
        {activities.map((activity) => (
          <div
            key={activity.id}
            className="glass-card p-4 rounded-xl flex items-center justify-between group hover:bg-slate-800/50 transition-colors"
          >
            <div className="flex items-center gap-4">
              {/* Icon based on source */}
              <div
                className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg shadow-lg ${
                  activity.performance?.source?.source === "strava"
                    ? "bg-[#FC4C02] text-white"
                    : "bg-emerald-500 text-slate-900"
                }`}
              >
                {activity.plan?.activityType === "cycling" ||
                    activity.performance?.notes?.toLowerCase().includes("ride")
                  ? "🚴"
                  : "🏃"}
              </div>
              ...
              <div>
                <div className="font-bold text-white text-sm flex items-center gap-2">
                  {activity.performance?.notes || activity.plan?.title ||
                    "Okänd Aktivitet"}
                  {activity.performance?.source?.source === "strava" && (
                ...
                <div className="text-xs text-slate-400 flex gap-3 mt-0.5">
                  ...
                  <span>
                    📏 {activity.performance?.distanceKm?.toFixed(1)} km
                  </span>
                  <span>
                    ⏱️ {activity.performance?.durationMinutes?.toFixed(1)} min
                  </span>
                </div>
              </div>
            </div>
