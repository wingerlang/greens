...
export function FollowMatrixModal(
  { isOpen, onClose, targetUserId, targetUserName }: FollowMatrixModalProps,
) {
  ...
  const toggleCategory = (cat: FeedEventCategory) => {
    setPreference((prev: Partial<FollowPreference>) => {
      ...
      if (current.includes(cat)) {
        return {
          ...prev,
          subscribedCategories: current.filter((c: FeedEventCategory) =>
            c !== cat
          ),
        };
      } else {
        ...
        <div className="p-6 pb-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400 font-medium">
              Välj vad du vill se från{" "}
              <span className="text-emerald-400">@{targetUserName}</span>
            </p>
          </div>
          <button
            onClick={onClose}
