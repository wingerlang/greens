                            ...
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                const res = await fetch(
                                  `/api/feed/events/${event.id}/visibility`,
                                  {
                                    method: "PATCH",
                                    headers: {
                                ...
                                alert(
                                  "Nätverksfel vid uppdatering av integritet.",
                                );
                              } finally {
                                setUpdating(false);
                              }
        ...
        <div className="flex-1 min-w-0">
          ...
          {event.metrics && event.metrics.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-2">
              {event.metrics.slice(0, expanded ? undefined : 3).map((
                metric,
                i,
              ) => (
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-3 gap-2">
        ...
        <StatBox
          label="Volym"
          value={`${Math.round(payload.totalVolume / 1000)}t`}
        />
      </div>
      {payload.exercises && payload.exercises.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          ...
          {payload.exercises.length > 6 && (
            <span className="text-[10px] text-slate-600">
              +{payload.exercises.length - 6} mer
            </span>
          )}
        </div>
      )}
...
function WorkoutCardioDetails({ payload }: { payload: any }) {
  return (
    <div className="grid grid-cols-3 gap-2">
      {payload.distance && (
        <StatBox label="Distans" value={`${payload.distance}km`} />
      )}
      ...
      {payload.avgHeartRate && (
        <StatBox label="Puls" value={`${payload.avgHeartRate}bpm`} />
      )}
      {payload.elevationGain && (
        <StatBox label="Höjdmeter" value={`${payload.elevationGain}m`} />
      )}
    </div>
  );
}
...
function NutritionDetails({ payload }: { payload: any }) {
  return (
    <div className="grid grid-cols-4 gap-2">
      <StatBox
        label="Kalorier"
        value={Math.round(payload.calories)}
        accent="amber"
      />
      <StatBox
        label="Protein"
        value={`${Math.round(payload.protein)}g`}
        accent="emerald"
      />
      <StatBox
        label="Kolhydrater"
        value={`${Math.round(payload.carbs)}g`}
        accent="blue"
      />
      <StatBox
        label="Fett"
        value={`${Math.round(payload.fat)}g`}
        accent="purple"
      />
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      ...
      {payload.bedtime && payload.wakeTime && (
        <StatBox
          label="Period"
          value={`${payload.bedtime.slice(0, 5)} - ${
            payload.wakeTime.slice(0, 5)
          }`}
        />
      )}
    </div>
