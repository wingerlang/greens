...
const MEASUREMENT_TYPES: Record<
  BodyMeasurementType,
  {
    label: string;
    unit: string;
...
> = {
  neck: {
    ...
    color: "#94a3b8",
    group: "upper",
    zone: { top: "16%", left: "42%", width: "16%", height: "5%" },
  },
  shoulders: {
    label: "Axlar",
    unit: "cm",
    color: "#38bdf8",
    group: "upper",
    zone: { top: "23%", left: "25%", width: "50%", height: "8%" },
  },
  chest: {
    label: "Bröst",
    unit: "cm",
    color: "#60a5fa",
    group: "upper",
    zone: { top: "31%", left: "30%", width: "40%", height: "10%" },
  },
  waist: {
    label: "Midja",
    unit: "cm",
    color: "#10b981",
    group: "upper",
    zone: { top: "44%", left: "35%", width: "30%", height: "8%" },
  },
  hips: {
    label: "Höft",
    unit: "cm",
    color: "#34d399",
    group: "lower",
    zone: { top: "50%", left: "32%", width: "36%", height: "9%" },
  },
  arm_left: {
    label: "Vänster Arm",
    unit: "cm",
    color: "#818cf8",
    group: "upper",
    zone: { top: "34%", left: "72%", width: "14%", height: "12%" },
  },
  arm_right: {
    label: "Höger Arm",
    unit: "cm",
    color: "#818cf8",
    group: "upper",
    zone: { top: "34%", left: "14%", width: "14%", height: "12%" },
  },
  forearm_left: {
    label: "V. Underarm",
    unit: "cm",
    color: "#a78bfa",
    group: "upper",
    zone: { top: "52%", left: "78%", width: "10%", height: "14%" },
  },
  forearm_right: {
    label: "H. Underarm",
    unit: "cm",
    color: "#a78bfa",
    group: "upper",
    zone: { top: "52%", left: "12%", width: "10%", height: "14%" },
  },
  thigh_left: {
    label: "Vänster Lår",
    unit: "cm",
    color: "#fbbf24",
    group: "lower",
    zone: { top: "62%", left: "52%", width: "16%", height: "15%" },
  },
  thigh_right: {
    label: "Höger Lår",
    unit: "cm",
    color: "#fbbf24",
    group: "lower",
    zone: { top: "62%", left: "32%", width: "16%", height: "15%" },
  },
  calf_left: {
    label: "Vänster Vad",
    unit: "cm",
    color: "#f87171",
    group: "lower",
    zone: { top: "82%", left: "54%", width: "12%", height: "15%" },
  },
  calf_right: {
    label: "Höger Vad",
    unit: "cm",
    color: "#f87171",
    group: "lower",
    zone: { top: "82%", left: "34%", width: "12%", height: "15%" },
  },
};

const DEFAULT_PINNED: BodyMeasurementType[] = [
  "waist",
  "hips",
  "chest",
  "arm_right",
...
export const BodyMeasurementsModule: React.FC = () => {
  ...
  // Quick Log State
  const [pinnedTypes, setPinnedTypes] = useState<BodyMeasurementType[]>(
    DEFAULT_PINNED,
  );
  const [inputValues, setInputValues] = useState<Record<string, string>>({});
  const [focusedType, setFocusedType] = useState<BodyMeasurementType | null>(
    null,
  );
            ...
            {Object.entries(MEASUREMENT_TYPES).map(([type, info]) => (
              <div
                ...
                className={`absolute cursor-pointer transition-all duration-300 border backdrop-blur-[1px]
                                    ${
                  focusedType === type
                    ? "bg-emerald-500/30 border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.5)] z-20"
                    : "bg-transparent border-transparent hover:bg-white/10 hover:border-white/20 z-10"
                }
                                `}
                style={{
                  top: info.zone.top,
              ...
              >
                ...
                {(focusedType === type) && (
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-slate-900 text-white text-xs font-bold px-2 py-1 rounded shadow-lg border border-slate-700 pointer-events-none">
                    {info.label}
                    {getLatestValue(type as BodyMeasurementType) && (
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full -mt-2 h-4 w-full bg-transparent z-40">
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            {["upper", "lower"].map((group) => {
              const groupTypes = pinnedTypes.filter((t) =>
                MEASUREMENT_TYPES[t].group === group
              );
              ...
              return (
                <div key={group} className="space-y-3 contents">
                  {groupTypes.map((type) => (
                    <div
                      key={type}
                      className={`flex items-center gap-3 p-2 pl-3 rounded-xl border transition-all duration-300 ${
                        focusedType === type
                    ...
                    >
                      <div
                        className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-slate-900 shrink-0"
                        style={{
                          backgroundColor: MEASUREMENT_TYPES[type].color,
                        }}
                      >
                    ...
                    if (history.some((h) => h.type === type)) {
                      return (
                        <Line
                          ...
                          type="monotone"
                          dataKey={type}
                          name={MEASUREMENT_TYPES[type as BodyMeasurementType]
                            .label}
                          stroke={MEASUREMENT_TYPES[type as BodyMeasurementType]
                            .color}
                          strokeWidth={2}
                          dot={{
                            r: 3,
                            fill: MEASUREMENT_TYPES[type as BodyMeasurementType]
                              .color,
                          }}
                          activeDot={{ r: 5 }}
                          connectNulls
                  ...
                  <tr key={entry.id} className="hover:bg-white/[0.02]">
                    ...
                    <td className="px-6 py-4 text-white font-medium">
                      <span
                        ...
                        style={{
                          backgroundColor: MEASUREMENT_TYPES[entry.type].color,
                        }}
                      >
                      </span>
