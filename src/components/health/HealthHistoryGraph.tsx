...
export function HealthHistoryGraph({
  ...
  const graphMeta = useMemo(() => {
    ...
    }).filter((p) => p.val !== undefined && p.val > 0) as {
      val: number;
      date: string;
      i: number;
    }[];
    ...
    let min = validPoints.length > 0
      ? Math.min(...validPoints.map((p) => p.val))
      : 70;
    let max = validPoints.length > 0
      ? Math.max(...validPoints.map((p) => p.val))
      : 80;
  ...
  const getY = (val: number) =>
    100 - ((val - graphMeta.min) / graphMeta.range) * 100;
  const getYCm = (val: number) =>
    100 - ((val - graphMeta.cmMin) / graphMeta.cmRange) * 100;
      ...
      <div className={`${height} w-full relative flex gap-6`}>
        ...
        <div className="absolute -left-10 top-0 bottom-0 w-8 flex flex-col items-end text-[10px] text-slate-500 font-bold pointer-events-none">
          ...
          <span className="absolute top-1/2 transform -translate-y-1/2">
            {Math.round(graphMeta.min + graphMeta.range / 2)}
          </span>
        ...
        {isWeight && (
          <div className="absolute -right-10 top-0 bottom-0 w-8 flex flex-col items-start text-[10px] text-slate-500 font-bold pointer-events-none">
            ...
            <span className="absolute top-1/2 transform -translate-y-1/2">
              {Math.round(graphMeta.cmMin + graphMeta.cmRange / 2)}
            </span>
            <span className="absolute bottom-0 transform translate-y-1/2">
              {graphMeta.cmMin}
            </span>
            <span className="absolute -top-6 left-0 text-[8px] font-black uppercase text-slate-600">
              CM
            </span>
          </div>
        )}
        {isWeight && (
          <div className="absolute -left-10 -top-6 text-[8px] font-black uppercase text-slate-600">
            KG
          </div>
        )}
            ...
            <>
              ...
              {snapshots.filter((s) => s.chest).length > 1 && (
                <path
                  d={`M ${
                    snapshots.map((s, i) =>
                      s.chest ? `${getX(i)} ${getYCm(s.chest)}` : null
                    ).filter((p) => p).join(" L ")
                  }`}
                  fill="none"
                  stroke="#6366f1"
              ...
              {snapshots.filter((s) => s.waist).length > 1 && (
                <path
                  d={`M ${
                    snapshots.map((s, i) =>
                      s.waist ? `${getX(i)} ${getYCm(s.waist)}` : null
                    ).filter((p) => p).join(" L ")
                  }`}
                  fill="none"
                  stroke="#10b981"
            ...
            ? (
              <>
                ...
                {graphMeta.points.length > 1 && (
                  <path
                    d={`M ${
                      graphMeta.points.map((p) => `${getX(p.i)} ${getY(p.val)}`)
                        .join(" L ")
                    }`}
                    fill="none"
                    stroke={themeColor}
        ...
        {hoverIndex !== null && snapshots[hoverIndex] && (
          <div
            ...
            style={{
              ...
              top: "10%",
              transform: `translate(${
                hoverIndex > snapshots.length / 2 ? "-110%" : "10%"
              }, 0)`,
            }}
          >
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest border-b border-white/5 pb-1 mb-1">
              {snapshots[hoverIndex].date}
            </span>
              ...
              <>
                {snapshots[hoverIndex].weight && (
                  <div className="flex justify-between items-baseline gap-4">
                    ...
                    <span className="text-sm font-black text-white">
                      {snapshots[hoverIndex].weight}{" "}
                      <span className="text-[9px] opacity-40">kg</span>
                    </span>
                  </div>
                )}
                {snapshots[hoverIndex].waist && (
                  <div className="flex justify-between items-baseline gap-4">
                    ...
                    <span className="text-sm font-black text-emerald-400">
                      {snapshots[hoverIndex].waist}{" "}
                      <span className="text-[9px] opacity-40">cm</span>
                    </span>
                  </div>
                )}
                ...
                <div className="text-[10px] text-slate-500 mt-1 pt-1 border-t border-white/5 flex justify-between">
                  ...
                  <span className="text-white font-bold">
                    {graphMeta.trendPoints.find((p) => p.i === hoverIndex)?.val
                      .toFixed(1) || "--"} kg
                  </span>
                </div>
              </>
            ...
            {isSleep && (
              <div className="flex justify-between items-baseline gap-4">
                ...
                <span className="text-sm font-black text-white">
                  {snapshots[hoverIndex].vitals?.sleep || 0}{" "}
                  <span className="text-[9px] opacity-40">h</span>
                </span>
              </div>
            )}
          </div>
