...
export function ComparisonBar({
  ...
  const safeUserValue = (typeof userValue === "number" && !isNaN(userValue))
    ? userValue
    : 0;
  const safeAvgValue = (typeof avgValue === "number" && !isNaN(avgValue))
    ? avgValue
    : 0;
  const safeMaxValue = (typeof maxValue === "number" && !isNaN(maxValue))
    ? maxValue
    : 0;
    ...
    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
      <div className="flex items-center gap-2 mb-4">
        ...
        {explanation && (
          <div className="relative group">
            <span className="text-gray-500 cursor-help hover:text-gray-300 transition-colors">
              ℹ️
            </span>
      ...
      <div className="flex justify-between items-center mt-4 pt-4 border-t border-slate-800">
        <div className="text-center flex-1 relative group/pb">
          <div
            className={`text-2xl font-black ${pbDetails ? "cursor-help" : ""}`}
            style={{ color }}
          >
            {safeUserValue.toLocaleString(undefined, {
              maximumFractionDigits: 1,
            })}
          </div>
          <div className="text-xs text-gray-500 uppercase">Du</div>
          {pbDetails && <PBHoverCard pb={pbDetails} rmMode={rmMode} />}
        </div>
        <div className="text-center">
          {(() => {
            if (safeAvgValue === 0) {
              return <div className="text-gray-600 text-lg font-bold">vs</div>;
            }
        ...
        <div className="text-center flex-1">
          <div className="text-2xl font-black text-slate-400">
            {safeAvgValue.toLocaleString(undefined, {
              maximumFractionDigits: 1,
            })}
          </div>
