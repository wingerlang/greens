...
export function BodyMeasurementsSection(
  { targetWeight, height }: BodyMeasurementsSectionProps,
) {
  const {
    history,
    loading,
  ...
  if (loading) {
    return (
      <div className="text-slate-500 text-center py-4">Laddar mätdata...</div>
    );
  }
    ...
    <div className="space-y-4">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                ...
                <div className="text-white font-bold text-lg">
                  {formatSwedishDate(latestWeight.date)}
                </div>
                <div className="text-slate-500 text-xs">
                  {getRelativeTime(latestWeight.date)}
                </div>
              </div>
            </div>
                ...
                <div className="text-center">
                  ...
                  >
                    {latestWeight.weight < previousWeight.weight
                      ...
                      ? "↑"
                      : "→"}
                    {Math.abs(latestWeight.weight - previousWeight.weight)
                      .toFixed(1)} kg
                  </div>
                </div>
              )}
              ...
              {toGoal !== null
                ? (
                  <div className="text-center">
                    ...
                    <div
                      className={`text-lg font-bold ${
                        toGoal <= 0 ? "text-emerald-400" : "text-amber-400"
                      }`}
                    >
                      {toGoal <= 0
                        ? "🎯 Uppnått!"
                        : `${toGoal.toFixed(1)} kg kvar`}
                    </div>
                  </div>
                )
                : (
        ...
        <div className="bg-slate-800/70 rounded-xl p-4 border border-slate-700">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-5 py-3 bg-emerald-500 text-white rounded-lg font-bold hover:bg-emerald-600"
            >
              Spara
            </button>
            <button
              onClick={() => setShowAddForm(false)}
              className="px-5 py-3 bg-slate-700 text-slate-300 rounded-lg font-bold hover:bg-slate-600"
            >
              Avbryt
            </button>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-800/50 rounded-xl p-4">
          <div className="text-slate-500 text-xs uppercase font-bold">
            Längd
          </div>
          <div className="text-white text-2xl font-black">
            {height || "—"} <span className="text-sm text-slate-400">cm</span>
          </div>
        </div>
        <div className="bg-slate-800/50 rounded-xl p-4">
          <div className="text-slate-500 text-xs uppercase font-bold">
        ...
        <div>
          ...
          <div className="space-y-1">
            {Array.from(
              new Map(
                history.map(
                  (item) => [item.date + (item.createdAt || ""), item],
                ),
              ).values(),
            )
              .slice(0, 5)
              .map((h, i) => (
                <div
                  key={`${h.date}-${h.createdAt || i}`}
                  className={`flex items-center justify-between p-2 rounded-lg ${
                    i === 0 ? "bg-emerald-500/10" : "bg-slate-800/30"
                  }`}
                >
                  <span className="text-slate-400 text-sm">
                    {formatSwedishDate(h.date)}
                  </span>
                  <span
                    className={`font-bold ${
