...
export function MeasurementsModule(
  { targetWeight, height }: MeasurementsModuleProps,
) {
  const { weightEntries, bodyMeasurements, addBodyMeasurement } = useData();
  const [activeTab, setActiveTab] = useState<"weight" | "measurements">(
    "weight",
  );
  ...
  const currentMeasurementLabel = MEASUREMENT_TYPES.find((t) =>
    t.id === newType
  )?.label;
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6 h-[300px]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-white flex items-center gap-2">
                📈 {activeTab === "weight"
                  ? "Viktutveckling"
                  : `${currentMeasurementLabel} - Utveckling`}
              </h3>
              {activeTab === "measurements" && (
                <select
                  value={newType}
                  onChange={(e) =>
                    setNewType(e.target.value as BodyMeasurementType)}
            ...
            <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-white">
                  Historik ({currentMeasurementLabel})
                </h3>
                <button
        ...
        <div className="space-y-4">
          <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6">
            <h3 className="font-bold text-white mb-4 text-sm uppercase text-slate-500">
              Senaste Mätningar
            </h3>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${
                      newType === type.id && activeTab === "measurements"
                  ...
                  >
                    ...
                    <span
                      className={`font-bold ${
                        latest ? "text-white" : "text-slate-600"
                      }`}
                    >
