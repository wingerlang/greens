...
export function ProfileFeedSection({ userId, compact = true }: ProfileFeedSectionProps) {
    ...
    const fetchFeed = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const userEvents = (data.events || []).filter((e: FeedEvent) => e.userId === targetUserId);
            ...
            <div className="space-y-3">
                {loading && !refreshing ? (
                    ...
                    <div className="text-center py-12 bg-slate-800/30 rounded-xl border border-dashed border-white/10">
                        ...
                        <p className="text-slate-400 text-sm">Inga aktiviteter att visa ännu.</p>
                        <p className="text-slate-500 text-xs mt-1">Logga träning, kost eller hälsodata för att se dem här.</p>
                    </div>
                )}
            </div>
        </section>
