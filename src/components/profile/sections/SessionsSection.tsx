...
export function SessionsSection() {
  ...
  if (loading) {
    return <div className="text-slate-500 text-center py-4">Laddar...</div>;
  }
  ...
  return (
    <div className="space-y-4">
      {sessions.length === 0
        ? (
          <p className="text-slate-500 text-sm">
            Inga aktiva sessioner hittades.
          </p>
        )
        : (
      ...
      <div className="text-slate-500 text-xs text-center">
        {sessions.length} aktiv{sessions.length !== 1 ? "a" : ""}{" "}
        session{sessions.length !== 1 ? "er" : ""}
      </div>
    </div>
  );
}
