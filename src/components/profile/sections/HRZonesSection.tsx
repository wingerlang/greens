        ...
        <div className="bg-emerald-500/10 rounded-xl p-4 border border-emerald-500/20">
          <div className="flex items-center justify-between mb-2">
            <div>
              <span className="text-emerald-400 font-bold">
                🎯 Automatiskt detekterat från{" "}
                {detectedZones.activitiesAnalyzed} aktiviteter
              </span>
              <span
                className={`ml-2 text-xs px-2 py-0.5 rounded ${
                  detectedZones.confidence === "high"
                    ? "bg-emerald-500/20 text-emerald-400"
                    : detectedZones.confidence === "medium"
          ...
          {detectedZones.maxHRActivity && (
            <div className="text-slate-400 text-xs">
              Max puls {detectedZones.maxHR}{" "}
              bpm under "{detectedZones.maxHRActivity.name}" ({detectedZones
                .maxHRActivity.date})
            </div>
          )}
        </div>
                ...
                <div className="w-full h-2 bg-slate-800 rounded-full mt-1 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        ((zone.max - zone.min) /
                          (zones.maxHR - (zones.estimatedRestingHR || 50))) *
                        100
                      }%`,
                    }}
                  />
