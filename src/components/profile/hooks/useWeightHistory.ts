...
export function useWeightHistory(): UseWeightHistoryResult {
  ...
  const logWeight = useCallback(
    async (weight: number, date?: string): Promise<boolean> => {
      try {
  ...
  const latestWeight = useMemo(() => history.length > 0 ? history[0] : null, [
    history,
  ]);
  const previousWeight = useMemo(() => history.length > 1 ? history[1] : null, [
    history,
  ]);
