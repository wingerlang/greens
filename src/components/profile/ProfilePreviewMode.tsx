...
export function ProfilePreviewMode({ profile, currentUserId, users, onExit, children }: ProfilePreviewModeProps) {
    ...
    if (viewerType === 'friend' || (viewerType === 'specific' && simulateFriend)) accessLevel = 'friend';

    // If specific user, check overrides
    let specificOverrides = selectedUserId ? priv.categoryOverrides?.[selectedUserId] : null;
        ...
        return (
            <div className="min-h-screen bg-slate-900 pt-32 px-6">
                <div className="bg-slate-800 p-8 rounded-2xl border border-rose-500/20 text-center max-w-md mx-auto">
                    ...
                    <p className="text-slate-400">Denna användare har blivit blockerad och kan inte se din profil.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="relative">
            ...
            <div className="sticky top-16 z-40 bg-indigo-600 text-white px-6 py-3 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <div className="font-bold text-sm uppercase opacity-80">Förhandsgranska som</div>
                        <div className="font-black text-lg flex items-center gap-2">
                             {viewerType === 'specific' ? (
                                 users.find(u => u.id === selectedUserId)?.name || 'Specifik person'
                             ) : (
                                 viewerType === 'public' ? 'Publik (Utloggad)' :
                                 viewerType === 'friend' ? 'Vän' :
                                 viewerType === 'blocked' ? 'Blockerad' : 'Följare'
                             )}
                        </div>
                    </div>
                </div>
                    ...
                    {viewerType === 'specific' && (
                        <div className="flex items-center gap-2">
                            ...
                            <label className="flex items-center gap-2 cursor-pointer bg-indigo-700/50 hover:bg-indigo-700 px-3 py-2 rounded-lg border border-indigo-500/50 transition-colors">
                                ...
                                />
                                <span className="text-xs font-bold text-indigo-100">Är vän?</span>
                            </label>
                        </div>
                    )}
            ...
            {/* Content Frame */}
            <div className={viewerType === 'blocked' ? 'opacity-50 pointer-events-none blur-sm select-none h-screen overflow-hidden relative' : ''}>
                 {/*
