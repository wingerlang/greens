...
import { generateId, getISODate } from "../models/types.ts";
...
export function NoccoOClock() {
      ...
      m.items.some((i) => {
        if (i.type === "foodItem") {
          ...
          return item?.name.toLowerCase().includes("nocco") ||
            item?.name.toLowerCase().includes("kaffe");
        }
        return false;
      })
    );
  ...
  if (
    (!isCountdownPhase && !isActionPhase) || hasRegisteredToday || isDismissed
  ) {
    return null;
  }
  ...
  const handleRegisterCoffee = () => {
    ...
    let coffeeItem = foodItems.find((f) =>
      f.name.toLowerCase() === "kaffe" || f.name.toLowerCase() === "coffee"
    );
