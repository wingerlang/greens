...
export function RecipeDetailModal({
  ...
  const [excludedIngredients, setExcludedIngredients] = useState<Set<number>>(
    new Set(),
  ); // Excluded ingredient indices
  const [ingredientMultipliers, setIngredientMultipliers] = useState<
    Map<number, number>
  >(new Map()); // Per-ingredient quantity adjustments
  ...
  const toggleSwap = useCallback(
    (originalItemId: string, newItemId: string) => {
      setActiveSwaps((prev) => {
        const next = { ...prev };
  ...
  useEffect(() => {
    try {
      const recipeSlug = recipe.name.toLowerCase().replace(/\s+/g, "-").replace(
        /[^a-z0-9-åäö]/g,
        "",
      );
      ...
      if (
        currentPath.startsWith("/vecka") || currentPath === "/" ||
        currentPath === ""
      ) {
  ...
  const scaledIngredients = useMemo(() => {
    ...
    return recipe.ingredientsText
      ...
      .map((ing, index) => {
        const isProteinFood = proteinFoods.some((p) =>
        ...
        const finalMultiplier = isExcluded
          ? 0
          : (isProteinFood ? multiplier * boostMultiplier : multiplier) *
            customMultiplier;
        return {
          ...ing,
          scaledAmount: isExcluded
            ? "0"
            : scaleAmount(ing.amount, finalMultiplier),
  ...
  const nutrition = useMemo(() => {
    ...
    scaledIngredients.forEach((ing: ScaledIngredient) => {
      ...
      if (foodItem) {
        ...
        const normalizeAmount = (str: string) =>
  ...
  return (
    <div className="recipe-detail-overlay" onClick={onClose}>
      <div className="recipe-detail-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="recipe-stats">
          ...
          >
            <span className="stat-label">
              PER PORTION {showBreakdown ? "▼" : "▶"}
            </span>
            <span
              className="stat-value"
              style={{ textDecoration: "underline dotted" }}
            >
              {nutrition.perPortion.calories} kcal
            </span>
          </div>
          <div className="stat">
            <span className="stat-label">PROTEIN/PORT</span>
            <span className="stat-value">
              💪 {nutrition.perPortion.protein}g
            </span>
          </div>
          <div className="stat">
            <span className="stat-label">PRIS/PORT</span>
            <span className="stat-value">
              💰 {nutrition.perPortion.price} kr
            </span>
          </div>
          <div className="stat">
        ...
        {showBreakdown && (
          <div
            className="ingredient-breakdown"
            style={{
          ...
          >
            ...
            {scaledIngredients.map((ing: ScaledIngredient, idx: number) => {
              ...
              const amountMatch = String(ing.scaledAmount).match(
                /(\d+(?:[.,]\d+)?)/,
              );
              const grams = amountMatch
                ? parseFloat(amountMatch[1].replace(",", "."))
                : 100;
              const gramsPerPortion = Math.round(grams / portions);
              const kcalPerIng = matched
                ? Math.round((matched.calories / 100) * grams / portions)
                : 0;
              const proteinPerIng = matched
                ? Math.round((matched.protein / 100) * grams / portions * 10) /
                  10
                : 0;
              const fatPerIng = matched
                ? Math.round((matched.fat / 100) * grams / portions * 10) / 10
                : 0;
              return (
                <div
                  key={idx}
        ...
        <button
          ...
          onClick={() =>
            setPortions((prev: number) => prev >= 8 ? basePortions : 8)}
        >
          🍲{" "}
          {portions >= 8
            ? "Tillbaka till normalt"
            : "Gör till Storkok (8 port)"}
        </button>
          ...
          <div className="ingredients-section">
            <div className="section-header">
              ...
              <button
                ...
                title={customizeMode
                  ? "Stäng anpassning"
                  : "Anpassa ingredienser för denna måltid"}
            ...
            {customizeMode && (
              ...
              >
                💡 Klicka på ✕ för att exkludera eller justera mängd.
                Ändringarna påverkar endast denna måltid.
              </div>
            )}
            ...
            {proteinSuggestion && proteinBoost < 2 && (
              <button
                className={`protein-suggestion ${
                  proteinAnimating ? "animating" : ""
                }`}
              ...
              >
                ...
                <div className="suggestion-text">
                  ...
                  <span>
                    Klicka för att öka {proteinSuggestion.name} med 50%
                  </span>
                </div>
            ...
            {false &&
              (smartSwaps.length > 0 || Object.keys(activeSwaps).length > 0) &&
              (
                <div
                  className="smart-swaps-section"
                  style={{
                ...
                >
                  ...
                  >
                    {smartSwaps.map((swap, idx) => {
                      const isApplied = activeSwaps[swap.originalItem.id] ===
                        swap.suggestion.id;
                      return (
                        <div
                          key={idx}
            ...
            <button
              ...
              onClick={() => {
                ...
                scaledIngredients.forEach(
                  (ing: ScaledIngredient, idx: number) => {
