            ...
            {timer && timer.status === 'running' && (
                ...
                >
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <span className="timer-label" style={{ fontSize: '0.8em', opacity: 0.9 }}>
                            ⏱ {timer.label}
                        </span>
                        <span style={{ fontSize: '0.7em', textTransform: 'uppercase', opacity: 0.7 }}>
                            Steg {timer.stepIndex + 1}
                        </span>
                    </div>
                    <span className="timer-value">{formatTime(timer.remainingSeconds)}</span>
                </div>
            )}
        </div>
    );
