...
import { type Timer, formatTime } from '../../hooks/useTimer.ts';
        ...
        <div className="zen-mode">
                ...
                {step.ingredients.length > 0 && (() => {
                    ...
                    const reusedIngredients = step.ingredients.map(ing => {
                        for (let i = 0; i < currentStep; i++) {
                            ...
                            const wasUsed = prevStep.ingredients.some(pi => pi.originalText === ing.originalText);
                            if (wasUsed) {
                    ...
                    return (
                        <div className="zen-ingredient-context">
                            ...
                            <div className="context-ingredients-list" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
                                {step.ingredients.map((stepIng, idx) => {
                                    // Find matched scaled ingredient
                                    const scaled = ingredients.find(si => si.originalText === stepIng.originalText);
                                    ...
                                    const quantity = parseFloat(scaled.scaledAmount.replace(',', '.'));
                                    ...
                                    const formattedText = !isNaN(quantity)
                                        ? formatIngredientQuantity(quantity, scaled.unit, foodItem || undefined)
                                    ...
                                    return (
                                        <span key={idx} className="context-ingredient-chip" style={{
                                            background: reuseInfo.used ? 'rgba(16, 185, 129, 0.2)' : 'rgba(255,255,255,0.1)',
                                            border: reuseInfo.used ? '1px solid rgba(16, 185, 129, 0.3)' : 'none',
                                            padding: '4px 8px',
                                            borderRadius: '4px',
                                            whiteSpace: 'nowrap'
                                        }}>
                                            {/* If reused, maybe smaller amount or just the name? Let's keep amount for clarity but maybe dim it */}
                                            <span style={{ opacity: reuseInfo.used ? 0.7 : 1 }}>{formattedText}</span> {displayText}
                                            {scaled.customMultiplier && scaled.customMultiplier !== 1 && (
                                                <span style={{ color: '#10b981', marginLeft: '4px', fontSize: '0.8em' }}>
                    ...
                    <div className="zen-timer-section">
                        ...
                        <div className="zen-timer-controls">
                            {!timer && step.isManualTimer && (
                                <div className="timer-adjust-controls" style={{ display: 'flex', gap: '8px', marginBottom: '8px', justifyContent: 'center' }}>
                                    <button className="zen-adjust-btn" onClick={() => setManualTime((m: number) => Math.max(1, m - 1))}>-1m</button>
                                    <button className="zen-adjust-btn" onClick={() => setManualTime((m: number) => Math.max(1, m - 5))}>-5m</button>
                                    <button className="zen-adjust-btn" onClick={() => setManualTime((m: number) => m + 5)}>+5m</button>
                                    <button className="zen-adjust-btn" onClick={() => setManualTime((m: number) => m + 1)}>+1m</button>
                                </div>
                            )}
                            {timer?.status === 'running' ? (
                                ...
                                <button className="timer-btn timer-btn-primary" onClick={() => onStartTimer(manualTime)}>
                                    ▶ STARTA TIMER
                                </button>
                            )}
                            <button className="timer-btn timer-btn-icon" onClick={onResetTimer}>
                                ↺
                            </button>
                        </div>
                    </div>
            ...
            <footer className="zen-nav">
                ...
                {(() => {
                    ...
                    const overlayTimers = activeTimers.filter(t => t.status === 'running');

                    if (overlayTimers.length === 0) return null;

                    return (
                        <div className="zen-timers-overlay" style={{
                            ...
                            {overlayTimers.map(t => (
                                <div key={t.id} className="zen-timer-toast" style={{
                                    ...
                                    <div
                                        style={{ display: 'flex', flexDirection: 'column', cursor: 'pointer', minWidth: '80px' }}
                                        onClick={() => onGoToStep && onGoToStep(t.stepIndex)}
                                        title={`Gå till ${t.label}`}
                                    >
                                        <span style={{ fontSize: '0.75em', opacity: 0.8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                            {t.label}
                                        </span>
                                        <span style={{ fontWeight: 'bold', fontFamily: 'monospace', fontSize: '1.1em' }}>
                ...
                <button
                    className={`zen-nav-btn zen-nav-next ${currentStep === totalSteps - 1 ? 'zen-nav-finish' : ''}`}
                    onClick={currentStep === totalSteps - 1 ? onExit : onNextStep}
                >
                    {currentStep === totalSteps - 1 ? 'Smaklig måltid! 🌟' : 'Nästa steg →'}
                </button>
            </footer>
        </div>
    );
