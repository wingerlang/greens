...
import {
  ...
  RequestSummary,
  useDebugInterceptor,
} from "../../hooks/useDebugInterceptor";
import {
  Activity,
  ChevronDown,
  ChevronRight,
...
import {
  ComponentDebugInfo,
  getComponentInfo,
} from "../../utils/debug/fiber-inspector.ts";
...
export default function DebugBar() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "db" | "memory" | "ui" | "structure"
  >("overview");
  ...
  return (
    <div
      id="debug-bar-root"
      className="fixed bottom-0 left-0 right-0 z-[9999] font-mono text-xs hidden md:block"
    >
      {/* Inspector Overlay */}
      ...
      >
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="bg-orange-600 text-white px-1.5 py-0.5 rounded font-bold">
              DEV MODE
            </span>
            {hostIps.length > 0 && (
          ...
          <div className="flex items-center gap-3 text-[10px] text-slate-500">
            <div className="flex items-center gap-1">
              <Globe size={12} />
              <span
                className="truncate max-w-[150px] text-slate-300"
                title="Current Route"
              >
                {currentRoute}
              </span>
          ...
          {currentRequest && (
            <div className="flex items-center gap-3 border-l border-slate-700 pl-4 hidden md:flex">
              <span
                className={`font-bold ${getMethodColor(currentRequest.method)}`}
              >
          ...
          <div className="w-1/4 min-w-[200px] border-r border-slate-700 overflow-y-auto hidden sm:block">
            {requests.map((req) => (
              <div
                ...
                className={`p-3 border-b border-slate-800 cursor-pointer hover:bg-slate-800 flex flex-col gap-1 ${
                  selectedId === req.id
              ...
              >
                <div className="flex justify-between items-center">
                  <span className={`font-bold ${getMethodColor(req.method)}`}>
                    {req.method}
                  </span>
                  <span
                ...
                <div className="truncate opacity-70" title={req.url}>
                  {req.url}
                </div>
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-950">
            ...
            <div className="flex border-b border-slate-700 bg-slate-900 overflow-x-auto">
              ...
              <TabButton
                ...
                label={`DB ${
                  profile
                    ? `(${profile.logs.filter((l) => l.type === "kv").length})`
                    : ""
                }`}
              />
              <TabButton
            ...
            <div className="flex-1 overflow-y-auto p-4">
                  ...
                  {inspectedElement
                    ? (
                      <div className="grid grid-cols-1 gap-4">
                        <Card title="Selected Component">
                          ...
                          <Row
                            label="File"
                            value={inspectedElement.filePath || "Unknown"}
                          />
                          <Row
                            label="Line"
                            value={inspectedElement.lineNumber || "Unknown"}
                          />
                        </Card>
                        ...
                        <Card title="Current Route Info">
                          ...
                          <Row
                            label="Active Modals"
                            value={activeModals.join(", ") || "None"}
                          />
                        </Card>

                        {inspectedElement.props && (
                          <div className="bg-slate-900 border border-slate-800 rounded p-4">
                            <h3 className="font-bold mb-3 text-slate-400 uppercase tracking-wider text-[10px]">
                              Props
                            </h3>
                    ...
                    : (
                      <div className="text-center py-10 opacity-50 border-2 border-dashed border-slate-800 rounded">
                        ...
                        <p>
                          Click "Pick Component" and select an element on the
                          page to inspect it.
                        </p>
                      </div>
                    )}
              ...
              {activeTab === "structure" && (
                <div className="space-y-4">
                  ...
                  <p className="opacity-60 mb-4">
                    List of all detected page components in{" "}
                    <code>src/pages</code>.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {pageFiles.map((file) => (
                      <div
                        key={file}
                        className="bg-slate-900 p-2 rounded border border-slate-800 text-[10px] flex items-center gap-2 hover:border-blue-500 transition-colors group relative"
                      >
                        <FileCode
                          size={14}
              ...
              {(activeTab === "overview" || activeTab === "db" ||
              ...
              {activeTab === "overview" && profile && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <Row
                      label="Time"
                      value={new Date(profile.startTime).toLocaleString()}
                    />
                  </Card>
                  <Card title="Performance">
                    <Row
                      label="Total Duration"
                      value={`${profile.duration.toFixed(2)} ms`}
                      highlight={profile.duration > 200}
                    />
                    <Row
                      label="DB Queries"
                      value={profile.logs.filter((l) => l.type === "kv").length}
                    />
                    <Row
                      label="DB Time"
                      value={`${
                        profile.logs.reduce(
                          (acc, l) => acc + (l.type === "kv" ? l.duration : 0),
                          0,
                        ).toFixed(2)
                      } ms`}
                    />
              ...
              {activeTab === "db" && profile && (
                <div className="space-y-2">
                  {profile.logs.filter((l) => l.type === "kv").length === 0 && (
                  ...
                  {profile.logs.filter((l) =>
                    ...
                    <div
                      key={idx}
                      className="bg-slate-900 border border-slate-800 rounded p-2 text-xs font-mono"
                    >
                      ...
                      {log.key && (
                        <div className="opacity-70 break-all">{log.key}</div>
                      )}
                      {log.details && (
              ...
              {activeTab === "memory" && profile && (
                <div className="grid grid-cols-2 gap-4">
                  <Card title="Memory Delta (Backend)">
                    ...
                    <Row
                      label="Heap Total"
                      value={formatBytes(profile.memory.heapTotal)}
                    />
                    <Row
                      label="Heap Used"
                      value={formatBytes(profile.memory.heapUsed)}
                    />
                    <Row
                      label="External"
                      value={formatBytes(profile.memory.external)}
                    />
                  </Card>
                  <div className="text-slate-500 text-[10px] col-span-2">
                    * Values represent the change in memory usage during the
                    request lifecycle.
                  </div>
                </div>
              )}
...
function TabButton(
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-4 py-3 hover:bg-slate-800 transition-colors border-b-2 whitespace-nowrap ${
        active
