...
import { Camera, Bug, X, Check, Loader2 } from 'lucide-react';
...
export function BugReporter() {
    ...
    if (active && snapshot) {
        return (
            <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4">
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setActive(false)} />
                <div className="relative bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden max-h-[90vh]">
                    <div className="p-4 border-b border-slate-700 bg-slate-800 flex justify-between items-center">
                        ...
                        <button onClick={() => setActive(false)}><X className="text-slate-400" /></button>
                    </div>

                    <div className="p-6 overflow-auto">
                        <div className="mb-4">
                            <label className="block text-xs font-mono text-slate-400 mb-2">SNAPSHOT</label>
                            <img src={snapshot} className="w-full rounded border border-slate-700 object-contain max-h-60 bg-black" alt="Bug Snapshot" />
                        </div>
                    ...
                    <div className="p-4 border-t border-slate-700 bg-slate-800 flex justify-end gap-3">
                        <button onClick={() => setActive(false)} className="px-4 py-2 text-slate-300 hover:text-white transition-colors">Cancel</button>
                        <button
                            onClick={handleSubmit}
                            disabled={loading || !description}
                            className={`px-6 py-2 rounded font-semibold text-white flex items-center gap-2 transition-all ${success ? 'bg-green-600' : 'bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed'}`}
                        >
                            {loading ? <Loader2 className="animate-spin" size={18} /> : success ? <Check size={18} /> : <Bug size={18} />}
    ...
    return (
        <div id="bug-reporter-fab" className="fixed top-1/2 right-0 -translate-y-1/2 z-[100000] group flex items-center">
            <button
