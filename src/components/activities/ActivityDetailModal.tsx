...
function ExpandableExercise({ exercise }: { exercise: any }) {
  ...
  const totalReps = exercise.sets.reduce(
    (s: number, set: any) => s + (set.reps || 0),
    0,
  );
  const totalDistance = exercise.sets.reduce(
    (s: number, set: any) => s + (set.distance || 0),
    0,
  );
      ...
      {expanded && (
        <div className="px-3 pb-2 space-y-1 border-t border-white/5">
          {exercise.sets.map((set: any, i: number) => (
            <div
              key={i}
              className="flex justify-between text-xs py-1 text-slate-400"
            >
              ...
              <div className="flex gap-4">
                ...
                {set.time && (
                  <span className="text-slate-500 font-mono">{set.time}</span>
                )}
                {set.rpe && (
                  <span className="text-amber-400">RPE {set.rpe}</span>
                )}
              </div>
            </div>
          ))}
...
export function ActivityDetailModal({
  ...
  const [viewMode, setViewMode] = useState<"combined" | "diff" | "raw">(
    "combined",
  );
  const [activeTab, setActiveTab] = useState<
    "stats" | "compare" | "splits" | "merge" | "analysis"
  >("stats");
  ...
  const [editForm, setEditForm] = useState({
    title: universalActivity?.plan?.title ||
      activity._mergeData?.universalActivity?.plan?.title || activity.title ||
      activity.notes || "",
    type: activity.type,
  ...
  const [displayTitle, setDisplayTitle] = useState(
    universalActivity?.plan?.title ||
      activity._mergeData?.universalActivity?.plan?.title || activity.title ||
      activity.notes || activity.type || "Aktivitet",
  );

  // Sync display title if prop changes (e.g. on load)
  useEffect(() => {
    const t = universalActivity?.plan?.title ||
      activity._mergeData?.universalActivity?.plan?.title || activity.title ||
      activity.notes || activity.type;
    if (t) setDisplayTitle(t);
  }, [
    universalActivity?.plan?.title,
    activity._mergeData?.universalActivity?.plan?.title,
    activity.title,
    activity.notes,
    activity.type,
  ]);
  ...
  const parsedWorkout = React.useMemo(() => {
    const title = universalActivity?.plan?.title ||
      activity._mergeData?.universalActivity?.plan?.title || activity.type ||
      "Workout";
    const desc = universalActivity?.plan?.description ||
      activity._mergeData?.universalActivity?.plan?.description ||
      activity.notes || "";
  ...
  const splits = universalActivity?.performance?.splits ||
    activity._mergeData?.universalActivity?.performance?.splits || [];
  const hasSplits = splits.length > 0;

  // Analysis visibility criteria - Strict check for meaningful content
  const perf = universalActivity?.performance ||
    activity._mergeData?.universalActivity?.performance;
  const hasHeartRate = (perf?.avgHeartRate && perf.avgHeartRate > 0) ||
    (activity.avgHeartRate && activity.avgHeartRate > 0);
  ...
  const isWorthyOfAnalysis = hasSplits ||
    (hasHeartRate && activity.type !== "strength") || hasWorkoutStructure;
  ...
  const handleRecategorize = async (newType: ExerciseType) => {
    ...
    console.log(
      `🔄 Omkategoriserar aktivitet ${activity.id} till ${newType}...`,
    );
    ...
    if (token) {
      try {
        ...
        const res = await fetch(
          `/api/activities/${activity.id}?date=${dateParam}`,
          {
            method: "PATCH",
            headers: {
        ...
        if (res.ok) {
          ...
          const updatedActivity = {
            ...
            plan: {
              title: universalActivity.plan?.title || activity.title ||
                activity.notes || "Aktivitet",
  ...
  useEffect(() => {
    if (
      isEditing && editForm.subType === "default" &&
      parsedWorkout?.suggestedSubType &&
      parsedWorkout.suggestedSubType !== "default" &&
      parsedWorkout.suggestedSubType !== "tempo"
    ) {
      setEditForm((prev) => ({
        ...prev,
        subType: parsedWorkout.suggestedSubType as any,
      }));
    }
  ...
  const originalActivities = React.useMemo(() => {
    ...
    if (isTrulyMerged && effectiveMergeInfo?.originalActivityIds?.length > 0) {
      const found = universalActivities.filter((u) =>
        effectiveMergeInfo.originalActivityIds!.includes(u.id)
      );
  ...
  const handleUnmerge = async () => {
    ...
    try {
      const response = await fetch(
        `/api/activities/${universalActivity.id}/separate`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    ...
    if (
      ...
      const isKeywordMatch = parsedWorkout.suggestedSubType === "long-run" ||
        parsedWorkout.suggestedSubType === "tempo";
      const intervalCount = parsedWorkout.segments.filter((s) =>
        s.type === "INTERVAL"
      ).length;
      const isSolidIntervals = parsedWorkout.suggestedSubType === "interval" &&
        intervalCount >= 2;
          ...
          if (universalActivity) {
            ...
            const updatedActivity = {
              ...
              plan: {
                ...universalActivity.plan,
                title: newTitle,
                activityType: universalActivity.plan?.activityType ||
                  universalActivity.performance?.activityType || "other",
                distanceKm: universalActivity.plan?.distanceKm ||
                  universalActivity.performance?.distanceKm || 0,
              },
            };
            ...
            console.warn(
              "Skipping fallback POST: universalActivity not available, would corrupt data",
            );
          }
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    if (isTrulyMerged && originalActivities.length > 0) {
      const stravaSource = originalActivities.find((a) =>
        a.performance?.source?.source === "strava"
      );
      const currentTitle = universalActivity?.plan?.title || activity.title;
      const stravaTitle = stravaSource?.plan?.title ||
        stravaSource?.performance?.notes;
          ...
          fetch(`/api/activities/${activity.id}?date=${dateParam}`, {
            ...
            if (res.status === 404) {
              ...
              if (universalActivity) {
                ...
                console.warn(
                  "Skipping fallback POST: universalActivity not available",
                );
              }
            }
          ...
          ? (
            <form onSubmit={handleSave} className="space-y-6">
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border flex flex-col items-center gap-1 transition-all ${
                      editForm.type === t.type
                        ...
                        : "bg-slate-800 border-white/5 text-slate-400 opacity-60 hover:opacity-100"
                    }`}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  {perf?.elapsedTimeSeconds &&
                    Math.abs(
                        (perf.elapsedTimeSeconds / 60) -
                          parseInt(editForm.duration || "0"),
                      ) > 1 &&
                    (
                      <div className="flex gap-2 mt-1">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setEditForm({
                              ...editForm,
                              duration: Math.round(perf.durationMinutes || 0)
                                .toString(),
                            })}
                          className="text-[10px] bg-slate-800 border border-white/10 px-2 py-1 rounded text-slate-400 hover:text-white"
                        >
                          Använd rörelsetid ({Math.round(
                            perf.durationMinutes || 0,
                          )} min)
                        </button>
                      </div>
                    )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {(editForm.type === "running" || editForm.type === "cycling" ||
                  editForm.type === "walking" || editForm.type === "swimming" ||
                  editForm.type === "hyrox") && (
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase">
                      Distans (km)
                ...
                {editForm.type === "strength" && (
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditForm({ ...editForm, tonnage: e.target.value })}
                      className="w-full bg-slate-800 border-white/5 rounded-xl p-3 text-white text-xs focus:outline-none focus:border-emerald-500/50"
                    />
                  </div>
                )}
                ...
                <div className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in slide-in-from-bottom-4">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-xs text-slate-500 font-mono">
                      ...
                      <span className="text-emerald-400 font-bold">
                        {formatDuration(
                          (editForm.hyroxStats?.runSplits?.reduce((a, b) =>
                            a + (b || 0), 0) || 0) +
                            (Object.values(editForm.hyroxStats?.stations || {})
                              .reduce((a, b) =>
                                a + (b || 0), 0) || 0),
                        )}
                      </span>
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div
                        key={i}
                        className="bg-slate-800/50 p-3 rounded-xl border border-white/5 space-y-2"
                      >
                        {/* Run Split */}
                            ...
                            <div className="flex gap-2 items-center">
                              <input
                                ...
                                value={formatDuration(
                                  editForm.hyroxStats?.runSplits?.[i] || 0,
                                )}
                                onChange={(e) => {
                        ...
                        <div className="flex items-center gap-3 pl-8 border-l-2 border-slate-700/50 ml-2">
                          <span className="text-xl">
                            {HYROX_STATIONS[i].icon}
                          </span>
                          <div className="flex-1 space-y-1">
                            <label className="text-[10px] uppercase font-bold text-amber-500/80">
                              {HYROX_STATIONS[i].label}
                            </label>
                            <input
                              type="text"
                              placeholder="mm:ss"
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    className={`text-lg ${
                      editForm.excludeFromStats ? "opacity-100" : "opacity-40"
                    }`}
                  >
                    🚫
                  </span>
                  <div>
                    ...
                    >
                      Exkludera från Beast Mode
                    </p>
                    <p className="text-[10px] text-slate-500">
                      Aktiviteten räknas inte med i statistik och poäng
                    </p>
                  </div>
                </div>
                <div
                  className={`w-10 h-6 rounded-full relative transition-all ${
                    editForm.excludeFromStats ? "bg-rose-500" : "bg-slate-700"
                  }`}
                >
                  <div
                    className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${
                      editForm.excludeFromStats ? "left-5" : "left-1"
                    }`}
                  />
                </div>
              </div>
            ...
            <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {(() => {
                      ...
                      if (activity.source === "strava" && activity.externalId) {
                        stravaLink = `https://www.strava.com/activities/${
                          activity.externalId.replace("strava_", "")
                        }`;
                      } else if (isTrulyMerged) {
                        const stravaOriginals = originalActivities
                          .filter((o) =>
                            o.performance?.source?.source === "strava" &&
                            o.performance?.source?.externalId
                          )
                          .sort((a, b) => (b.performance?.distanceKm || 0) -
                            (a.performance?.distanceKm || 0)
                          );
                        if (stravaOriginals.length > 0) {
                          const longest = stravaOriginals[0];
                          const extId = longest.performance?.source?.externalId
                            ?.replace("strava_", "");
                          stravaLink =
                            `https://www.strava.com/activities/${extId}`;
                          stravaTitle = longest.plan?.title ||
                            longest.performance?.notes;
                        }
                      }
                        ...
                        if (stravaLink) {
                          return (
                            <a
                              ...
                              title={`Sammanslagen (Strava: ${
                                stravaTitle || "Activity"
                              })`}
                            >
                        ...
                        {hyroxStats && (
                          <div className="text-[10px] uppercase font-bold text-slate-500 bg-slate-800/50 px-2 py-1 rounded">
                            ...
                            <span className="text-amber-400 text-sm">
                              {formatDuration(
                                Object.values(hyroxStats.stations || {}).reduce(
                                  (a, b) => a + (b || 0),
                                  0,
                                ),
                              )}
                        ...
                        {hyroxStats && (
                          <div className="text-[10px] uppercase font-bold text-slate-500 bg-slate-800/50 px-2 py-1 rounded">
                            ...
                            <span className="text-emerald-400 text-sm">
                              {formatDuration(
                                hyroxStats.runSplits?.reduce((a, b) =>
                                  a + (b || 0), 0) || 0,
                              )}
                            </span>
                          </div>
                          ...
                          <div key={i} className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-3 p-2 rounded-lg bg-amber-500/5 border border-amber-500/10">
                              ...
                              <span className="font-mono font-bold text-sm text-amber-400">
                                {formatDuration(
                                  hyroxStats.stations?.[HYROX_STATIONS[i].id] ||
                                    0,
                                )}
                              </span>
                            </div>
                            ...
                            return (
                              <>
                                <div className="flex-1 flex flex-col gap-2 items-center group">
                                  <div
                                    className="w-full bg-emerald-500/20 rounded-t-xl relative overflow-hidden transition-all group-hover:bg-emerald-500/30"
                                    style={{
                                      height: `${(totalRun / total) * 100}%`,
                                    }}
                                  >
                                    <div className="absolute inset-x-0 bottom-0 bg-emerald-500 opacity-20 h-full" />
                                  </div>
                                  <span className="text-xs font-bold text-emerald-400">
                                    {Math.round((totalRun / total) * 100)}%
                                  </span>
                                ...
                                <div className="flex-1 flex flex-col gap-2 items-center group">
                                  <div
                                    className="w-full bg-amber-500/20 rounded-t-xl relative overflow-hidden transition-all group-hover:bg-amber-500/30"
                                    style={{
                                      height: `${
                                        (totalStation / total) * 100
                                      }%`,
                                    }}
                                  >
                                  ...
                                  <span className="text-xs font-bold text-amber-400">
                                    {Math.round((totalStation / total) * 100)}%
                                  </span>
                                  <span className="text-[10px] font-black uppercase text-slate-500">
                                    Stations
                                  </span>
                      ...
                      <div className="p-4 bg-slate-800/30 rounded-2xl border border-white/5 flex-1">
                        ...
                        <div className="space-y-2">
                          {Object.entries(hyroxStats.stations || {})
                            ...
                            .map(([key, duration]) => {
                              const station = HYROX_STATIONS.find((s) =>
                                s.id === key
                              );
                ...
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="text-amber-400 font-bold text-sm">
                      Denna aktivitet är dold
                    </h4>
                    <p className="text-amber-300/70 text-xs">
                      Den ingår i en sammanslagen aktivitet och visas normalt
                      inte i listor.
                    </p>
                  </div>
                  <button
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                {(activity.type === "running" || activity.type === "cycling" ||
                  ...
                  <button
                    onClick={async () => {
                      const newSubType = activity.subType === "race"
                        ? "default"
                        : "race";
                      ...
                      if (token && universalActivity) {
                        try {
                          const dateParam = activity.date.split("T")[0];
                          console.log(
                            `🔄 Sparar subType=${newSubType} för aktivitet ${activity.id}...`,
                          );
                          const res = await fetch(
                            `/api/activities/${activity.id}?date=${dateParam}`,
                            {
                          ...
                          if (res.ok) {
                            console.log(
                              `✅ Aktivitet markerad som ${
                                newSubType === "race" ? "tävling" : "träning"
                              } - sparad i KV`,
                            );
                          } else if (res.status === 404) {
                            // Fallback to upsert
                            console.log("⚠️ PATCH 404, försöker POST...");
                            const { userId: _u, ...activityData } =
                              universalActivity;
                            const updatedActivity = {
                              ...activityData,
                              performance: {
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${
                      activity.subType === "race"
                        ? "bg-rose-500 text-white shadow-lg shadow-rose-500/20"
                        : "bg-rose-500/10 text-rose-400 border border-rose-500/30 hover:bg-rose-500/20"
                    }`}
                    title={activity.subType === "race"
                      ? "Avmarkera som tävling"
                      : "Markera som tävling"}
                  >
                    <span>🏅</span> {activity.subType === "race"
                      ? "Tävling ✓"
                      : "Markera tävling"}
                  </button>
                )}
                              ...
                              >
                                <div
                                  className={`flex flex-col gap-1 ${
                                    original.performance?.source?.source ===
                                        "strava"
                                      ? "text-[#FC4C02]"
                                      : "text-purple-400"
                                  }`}
                                >
                                  ...
                                  <span className="text-[9px] opacity-70 font-normal capitalize">
                                    {original.plan?.title ||
                                      original.performance?.activityType}
                                  </span>
                                </div>
                              </th>
                            ))}
                        ...
                        <tbody className="divide-y divide-white/5">
                          ...
                          <tr className="hover:bg-white/5 transition-colors">
                            ...
                            {originalActivities.map((a) => {
                              const sourceTitle = a.plan?.title ||
                                a.performance?.notes || "-";
                              ...
                              return (
                                ...
                                >
                                  <button
                                    onClick={() =>
                                      handleUpdateTitle(sourceTitle)}
                                    disabled={isActive}
                                    className={`font-mono text-xs truncate max-w-[150px] px-2 py-1 rounded transition-all ${
                                      isActive
                          ...
                          <tr className="hover:bg-white/5 transition-colors">
                            ...
                            {originalActivities.map((a) => (
                              <td
                                key={a.id}
                                className="px-6 py-4 text-center text-slate-500 text-xs truncate max-w-[150px]"
                                title={a.plan?.description ||
                                  a.performance?.notes || ""}
                              >
                            ...
                            <td className="px-6 py-4 text-right text-slate-400 text-xs">
                              {universalActivity?.plan?.description ||
                                activity.notes || "-"}
                            </td>
                          </tr>
                          {/* Duration */}
                          <tr className="hover:bg-white/5 transition-colors">
                            ...
                            {originalActivities.map((a) => (
                              <td
                                key={a.id}
                                className="px-6 py-4 text-center text-slate-300 font-mono"
                              >
                                {formatDuration(
                                  (a.performance?.durationMinutes || 0) * 60,
                                )}
                              </td>
                            ))}
                            ...
                            {originalActivities.map((a) => (
                              ...
                              >
                                {a.performance?.distanceKm
                                  ? `${a.performance.distanceKm.toFixed(2)} km`
                                  : "-"}
                              </td>
                            ))}
                            <td className="px-6 py-4 text-right font-bold text-emerald-400 font-mono">
                          ...
                          <tr className="hover:bg-white/5 transition-colors">
                            ...
                            {originalActivities.map((a) => (
                              <td
                                ...
                                className="px-6 py-4 text-center text-slate-300 font-mono"
                              >
                                {a.performance?.calories
                                  ? `${a.performance.calories} kcal`
                                  : "-"}
                              </td>
                            ))}
                            <td
                              className={`px-6 py-4 text-right font-bold text-emerald-400 font-mono ${
                                activity.calorieBreakdown
                                  ? "cursor-help border-b border-emerald-400/20 md:border-b-0"
                                  : ""
                              }`}
                          ...
                          <tr className="hover:bg-white/5 transition-colors">
                            ...
                            {originalActivities.map((a) => (
                              ...
                              >
                                {a.performance?.avgHeartRate
                                  ? `${
                                    Math.round(a.performance.avgHeartRate)
                                  } bpm`
                                  : "-"}
                              </td>
                            ))}
                            <td className="px-6 py-4 text-right font-bold text-emerald-400 font-mono">
                              {perf?.avgHeartRate
                                ? `${Math.round(perf.avgHeartRate)} bpm`
                                : "-"}
                            </td>
                          </tr>
                        </tbody>
              ...
              {/* COMBINED VIEW */}
              {!isHyrox && (viewMode === "combined" || !isMerged) &&
                activeTab === "stats" && (
                <>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {(() => {
                              ...
                              if (
                                ...
                                const stravaOriginals = originalActivities
                                  .filter((o) =>
                                    o.performance?.source?.source ===
                                      "strava" &&
                                    o.performance?.source?.externalId
                                  )
                                  .sort((a, b) =>
                                    (b.performance?.distanceKm || 0) -
                                    (a.performance?.distanceKm || 0)
                                  );
                                if (stravaOriginals.length > 0) {
                                  const extId = stravaOriginals[0].performance
                                    ?.source?.externalId?.replace(
                                      "strava_",
                                      "",
                                    );
                                  stravaLink =
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4">
                                ...
                                {perf?.elapsedTimeSeconds &&
                                  ...
                                  (
                                    <span className="text-xs text-slate-500 font-bold">
                                      /{" "}
                                      {formatDuration(perf.elapsedTimeSeconds)}
                                    </span>
                                  )}
                              </div>
                            </div>
                              ...
                              <div className="flex items-baseline gap-1">
                                <span
                                  className={`text-xl font-black text-white ${
                                    activity.calorieBreakdown
                                      ? "cursor-help border-b border-white/20 md:border-b-0"
                                      : ""
                                  }`}
                                  title={activity.calorieBreakdown}
                                >
                                ...
                                {perf?.kilojoules && (
                                  <span className="text-[9px] text-[#FC4C02] font-black ml-2">
                                    {perf.kilojoules} KJ
                                  </span>
                                )}
                              </div>
                            ...
                            {(!!perf?.avgHeartRate ||
                              ...
                              <div className="flex flex-col">
                                ...
                                <div className="flex items-baseline gap-1">
                                  ...
                                  {perf?.maxHeartRate && (
                                    <span className="text-[9px] text-red-500 font-black ml-1">
                                      MAX {Math.round(perf.maxHeartRate)}
                                    </span>
                                  )}
                                </div>
                              </div>
                            ...
                            {!!perf?.averageWatts && (
                              <div className="flex flex-col">
                                ...
                                <div className="flex items-baseline gap-1">
                                  <span className="text-xl font-black text-white">
                                    {Math.round(perf.averageWatts)}
                                  </span>
                                  <span className="text-[10px] uppercase text-slate-500">
                                    w
                                  </span>
                                  {perf?.maxWatts && (
                                    <span className="text-[9px] text-indigo-400 font-black ml-1">
                                      MAX {Math.round(perf.maxWatts)}
                                    </span>
                                  )}
                                </div>
                              </div>
                            ...
                            {/* Achievements */}
                            {(perf?.achievementCount || perf?.prCount ||
                                perf?.kudosCount)
                              ? (
                                <div className="flex flex-col col-span-2">
                                  <span className="text-[10px] text-slate-500 uppercase font-black tracking-tighter mb-1">
                                    Engagemang & Prestationer
                                  </span>
                                  <div className="flex items-center gap-4">
                                    {(perf?.prCount || 0) > 0 && (
                                      <div
                                        className="flex items-center gap-1.5 sh-tooltip"
                                        title={`${perf.prCount} Personbästa`}
                                      >
                          ...
                          {(activity.tonnage && activity.tonnage > 0) && (
                            <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                              <p className="text-2xl font-black text-purple-400">
                                {(activity.tonnage / 1000).toFixed(1)}
                              </p>
                              ...
                              <svg className="w-full h-full -rotate-90">
                                ...
                                <circle
                                  ...
                                  strokeDashoffset={314 -
                                    (314 * perfBreakdown.totalScore) / 100}
                                  strokeLinecap="round"
                              ...
                              <div key={idx} className="space-y-1.5">
                                <div className="flex justify-between items-end">
                                  <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1.5">
                                    ...
                                    {comp.isPersonalBest && (
                                      <span className="text-[8px] bg-yellow-500/10 text-yellow-500 px-1 rounded border border-yellow-500/20">
                                        PB
                                      </span>
                                    )}
                                  </span>
                                  <span
                                    className={`text-xs font-mono font-bold ${comp.color}`}
                                  >
                                    {comp.value}
                                  </span>
                                </div>
                  ...
                  {(perf?.avgHeartRate || activity.avgHeartRate) &&
                    ...
                    <HeartRateZones
                      avgHeartRate={Math.round(
                        perf?.avgHeartRate || activity.avgHeartRate || 0,
                      )}
                      maxHeartRate={(perf?.maxHeartRate ||
                          activity.maxHeartRate)
                        ? Math.round(
                          perf?.maxHeartRate || activity.maxHeartRate || 0,
                        )
                        : undefined}
                  ...
                  {strengthWorkout?.exercises &&
                    ...
                    <div className="bg-purple-950/30 border border-purple-500/20 rounded-xl p-4">
                      <h3 className="text-xs font-bold text-purple-400 uppercase mb-3">
                        💪 Övningar{" "}
                        {isMerged && (
                      ...
                      <div className="space-y-1 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                        {strengthWorkout.exercises
                          .filter((ex: any) =>
                            ex.totalVolume > 0 || ex.sets.length > 0
                          )
                          .map((ex: any, i: number) => (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {(perf ? perf.elevationGain > 0 : false) && (
                      <div className="bg-emerald-950/30 border border-emerald-500/20 rounded-xl p-4">
                        ...
                        <span className="text-2xl font-black text-white">
                          {Math.round(perf.elevationGain)}
                        </span>
                        <span className="text-xs text-slate-400 ml-1">m</span>
                      </div>
                    )}
                    {((activity.distance || 0) > 0 &&
                        (perf?.elevationGain || 0) > 0)
                      ? (
                  ...
                  {(() => {
                    ...
                    const shouldShowNotes = notes &&
                      (notes.length >= 50 || notes.includes("\n"));
              ...
              {activeTab === "analysis" && (
                <div className="space-y-4">
                    ...
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 flex items-center justify-between animate-in slide-in-from-top-2">
                      <div>
                        <p className="text-xs font-bold text-amber-400 uppercase flex items-center gap-2">
                          <span>💡</span> Förslag:{" "}
                          {parsedWorkout.suggestedSubType === "interval"
                            ? "Intervaller"
                            : parsedWorkout.suggestedSubType === "long-run"
                            ? "Långpass"
                            : parsedWorkout.suggestedSubType}
                        </p>
                        <p className="text-[10px] text-amber-200/70">
                          Analysen tyder på att detta är ett{" "}
                          {parsedWorkout.suggestedSubType === "interval"
                            ? "intervallpass"
                            : "långpass"}.
                        </p>
                      ...
                      <button
                        onClick={() => {
                          handleApplyCategory(
                            parsedWorkout.suggestedSubType as any,
                          );
                        }}
                  ...
                  <WorkoutStructureCard
                    title={universalActivity?.plan?.title || activity.type ||
                      "Workout"}
                    description={universalActivity?.plan?.description ||
                      activity.notes || ""}
                  />
                </div>
              )}
              ...
              {viewMode === "raw" && (
                <div className="space-y-4">
                  ...
                  <p className="text-[10px] text-slate-600 italic">
                    Denna vy visar den kombinerade JSON-strukturen för
                    aktiviteten, inklusive merge-data och källinfo.
                  </p>
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-indigo-400 uppercase tracking-wider">
                      ⚖️ Jämför med liknande pass
                    </h3>
                    <span className="text-[10px] text-slate-500 uppercase font-mono">
                      Baserat på distans (+/- 25%)
                    </span>
                  </div>
                        ...
                        <table className="w-full text-sm">
                          ...
                          <tbody className="divide-y divide-white/5 font-mono">
                              ...
                              <td className="px-4 py-4">
                                <div className="flex flex-col">
                                  <span className="text-white font-bold">
                                    {formatSwedishDate(activity.date)}
                                  </span>
                            ...
                            {similarActivities.map((a) => {
                              const aPaceSec = a.distance
                                ? (a.durationMinutes * 60 / a.distance)
                                : 0;
                    ...
                    : (
                      <div className="text-center py-12 bg-slate-800/30 rounded-2xl border border-dashed border-white/5">
                        ...
                        <div className="text-slate-500 italic text-sm">
                          Inga liknande pass inom +/- 25% distans hittades.
                        </div>
                      </div>
                    )}
                </div>
                          ...
                          return (
                            <tr key={i}>
                              ...
                              <td className="px-4 py-3 text-right text-slate-300">
                                {Math.floor(
                                  s.movingTime / 60,
                                )}:{(s.movingTime % 60).toString().padStart(
                                  2,
                                  "0",
                                )}
                              </td>
                              <td className="px-4 py-3 text-right text-indigo-400">
                                {Math.floor(
                                  splitPace / 60,
                                )}:{(Math.round(splitPace % 60)).toString()
                                  .padStart(2, "0")} /km
                              </td>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wider">
                      ⚡ Sammanslagen aktivitet
                    </h3>
                    <span className="text-[10px] text-slate-500 uppercase font-mono">
                      Skapad {effectiveMergeInfo?.mergedAt
                        ? formatSwedishDate(
                          effectiveMergeInfo.mergedAt.split("T")[0],
                        )
                        : "-"}
                    </span>
                  </div>
                  ...
                  <div className="bg-amber-950/20 border border-amber-500/20 rounded-xl p-4">
                    <p className="text-sm text-slate-300 mb-4">
                      Denna aktivitet är en sammanslagning av{" "}
                      <strong className="text-amber-400">
                        {originalActivities.length}
                      </strong>{" "}
                      separata aktiviteter. De kombinerade värdena (distans,
                      tid, kalorier osv.) har räknats ut automatiskt.
                    </p>
                      ...
                      {originalActivities.length > 0
                        ? (
                          originalActivities.map((orig, idx) => {
                            const title = orig.plan?.title ||
                              orig.performance?.notes || null;
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      {title
                                        ? (
                                          <>
                                            <p className="text-white font-bold group-hover:text-amber-400 transition-colors truncate max-w-[200px]">
                                              {title}
                                            </p>
                                            <p className="text-xs text-slate-500 capitalize">
                                              {orig.performance?.activityType ||
                                                "Aktivitet"} •{" "}
                                              {formatSwedishDate(orig.date)}
                                            </p>
                                          </>
                                        )
                                        : (
                                          <>
                                            <p className="text-white font-bold capitalize group-hover:text-amber-400 transition-colors">
                                              {orig.performance?.activityType ||
                                                "Aktivitet"}
                                            </p>
                                    ...
                                    {orig.performance?.distanceKm && (
                                      <div className="text-right">
                                        <p className="text-emerald-400 font-mono font-bold">
                                          {orig.performance.distanceKm.toFixed(
                                            1,
                                          )} km
                                        </p>
                                      </div>
                                    ...
                                    {orig.performance?.distanceKm &&
                                      ...
                                      <div className="text-right">
                                        <p className="text-indigo-400 font-mono">
                                          {formatPace(
                                            (orig.performance.durationMinutes *
                                              60) / orig.performance.distanceKm,
                                          )}
                                        </p>
                                      </div>
                                    )}
                        ...
                        : (
                          <p className="text-slate-500 italic text-sm">
                            Originalaktiviteterna kunde inte hittas. De kan ha
                            tagits bort.
                          </p>
                        )}
                    </div>
                  ...
                  <div className="bg-slate-800/30 border border-white/5 rounded-xl p-4">
                    ...
                    <p className="text-sm text-slate-400 mb-4">
                      Om sammanslagningen blev fel kan du separera dem igen. Den
                      sammanslagna aktiviteten försvinner och de ursprungliga
                      aktiviteterna visas på nytt.
                    </p>
                    <button
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <button
                  onClick={() => {
                    window.location.href =
                      `/workouts/builder?fromActivity=${activity.id}`;
                  }}
                ...
                <button
                  onClick={() =>
                    setViewMode(viewMode === "raw" ? "combined" : "raw")}
