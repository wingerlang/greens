...
const SegmentRow = ({ segment, index }: { segment: WorkoutSegment; index: number }) => {
    ...
    return (
        <div className={`relative flex items-center gap-3 p-3 rounded-xl border ${borderColor} ${bgColor} mb-2`}>
            <div className="w-6 h-6 flex items-center justify-center text-sm">{icon}</div>

            <div className="flex-1">
                <div className="flex items-baseline justify-between">
                    <h4 className={`text-xs font-bold uppercase ${isInterval ? 'text-amber-400' : 'text-slate-400'}`}>
                        {segment.type} {segment.reps > 1 && <span className="text-white ml-2">x{segment.reps}</span>}
                    </h4>
                    {segment.work.pace && (
                ...
                <div className="flex items-baseline gap-2 mt-0.5">
                    {segment.work.dist ? (
                        <span className="text-lg font-black text-white">{segment.work.dist} <span className="text-xs font-normal text-slate-500">m</span></span>
                    ) : segment.work.time ? (
                        <span className="text-lg font-black text-white">{Math.round(segment.work.time / 60)} <span className="text-xs font-normal text-slate-500">min</span></span>
                    ) : (
...
export function WorkoutStructureCard({ title, description }: WorkoutStructureCardProps) {
