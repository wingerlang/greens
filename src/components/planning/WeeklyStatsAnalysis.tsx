...
export function WeeklyStatsAnalysis({
  ...
  const intensityStats = useMemo(() => {
    ...
    weekActivities.forEach((a) => {
      ...
      if (a.intensity) {
        ...
        else if (a.intensity === "high" || a.intensity === "ultra") {
          z4_5 += duration;
        }
  ...
  const volumeAnalysis = useMemo(() => {
    ...
    const totalRecentKm = recentRuns.reduce(
  ...
  const reviewInsights = useMemo(() => {
    ...
    const weekPlan = plannedActivities.filter((p) =>
      p.date >= weekStart && p.date <= endStr
    );
    const weekActual = unifiedActivities.filter((a) =>
      a.date >= weekStart && a.date <= endStr
    );

    const insights: string[] = [];
    const completed = weekPlan.filter((p) => p.status === "COMPLETED");
    const missed = weekPlan.filter((p) =>
      p.status === "PLANNED" && new Date(p.date) < new Date()
    );

    // Adherence score
    const adherencePct = weekPlan.length > 0
      ? (completed.length / weekPlan.length) * 100
      : 100;

    if (weekPlan.length === 0 && weekActual.length > 0) {
      insights.push(
        "Du tränade bra trots att inget var planerat. Snyggt jobbat!",
      );
    } else if (adherencePct === 100 && weekPlan.length > 0) {
      insights.push(
        "100% följsamhet! Du har genomfört alla planerade pass denna vecka.",
      );
    } else if (adherencePct >= 75) {
      insights.push(
        `Hög följsamhet (${Math.round(adherencePct)}%). Du missade bara ${
          weekPlan.length - completed.length
        } pass.`,
      );
    } else if (adherencePct > 0) {
      insights.push(
        `Du har genomfört ${completed.length} av ${weekPlan.length} planerade pass.`,
      );
    }
      ...
      if (p.actualDistance && p.estimatedDistance) {
        ...
        if (diff > 2) {
          insights.push(
            `Passet "${p.title}" blev ${
              diff.toFixed(1)
            }km längre än planerat. Starkt!`,
          );
        } else if (diff < -2) {
          insights.push(
            `Passet "${p.title}" blev ${
              Math.abs(diff).toFixed(1)
            }km kortare än planerat.`,
          );
        }
      }
    });
    ...
    // Extra activities
    const extraActivities = weekActual.filter((a) =>
      !weekPlan.some((p) => p.externalId === a.id)
    );
    if (extraActivities.length > 0) {
      insights.push(
        `Du körde ${extraActivities.length} extra pass som inte fanns i planen.`,
      );
    }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
        ...
        <div className="flex items-end gap-2 h-32 mb-4">
          ...
          <div className="flex-1 flex flex-col justify-end gap-2 group">
            ...
            />
            <div className="text-[10px] font-black uppercase text-center text-slate-500">
              Låg
            </div>
          </div>
        ...
        <div className="text-xs text-slate-500 font-medium text-center">
          Totalt {formatDuration(intensityStats.totalTime * 60)}{" "}
          konditionsträning
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="text-2xl font-black text-slate-900 dark:text-white">
              {volumeAnalysis.current.toFixed(1)}{" "}
        ...
        <div className="relative pt-2">
          <div className="flex justify-between text-[10px] font-bold uppercase text-slate-400 mb-1">
            <span>Trend</span>
            <span
              className={volumeAnalysis.pctDiff > 0
                ? "text-emerald-500"
                : "text-rose-500"}
            >
              {volumeAnalysis.pctDiff > 0 ? "+" : ""}
              {volumeAnalysis.pctDiff.toFixed(0)}%
            </span>
          </div>
          <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${
                volumeAnalysis.pctDiff > 10
                  ? "bg-amber-500"
                  : volumeAnalysis.pctDiff < -10
                  ? "bg-blue-400"
                  : "bg-emerald-500"
            ...
            <div className="flex items-start gap-2 p-3 bg-white/5 rounded-xl border border-white/10">
              ...
              <div className="text-xs">
                ...
                <p className="text-slate-400 mt-0.5">
                  Mer än 25% av tiden i zon 4-5. Se till att de lugna passen
                  verkligen är lugna.
                </p>
              </div>
            </div>
      ...
      <div className="md:col-span-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-black uppercase tracking-wider text-slate-800 dark:text-white">
              Veckans Review
            </h3>
          </div>
        ...
        {reviewInsights.insights.length > 0
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                  Insikter & Observationer
                </h4>
                {reviewInsights.insights.map((insight, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800"
                  >
          ...
          : (
            <div className="flex flex-col items-center py-8 text-slate-400">
              ...
              <p className="text-sm">
                Ingen träningsdata tillgänglig för denna vecka.
              </p>
            </div>
          )}
      </div>
