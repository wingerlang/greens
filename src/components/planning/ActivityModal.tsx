...
export function ActivityModal({
  ...
  const [formType, setFormType] = useState<
    "RUN" | "STRENGTH" | "HYROX" | "BIKE" | "REST"
  >("RUN");
  // New: Sub-category state for UI chips (only for RUN)
  const [runSubCategory, setRunSubCategory] = useState<
    "EASY" | "LONG_RUN" | "INTERVALS" | "RECOVERY"
  >("EASY");
  ...
  const lastChanged = useRef<
    "pace" | "duration" | "distance" | "preset" | null
  >(null);

  // Hyrox specific
  const [formIncludesRunning, setFormIncludesRunning] = useState(true);
  const [formHyroxFocus, setFormHyroxFocus] = useState<
    "hybrid" | "strength" | "cardio"
  >("hybrid");
  const [formStatus, setFormStatus] = useState<
    "PLANNED" | "COMPLETED" | "SKIPPED" | "CHANGED"
  >("PLANNED");
  ...
  const smartSuggestions = useSmartTrainingSuggestions(
    selectedDate,
    weeklyStats,
    goalProgress,
  );
  ...
  const runStats = useMemo(() => {
    ...
    const stravaRuns = (universalActivities || [])
      .filter((ua) => {
        const actType = (ua.performance?.activityType as string || "")
          .toLowerCase();
        return (
    ...
    const easyRuns = allRuns.filter((r) =>
      r.category === "EASY" && r.durationMinutes > 0 && r.distance > 0
    );
    const avgEasyPace = easyRuns.length > 0
      ? easyRuns.reduce((sum, r) => sum + (r.durationMinutes / r.distance), 0) /
        easyRuns.length
      : 5.5; // Default

    // Bucketing for presets
    const distanceBuckets: Record<
      string,
      { count: number; paceCount: number; totalPace: number }
    > = {};
    allRuns.forEach((run) => {
  ...
  useEffect(() => {
    ...
    if (
      formType === "RUN" && formDistance && formPace &&
      lastChanged.current !== "duration"
    ) {
      ...
      const newDuration = `${hours.toString().padStart(2, "0")}:${
  ...
  useEffect(() => {
    ...
    if (
      formType === "RUN" && formDistance && formDuration &&
      ...
      const newPace = `${paceMins.toString().padStart(2, "0")}:${
        paceSecs.toString().padStart(2, "0")
      }`;
  ...
  useEffect(() => {
    if (isOpen && selectedDate) {
      ...
      if (editingActivity) {
        setFormType(
          editingActivity.type === "REST"
            ? "REST"
            : (editingActivity.category === "STRENGTH"
              ? "STRENGTH"
              : (editingActivity.title.toLowerCase().includes("hyrox")
                ? "HYROX"
                : (editingActivity.type === "BIKE" ? "BIKE" : "RUN"))),
        );

        if (editingActivity.category === "LONG_RUN") {
          setRunSubCategory("LONG_RUN");
        } else if (
          editingActivity.category === "INTERVALS" ||
          editingActivity.category === "TEMPO"
        ) setRunSubCategory("INTERVALS");
        else if (editingActivity.category === "RECOVERY") {
          setRunSubCategory("RECOVERY");
        } else setRunSubCategory("EASY");

        const durMatch = editingActivity.description?.match(
          /\((\d{2}:\d{2})\)$/,
        );
        setFormDuration(durMatch ? durMatch[1] : "00:45");
        setFormDistance(
          editingActivity.estimatedDistance
            ? Number(editingActivity.estimatedDistance).toFixed(1)
            : "",
        );
        setFormTonnage(
          editingActivity.tonnage ? editingActivity.tonnage.toString() : "",
        );
        setFormMuscleGroups(editingActivity.muscleGroups || []);
        setFormNotes(editingActivity.description || "");
        setFormIntensity(
          editingActivity.targetHrZone <= 2
            ? "low"
            : editingActivity.targetHrZone >= 4
            ? "high"
            : "moderate",
        );
        ...
        setFormStatus(
          editingActivity.status === "DRAFT"
            ? "PLANNED"
            : editingActivity.status as any,
        );
  ...
  const handleRunSubCategoryClick = (
    sub: "EASY" | "LONG_RUN" | "INTERVALS" | "RECOVERY",
  ) => {
  ...
  const handleSave = () => {
    ...
    const activityData: PlannedActivity = {
      ...
      description: formNotes ||
        `${
          formType === "REST" ? "Vila och återhämtning" : title + " pass"
        } (${formDuration})`,
      estimatedDistance: formType === "RUN" && formDistance
        ? parseFloat(formDistance)
        : 0,

      // Hyrox & Strength
      tonnage: (formType === "STRENGTH" || formType === "HYROX") && formTonnage
        ? parseInt(formTonnage)
        : undefined,
      muscleGroups: formType === "STRENGTH"
        ? formMuscleGroups as any
        : undefined,
      ...
      targetPace: "",
      targetHrZone: formType === "REST"
        ? 1
        : (formIntensity === "low" ? 2 : formIntensity === "moderate" ? 3 : 4),
      structure: {
        warmupKm: 0,
        mainSet: [],
        cooldownKm: 0,
      } as PlannedActivity["structure"],
  ...
  const handleApplySuggestion = (s: TrainingSuggestion) => {
    ...
    if (s.type === "STRENGTH") category = "STRENGTH";
      ...
      s.label.includes("Intervaller") || s.label.includes("Tempo") ||
      s.label.includes("Kvalitet")
    ) category = "INTERVALS";
  ...
  const getSuggestionColor = (s: TrainingSuggestion) => {
    if (s.label.includes("Måljakt")) {
      return "from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400";
    }
    if (s.type === "STRENGTH") {
      return "from-purple-50 to-fuchsia-50 dark:from-purple-900/20 dark:to-fuchsia-900/20 border-purple-200 dark:border-purple-800 text-purple-700 dark:text-purple-400";
    }
    if (s.label.includes("Återhämtning") || s.type === "REST") {
      return "from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400";
    }
    if (
      s.label.includes("Intervaller") || s.label.includes("Tempo") ||
      s.label.includes("Kvalitet") || s.intensity === "high"
    ) {
      return "from-rose-50 to-orange-50 dark:from-rose-900/20 dark:to-orange-900/20 border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-400";
    }
            ...
            <div className="mb-6 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-700 dark:text-amber-300 leading-tight mt-0.5">
                  Det finns redan träning registrerad eller planerad på detta
                  datum.
                </p>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {smartSuggestions
                  .filter((s) => {
                    ...
                    if (formType === "RUN") {
                      return s.type === "RUN" || s.type === "REST";
                    }
                    if (formType === "HYROX") {
                      return s.type === "HYROX" || s.type === "STRENGTH" ||
                        s.type === "RUN";
                    }
                    return true;
                  })
              ...
              {runStats.avgDistance && !editingActivity && (
                ...
                >
                  ...
                  <p className="text-[10px] text-blue-600/80 dark:text-blue-400/80 font-medium">
                    Baserat på senaste 5 veckor.{" "}
                    <span className="italic underline decoration-blue-300">
                      Sträva efter att slå detta!
                    </span>{" "}
                    🚀
                  </p>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                    {runStats.frequentPresets[0]?.isDefault
                      ? "Populära distanser"
                      : "Dina vanliga distanser"}
                  </span>
                </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const paceSeconds = Math.round(
                          (preset.avgPace - paceMinutes) * 60,
                        );
                        setFormPace(
                          `${paceMinutes.toString().padStart(2, "0")}:${
                      ...
                      className={`px-2.5 py-1.5 rounded-xl border text-sm font-bold transition-all hover:scale-105 ${
                        formDistance === preset.distance.toFixed(1)
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-black uppercase tracking-wide transition-all ${
                  formType === "REST"
                    ? "bg-white dark:bg-slate-700 shadow-sm text-slate-600"
                    : "text-slate-400 hover:text-slate-600"
                }`}
              >
                Vila
              ...
              <div className="flex gap-1.5 pb-1">
                {[
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide whitespace-nowrap transition-all border ${
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl border flex items-center gap-2 transition-all ${
                    isRace
                      ? "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-400 text-yellow-600 dark:text-yellow-400"
                      : "bg-transparent border-slate-200 dark:border-slate-700 text-slate-400"
                  }`}
                >
                  <Trophy
                    size={16}
                    className={isRace ? "fill-yellow-500" : ""}
                  />
                    ...
                    : (
                      <div>
                        ...
                        <select
                          value={formIntensity}
                          onChange={(e) =>
                            setFormIntensity(e.target.value as any)}
                        ...
                        {[
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                              formMuscleGroups.includes(group.id)
                  ...
                  <div className="space-y-4 animate-in slide-in-from-top-2 p-3 bg-amber-500/5 rounded-xl border border-amber-500/10">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <button
                          ...
                          className={`py-2 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all border ${
                            formHyroxFocus === "hybrid"
                              ? "bg-amber-500 text-white border-amber-500"
                              : "bg-transparent border-slate-200 dark:border-slate-700 text-slate-400 hover:border-amber-300"
                          }`}
                        >
                          Hybrid
                        </button>
                        <button
                          ...
                          className={`py-2 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all border ${
                            formHyroxFocus === "strength"
                              ? "bg-purple-500 text-white border-purple-500"
                              : "bg-transparent border-slate-200 dark:border-slate-700 text-slate-400 hover:border-purple-300"
                          }`}
                        >
                          💪 Styrka
                        </button>
                        <button
                          ...
                          className={`py-2 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all border ${
                            formHyroxFocus === "cardio"
                              ? "bg-blue-500 text-white border-blue-500"
                              : "bg-transparent border-slate-200 dark:border-slate-700 text-slate-400 hover:border-blue-300"
                          }`}
                        >
                          🏃 Cardio
                        </button>
                      ...
                      <p className="text-[10px] text-slate-500 italic">
                        {formHyroxFocus === "strength"
                          ? "Räknas som styrkepass i statistiken"
                          : formHyroxFocus === "cardio"
                          ? "Räknas som cardiopass i statistiken"
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() =>
                          setFormIncludesRunning(!formIncludesRunning)}
                        className={`w-12 h-6 rounded-full transition-colors relative ${
                          formIncludesRunning ? "bg-amber-500" : "bg-slate-700"
                        }`}
                      >
                        <div
            ...
            >
              {editingActivity
                ? "Uppdatera Pass"
                : (isRace ? "Spara Tävling" : "Spara Pass")}
            </button>
          </div>
        </div>
      </div>
