...
export function WorkoutComparisonView({ workout }: Props) {
  ...
  const matches = useMemo(() => {
    ...
    if (signature.type === "RUNNING") {
      ...
      const checkMatch = (
        ...
        item: ExerciseEntry | StrengthSession,
      ) => {
        const historySet = new Set(
          historyExercises.map((n) => n.toLowerCase().trim()),
        );
        const intersection = new Set(
          [...targetExercises].filter((x) => historySet.has(x)),
        );
        ...
        if (overlapCount === targetCount && historyCount === targetCount) {
          type = "EXACT";
        } else if (overlapCount === targetCount && historyCount > targetCount) {
          type = "SUPERSET";
        } else if (
          overlapCount === historyCount && targetCount > historyCount
        ) type = "SUBSET";
  ...
  const renderBadge = (type: MatchType) => {
    switch (type) {
      case "EXACT":
        return (
          <span className="bg-emerald-500/20 text-emerald-400 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider border border-emerald-500/20">
            Perfekt
          </span>
        );
      case "SUBSET":
        return (
          <span className="bg-blue-500/20 text-blue-400 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider border border-blue-500/20">
            Delmängd
          </span>
        );
      case "SUPERSET":
        return (
          <span className="bg-purple-500/20 text-purple-400 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider border border-purple-500/20">
            Utökad
          </span>
        );
      case "SIMILAR_DIST":
        return (
          <span className="bg-indigo-500/20 text-indigo-400 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider border border-indigo-500/20">
            Liknande
          </span>
        );
      default:
        return (
          <span className="bg-slate-500/20 text-slate-400 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider border border-slate-500/20">
            Match
          </span>
        );
    }
    ...
    return (
      <div className="p-12 text-center border-2 border-dashed border-white/5 rounded-[2.5rem] bg-slate-900/10 mx-6 mt-6">
        <p className="text-slate-500 font-bold text-sm mb-2">
          Inga matchande pass hittades.
        </p>
        <div className="text-[10px] text-slate-600 uppercase tracking-widest font-black opacity-50">
          {signature.type === "RUNNING"
            ? `Söker efter pass kring ${
              (signature as any).distance.toFixed(1)
            } km`
            : `Söker pass med liknande övningar`}
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-[#080815]">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em]">
            Matchade Pass ({matches.length})
          </h3>
          <div className="text-xs font-mono text-slate-600">
            {signature.type === "RUNNING"
              ? `${((signature as any).distance as number).toFixed(1)} km @ ${
                formatPace((signature as any).pace as number)
              }`
              : `${
                ((signature as any).exercises as Set<string>).size
              } övningar`}
          </div>
        </div>
        ...
        {matches.map((m) => (
          <div
            key={m.id}
            className="relative bg-slate-900/40 border border-white/5 p-5 rounded-3xl hover:border-indigo-500/40 transition-all group cursor-pointer overflow-hidden backdrop-blur-sm"
          >
            {/* SCORE INDICATOR */}
            <div
              className="absolute top-0 left-0 bottom-0 w-1 bg-indigo-500/20 group-hover:bg-indigo-500 transition-all"
              style={{ opacity: m.score }}
            />
            ...
            <div className="grid grid-cols-2 gap-4 mt-4 py-3 border-t border-white/5">
              ...
              <div className="flex flex-col items-end">
                ...
                >
                  {m.diffs.pace
                    ? `${m.diffs.pace < 0 ? "-" : "+"}${
                      formatPace(Math.abs(m.diffs.pace))
                    }/km`
                    : "-"}
                </div>
              </div>
