...
export function WorkoutCard({ workout, onClick }: Props) {
    ...
    return (
        ...
        >
            {/* Background Glow */}
            <div className={`absolute top-0 right-0 w-32 h-32 bg-${c}-500/5 rounded-full blur-3xl -mr-16 -mt-16 group-hover:bg-${c}-500/10 transition-colors pointer-events-none`} />

            <div className="flex justify-between items-start mb-3 relative z-10">
                <div className="flex gap-2">
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded bg-${c}-500/10 text-${c}-400 border border-${c}-500/20`}>
                        {workout.category}
                    </span>
