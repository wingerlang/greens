  ...
  const getColor = (assignedTo: string) => {
    switch (assignedTo) {
      ...
      case "PARTNER":
        return "bg-rose-500";
      case "BOTH":
        return "bg-purple-500"; // Mixed
      case "SPLIT":
        return "bg-emerald-500"; // Efficient split
      default:
        return "bg-slate-700";
    }
  };
    ...
    <div className="space-y-4 select-none">
        ...
        {trace.map((step, i) => {
          ...
          return (
            <div
              key={i}
              className={`h-full ${
                getColor(step.assignedTo)
              } relative group hover:brightness-110 transition-all`}
      ...
      <div className="flex justify-center gap-4 text-[10px] font-bold uppercase text-slate-500">
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 bg-cyan-500 rounded-full"></div> You
        </div>
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 bg-rose-500 rounded-full"></div> Partner
        </div>
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 bg-purple-500 rounded-full"></div> Both (Run)
        </div>
      </div>
    </div>
  );
