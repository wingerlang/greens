...
const STATION_TIPS: Record<HyroxStation, string> = {
  ski_erg:
    "Håll armarna raka i starten av draget (använd lats). Gå inte ut för hårt första 200m.",
  sled_push:
    "Låga höfter, raka armar. Försök att inte stanna. Små snabba steg.",
  sled_pull:
    "Gå baklänges snabbt, använd kroppsvikten. Korta armtag, ingen biceps-curl.",
  burpee_broad_jumps:
    "Falla framåt, landa mjukt. Stega fram fötterna (step in) för att spara energi. Hoppa inte för långt.",
  rowing: "Långa drag. Håll tempot nere (lägre strock rate) men kraftfullt.",
  farmers_carry:
    "Snabba steg. Lås skulderbladen. Släpp INTE vikten (straffet att plocka upp är för dyrt).",
  sandbag_lunges:
    "Vila med säcken på axlarna om du måste stanna. Byt axel halvvägs.",
  wall_balls:
    "Håll bollen högt på bröstet. Andas ut när du kastar. Använd benen!",
...
export function HyroxDashboard() {
  ...
  const [viewMode, setViewMode] = useState<
    "status" | "simulate" | "goals" | "training" | "guide" | "duo" | "pro_stats"
  >("status");
  ...
  const [selectedStation, setSelectedStation] = useState<HyroxStation | null>(
    null,
  );

  // Feature 11: Doubles Strategy
  const [doublesStrategy, setDoublesStrategy] = useState<
    Record<string, "ME" | "PARTNER" | "SPLIT">
  >({});

  const isDoubles = selectedClass.includes("DOUBLES") ||
    selectedClass === "RELAY";

  // 1. Gather Data & 5k Form
  const stats = useMemo(
    () => parseHyroxStats(exerciseEntries, strengthSessions),
    [exerciseEntries, strengthSessions],
  );
  ...
  const runnerType = useMemo(() => {
    if (!prediction) {
      return { title: "Unknown", desc: "No prediction data.", weakness: "N/A" };
    }
    const runRatio = prediction.runTotal / prediction.totalTimeSeconds;
    if (runRatio < 0.45) {
      return {
        title: "The Sled Dog 🐕",
        desc: "Station Dominant. Your engine overcomes your running.",
        weakness: "Pure Running Speed",
      };
    }
    if (runRatio > 0.55) {
      return {
        title: "The Gazelle 🦌",
        desc:
          "Running Dominant. You fly on the track but struggle with weights.",
        weakness: "Heavy Sleds",
      };
    }
    return {
      title: "The Hybrid 🤖",
      desc: "Perfectly Balanced. You are the ideal Hyrox athlete.",
      weakness: "None (Jack of all trades)",
    };
  }, [prediction]);
  ...
  const fuelPlan = useMemo(() => {
    ...
    if (totalMin > 90) {
      plan.push({
        at: "Roxzone 6 (Post-Farm)",
        what: "⚡ Caffeine / Carb Drink",
        why: "Kick inför Lunges",
      });
    }
    ...
    <div className="space-y-6 max-w-5xl mx-auto text-slate-200">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950/30 border border-white/10 rounded-3xl p-6 md:p-8 relative overflow-hidden shadow-2xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="bg-amber-500 text-slate-950 font-black px-3 py-1 rounded text-xs uppercase tracking-tighter shadow-amber-500/20 shadow-lg">
                v4.0
              </div>
              <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase">
                Hyrox <span className="text-slate-500">Predictor</span>
              </h2>
            </div>
            <div className="flex items-center gap-4 mt-4 flex-wrap">
              ...
              <div className="flex gap-2 text-[10px] text-slate-400 font-mono">
                <span className="bg-slate-950/50 px-2 py-1.5 rounded border border-white/5">
                  SP: {standards.sledPush}kg
                </span>
                <span className="bg-slate-950/50 px-2 py-1.5 rounded border border-white/5">
                  WB: {standards.wallBall}kg
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-1 opacity-75">
              Estimerad Sluttid
            </span>
            <div className="text-7xl font-black text-white tracking-tighter tabular-nums drop-shadow-xl leading-none">
            ...
            <div className="mt-4 w-48 h-8 relative">
              <svg viewBox="0 0 100 20" className="w-full h-full opacity-50">
                <path
                  d="M0,20 Q20,20 35,5 Q50,-10 65,5 Q80,20 100,20"
                  fill="none"
                  stroke="#64748b"
                  strokeWidth="1"
                />
        ...
        <div className="flex gap-2 mt-8 border-t border-white/5 pt-4 overflow-x-auto pb-1 scrollbar-none">
          ...
          <TabBtn
            label="📊 Pro Stats"
            active={viewMode === "pro_stats"}
            onClick={() => setViewMode("pro_stats")}
            color="indigo"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              ...
              <div className="bg-slate-900 border border-white/10 rounded-3xl p-6">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6">
                  The Ghost Protocol (vs Elite)
                </h3>
                <div className="space-y-4">
                  {["ski_erg", "sled_push", "burpee_broad_jumps", "wall_balls"]
                    .map((st) => (
                      <div key={st}>
                        <div className="flex justify-between text-xs font-bold mb-1 uppercase">
                          ...
                          <span className="text-slate-500">
                            Elite: {fmtSec(ELITE_SPLITS[st as HyroxStation])}
                          </span>
                        </div>
                        <div className="h-2 bg-slate-800 rounded-full overflow-hidden flex">
                          <div
                            ...
                            style={{
                              width: `${
                                (ELITE_SPLITS[st as HyroxStation] /
                                  prediction.splits[st as HyroxStation]) * 100
                              }%`,
                            }}
                          >
                          </div>
                        ...
                        <div className="text-right text-[10px] text-rose-400 font-mono mt-0.5">
                          +{fmtSec(
                            prediction.splits[st as HyroxStation] -
                              ELITE_SPLITS[st as HyroxStation],
                          )}
                        </div>
                      </div>
                ...
                <div className="relative border-l-2 border-emerald-500/20 ml-2 space-y-6">
                  {fuelPlan.map((step, i) => (
                    <div key={i} className="pl-6 relative">
                      <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-slate-900 border-2 border-emerald-500">
                      </div>
                      <div className="text-xs font-black text-emerald-500 uppercase tracking-wider mb-1">
                        {step.at}
                      </div>
        ...
        {(viewMode === "status" || viewMode === "simulate" ||
          viewMode === "goals") && (
          <>
            {/* LEFT: DATA SPLITS */}
            <div className="bg-slate-900 border border-white/5 rounded-3xl p-6 shadow-xl">
              <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6 border-b border-white/5 pb-2">
                Race Breakdown
              </h3>
              ...
              <div className="mb-6 bg-slate-950/50 p-4 rounded-xl border border-white/5">
                ...
                <div className="flex items-end h-16 gap-1">
                  {[1, 1.02, 1.05, 1.08, 1.10, 1.12, 1.15, 1.20].map((
                    mod,
                    i,
                  ) => (
                    <div
                      key={i}
                      className="flex-1 bg-indigo-500/20 hover:bg-indigo-500/40 transition-colors rounded-t-sm relative group"
                      style={{ height: `${20 + (mod - 1) * 200}%` }}
                    >
                            ...
                            <div className="absolute -right-2 top-full mt-1 flex gap-1 items-center">
                              {["ME", "PARTNER", "SPLIT"].map((type) => (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setDoublesStrategy((p) => ({
                                      ...p,
                                      [station]: type as any,
                                    }));
                                  }}
                                  className={`text-[8px] px-1.5 py-0.5 rounded border ${
                                    (doublesStrategy[station] || "SPLIT") ===
                                        type
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-slate-950/50 p-4 rounded-xl border border-white/5 space-y-4">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setRunImprovement(Number(e.target.value))}
                        className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                      />
                      <p className="text-[10px] text-slate-500 italic">
                        Simulerar bättre kondition och högre tröskel.
                      </p>
                    </div>
                    ...
                    <div className="bg-slate-950/50 p-4 rounded-xl border border-white/5 space-y-4">
                      <div className="flex justify-between items-end">
                        <label className="text-[10px] font-bold text-amber-400 uppercase tracking-widest">
                          🏋️ Stationseffektivitet
                        </label>
                        <span className="text-xs font-mono font-bold text-white">
                          {stationEfficiency}% effektivare
                        </span>
                      </div>
                      <input
                        type="range"
                        ...
                        onChange={(e) =>
                          setStationEfficiency(Number(e.target.value))}
                        className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-amber-500"
                      />
                      <p className="text-[10px] text-slate-500 italic">
                        Snabbare burpees, wallballs och utfall utan extra puls.
                      </p>
                    </div>
                    ...
                    <div className="bg-slate-950/50 p-4 rounded-xl border border-white/5 space-y-4">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setRoxzoneImprovement(Number(e.target.value))}
                        className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                      />
                      <p className="text-[10px] text-slate-500 italic">
                        Minimera "dötid" och gåvila mellan stationerna.
                      </p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl border border-amber-500/20 text-center relative overflow-hidden group">
                      ...
                      <span className="text-[10px] text-slate-400 uppercase tracking-widest block mb-2 relative z-10">
                        Simulerad Sluttid
                      </span>
                      ...
                      {(() => {
                        ...
                        const baseline = predictHyroxTime(
                          ...
                          selectedClass,
                          {
                            runGlobalImprovement: 0,
                            stationGlobalImprovement: 0,
                          },
                        )?.totalTimeSeconds || 0;
                ...
                <div className="grid gap-4">
                  <div className="bg-slate-900 p-5 rounded-2xl border border-white/5">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {/* Should differ based on station, hardcoded for now or map icon */}
                      <div>
                        <div className="font-bold text-white text-lg leading-none mb-1">
                          {prediction.strongestStation.replace(/_/g, " ")}
                        </div>
                        <p className="text-xs text-slate-400">
                          Du är 15% snabbare än snittet här.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    return (
                      <div className="grid gap-4">
                        <div className="bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/10 space-y-4">
                          <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-widest border-b border-emerald-500/10 pb-2">
                            Kravspecifikation
                          </h4>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-mono text-amber-400 font-bold max-w-[100px] text-right truncate">
                              Max {Math.round(targetRox / 60)} min
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-950 p-4 rounded-xl border border-white/5">
                          ...
                          <div className="space-y-1 text-xs">
                            <p className="text-slate-400">
                              ...
                              {" "}
                              Gå ut i {fmtSec(runPaceSec - 5)} tempo första 2km.
                            </p>
                            <p className="text-slate-400">
                              <strong className="text-white">Mitten:</strong>
                              {" "}
                              Håll {fmtSec(runPaceSec)} mellan Sled & Row.
                            </p>
                            <p className="text-slate-400">
                              <strong className="text-white">Slutet:</strong>
                              {" "}
                              Överlev lunges, töm tanken på Wall Balls.
                            </p>
                          </div>
                        </div>
                      </div>
          ...
          <div className="col-span-1 lg:col-span-2 space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="flex justify-between items-center bg-slate-900/50 p-4 rounded-2xl border border-white/5">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Sök bland alla dina övningar och Hyrox-stationer
                </p>
              </div>
              <a
                href="/exercises"
                className="px-5 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold uppercase tracking-widest transition-all"
              >
                Gå till databas
              </a>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {HYROX_WORKOUTS.map((workout) => (
                <div
                  key={workout.id}
                  className="bg-slate-900 border border-white/5 rounded-3xl p-6 hover:border-amber-500/30 transition-all group relative flex flex-col"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex gap-2">
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${
                          workout.category === "SIMULATION"
                            ? "bg-amber-500 text-black"
                            : workout.category === "COMPROMISED"
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-white/5 mb-4 space-y-1 flex-1">
                    {workout.structure.slice(0, 4).map((line, i) => (
                      <div
                        key={i}
                        className="text-[10px] text-slate-300 font-mono border-b border-white/5 last:border-0 pb-1 last:pb-0"
                      >
                        {line}
                      </div>
                    ))}
                  ...
                  <a
                    href="/pass"
                    className="w-full py-3 bg-white/5 hover:bg-amber-500 hover:text-black rounded-xl text-xs font-bold uppercase tracking-widest transition-all text-center block"
                  >
                    Visa pass
                  </a>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-slate-900 p-8 rounded-3xl border border-white/5">
                ...
                <ul className="space-y-2 text-sm text-slate-300">
                  <li>
                    🚨 <strong className="text-white">Post-Sled:</strong>{" "}
                    De första 200m kommer kännas som att du springer i lera.
                    Acceptera det. Öka farten gradvis.
                  </li>
                  <li>
                    🚨 <strong className="text-white">Post-Lunges:</strong>{" "}
                    "The Penguin Waddle". Ta korta steg. Försök inte sträcka ut
                    steget förrän efter 400m.
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-slate-900 p-8 rounded-3xl border border-white/5">
                ...
                <p className="text-slate-400 text-sm leading-relaxed mb-4">
                  Roxzone är där tävlingen förloras. Många går och dricker
                  vatten. Gör inte det.
                </p>
                <ul className="space-y-2 text-sm text-slate-300">
                  <li>
                    ...
                    Ingen gåvila i Roxzone. Joggvila är 2x snabbare än gåvila.
                  </li>
                  <li>
                    ✅ <strong className="text-emerald-400">Vatten:</strong>
                    {" "}
                    Drick ENDAST om du måste (max 2 ggr/lopp). Det kostar 10-15
                    sekunder per stopp.
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-slate-900 border border-sky-500/20 rounded-3xl p-8 relative overflow-hidden">
                ...
                <h3 className="text-xl font-black text-white mb-6 relative z-10">
                  {DEEP_TIPS.nutrition.title}
                </h3>
                <ul className="space-y-4 relative z-10">
                  {DEEP_TIPS.nutrition.points.map((p, i) => (
                    <li key={i} className="flex gap-4">
                      <span className="bg-sky-500/20 text-sky-400 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0">
                        {i + 1}
                      </span>
              ...
              <div className="bg-slate-900 border border-emerald-500/20 rounded-3xl p-8 relative overflow-hidden">
                ...
                <h3 className="text-xl font-black text-white mb-6 relative z-10">
                  {DEEP_TIPS.pacing.title}
                </h3>
                <ul className="space-y-4 relative z-10">
                  {DEEP_TIPS.pacing.points.map((p, i) => (
                    <li key={i} className="flex gap-4">
                      <span className="bg-emerald-500/20 text-emerald-400 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0">
                        {i + 1}
                      </span>
                      <p className="text-sm text-slate-300 leading-relaxed">
                        {p}
