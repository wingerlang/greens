import { PartnerStats, TeamArchetype, SplitStrategy, SimulationStep } from './DuoLabTypes.ts';

// ------------------------------------------------------------------
// CALIBRATION LOGIC
// ------------------------------------------------------------------
export function calculateLevelsFromRealStats(stats: PartnerStats): PartnerStats {
    ...
    if (stats.best5k) {
        ...
        if (parts.length === 2) {
            ...
            let lvl = 100 - ((totalSec - maxScoreSec) / (minScoreSec - maxScoreSec)) * 100;
    ...
    if (stats.wallBallsUnbroken) {
        newStats.strengthLevel = Math.min(100, Math.max(0, stats.wallBallsUnbroken));
    }
...
// ARCHETYPE DETECTION
// ------------------------------------------------------------------
export function detectArchetype(p1: PartnerStats, p2: PartnerStats): TeamArchetype {
    ...
    // Complementary?
    if ((p1.runLevel > p2.runLevel + 15 && p2.strengthLevel > p1.strengthLevel + 15) ||
    ...
    if (runDiff < 15 && strDiff < 15 && engDiff < 15 && avgRun > 60) return 'BALANCED_ASSAULT';
...
// ------------------------------------------------------------------
export function optimizeSplits(p1: PartnerStats, p2: PartnerStats, config: { transitionPenalty: number }): SplitStrategy[] {
    const stations = [
        { id: 'ski', type: 'engine' },
    ...
    return stations.map(s => {
        ...
        return {
            stationId: s.id,
            assignedTo,
            rationale: `Effective score: ${assignedTo === 'ME' ? s1Eff.toFixed(0) : s2Eff.toFixed(0)} vs ${assignedTo === 'ME' ? s2Eff.toFixed(0) : s1Eff.toFixed(0)}`,
...
export function simulateRace(p1: PartnerStats, p2: PartnerStats, archetype: TeamArchetype): SimulationResult {
    const trace: SimulationStep[] = [];
    let currentTime = 0;

    // Stations array representing a full Hyrox race
    const stations = [
        "Run 1 (1km)", "Ski Erg", "Run 2 (1km)", "Sled Push", "Run 3 (1km)", "Sled Pull",
        "Run 4 (1km)", "Burpees", "Run 5 (1km)", "Row", "Run 6 (1km)", "Farmers",
        "Run 7 (1km)", "Lunges", "Run 8 (1km)", "Wall Balls"
    ];
    ...
    stations.forEach((station) => {
        ...
        if (isRun) {
            ...
            const isStrength = station.includes("Sled") || station.includes("Lunges") || station.includes("Farmers") || station.includes("Wall");
            ...
            // Archetype Bonuses
            if (archetype === 'THUNDER_AND_LIGHTNING' && Math.abs(p1Ability - p2Ability) > 30) sectorDuration *= 0.9; // Efficient specialist usage
            if (archetype === 'THE_TOW_TRUCK' && (p1Ability > 80 || p2Ability > 80)) sectorDuration *= 0.95;
...
export function predictDoublesTime(p1: PartnerStats, p2: PartnerStats, archetype: TeamArchetype): number {
