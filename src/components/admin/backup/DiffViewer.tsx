      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Visa skillnader mellan två backups
          </p>
        </div>
        {onClose && (
          <button
          ...
          >
            ...
            {snapshots.map((s) => (
              <option key={s.id} value={s.id} disabled={s.id === toId}>
                {s.label || new Date(s.timestamp).toLocaleString("sv-SE")}{" "}
                ({backupService.formatBytes(s.size)})
              </option>
            ))}
          </select>
        </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-sm">
                          {getCategoryLabel(
                            cat.name as keyof BackupEntityCounts,
                          )}
                        </span>
                      </div>
                            ...
                            >
                              ...
                              {change.type === "MODIFIED" &&
                                ...
                                <div className="mt-2 pt-2 border-t border-current/10 space-y-1">
                                  {change.fieldChanges.slice(0, 5).map((
                                    fc,
                                    j,
                                  ) => (
                                    <div
                                    ...
                                    >
                                      <span className="opacity-60 w-20 truncate">
                                        {fc.field}:
                                      </span>
                                      <span className="line-through opacity-50">
                                        {formatFieldValue(fc.oldValue)}
                                      </span>
                                      <span>→</span>
                                      <span>
                                        {formatFieldValue(fc.newValue)}
                                      </span>
                                    </div>
                                  ))}
                                </div>
            ...
            {filteredChanges.length === 0
              ? (
                <div className="text-center py-8 text-slate-500">
                  <p className="text-sm">
                    Inga {filterType !== "all" ? filterType.toLowerCase() : ""}
                    {" "}
                    ändringar
                  </p>
                </div>
