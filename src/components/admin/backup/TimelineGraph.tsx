...
export function TimelineGraph({ snapshots, onSelectSnapshot, selectedId }: TimelineGraphProps) {
    ...
    const timelineData = useMemo(() => {
        ...
        const now = new Date();
        const days: { date: string; snapshots: BackupSnapshot[]; totalSize: number }[] = [];
        ...
        for (let i = 29; i >= 0; i--) {
            ...
            const daySnapshots = snapshots.filter(s => s.timestamp.startsWith(dateStr));
            days.push({
                date: dateStr,
    ...
    if (snapshots.length === 0) {
        return (
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-8 text-center">
                ...
                <p className="text-sm text-slate-500">Skapa några backups för att se tidslinjen</p>
            </div>
        );
    }
                ...
                <div className="flex items-end gap-1 h-32">
                    {timelineData.days.map((day, i) => {
                        const barHeight = day.snapshots.length > 0
                            ? Math.max(10, (day.snapshots.length / timelineData.maxCount) * 100)
                            : 0;
                        ...
                        return (
                            ...
                            >
                                    ...
                                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                        <div className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1 text-[10px] whitespace-nowrap shadow-xl">
                                            ...
                                            <div className="text-slate-400">
                                                {day.snapshots.length} backup{day.snapshots.length > 1 ? 's' : ''}
                                            </div>
                                ...
                                {/* Backup count bar */}
                                <div
                                    className={`w-full rounded-t transition-all duration-300 cursor-pointer ${hasSnapshots
                                            ? 'bg-indigo-500/80 hover:bg-indigo-400'
                                            : 'bg-slate-800/50'
                                        }`}
                                    style={{ height: `${barHeight}%`, minHeight: hasSnapshots ? '4px' : '2px' }}
                                    onClick={() => hasSnapshots && onSelectSnapshot(day.snapshots[0])}
                                />
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500">Kumulativ storlek över tid</p>
                    </div>
                    ...
                    <svg className="w-full h-full" preserveAspectRatio="none">
                        ...
                        {storageTrend.length > 1 && (
                            <>
                                ...
                                <path
                                    d={`
                                        M ${0} ${100 - (storageTrend[0].size / maxCumulativeSize) * 100}
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500">Backup-frekvens per dag</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-7 gap-1">
                    ...
                    {Array.from({ length: 35 }).map((_, i) => {
                        ...
                        const dateStr = d.toISOString().split('T')[0];
                        const dayData = timelineData.days.find(day => day.date === dateStr);
                ...
                <div className="space-y-2">
                    {snapshots.slice(0, 5).map((snapshot) => (
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all text-left ${selectedId === snapshot.id
                                    ? 'border-indigo-500 bg-indigo-500/10'
                                    : 'border-white/5 bg-white/[0.02] hover:bg-white/5'
                                }`}
                        >
                            <div className="flex items-center gap-3">
                                <div className={`w-2 h-2 rounded-full ${snapshot.trigger === 'MANUAL' ? 'bg-blue-500' :
                                        snapshot.trigger === 'PRE_RESTORE' ? 'bg-amber-500' :
                                            'bg-slate-500'
                                    }`} />
                                <div>
                                    <div className="text-xs font-medium text-white">
                                        {snapshot.label || new Date(snapshot.timestamp).toLocaleString('sv-SE', {
                                            month: 'short',
                                            day: 'numeric',
                                            hour: '2-digit',
                                            minute: '2-digit'
                                    ...
                                    <div className="text-[10px] text-slate-500">
                                        {snapshot.entityCounts.meals} måltider • {snapshot.entityCounts.exercises} aktiviteter
                                    </div>
                                </div>
                            </div>
