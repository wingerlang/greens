...
export function BackupModule() {
  ...
  const handleCreateBackup = async (label?: string) => {
    ...
    try {
      ...
      showNotification(
        "success",
        `Backup skapad: ${backupService.formatBytes(snapshot.size)}`,
      );
    } catch (e) {
  ...
  const confirmRestore = async (
    mode: "FULL" | "SELECTIVE",
    categories?: (keyof typeof snapshots[0]["entityCounts"])[],
  ) => {
    ...
    if (result.success) {
      showNotification(
        "success",
        `Återställning lyckades! Du måste ladda om sidan.`,
      );
      await loadSnapshots();
      ...
      showNotification(
        "error",
        result.errors?.join(", ") || "Återställning misslyckades",
      );
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Hantera snapshots och återställ data
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
          ...
          <div className="text-2xl font-black text-indigo-400">
            {backupService.formatBytes(stats?.totalSize ?? 0)}
          </div>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">
          ...
          <div className="text-sm font-bold text-white">
            {stats?.newestSnapshot
              ? new Date(stats.newestSnapshot).toLocaleDateString("sv-SE", {
                month: "short",
                day: "numeric",
                hour: "2-digit",
                minute: "2-digit",
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
          ...
          <div className="text-sm font-bold text-white">
            {stats?.oldestSnapshot
              ? new Date(stats.oldestSnapshot).toLocaleDateString("sv-SE", {
                month: "short",
                day: "numeric",
              })
              : "-"}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const updated = {
                        ...settings,
                        maxSnapshots: parseInt(e.target.value) || 100,
                      };
                      setSettings(updated);
                      backupService.saveSettings(updated);
                    }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const updated = {
                        ...settings,
                        autoBackupIntervalHours: parseInt(e.target.value) || 24,
                      };
                      setSettings(updated);
                      backupService.saveSettings(updated);
                    }}
