...
import type { BackupSnapshot, BackupTrack } from "../../../models/backup.ts";
...
export function TrackManager({ onTrackChange }: TrackManagerProps) {
  ...
  const handleCreateTrack = () => {
    ...
    try {
      ...
      const track = backupService.createTrack(
        newTrackName.trim(),
        newTrackDescription.trim() || undefined,
      );
  ...
  const handleSwitchTrack = (trackId: string) => {
    ...
    showNotification(
      "success",
      `Bytte till spår "${tracks.find((t) => t.id === trackId)?.name}"`,
    );
    onTrackChange?.();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {notification && (
        <div
          className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-xl shadow-2xl border backdrop-blur-xl transition-all duration-300 ${
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Git-liknande branching för din data
          </p>
        </div>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {track.isDefault && (
                        <span className="text-[8px] px-1.5 py-0.5 rounded bg-slate-700 text-slate-400 uppercase">
                          Standard
                        </span>
                      )}
                      {isActive && (
                        <span className="text-[8px] px-1.5 py-0.5 rounded bg-indigo-500/30 text-indigo-400 uppercase">
                          Aktiv
                        </span>
                      )}
                    </div>
                    <div className="text-[10px] text-slate-500 mt-0.5">
                      {track.description || "Ingen beskrivning"} • Skapad{" "}
                      {new Date(track.createdAt).toLocaleDateString("sv-SE")}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right mr-4">
                    ...
                    <div className="text-[9px] text-slate-500">
                      {stats?.latestDate
                        ? `Senast ${
                          new Date(stats.latestDate).toLocaleDateString("sv-SE")
                        }`
                        : "Inga backups"}
                    </div>
                  </div>
              ...
              {track.parentTrackId && (
                <div className="mt-3 pt-3 border-t border-white/5 text-[10px] text-slate-500">
                  <span className="opacity-50">↳</span> Grenad från:{" "}
                  {tracks.find((t) => t.id === track.parentTrackId)?.name ||
                    "Okänt spår"}
                </div>
              )}
            </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
          ...
          <div className="flex items-start gap-4">
            <div className="flex flex-col items-center">
              ...
              <div className="w-0.5 h-8 bg-gradient-to-b from-emerald-500 to-slate-700">
              </div>
            </div>
            <div className="flex-1 pt-0.5">
              <div className="text-xs font-bold text-white">
                {tracks.find((t) => t.isDefault)?.name || "main"}
              </div>
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3 mt-4">
              <div className="text-xs text-amber-400">
                <span className="font-bold">⚠️ OBS:</span>{" "}
                Ett nytt spår startar med en kopia av nuvarande data. En backup
                skapas automatiskt innan greningen.
              </div>
            </div>
...
function TrackCompareModal(
  { tracks, onClose }: { tracks: BackupTrack[]; onClose: () => void },
) {
  const [leftTrackId, setLeftTrackId] = useState(tracks[0]?.id || "");
  const [rightTrackId, setRightTrackId] = useState(
    tracks[1]?.id || tracks[0]?.id || "",
  );

  const leftSnapshots = useMemo(() => backupService.getSnapshots(leftTrackId), [
    leftTrackId,
  ]);
  const rightSnapshots = useMemo(
    () => backupService.getSnapshots(rightTrackId),
    [rightTrackId],
  );
  ...
  const comparison = useMemo(() => {
    ...
    const categories = Object.keys(
      leftLatest.entityCounts,
    ) as (keyof typeof leftLatest.entityCounts)[];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-slate-900 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
              ...
              <div className="rounded-xl border border-white/10 overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-slate-800/50">
                    <tr>
                      <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase">
                        Kategori
                      </th>
                      <th className="px-4 py-2 text-right text-[10px] font-bold text-slate-500 uppercase">
                        {tracks.find((t) => t.id === leftTrackId)?.name}
                      </th>
                      <th className="px-4 py-2 text-right text-[10px] font-bold text-slate-500 uppercase">
                        {tracks.find((t) => t.id === rightTrackId)?.name}
                      </th>
                      <th className="px-4 py-2 text-right text-[10px] font-bold text-slate-500 uppercase">
                        Skillnad
                      </th>
          ...
          {/* Future: Merge button */}
          {leftTrackId !== rightTrackId && comparison &&
            comparison.length > 0 && (
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
              <div className="flex items-center justify-between">
                <div>
