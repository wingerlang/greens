...
export function SessionsModule() {
  ...
  if (loading && sessions.length === 0) {
    return (
      <div className="p-8 text-center text-gray-500">Laddar sessioner...</div>
    );
  }
    ...
    return (
      <div className="p-8 text-center">
        <div className="text-red-400 mb-2">
          Ett fel inträffade vid laddning av sessioner
        </div>
        <div className="text-xs font-mono bg-slate-900 inline-block px-2 py-1 rounded text-red-300">
          {error}
        </div>
        <button
          onClick={fetchData}
      ...
      <section className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6 md:p-8 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold flex items-center gap-2 text-white">
            ...
            Aktiva Sessioner
            <span className="text-xs bg-slate-800 px-2 py-1 rounded-full text-gray-400">
              {sessions.length}
            </span>
          </h2>
          <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="text-gray-500 border-b border-slate-800">
              <tr>
                <th className="px-4 py-3 font-semibold text-[10px] uppercase">
                  Senast Aktiv
                </th>
                <th className="px-4 py-3 font-semibold text-[10px] uppercase">
                  Användare
            ...
            <tbody className="divide-y divide-slate-800/50">
              {sessions.map((s) => {
                const isFresh =
                  (Date.now() - new Date(s.lastSeen).getTime()) < 60000; // < 1 min
                return (
                  <tr
                    key={s.id}
                    className={`hover:bg-white/[0.02] transition-colors ${
                      isFresh ? "bg-emerald-500/5" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-3 text-xs">
                      {s.username
                        ? (
                          <span className="font-bold text-white bg-blue-500/20 px-2 py-0.5 rounded-full">
                            {s.username}
                          </span>
                        )
                    ...
                    <td className="px-4 py-3">
                      {s.errorCount > 0
                        ? (
                          <span className="text-red-400 font-bold bg-red-500/10 px-2 py-1 rounded-full text-xs">
                            {s.errorCount} fel
                          </span>
                        )
                        : (
          ...
          {errors.length === 0
            ? (
              <div className="text-center py-8 text-gray-500 text-sm">
                Inga fel rapporterade än. (Hoppas det förblir så!)
              </div>
            )
            : (
              errors.map((err, i) => (
                <div
                  key={i}
                  className="bg-slate-900 border border-red-900/30 p-4 rounded-xl space-y-2"
                >
                  <div className="flex justify-between items-start">
                    <h3 className="text-red-400 font-bold text-sm font-mono break-all">
                      {err.message}
                    </h3>
