...
export const HealthModule: React.FC = () => {
    ...
    if (loading && !system) {
        return <div className="p-8 text-center text-gray-500 animate-pulse">Laddar systemstatus...</div>;
    }
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
                    <div className="text-xs text-gray-500 font-bold uppercase mb-1">Uptime</div>
                    <div className="text-xl font-mono text-emerald-400">{system?.uptime || '-'}</div>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
                    <div className="text-xs text-gray-500 font-bold uppercase mb-1">RAM (RSS)</div>
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="font-bold text-gray-300">Responstid (ms) - Senaste 50 förfrågningarna</h3>
                    <div className="text-xs font-mono text-gray-500">
                        Avg: <span className="text-emerald-400">{Math.round(metrics?.avgResponseTime || 0)}ms</span>
                    </div>
                </div>
                <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={metrics?.recentResponseTimes ? [...metrics.recentResponseTimes].reverse() : []}>
                ...
                <div className="max-h-96 overflow-y-auto">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-slate-800/50 text-gray-500 sticky top-0">
                            <tr>
                                <th className="px-6 py-3 font-semibold uppercase tracking-wider text-[10px]">Tid</th>
                                <th className="px-6 py-3 font-semibold uppercase tracking-wider text-[10px]">Meddelande</th>
                                <th className="px-6 py-3 font-semibold uppercase tracking-wider text-[10px]">Path</th>
                            </tr>
                        </thead>
                                ...
                                logs.map(log => (
                                    <tr key={log.id} className="hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-red-400 font-medium truncate max-w-md" title={log.message}>
                                                {log.message}
                                            </div>
                                            {log.stack && (
