            ...
            {apiUsers.map((user) => (
              <tr
                ...
                className={`hover:bg-white/[0.02] transition-colors ${
                  authUser?.id === user.id ? "bg-blue-500/5" : ""
                }`}
              >
                ...
                <td className="px-6 py-4">
                  <select
                    ...
                    className={`text-[10px] px-2 py-1 rounded-lg uppercase tracking-widest font-bold border-none outline-none cursor-pointer ${
                      user.role === "admin"
                  ...
                  {authUser?.id !== user.id && (
                    <button
                      onClick={async () => {
                        if (
                          !confirm(
                            `Är du HELT SÄKER på att du vill ta bort användaren "${user.username}" och ALL deras data? Detta kan INTE ångras!`,
                          )
                        ) return;
                        ...
                        try {
                          ...
                          if (res.ok) {
                            ...
                            alert(
                              `✅ Användaren ${user.username} har tagits bort.`,
                            );
                          } else {
