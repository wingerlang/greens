...
export function ExerciseMapperModule({ unmapped, onSave }: ExerciseMapperModuleProps) {
    ...
    if (unmapped.length === 0) {
        return (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-6 text-center">
                <p className="text-emerald-400 font-medium">✨ Alla övningar är mappade!</p>
                <p className="text-slate-400 text-sm mt-1">Systemet kan analysera all din data korrekt.</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
            ...
            <div className="max-h-96 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                {unmapped.map(name => (
                    <div key={name} className="flex items-center justify-between gap-4 bg-slate-800/50 p-3 rounded-lg border border-white/5">
                        <span className="text-sm font-medium text-slate-200 truncate flex-1" title={name}>
                            {name}
                        </span>
                        ...
                        <select
                            ...
                            onChange={(e) => handleAssign(name, e.target.value as MuscleGroup)}
                        >
