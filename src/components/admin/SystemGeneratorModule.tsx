...
export function SystemGeneratorModule() {
  ...
  const generateMyData = async () => {
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      const hasMealsForDay = mealEntries.some((entry) =>
        entry.date === dateStr
      );
      ...
      const shouldGenerateForDay = !hasMealsForDay ||
        (generatedDays < MAX_DAYS_TO_GENERATE && Math.random() < 0.5);
      ...
      if (shouldGenerateForDay) {
        ...
        if (Math.random() > 0.6) {
          ...
          addExercise({
            ...
            intensity: (["low", "moderate", "high"] as ExerciseIntensity[])[
    ...
    addLog(
      `✅ Klar! Skapade ${countMeals} måltider och ${countWorkout} träningspass över ${generatedDays} dagar.`,
    );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <section className="bg-slate-900/50 rounded-3xl border border-slate-800 p-8 shadow-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">
              System & Datagenerering
            </h2>
            <p className="text-slate-400 text-sm">
              Verktyg för att populera databasen och stresstesta systemet.
            </p>
          </div>
        </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-bold text-emerald-400 mb-1">
                  Generera Min Historik
                </h3>
                <p className="text-xs text-slate-400">
                  Skapar 30 dagars måltids- och träningshistorik slumpmässigt.
                </p>
              </div>
          ...
          <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Lägger till 5 fiktiva användare för att testa tävlingar.
                </p>
              </div>
              <span className="text-2xl">🤖</span>
            </div>
        ...
        <div className="mt-8 bg-slate-950 rounded-xl p-4 font-mono text-xs h-40 overflow-y-auto border border-slate-800">
          <div className="text-slate-500 mb-2 uppercase tracking-widest font-bold">
            System Log
          </div>
          {log.length === 0 && (
          ...
          {log.map((line, i) => (
            <div
              key={i}
              className="text-emerald-400/80 mb-1 border-l-2 border-emerald-500/20 pl-2"
            >
        ...
        <div className="mt-8 pt-8 border-t border-red-500/20">
          <h3 className="text-red-500 font-bold mb-4 uppercase tracking-widest text-xs">
            Danger Zone
          </h3>
          <button
