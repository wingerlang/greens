...
const TreeNode: React.FC<{ node: MuscleNode; depth: number }> = ({ node, depth }) => {
    return (
        <div className="ml-4">
            <div className={`flex items-center py-1 ${node.isLeaf ? 'text-gray-600' : 'font-medium text-gray-900'}`}>
...
export const MuscleTree: React.FC<MuscleTreeProps> = ({ hierarchy }) => {
    if (!hierarchy) return <div className="text-sm text-gray-500">No hierarchy data.</div>;

    return (
        <div className="space-y-6">
            {hierarchy.categories.map(cat => (
                <div key={cat.id}>
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{cat.name}</h4>
