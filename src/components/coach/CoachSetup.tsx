...
export function CoachSetup() {
  ...
  const [currentTimeStr, setCurrentTimeStr] = useState(
    formatSeconds(profile.currentForm?.timeSeconds || 0),
  );
  const [newGoalTimeStr, setNewGoalTimeStr] = useState(
    formatSeconds(newGoal.targetTimeSeconds || 0),
  );
  ...
  const handleSaveProfile = () => {
    ...
    const updatedProfile = {
      ...profile,
      currentForm: { ...profile.currentForm!, timeSeconds: time },
    };
    ...
    <div className="coach-setup max-w-5xl mx-auto space-y-6 pb-12 text-white animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <section className="glass-card p-5 bg-gradient-to-br from-blue-500/10 via-blue-500/5 to-transparent border-blue-500/20">
        <div className="flex items-center gap-4 mb-5">
          <div className="w-10 h-10 rounded-xl bg-blue-500/20 text-blue-400 flex items-center justify-center text-xl shadow-lg shadow-blue-500/10">
            🚀
          </div>
          <div>
            ...
            <p className="text-[8px] text-slate-500 font-black uppercase tracking-[0.2em] opacity-80">
              Dina fysiologiska förutsättningar
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end mt-6 gap-4">
          {savedProfile && (
            <span className="text-emerald-400 font-black uppercase text-[10px] animate-bounce">
              ✓ Sparat!
            </span>
          )}
          <button
            onClick={handleSaveProfile}
            className="px-6 py-2.5 bg-blue-500/10 text-blue-400 font-black rounded-xl hover:bg-blue-500/20 transition-all uppercase tracking-widest text-[10px] border border-blue-500/20 shadow-xl shadow-blue-500/5"
          >
            Spara Profil
          </button>
        </div>
      ...
      <section className="glass-card p-5 bg-gradient-to-br from-emerald-500/5 to-transparent border-emerald-500/10">
        <div className="flex items-center gap-4 mb-5">
          <div className="w-10 h-10 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center text-xl shadow-lg shadow-emerald-500/10">
            🎯
          </div>
          <div>
            <h3 className="text-lg font-black italic uppercase tracking-tighter">
            ...
            >
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h4 className="font-black text-xl tracking-tight uppercase italic">
                    {goal.type}
                  </h4>
                  {goal.isActive && (
                    <span className="px-2 py-0.5 bg-emerald-500 text-slate-950 text-[9px] font-black rounded-full uppercase tracking-widest shadow-lg shadow-emerald-500/20 animate-pulse">
                      Aktiv
                    </span>
                  )}
                </div>
                <p className="text-sm text-slate-400 font-medium tracking-wide">
                  ...
                  Måltid:{" "}
                  <span className="text-emerald-400/80 font-black">
                    {formatSeconds(goal.targetTimeSeconds || 0)}
                  </span>
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => deleteCoachGoal(goal.id)}
        ...
        <div
          className={`p-5 rounded-2xl bg-slate-950/40 border-2 border-dashed transition-all ${
            savedGoal
        ...
        >
          ...
          <div className="flex items-center justify-end mt-4 gap-3">
            ...
            <button
              ...
              className="px-5 py-2 bg-emerald-500 text-slate-950 font-black rounded-lg hover:bg-emerald-400 transition-all uppercase tracking-widest text-[9px] shadow-lg shadow-emerald-500/20 active:scale-95"
            >
              Spara Mål
            </button>
          </div>
      ...
      <section className="glass-card p-4">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-lg bg-indigo-500/20 text-indigo-400 flex items-center justify-center text-lg">
            📅
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setPrefs({ ...prefs, weeklyVolumeKm: Number(e.target.value) })}
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setPrefs({ ...prefs, weightGoal: Number(e.target.value) })}
              className="w-full bg-slate-900/50 border border-white/5 rounded-2xl p-4 text-white font-black focus:border-emerald-500/50 outline-none transition-all"
            />
          </div>
        </div>
        <div className="flex items-center justify-start mt-4 gap-3">
          <button
            type="button"
            onClick={() => {
              updateCoachConfig({ preferences: prefs });
              setSavedPrefs(true);
              setTimeout(() => setSavedPrefs(false), 2000);
            }}
            className="px-5 py-2 bg-indigo-500/20 text-indigo-400 font-bold rounded-lg hover:bg-indigo-500/30 transition-all uppercase tracking-widest text-[9px] border border-indigo-500/20"
          >
            Spara Preferenser
          </button>
          {savedPrefs && (
      ...
      <div className="flex flex-col items-center gap-2 pt-2">
        {!activeGoal && (
          <p className="text-[9px] text-rose-400 font-black uppercase tracking-widest italic animate-pulse mb-1">
            ⚠️ Välj ett mål ovan först
          </p>
        )}
        <button
          type="button"
        ...
        {activeGoal && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest italic">
            Mål: {activeGoal.type} ({activeGoal.targetDate})
          </p>
        )}
      </div>
    </div>
