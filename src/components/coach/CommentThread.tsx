...
export function CommentThread({
    ...
    const rootComments = comments.filter(c => !c.parentId && c.targetType === targetType && c.targetId === targetId);
    ...
    const CommentCard = ({ comment, depth = 0 }: { comment: Comment; depth?: number }) => {
        ...
        return (
            <div className={`${depth > 0 ? 'ml-6 pl-4 border-l-2 border-white/5' : ''}`}>
                <div className={`p-3 rounded-xl mb-2 ${comment.authorRole === 'coach' ? 'bg-amber-500/5 border border-amber-500/10' : 'bg-indigo-500/5 border border-indigo-500/10'}`}>
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${comment.authorRole === 'coach' ? 'bg-amber-500/20' : 'bg-indigo-500/20'}`}>
                                {comment.authorRole === 'coach' ? '👨‍🏫' : '🏃'}
                            </span>
                            <span className={`text-[10px] font-black uppercase ${comment.authorRole === 'coach' ? 'text-amber-400' : 'text-indigo-400'}`}>
                                {comment.authorName}
                            </span>
                            <span className="text-[9px] text-slate-600">{formatDate(comment.createdAt)}</span>
                            {comment.isEdited && <span className="text-[8px] text-slate-600 italic">(redigerad)</span>}
                        </div>
                        <div className="flex items-center gap-2">
                            {isOwn && onDeleteComment && (
                                <button onClick={() => onDeleteComment(comment.id)} className="text-[9px] text-slate-600 hover:text-rose-400 transition-colors">
                                    🗑️
                                </button>
                            )}
                        </div>
                    ...
                    {comment.reactions && comment.reactions.length > 0 && (
                        <div className="flex gap-1 mt-2">
                            {comment.reactions.map((r, i) => (
                                <span key={i} className="px-1.5 py-0.5 bg-slate-800 rounded-md text-[10px]">
                                    {r.emoji} {r.count}
                                </span>
                            ))}
                        </div>
        ...
        <div className="comment-thread space-y-4">
            ...
            <div className="space-y-2">
                {rootComments.length === 0 ? (
                    <p className="text-center text-slate-600 text-sm py-6">Inga kommentarer ännu. Var först!</p>
                ) : (
                    rootComments.map(comment => <CommentCard key={comment.id} comment={comment} />)
                )}
            </div>
        </div>
    );
