...
const SAMPLE_ENTRIES: LeaderboardEntry[] = [
  {
    ...
    prCount: 5,
    completionRate: 98,
    badges: [{ type: "volume_king", name: "Marsmaskin", icon: "👑" }],
    lastActiveDate: "2024-03-23",
  },
  {
    userId: "2",
    userName: "Erik Uthållig",
    rank: 2,
    weeklyVolumeKm: 72,
    monthlyVolumeKm: 285,
    currentStreak: 45,
    prCount: 3,
    completionRate: 100,
    badges: [{ type: "streak_master", name: "Strävare", icon: "🔥" }],
    lastActiveDate: "2024-03-23",
  },
  {
    userId: "3",
    ...
    prCount: 8,
    completionRate: 94,
    badges: [{ type: "speed_demon", name: "Fartfantom", icon: "⚡" }],
    lastActiveDate: "2024-03-22",
  },
  {
    userId: "4",
    userName: "Magnus Marathon",
    rank: 4,
    weeklyVolumeKm: 58,
    monthlyVolumeKm: 220,
    currentStreak: 8,
...
export function LeaderboardWidget(
  { entries = SAMPLE_ENTRIES, currentUserId, compact = false }:
  ...
  const getRankStyle = (rank: number) => {
    if (rank === 1) {
      return "bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-950";
    }
    if (rank === 2) {
      return "bg-gradient-to-r from-slate-400 to-slate-300 text-slate-950";
    }
    if (rank === 3) {
    ...
    <div className="leaderboard-widget text-white">
      ...
      <div className="space-y-2">
        {sortedEntries.map((entry) => {
          ...
          const value = view === "weekly"
            ? entry.weeklyVolumeKm
            : view === "monthly"
            ? entry.monthlyVolumeKm
            : entry.currentStreak;
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {isMe && (
                      <span className="text-[8px] bg-indigo-500 text-white px-1.5 py-0.5 rounded-full font-bold uppercase">
                        Du
                      </span>
                    )}
