...
export function SignalTimelineChart(
  { activities, showOptimalWindow = true }: Props,
) {
  ...
  const getInterferenceZones = () => {
    const zones: {
      ...
      type: "interference" | "suboptimal";
    }[] = [];

    const mtorActs = sorted.filter((a) => a.signal === "MTOR");
    const ampkActs = sorted.filter((a) =>
      a.signal === "AMPK_HIGH" || a.signal === "HYBRID"
    );
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 space-y-4">
      ...
      <div className="relative h-32">
        ...
        {interferenceZones.map((zone, i) => (
          ...
          >
            <div
              className={`absolute -top-1 left-1/2 -translate-x-1/2 text-[9px] font-bold uppercase px-1 rounded ${
                zone.type === "interference"
        ...
        {sorted.filter((a) => a.signal === "MTOR").map((act, i) => (
          <div
            ...
            style={{
              ...
              background:
                "linear-gradient(90deg, rgba(16, 185, 129, 0.6) 0%, rgba(16, 185, 129, 0) 100%)",
              top: "10%",
            }}
          />
        ))}
          ...
          >
            ...
            <div
              className={`w-4 h-4 rounded-full ${
                getSignalColor(act.signal)
              } ring-2 ring-white dark:ring-slate-900 shadow-lg ${
                act.source === "PLAN" ? "ring-dashed opacity-70" : ""
              }`}
            />
