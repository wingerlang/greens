...
import { MealEntry, MealType, MEAL_TYPE_LABELS, FoodItem, Recipe } from '../../models/types.ts';
...
export function MacroDistribution({ entries, foodItems, recipes }: MacroDistributionProps) {
    ...
    const totalProtein = Object.values(distribution).reduce((sum, s) => sum + s.protein, 0);
    const morningProtein = distribution.breakfast.protein;
    const proteinTimingScore = totalProtein > 0 ? (morningProtein / totalProtein) * 100 : 0;
