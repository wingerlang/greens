...
function isLikelyUncooked(name: string): boolean {
  ...
  // Uncooked indicators
  if (
    lower.includes("okokt") || lower.includes("torr") || lower.includes("rå")
  ) {
    return true;
  }
  // Cooked indicators = NOT uncooked
  if (
    lower.includes("kokt") || lower.includes("tillagad") ||
    lower.includes("stekt")
  ) {
    return false;
  }
  return false;
          ...
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-tight">
              <span className="text-emerald-500">
                {Math.round(totals.calories * pieces)} kcal
              </span>
              <span className="text-rose-400">
                {Math.round(totals.protein * pieces)}g P
              </span>
            </div>
          </div>
...
export function QuickAddModal({
  ...
  const [hoveredSnabbvalId, setHoveredSnabbvalId] = useState<string | null>(
    null,
  );
  const [hoveredIngredientIndex, setHoveredIngredientIndex] = useState<
    number | null
  >(null);
  ...
  const filteredQuickMeals = useMemo(() => {
    ...
    return base.filter((qm) =>
      qm.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const renderResultItem = (result: SearchResult, keyPrefix: string) => {
    ...
    const { canCook: showCookedToggle, effectiveYieldFactor } = canLogAsCooked(
      result,
    );
      ...
      >
        ...
        <div className="result-actions">
          ...
          {showCookedToggle && (
            <button
              type="button"
              className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm transition-all mr-2 ${
                isCooked
          ...
          <div className="portion-control">
            <button
              className="btn-portion"
              onClick={() =>
                setQuickAddServings(Math.max(0.25, quickAddServings - 0.25))}
            >
              −
            </button>
            ...
            <div>
              <div
                className={`text-sm font-bold ${
                  isToday ? "text-slate-400" : "text-amber-400"
                }`}
              >
          ...
          <div className="search-results proposals-results mb-4">
            ...
            {filteredQuickMeals.map((qm) => (
              <QuickMealRow
                ...
                highlightedIndex={hoveredSnabbvalId === qm.id
