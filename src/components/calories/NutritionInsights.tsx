...
export function NutritionInsights({ onDateSelect }: NutritionInsightsProps) {
  const { mealEntries, recipes, foodItems, calculateDailyNutrition } =
    useData();
  ...
  const last7Days = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      days.push({
        ...
        label: i === 0
          ? "Idag"
          : new Date(dateStr).toLocaleDateString("sv-SE", { weekday: "short" }),
  ...
  const completeDays = last7Days.filter((d: any) =>
    !settings.incompleteDays?.[d.date]
  );

  // Safety check to avoid division by zero
  const divisor = completeDays.length || 1;

  const calorieAvegare = Math.round(
    completeDays.reduce((acc: number, d: any) => acc + d.calories, 0) / divisor,
  );
  const proteinAverage = Math.round(
    completeDays.reduce((acc: number, d: any) => acc + d.protein, 0) /
      divisor * 10,
  ) / 10;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="trend-card">
          <div className="flex justify-between items-end mb-2">
            <div>
              <span className="text-xs text-slate-500 block">
                Kaloritrend ({completeDays.length} dgr)
              </span>
          ...
          <svg
            ...
            className="w-full h-24 overflow-visible group/cal"
          >
            {/* Goal line */}
            <line
              x1="0"
              y1={chartHeight - (calorieGoal / calScale) * chartHeight}
              x2={chartWidth}
              y2={chartHeight - (calorieGoal / calScale) * chartHeight}
              stroke="#334155"
              strokeDasharray="4 2"
            />
              ...
              return (
                ...
                >
                  ...
                  <text
                    ...
                    y={chartHeight + 12}
                    textAnchor="middle"
                    className={`text-[8px] uppercase tracking-tighter transition-colors ${
                      isToday ? "fill-emerald-400 font-bold" : "fill-slate-500"
                    } group-hover/bar:fill-white`}
                  >
                    {day.label}
                  </text>
        ...
        <div className="trend-card">
          <div className="flex justify-between items-end mb-2">
            <div>
              <span className="text-xs text-slate-500 block">
                Proteintrend ({completeDays.length} dgr)
              </span>
          ...
          <svg
            viewBox={`0 0 ${chartWidth} ${chartHeight}`}
            className="w-full h-24 overflow-visible group/prot"
          >
            {/* Goal line */}
            <line
              x1="0"
              y1={chartHeight - (proteinGoal / protScale) * chartHeight}
              x2={chartWidth}
              y2={chartHeight - (proteinGoal / protScale) * chartHeight}
              stroke="#334155"
              strokeDasharray="4 2"
            />
            {last7Days.map((day: any, i: number) => {
              ...
              return (
                ...
                >
                  ...
                  <text
                    ...
                    textAnchor="middle"
                    className={`text-[8px] uppercase tracking-tighter transition-colors ${
                      isToday ? "fill-violet-400 font-bold" : "fill-slate-500"
                    } group-hover/bar:fill-white`}
                  >
                    {day.label}
                  </text>
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/30">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Kombinera <span className="text-amber-400">baljväxter</span> med
            {" "}
            <span className="text-sky-400">spannmål</span>{" "}
            för att få i dig alla essentiella aminosyror!
          </p>
        </div>
        <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/30">
          <span className="text-[10px] text-slate-500 uppercase font-bold block mb-1">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Baserat på veckan ligger du på{" "}
            <span className="text-emerald-400">
              {Math.round((calorieAvegare / calorieGoal) * 100)}%
            </span>{" "}
            av ditt kaloriemål.
            {calorieAvegare < calorieGoal
              ? " Ett litet överskott kan behövas för muskelbygge."
              : " Snyggt jobbat!"}
          </p>
        </div>
        <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-700/30">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Har du ätit <span className="text-rose-400">broccoli</span> eller
            {" "}
            <span className="text-rose-400">spenat</span>{" "}
            idag? Bra källor till både järn och kalcium för veganer.
          </p>
        </div>
      </div>
    </div>
