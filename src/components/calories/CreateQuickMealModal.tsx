...
import { X, Zap, ChevronRight, History } from 'lucide-react';
            ...
            >
                ...
                <div className="relative p-6 border-b border-slate-800/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-white tracking-tight">Spara som Snabbval</h2>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Skapa en återanvändbar kombination</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="bg-slate-800/30 border border-slate-800 rounded-2xl p-4">
                        <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-800/50">
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Innehåll</span>
                            <div className="flex items-center gap-3 text-[10px] font-black">
                                <span className="text-emerald-500">{Math.round(totals.calories)} kcal</span>
                                <span className="text-rose-400">{Math.round(totals.protein)}g P</span>
                            </div>
                        </div>
                        <div className="space-y-2">
                            {items.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between text-xs">
                                    <span className="text-slate-300 font-medium truncate">{getItemName(item)}</span>
                    ...
                    {/* Input Field */}
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-emerald-500 uppercase tracking-widest ml-1">Namn på snabbvalet</label>
                        <input
                            autoFocus
                            type="text"
                            value={name}
                                ...
                                {recentQuickMeals.slice(0, 3).map(qm => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-xs font-bold text-slate-300 group-hover:text-white transition-colors">{qm.name}</span>
                                        </div>
                                        <ChevronRight size={14} className="text-slate-600 group-hover:text-emerald-500 transition-all translate-x-0 group-hover:translate-x-1" />
                                    </div>
                                ))}
                            </div>
                        </div>
