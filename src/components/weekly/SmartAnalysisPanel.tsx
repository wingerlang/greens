...
import { WEEKDAYS, type Weekday } from '../../models/types.ts';
...
const SmartAnalysisPanel: React.FC<SmartAnalysisPanelProps> = ({ analysis, onOptimize }) => {
    const {
        score,
        proteinScore,
        varietyScore,
        ...
        <div className="bg-[#1a1c1e] rounded-2xl p-6 border border-white/5 shadow-xl transition-all duration-300 hover:border-emerald-500/20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div>
                    ...
                    <p className="text-gray-400 mt-1">Optimerad för vegansk näring & hälsa</p>
                </div>

                <div className="flex items-center gap-4">
                    <div className={`px-6 py-4 rounded-xl ${getScoreBg(score)} border border-white/5 flex flex-col items-center min-w-[120px]`}>
                        <span className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Greens Score</span>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex gap-1">
                                    ...
                                    <StatusDot active={da.tags.includes('budget-win')} icon="💰" label="Budget" />
                                </div>
                            </div>
                        );
                    })}
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {/* General Tips */}
                    {tips.map((tip, i) => (
                        <div key={`tip-${i}`} className="p-3 bg-white/5 border border-white/5 rounded-lg text-gray-300 text-sm leading-relaxed flex gap-3">
                    ...
                    {WEEKDAYS.map(day => {
                        ...
                        const tips = da.tips.map((tip, i) => (
                            <div key={`day-tip-${day}-${i}`} className="p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-lg text-emerald-100/90 text-sm leading-relaxed flex gap-3">
                                <span className="text-emerald-500 font-bold mt-0.5">{DAY_ABBREV[day]}</span>
                                {tip}
                            </div>
                        ));

                        const synergies = da.synergies.map((syn, i) => (
                            <div key={`syn-${day}-${i}`} className="p-3 bg-cyan-500/5 border border-cyan-500/10 rounded-lg text-cyan-50 text-sm leading-relaxed flex gap-3">
                                <span className="text-cyan-400 font-bold">{syn.icon}</span>
                                <div>
                                    <span className="font-bold block text-cyan-300">{syn.name}</span>
                                    {syn.description}
                                </div>
                            </div>
                        ));
...
const ScoreCard: React.FC<ScoreCardProps> = ({ label, score, icon }) => {
    ...
    return (
        <div className="bg-white/5 border border-white/5 p-4 rounded-xl">
            ...
            <div className="text-xs text-gray-400 font-medium mb-3 truncate">{label}</div>
...
const StatusDot: React.FC<{ active: boolean; icon: string; label: string }> = ({ active, icon, label }) => (
    <span
        title={active ? label : `Saknar ${label.toLowerCase()}`}
        className={`text-[10px] transition-all duration-500 ${active ? 'opacity-100 scale-100 filter-none' : 'opacity-20 scale-90 grayscale'}`}
    >
        {icon}
    </span>
);
