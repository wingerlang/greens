...
export function WeeklyDistanceChart(
  { activities, fixedYear, fixedDateRange }: WeeklyDistanceChartProps,
) {
  const [range, setRange] = useState<"3m" | "6m" | "12m" | "year" | "all">(
    fixedYear || fixedDateRange ? "year" : "12m",
  );
  ...
  if (weeklyData.length === 0) {
    return <p className="text-slate-500">Ingen data för den valda perioden.</p>;
  }
    ...
    <div className="space-y-6">
            ...
            if (indices.length === 0) {
              yearMarkers.forEach((ym) => {
                items.push(
                  <div
                    key={`year-${ym.year}`}
                    className="flex-shrink-0 w-[1px] h-full bg-emerald-500/20 relative mx-0.5"
                  >
                    <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-emerald-500/50 px-1 bg-emerald-500/10 rounded">
                      {ym.year}
                    </span>
                  </div>,
                );
                  ...
                  {yearMarkers.map((ym) => {
                    ...
                    return (
                      <div
                        ...
                        className="absolute top-0 bottom-0 w-[1px] bg-emerald-500/20 z-0"
                        style={{ left: `${leftPos}%` }}
                      >
                        <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-emerald-500/30 px-1 bg-emerald-500/5 rounded">
                          {ym.year}
                        </span>
                      </div>
                    );
                ...
                items.push(
                  ...
                  >
                    ...
                    <div className="h-4 flex items-center justify-center">
                      <span className="text-[10px] text-slate-800 font-bold group-hover/cross:text-slate-700 transition-colors">
                        ·
                      </span>
                    </div>
                  </div>,
          ...
          weeklyData.forEach(({ week, volume, rollingAvg }, i) => {
            ...
            if (volume === 0 && !isCurrentWeek) {
              ...
              if (isGold) {
                ...
                barColorClass =
                  "bg-gradient-to-t from-emerald-400 to-white animate-pulse";
              }
      ...
      <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase tracking-widest pt-4 border-t border-white/5">
        <div className="flex items-center gap-3">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-0.5 bg-emerald-500/50 border-t border-dashed border-emerald-400">
            </span>
            Snitt (4v)
          </span>
        </div>
