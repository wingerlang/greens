...
export function WeeklyDistanceChart({ activities, fixedYear }: WeeklyDistanceChartProps) {
    ...
    }, [weeklyData, range, activities, layoutTick]);
    ...
    if (weeklyData.length === 0) return <p className="text-slate-500">Ingen data för den valda perioden.</p>;
        ...
        <div className="space-y-6">
                ...
                <div className="flex gap-2 mb-2">
                    {[
                        ...
                        <button
                            ...
                            className={`text-[9px] font-black uppercase px-3 py-1.5 rounded-lg border transition-all ${range === p.id ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-slate-950 border-white/5 text-slate-500 hover:border-white/10'}`}
                        >
                            {p.label}
                        </button>
                    ))}
                ...
                {(() => {
                    ...
                    const renderGap = (indices: number[], yearMarkers: { index: number, year: number }[]) => {
                        if (indices.length === 0) {
                            yearMarkers.forEach(ym => {
                                items.push(
                                    <div key={`year-${ym.year}`} className="flex-shrink-0 w-[1px] h-full bg-emerald-500/20 relative mx-0.5">
                                        <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-emerald-500/50 px-1 bg-emerald-500/10 rounded">{ym.year}</span>
                                    </div>
                                );
                            });
                            return;
                    ...
                    weeklyData.forEach(({ week, volume, rollingAvg }, i) => {
                        ...
                        if (volume === 0) {
                            ...
                            } else if (isCurrentWeek && !fixedYear) {
                                barColorClass = "bg-gradient-to-t from-emerald-400 to-white animate-pulse";
                            }
                            ...
                            items.push(
                                <div key={`bar-${i}`} className="flex-1 min-w-[4px] max-w-[40px] flex flex-col items-center h-full justify-end group relative z-10 transition-all hover:z-50">
                                    {/* Tooltip */}
                                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-200 absolute bottom-[calc(100%+8px)] left-1/2 -translate-x-1/2 bg-slate-900 border border-white/10 p-3 rounded-xl shadow-2xl pointer-events-none whitespace-nowrap min-w-[120px] scale-95 group-hover:scale-100 origin-bottom z-50">
                                        ...
                                        <div className="flex items-baseline gap-1">
                                            <p className="font-black text-white text-2xl leading-none">{volume.toLocaleString('sv-SE', { maximumFractionDigits: 1 })}</p>
                                        ...
                                        {rollingAvg > 0 && (
                                            <div className="mt-1 pt-1 border-t border-white/5 flex items-center justify-between text-[10px]">
                                                ...
                                                <span className="font-mono text-emerald-400">{rollingAvg.toFixed(1)}km</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    >
                                        <div className="opacity-0 group-hover/trend:opacity-100 transition-opacity absolute bottom-full mb-4 bg-slate-900 border border-emerald-500/30 p-2 rounded-lg shadow-2xl z-50 pointer-events-none whitespace-nowrap">
                                            <p className="text-[10px] font-black text-emerald-400">Trend: {rollingAvg.toLocaleString('sv-SE', { maximumFractionDigits: 1 })}km</p>
                                            <p className="text-[8px] text-slate-500 font-bold">{fullLabel}</p>
                                        </div>
                                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 opacity-0 group-hover/trend:opacity-100 shadow-[0_0_8px_rgba(16,185,129,1)] transition-opacity" />
                                    </div>
                                    ...
                                    {(isCurrentWeek || i === 0 || (containerWidth < 20 && i % 2 === 0)) && (
                                        <div className={`absolute top-full mt-2 text-[8px] font-bold whitespace-nowrap ${isCurrentWeek ? 'text-white' : 'text-slate-500'}`}>
                                            {isCurrentWeek && !fixedYear ? 'V.NU' : startStr}
                                        </div>
                                    )}
                                </div>
                            );
            ...
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase tracking-widest pt-4 border-t border-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <span className="flex items-center gap-1.5">
                        <span className="w-4 h-0.5 bg-emerald-500/50 border-t border-dashed border-emerald-400"></span>
                        Snitt (4v)
                    </span>
                </div>
                <div>
