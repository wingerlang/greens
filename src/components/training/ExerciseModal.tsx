                    ...
                    <form onSubmit={onSave} className="space-y-8">
                                    ...
                                    <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700/50 flex items-center justify-between shadow-sm">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center text-3xl shadow-sm border border-slate-100 dark:border-slate-700">
                                                {EXERCISE_TYPES.find(t => t.type === effectiveExerciseType)?.icon}
                                            </div>
                                            <div>
                                                <div className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                                    {EXERCISE_TYPES.find(t => t.type === effectiveExerciseType)?.label}
                                                    <span className="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600" />
                                                    <span className={`${INTENSITIES.find(i => i.value === effectiveIntensity)?.color}`}>
                                                        {INTENSITIES.find(i => i.value === effectiveIntensity)?.label}
                                                    </span>
                                                </div>
                                                <div className="text-xs text-slate-500 font-medium mt-0.5">
                                                    {effectiveDuration} min
                                                    {exerciseForm.distance ? ` • ${exerciseForm.distance} km` : ''}
                                                </div>
                                            </div>
                                        </div>
                                        <div className="text-right px-3 py-1 bg-white dark:bg-slate-950 rounded-lg border border-slate-100 dark:border-slate-800">
                                            <div className="text-sm font-black text-emerald-500">
                                                {Math.round(calculateCalories(effectiveExerciseType, parseInt(effectiveDuration) || 0, effectiveIntensity))}
                                            </div>
                                            <div className="text-[9px] font-bold uppercase tracking-wider text-slate-400">kcal</div>
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="relative">
                            ...
                            <div className="relative flex justify-center">
                                <span className="bg-white dark:bg-slate-900 px-3 text-xs font-bold text-slate-400 uppercase tracking-widest">Eller välj manuellt</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                            {EXERCISE_TYPES.map(t => (
                                <button
                                    key={t.type}
                                    type="button"
                                    className={`aspect-square rounded-2xl border flex flex-col items-center justify-center gap-2 transition-all duration-200 ${effectiveExerciseType === t.type
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-12 gap-4">
                            ...
                            <div className="col-span-1 sm:col-span-4 space-y-1.5">
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Tid (min)</label>
                                <input
                                    type="number"
                                ...
                                <div className="grid grid-cols-4 gap-1 p-1 bg-slate-100 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                                    {INTENSITIES.map(i => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                setExerciseForm({ ...exerciseForm, intensity: i.value });
                                                setSmartInput('');
                                            }}
                                            title={i.label}
                                            className={`rounded-lg py-2 text-xs font-bold transition-all ${effectiveIntensity === i.value
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                            ...
                            {effectiveExerciseType === 'strength' && (
                                <div className="space-y-1.5 animate-in fade-in slide-in-from-right-4">
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Total Volym (kg)</label>
                                    <input
                                        type="number"
                                        placeholder="0"
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Anteckningar</label>
                            <textarea
                                rows={2}
