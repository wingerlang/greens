...
export function WorkoutCard(
  { workout, isAnnualBest, isPR, onClick }: WorkoutCardProps,
) {
  const topExercises = workout.exercises.slice(0, 3).map((e) => e.exerciseName)
    .join(", ");
  const category = workout.workoutCategory || classifyWorkout(workout);

  return (
    <div
      className={`bg-slate-900/50 border transition-all cursor-pointer relative overflow-hidden rounded-xl p-4 ${
        isPR
    ...
    >
      {isPR && (
        <div className="absolute -right-6 -top-6 w-16 h-16 bg-amber-500/20 blur-2xl rounded-full pointer-events-none">
        </div>
      )}
      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {workout.date} • {workout.uniqueExercises} övningar •{" "}
        ...
        <div className="text-right">
          <p className="text-emerald-400 font-bold">
            {Math.round(workout.totalVolume / 1000)}t volym
          </p>
...
export function RecordTrendLine({ pbs }: RecordTrendLineProps) {
  ...
  const points = displaySlots.map((s, i) => {
    ...
    const y = height -
      ((s.value - minVal) / valRange) * (height - padding * 2) - padding;
    return `${x},${y}`;
  }).join(" ");
      ...
      <div className="w-full h-full relative group/chart">
        ...
        >
          ...
          <line
            x1="0"
            y1={height -
              ((lastSlot.value - minVal) / valRange) * (height - padding * 2) -
              padding}
            x2={width}
            y2={height -
              ((lastSlot.value - minVal) / valRange) * (height - padding * 2) -
