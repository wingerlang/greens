...
export function GoalModal(
  { isOpen, onClose, onSave, cycles, editingGoal }: GoalModalProps,
) {
  // Basic state
  ...
  const [weightDirection, setWeightDirection] = useState<WeightDirection>(
    "down",
  );
  ...
  const volumeStats = useMemo(() => {
    if (type === "tonnage") {
      return calculateVolumeStats(exerciseEntries, "strength", "ton");
    }
    if (type === "distance") {
      return calculateVolumeStats(exerciseEntries, "running", "km"); // Assuming running
    }
    return null;
  ...
  useEffect(() => {
    if (isOpen && editingGoal) {
      ...
      setCustomStartDate(
        editingGoal.startDate || new Date().toISOString().split("T")[0],
      );
      ...
      if (editingGoal.type === "weight" || editingGoal.type === "measurement") {
        ...
        const startW = editingGoal.milestoneProgress ||
  ...
  useEffect(() => {
    if (isOpen && !editingGoal) {
      ...
      const active = trainingPeriods.find((p) =>
  ...
  useEffect(() => {
    if (
      (type === "weight" || type === "measurement") && latestWeight &&
      useExistingWeight && !currentWeight
    ) {
  ...
  useEffect(() => {
    ...
    const freqMatch = lower.match(
      /(\d+)\s*x?\s*(löpning|löp|styrka|cykling|promenad|simning|yoga|annat)/,
    );
    if (freqMatch) {
      setType("frequency");
    ...
    // Period detection
    if (
      lower.includes("/dag") || lower.includes("om dagen") ||
      lower.includes("per dag")
    ) {
      setPeriod("daily");
    } else if (
      lower.includes("/vecka") || lower.includes("per vecka") ||
      lower.includes("i veckan")
    ) {
      setPeriod("weekly");
    } else if (
      lower.includes("/månad") || lower.includes("per månad") ||
      lower.includes("i månaden")
    ) {
      setPeriod("monthly");
    }
  }, [smartInput]);
  ...
  const handleSubmit = () => {
    ...
    switch (type) {
      ...
      case "speed":
        ...
        const totalSeconds = (parseInt(targetTimeMinutes) || 0) * 60 +
          (parseInt(targetTimeSeconds) || 0);
        targets = [{
      ...
      case "measurement":
        category = "body";
        const weightTarget = includeWeight
          ? parseFloat(targetWeight)
          : undefined;
        const measureTarget = includeMeasurement
          ? parseFloat(targetMeasurement)
          : undefined;
        targets = [{
        ...
        if (!goalName) {
          ...
          if (includeWeight && type === "weight") {
            const dirLabel = weightDirection === "down"
              ? "Gå ner"
              : weightDirection === "up"
              ? "Gå upp"
              : "Håll";
          ...
          if (includeMeasurement || type === "measurement") {
            ...
            const dirLabel = measurementDirection === "down"
              ? "↓"
            ...
            parts.push(
              `${
                mt?.label || measurementType
              } ${dirLabel} ${targetMeasurement}cm`,
            );
          }
          goalName = parts.join(" + ") ||
            (type === "weight" ? "Viktmål" : "Måttmål");
        }
        break;
      ...
      case "streak":
        ...
        if (!goalName) {
          const label = EXERCISE_TYPES.find((e) =>
            e.type === streakT.exerciseType
          )?.label || "Träning";
    ...
    const goalData: Omit<PerformanceGoal, "id" | "createdAt"> = {
      ...
      targetWeightRate: type === "weight"
        ? parseFloat(weeklyRate) || undefined
        : undefined,
      targetWeight: type === "weight"
        ? parseFloat(targetWeight) || undefined
        : undefined,
      // Measurement-specific fields
      targetMeasurement: type === "measurement"
        ? parseFloat(targetMeasurement) || undefined
        : undefined,
      measurementType: type === "measurement"
        ? (measurementType as any)
        : undefined,
      goalDirection: type === "weight"
        ? (weightDirection as any)
        : type === "measurement"
        ? (measurementDirection as any)
        : undefined,
      // milestoneProgress used as start value for both
      milestoneProgress: type === "weight"
        ? (parseFloat(currentWeight) || undefined)
        : type === "measurement"
        ? (parseFloat(currentMeasurement) || undefined)
        : undefined,
  ...
  const renderMacroBreakdown = () => {
    ...
    const totalMacros = calculatedMacros.protein + calculatedMacros.carbs +
      calculatedMacros.fat;
    const getPct = (val: number) =>
      totalMacros > 0 ? Math.round((val / totalMacros) * 100) : 0;
      ...
      <div className="mt-4 p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
        ...
        <div className="h-2 w-full bg-slate-950 rounded-full flex overflow-hidden">
          <div
            ...
            style={{ width: `${getPct(calculatedMacros.protein)}%` }}
          />
          <div
            className="h-full bg-emerald-500 transition-all"
            style={{ width: `${getPct(calculatedMacros.carbs)}%` }}
          />
          <div
            className="h-full bg-amber-500 transition-all"
            style={{ width: `${getPct(calculatedMacros.fat)}%` }}
          />
        </div>
  ...
  return (
    ...
    >
      {showNutritionWizard
        ? (
          <div
            className="w-full max-w-xl bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95"
                ...
                {type === "frequency" && (
                  <div className="space-y-4">
                    {frequencyTargets.map((target, idx) => (
                      <div
                        key={idx}
                        className="p-4 bg-slate-950/30 rounded-xl border border-white/5 space-y-4 relative"
                      >
                    ...
                    <button
                      onClick={() =>
                        setFrequencyTargets([...frequencyTargets, {
                    ...
                    {volumeStats && (volumeStats.avg30d > 0) && (
                      <div className="p-3 bg-blue-500/5 rounded-xl border border-blue-500/10">
                        <div className="text-[10px] font-bold text-blue-400 uppercase mb-2 flex justify-between">
                          ...
                          <span>
                            {volumeStats.avg30d.toFixed(1)} {volumeStats.unit}
                            {" "}
                            (30d)
                          </span>
                        </div>
                        <div className="flex gap-2 overflow-x-auto no-scrollbar">
                          {[
                            ...
                            <button
                              key={opt.label}
                              onClick={() =>
                                setTargetValue(Math.round(opt.val).toString())}
                ...
                {isBodyGoal && (
                  <div className="space-y-4 p-4 bg-gradient-to-br from-blue-500/5 to-purple-500/5 rounded-xl border border-blue-500/10">
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        onClick={() => {
                          setIncludeMeasurement(!includeMeasurement);
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="text-xs">
                                  ...
                                  <span className="text-slate-500 ml-1">
                                    ({formatWeightDate(latestWeight.date)})
                                  </span>
                                </div>
                              </div>
                              <button
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            <div className="space-y-1.5">
                              ...
                              <input
                                ...
                                className={`w-full border rounded-lg p-2.5 text-white text-center font-bold ${
                                  !useExistingWeight
                            ...
                            <div className="space-y-1.5">
                              ...
                              <div className="relative">
                                ...
                                />
                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] text-slate-500 font-bold uppercase">
                                  ±
                                </span>
                              </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setTargetMeasurement(e.target.value)}
                              className="w-full bg-slate-900 border border-white/10 rounded-lg p-2.5 text-white text-center font-bold"
                            />
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8 gap-2">
                        {EXERCISE_TYPES.map((t) => (
                          <button
                            ...
                            title={t.label}
                            className={`p-2 rounded-lg border flex flex-col items-center gap-1 transition-all ${
                              frequencyTargets[0]?.exerciseType === t.type
                      ...
                      <div className="p-3 bg-emerald-500/5 rounded-xl border border-emerald-500/10">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <button
                            onClick={() =>
                              setTargetValue(
                                calorieSmartData.target.toString(),
                              )}
                  ...
                  <div className="p-4 bg-gradient-to-br from-blue-500/5 to-cyan-500/5 rounded-xl border border-blue-500/10 space-y-4">
                    <div className="space-y-2">
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <div className="flex gap-2">
                            <input
                              ...
                              onChange={(e) =>
                                setTargetTimeMinutes(e.target.value)}
                              placeholder="25"
                              className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white text-center text-lg font-bold"
                            />
                            <span className="self-center font-bold text-slate-500">
                              :
                            </span>
                            <input
                              type="number"
                              ...
                              onChange={(e) =>
                                setTargetTimeSeconds(e.target.value)}
                              placeholder="00"
                              className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white text-center text-lg font-bold"
                            />
                          </div>
                      ...
                      <div className="text-xs text-center text-slate-500">
                        Löphastighet:{" "}
                        <strong>
                          {targetDistance && targetTimeMinutes
                            ? ((parseFloat(targetTimeMinutes) +
                              (parseFloat(targetTimeSeconds) || 0) / 60) /
                              parseFloat(targetDistance)).toFixed(2)
                            : "-"}
                        </strong>{" "}
                        min/km
                      </div>
                  ...
                  {calculatedMacros
                    ...
                    : (
                      <div className="text-[10px] text-slate-500 italic px-1">
                        Använd Hälsoguiden för att räkna ut exakta behov för att
                        nå ditt mål.
                      </div>
                    )}
                </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      {activePeriod && (
                        <button
                          onClick={() =>
                            setCustomStartDate(activePeriod.startDate)}
                    ...
                    <div className="grid grid-cols-6 gap-1.5">
                      {[
                        ...
                        <button
                          key={d.id}
                          onClick={() =>
                            setDurationPreset(d.id as DurationPreset)}
                          title={d.label}
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center">
                          ...
                          {activePeriod && (
                            <button
                              onClick={() =>
                                setCustomEndDate(activePeriod.endDate)}
