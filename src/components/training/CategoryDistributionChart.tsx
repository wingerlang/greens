...
import { type StrengthWorkout, type WorkoutCategory } from '../../models/strengthTypes.ts';
import { classifyWorkout, WORKOUT_CATEGORY_INFO } from '../../utils/workoutClassifier.ts';
...
export function CategoryDistributionChart({ workouts, className = '' }: CategoryDistributionChartProps) {
    ...
    const getCumulativeHeight = (dataIndex: number, upToCategory: WorkoutCategory) => {
        ...
        <div className={`bg-slate-800/30 rounded-xl p-3 w-1/2 ${className}`}>
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold text-slate-500 uppercase">Fördelning/Månad</span>
                </div>
            ...
            <div className="relative">
                <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-28" preserveAspectRatio="xMidYMid meet">
                    ...
                    {displayData.map((data, dataIndex) => {
                        const barX = padding.left + dataIndex * (barWidth + barGap) + barGap / 2;
                ...
                {hoveredIndex !== null && displayData[hoveredIndex] && (
                    ...
                    >
                                ...
                                return (
                                    <div key={cat} className="flex items-center gap-1">
                                        <div
                                            ...
                                            style={{ backgroundColor: colors[cat] }}
                                        />
                                        <span className="text-slate-400">{WORKOUT_CATEGORY_INFO[cat].labelSe}:</span>
                        ...
                        <div className="border-t border-white/10 mt-1 pt-1 text-slate-500">
                            Totalt: <span className="text-white font-bold">{displayData[hoveredIndex].total}</span> pass
                        </div>
                    </div>
                )}
            </div>
