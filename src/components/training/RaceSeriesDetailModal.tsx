...
import { Trophy, Calendar, MapPin, TrendingUp, Clock, ArrowUpRight, ArrowDownRight, Minus } from 'lucide-react';
...
export function RaceSeriesDetailModal({ seriesName, races, onClose, onSelectRace }: RaceSeriesDetailModalProps) {
    ...
    const stats = useMemo(() => {
        ...
        if (sortedRaces.length >= 2) {
            ...
            const avgFirst = firstHalf.reduce((sum, r) => sum + r.durationMinutes, 0) / firstHalf.length;
            const avgSecond = secondHalf.reduce((sum, r) => sum + r.durationMinutes, 0) / secondHalf.length;

            if (avgSecond < avgFirst * 0.98) trend = 'improving';
            else if (avgSecond > avgFirst * 1.02) trend = 'declining';
        }

        const yearsActive = sortedRaces.map(r => r.date.substring(0, 4));
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200" onClick={onClose}>
            ...
            >
                ...
                <div className="p-8 pb-4 border-b border-white/5 flex justify-between items-start bg-slate-900 sticky top-0 z-20">
                    ...
                    <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors p-2 bg-slate-800/50 rounded-full hover:bg-slate-800">
                        ✕
                    </button>
                </div>
                ...
                <div className="p-8 space-y-8 overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="bg-slate-950/50 p-4 rounded-2xl border border-white/5">
                            <div className="text-xs text-slate-500 uppercase font-bold mb-1">Medeltid</div>
                            <div className="text-2xl font-bold text-slate-200 font-mono">{formatActivityDuration(stats.avg)}</div>
                        ...
                        <div className="bg-slate-950/50 p-4 rounded-2xl border border-white/5">
                            <div className="text-xs text-slate-500 uppercase font-bold mb-1">Utveckling</div>
                            <div className="flex items-center gap-2">
                                <div className={`text-2xl font-black ${stats.trend === 'improving' ? 'text-emerald-400' :
                                    stats.trend === 'declining' ? 'text-rose-400' : 'text-slate-400'
                                    }`}>
                                    {stats.trend === 'improving' ? 'Förbättring' :
                                        stats.trend === 'declining' ? 'Tapp' : 'Stabil'}
                                </div>
                            </div>
                            <div className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                                {stats.trend === 'improving' ? <ArrowDownRight size={14} className="text-emerald-500" /> :
                                    stats.trend === 'declining' ? <ArrowUpRight size={14} className="text-rose-500" /> :
                                        <Minus size={14} />}
                                {stats.trend === 'improving' ? 'Snabbare snitt' :
                                    stats.trend === 'declining' ? 'Långsammare snitt' : 'Jämn nivå'}
                            </div>
                        </div>

                        <div className="bg-slate-950/50 p-4 rounded-2xl border border-white/5">
                            <div className="text-xs text-slate-500 uppercase font-bold mb-1">Långsammaste</div>
                            <div className="text-2xl font-bold text-slate-400 font-mono">{formatActivityDuration(stats.pw.durationMinutes)}</div>
                            <div className="text-xs text-slate-500 mt-1">
                    ...
                    <div className="bg-slate-950/30 p-6 rounded-3xl border border-white/5">
                        <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                            <TrendingUp size={20} className="text-emerald-500" /> Tidsutveckling
                        </h3>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(value: any) => [<span className="text-amber-400 font-mono font-bold text-lg">{formatActivityDuration(Number(value))}</span>, <span className="text-white text-xs">Tid</span>]}
                                    />
                                    <Line
                                        type="monotone"
                                        dataKey="time"
                                    ...
                                    />
                                    {/* PB Line */}
                                    <ReferenceLine y={stats.pb.durationMinutes} stroke="#fbbf24" strokeDasharray="3 3" opacity={0.5} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-950/30 rounded-3xl border border-white/5 overflow-hidden">
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-slate-900/50 text-xs uppercase font-bold text-slate-500">
                                <tr>
                                    <th className="px-6 py-4 text-left cursor-pointer hover:text-white" onClick={() => handleSort('date')}>År / Datum <SortIcon colKey="date" /></th>
                                    ...
                                    <th className="px-6 py-4 text-right cursor-pointer hover:text-white" onClick={() => handleSort('time')}>Tid <SortIcon colKey="time" /></th>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {sortedRaces.map(race => {
                                    ...
                                    return (
                                        <tr
                                            key={race.id}
                                            className={`hover:bg-white/5 transition-colors cursor-pointer group ${isPb ? 'bg-amber-500/5' : ''}`}
                                            onClick={() => onSelectRace(race)}
                                        >
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="font-mono text-slate-300">{race.date}</div>
                                                    {isPb && <span className="bg-amber-500/20 text-amber-500 text-[10px] px-2 py-0.5 rounded font-bold uppercase">PB</span>}
                                                </div>
                                            </td>
