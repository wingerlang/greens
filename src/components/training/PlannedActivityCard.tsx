...
export function PlannedActivityCard({ activity, compact = false }: PlannedActivityCardProps) {
    const { updatePlannedActivity, completePlannedActivity, deletePlannedActivity } = useData();
    ...
    if (compact) {
        return (
            <div className={`p-4 rounded-2xl border transition-all ${activity.status === 'COMPLETED' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-slate-900/40 border-white/5 hover:border-white/10'}`}>
                <div className="flex items-center justify-between">
                    <div>
                        <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">{activity.category}</div>
                        <div className="font-black text-sm text-white">{activity.title}</div>
                    </div>
                    {activity.status === 'COMPLETED' ? (
                        <div className="text-emerald-500 text-lg">✅</div>
                    ) : (
                        <div className="text-indigo-400 font-black text-xs">{activity.targetPace}</div>
                    )}
                </div>
            </div>
        );
    ...
    const handleComplete = (feedback: PlannedActivity['feedback'], actualDist?: number, actualTime?: number) => {
        completePlannedActivity(activity.id, actualDist || activity.estimatedDistance, actualTime, feedback);
        ...
        <div className={`relative group glass-card p-4 transition-all overflow-hidden ${activity.status === 'COMPLETED' ? 'opacity-60 grayscale-[30%]' : 'hover:scale-[1.01]'} ${activity.category === 'LONG_RUN' ? 'border-amber-500/30 bg-amber-500/[0.02]' : ''}`}>
                ...
                <div className="space-y-4 flex-1">
                    ...
                    {activity.category === 'LONG_RUN' && (
                        <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-3">
                            <div className="flex justify-between items-end">
                                <div className="text-[10px] font-black text-amber-400 uppercase tracking-widest">Uthållighets-analys</div>
                                <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Bas + Stamina-delta</div>
                            </div>
                            <div className="flex h-3 gap-1 rounded-full overflow-hidden bg-slate-800">
                                <div className="h-full bg-blue-500/40 relative group/base" style={{ width: '60%' }}>
                                    <div className="absolute inset-0 flex items-center justify-center text-[7px] font-black opacity-0 group-hover/base:opacity-100 transition-opacity uppercase text-white pointer-events-none">Basmängd</div>
                                </div>
                                <div className="h-full bg-gradient-to-r from-amber-500 to-orange-500 relative group/extra animate-pulse" style={{ width: '40%' }}>
                                    <div className="absolute inset-0 flex items-center justify-center text-[7px] font-black text-slate-950 uppercase pointer-events-none">Endurance Delta</div>
                                </div>
                            </div>
                            <p className="text-[9px] text-slate-500 font-medium italic">
                                * Den orangea zonen markerar distansen där du faktiskt bygger ny stamina och växer din aeroba motor.
                            </p>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="p-3.5 rounded-2xl bg-slate-900/50 border border-white/5 group-hover:border-indigo-500/20 transition-colors">
                            <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Måltempo</div>
                            <div className="text-lg font-black text-white tracking-tight">{activity.targetPace} <span className="text-[10px] text-slate-500 font-medium italic">min/km</span></div>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-slate-900/50 border border-white/5 group-hover:border-indigo-500/20 transition-colors">
                            <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Distans</div>
                            <div className={`text-lg font-black tracking-tight ${activity.category === 'LONG_RUN' ? 'text-amber-400' : 'text-white'}`}>
                                {activity.estimatedDistance} <span className="text-[10px] text-slate-500 font-medium italic">km</span>
                            </div>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-slate-900/50 border border-white/5 group-hover:border-indigo-500/20 transition-colors">
                        ...
                        <>
                            {!showFeedback ? (
                                ...
                                <div className="flex flex-col gap-2 p-4 bg-slate-900/90 rounded-2xl border border-white/10 animate-in fade-in zoom-in duration-200 shadow-2xl">
                                    <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest text-center mb-1">Hur kändes det?</div>
                                    <div className="flex gap-2">
                                        <button onClick={() => handleComplete('EASY')} className="p-3 bg-blue-500/20 text-blue-400 rounded-xl hover:bg-blue-500/30 transition-all font-black text-[10px] uppercase">Lätt</button>
                                        <button onClick={() => handleComplete('PERFECT')} className="p-3 bg-emerald-500/20 text-emerald-400 rounded-xl hover:bg-emerald-500/30 transition-all font-black text-[10px] uppercase">Perfekt</button>
                                        <button onClick={() => handleComplete('HARD')} className="p-3 bg-rose-500/20 text-rose-400 rounded-xl hover:bg-rose-500/30 transition-all font-black text-[10px] uppercase">Hårt</button>
                                    </div>
                                    <button onClick={() => setShowFeedback(false)} className="text-[10px] text-slate-500 font-bold hover:text-white mt-1">Avbryt</button>
                                </div>
                            )
                            }
                        </>
            ...
            <div className={`transition-all duration-500 overflow-hidden ${isExpanded ? 'max-h-[2000px] opacity-100 mt-6' : 'max-h-0 opacity-0'}`}>
                {activity.structure.warmupKm > 0 && (
                    <div className="flex items-center gap-4 p-3 rounded-xl bg-blue-500/5 border border-blue-500/10">
                        ...
                        <span className="text-sm text-white font-black tabular-nums">{activity.structure.warmupKm} km</span>
                        <span className="text-[10px] text-slate-500 font-bold uppercase">Lugnt</span>
                    </div>
                )}

                {activity.structure.mainSet.map((set, i) => (
                    <div key={i} className={`flex flex-col gap-3 p-4 rounded-xl border relative overflow-hidden group/set ${activity.category === 'LONG_RUN' ? 'bg-amber-500/5 border-amber-500/10' : 'bg-white/5 border-white/5'
                        }`}>
                        <div className={`absolute inset-y-0 left-0 w-1 opacity-50 ${activity.category === 'LONG_RUN' ? 'bg-amber-500' : 'bg-indigo-500'
                            }`} />
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                <span className={`w-24 text-[9px] font-black uppercase tracking-wider ${activity.category === 'LONG_RUN' ? 'text-amber-400' : 'text-indigo-400'
                                    }`}>
                                    {activity.category === 'TEMPO' ? 'Tempo' : (activity.category === 'LONG_RUN' || activity.category === 'EASY') ? 'Uthållighet' : 'Intervaller'}
                                </span>
                                <span className="text-xl font-black text-white tabular-nums">
                                    {(activity.category === 'EASY' || activity.category === 'LONG_RUN') && set.reps === 1
                                        ? `${set.distKm}km`
                                        : `${set.reps}x ${set.distKm >= 1 ? `${set.distKm}km` : `${set.distKm * 1000}m`}`}
                                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="text-[10px] text-slate-500 font-bold uppercase">Tempo</span>
                                <span className={`text-2xl font-black tabular-nums drop-shadow-xl ${activity.category === 'LONG_RUN' ? 'text-amber-400' : 'text-emerald-400'
                                    }`}>
                                    {set.pace}
                                </span>
                            </div>
                        ...
                        {set.restMin > 0 && (
                            <div className="flex items-center gap-4 pl-28">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Vila</span>
                                    <span className="text-sm font-black text-indigo-400">{set.restMin} min</span>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                {activity.structure.cooldownKm > 0 && (
                    <div className="flex items-center gap-4 p-3 rounded-xl bg-blue-500/5 border border-blue-500/10">
                        <span className="w-24 text-[9px] font-black uppercase text-blue-400 tracking-wider">Nedvarvning</span>
                        <div className="h-1.5 flexible flex-1 bg-blue-500/20 rounded-full overflow-hidden">
                            <div className="h-full bg-blue-500 w-full opacity-50" />
                        </div>
                        <span className="text-sm text-white font-black tabular-nums">{activity.structure.cooldownKm} km</span>
                        <span className="text-[10px] text-slate-500 font-bold uppercase">Lugnt</span>
                    </div>
                )}
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-white/5">
                <div className="flex items-center gap-1.5 h-4 rounded-full overflow-hidden bg-slate-900 shadow-inner">
                    {activity.structure.warmupKm > 0 && <div className="h-full bg-blue-500/40 relative" style={{ width: `${(activity.structure.warmupKm / activity.estimatedDistance) * 100}%` }} />}
                    {activity.structure.mainSet.map((set, i) => (
                        <div key={i} className={`h-full relative ${activity.category === 'LONG_RUN' ? 'bg-amber-500' : 'bg-indigo-500'}`} style={{ width: `${((set.reps * set.distKm) / activity.estimatedDistance) * 100}%` }}>
                            <div className="absolute inset-x-0 top-0 h-1/2 bg-white/10" />
                        </div>
                    ))}
                    {activity.structure.cooldownKm > 0 && <div className="h-full bg-blue-500/40 relative" style={{ width: `${(activity.structure.cooldownKm / activity.estimatedDistance) * 100}%` }} />}
                </div>
                <div className="flex justify-between mt-3 px-1">
                    <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Start</div>
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{activity.estimatedDistance}km Totalt</div>
                </div>
            </div>
