...
import { ExerciseEntry, DailyVitals } from '../../models/types.ts';
...
export function SleepGainsAnalysis({ exerciseEntries, dailyVitals }: SleepGainsAnalysisProps) {
    const analysis = useMemo(() => {
        ...
        exerciseEntries.forEach(entry => {
            ...
            if (entry.type === 'running' && entry.distance && entry.distance > 0 && entry.durationMinutes > 0) {
            ...
            } else if (entry.type === 'strength' && entry.tonnage && entry.tonnage > 0) {
        ...
        return {
            running: {
                ...
                count: stats.running.low.length + stats.running.medium.length + stats.running.high.length
            },
            strength: {
                low: avg(stats.strength.low),
                medium: avg(stats.strength.medium),
                high: avg(stats.strength.high),
                count: stats.strength.low.length + stats.strength.medium.length + stats.strength.high.length
            }
        };
    ...
    const strengthGain = analysis.strength.low > 0 && analysis.strength.high > 0
        ? ((analysis.strength.high - analysis.strength.low) / analysis.strength.low * 100).toFixed(1) // % heavier
        : null;
        ...
        <div className="bg-slate-900 border border-indigo-500/20 rounded-2xl p-6 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-950/50 rounded-xl p-4 border border-white/5">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-slate-300 uppercase tracking-widest">Löpning</span>
                        </div>
                ...
                <div className="bg-slate-950/50 rounded-xl p-4 border border-white/5">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                            <span className="text-lg">💪</span>
                            <span className="text-xs font-bold text-slate-300 uppercase tracking-widest">Styrka</span>
                        </div>
                    ...
                    <div className="space-y-3">
                        <Bar
                            label="< 6.5h Sömn"
                            value={analysis.strength.low ? `${Math.round(analysis.strength.low / 1000)} ton` : '-'}
                            subValue="Trött"
                            color="bg-slate-700"
                            width="w-[60%]"
                        />
                        <Bar
                            label="> 7.5h Sömn"
                            value={analysis.strength.high ? `${Math.round(analysis.strength.high / 1000)} ton` : '-'}
                            subValue="Utvilad"
            ...
            <div className="mt-4 pt-4 border-t border-white/5 text-center">
                <p className="text-[10px] text-slate-500 italic">
                    Baserat på {analysis.running.count + analysis.strength.count} aktiviteter matchade med sömndata.
                </p>
            </div>
        </div>
    );
...
const Bar = ({ label, value, subValue, color, width }: { label: string, value: string, subValue: string, color: string, width: string }) => (
    <div className="flex items-center gap-3">
        ...
        <div className="flex-1 h-8 bg-slate-900 rounded-lg relative overflow-hidden flex items-center px-3 group">
            <div className={`absolute top-0 left-0 h-full ${color} opacity-20 transition-all duration-1000`} style={{ width }}></div>
            <div className={`absolute top-0 left-0 h-full w-[2px] ${color} opacity-50`}></div>
