...
export function SmartCoachDashboard(
  { stravaHistory }: SmartCoachDashboardProps,
) {
  const {
    coachConfig,
    plannedActivities,
    ...
    return (
      <div className="smart-coach-dashboard p-6 rounded-3xl bg-slate-900 border border-white/5 shadow-2xl">
        ...
        <p className="text-slate-400 text-xs leading-relaxed mb-6">
          Konfigurera din coach för att få en personlig träningsplan baserad på
          din fysiologi.
        </p>
        <Link
          to="/coach"
  ...
  const activeGoal = React.useMemo(
    () => coachConfig?.goals.find((g) => g.isActive) || coachConfig?.goals[0],
    [coachConfig],
  );

  const probability = React.useMemo(() => {
    if (!coachConfig || !activeGoal) return null;
    const currentVdot = coachConfig.userProfile.recentRaceTime
      ? calculateVDOT(
        coachConfig.userProfile.recentRaceTime.distance,
        coachConfig.userProfile.recentRaceTime.timeSeconds,
      )
      : 35;
  ...
  const nextActivity = plannedActivities.length > 0
    ? plannedActivities[0]
    : null;
    ...
    <div className="smart-coach-dashboard p-6 rounded-3xl bg-slate-900 border border-white/5 shadow-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/planner"
            className="bg-indigo-500 hover:bg-indigo-400 text-white text-[10px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider transition-all shadow-lg shadow-indigo-500/20"
          >
            Planera Vecka
          </Link>
        ...
        ? (
          <div className="text-center py-8">
            <p className="text-slate-400 text-xs mb-4">
              Ingen träningsplan genererad än.
            </p>
            <button
                ...
                {probability !== null && (
                  <span
                    className={`text-sm font-black ${
                      probability > 0.8 ? "text-emerald-400" : "text-amber-400"
                    }`}
                  >
