...
import { ExerciseType, PerformanceGoal } from "../../models/types.ts";
...
export function GoalCard({ goal, progress, onEdit, onDelete }: GoalCardProps) {
  ...
  const icon = primaryTarget?.exerciseType
    ? EXERCISE_ICONS[primaryTarget.exerciseType]
    : "🎯";
          ...
          <div className="text-[10px] text-slate-400 mb-2">
            {goal.targets.map((t, i) => (
              <span key={i}>
                ...
                {t.count && `${t.count}x `}
                {t.exerciseType &&
                  (EXERCISE_ICONS[t.exerciseType] || t.exerciseType)}
