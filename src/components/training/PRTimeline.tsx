...
export function PRTimeline({ workouts, exerciseName }: PRTimelineProps) {
  ...
  const sessions = useMemo(() => {
    ...
    workouts.forEach((w) => {
      const exerciseEntries = w.exercises.filter((e) =>
        normalizeExerciseName(e.exerciseName) === normName
      );
  ...
  const { weightChain, e1rmChain } = useMemo(() => {
    ...
    sessions.forEach((session, idx) => {
      ...
      if (session.maxWeight > currentMaxWeight) {
        ...
        const prevDate = wChain.length > 0
          ? wChain[wChain.length - 1].date
          : null;
      ...
      if (session.maxE1RM > currentMaxE1RM) {
        const prevDate = eChain.length > 0
          ? eChain[eChain.length - 1].date
          : null;
        const gapStats = calculateGapStats(sessions, prevDate, session.date);
  ...
  function calculateGapStats(
    ...
    relevantSessions.forEach((s) => {
      ...
      s.sets.forEach((set) => {
        sets++;
        reps += set.reps;
        const load = (set.isBodyweight || set.weight === 0)
          ? (set.extraWeight || 0)
          : set.weight;
  ...
  const renderNode = (
    ...
    <div key={node.id} className="relative flex flex-col items-center">
      {/* The Node Card */}
      <div
        className={`z-10 w-full p-3 rounded-2xl border backdrop-blur-md shadow-xl transition-all hover:scale-[1.01] ${
          type === "weight"
      ...
      >
        ...
        <div className="flex items-baseline gap-1 mb-1">
          <span className="text-3xl font-black text-white">
            {node.reps} <span className="text-xl text-slate-500 mx-0.5">×</span>
            {" "}
            {node.weight}
        ...
        <div className="flex flex-col items-center h-20 w-full relative">
          ...
          <div
            className={`absolute top-0 bottom-0 w-0.5 ${
              type === "weight"
                ? "bg-gradient-to-b from-emerald-500/30 to-emerald-500/5"
                : "bg-gradient-to-b from-amber-500/30 to-amber-500/5"
            }`}
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 relative">
        ...
        <div className="space-y-4">
          <div className="text-center mb-8 sticky top-0 bg-slate-950/95 py-4 z-30 backdrop-blur border-b border-white/5">
            <h3 className="text-emerald-500 font-black uppercase tracking-widest text-sm">
              Tyngsta Lyft
            </h3>
          ...
          <div className="px-4 pb-20">
            {weightChain.map((node, idx) =>
              renderNode(node, "weight", idx === weightChain.length - 1)
            )}
            {weightChain.length === 0 && (
              <p className="text-center text-slate-600 text-xs">
                Inga vikt-rekord hittades.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="text-center mb-8 sticky top-0 bg-slate-950/95 py-4 z-30 backdrop-blur border-b border-white/5">
            <h3 className="text-amber-500 font-black uppercase tracking-widest text-sm">
              Prestation (1eRM)
            </h3>
            <p className="text-[10px] text-slate-600 font-bold">
              Estimerat Max (kg)
          ...
          <div className="px-4 pb-20">
            {e1rmChain.map((node, idx) =>
              renderNode(node, "e1rm", idx === e1rmChain.length - 1)
            )}
            {e1rmChain.length === 0 && (
              <p className="text-center text-slate-600 text-xs">
                Inga e1RM-rekord hittades.
              </p>
            )}
          </div>
        </div>
