...
export function HeartRateZones(
  { avgHeartRate, maxHeartRate, duration, age = 30 }: HeartRateZonesProps,
) {
    ...
    <div className="bg-slate-900/50 border border-white/5 rounded-xl p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <span
            ...
            style={{ color: zoneInfo.zone.color }}
          >
            {avgHeartRate}
          </span>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            {zoneInfo.zone.minPct}-{zoneInfo.zone.maxPct}% av max ({Math.round(
              calculatedMaxHR * zoneInfo.zone.minPct / 100,
            )}-{Math.round(calculatedMaxHR * zoneInfo.zone.maxPct / 100)} bpm)
          </p>
        </div>
        <div className="text-right">
          <p
      ...
      <div className="text-xs text-slate-500 border-t border-white/5 pt-3">
        {zoneInfo.zone.name === "Z1" &&
          "Lätt aktivitet för återhämtning. Bygger inte kondition."}
        {zoneInfo.zone.name === "Z2" &&
          "Optimal zon för fettförbränning och långdistansträning."}
        {zoneInfo.zone.name === "Z3" &&
          "Förbättrar aerob kapacitet. Marathontempo."}
        {zoneInfo.zone.name === "Z4" && "Tröskelträning. Ökar laktattolerans."}
        {zoneInfo.zone.name === "Z5" &&
          "Maximal ansträngning. Förbättrar VO2 max."}
      </div>
 ...
 */
export function HeartRateZoneBadge(
  { avgHeartRate, maxHeartRate, age = 30 }: {
    avgHeartRate: number;
    maxHeartRate?: number;
    age?: number;
  },
  ...
  const zone = ZONES.find((z) => hrPct >= z.minPct && hrPct < z.maxPct) ||
    ZONES[0];
