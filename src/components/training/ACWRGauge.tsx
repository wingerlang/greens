...
export function ACWRGauge({ workouts }: ACWRGaugeProps) {
    ...
    return (
        <div className={`${config.bg} ${config.border} border rounded-2xl p-4 shadow-lg ${config.glow}`}>
            {/* Header */}
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-[9px] text-slate-600 font-bold">ACWR (Akut:Kronisk)</p>
                </div>
                ...
                <div className="absolute inset-x-0 bottom-0 h-16 overflow-hidden">
                    <div className="relative w-full h-32">
                        ...
                        <div
                            ...
                            style={{
                                background: 'conic-gradient(from 180deg, #3b82f6 0deg, #10b981 72deg, #10b981 117deg, #f59e0b 144deg, #ef4444 180deg, transparent 180deg)'
                            }}
                        />
                        {/* Inner cut-out */}
                        ...
                        >
                            <div className={`w-1 h-full ${config.bg.replace('/10', '')} rounded-full`} />
                        </div>
            ...
            {/* Message */}
            <div className={`text-center text-[10px] font-bold uppercase tracking-wider ${config.text} mb-3`}>
                {message}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-white/5">
                <div>
                    <p className="text-[8px] text-slate-600 uppercase tracking-widest">Akut (7d)</p>
                    <p className="text-sm font-bold text-white">{(acuteLoad / 1000).toFixed(1)}t</p>
                </div>
                <div>
                    <p className="text-[8px] text-slate-600 uppercase tracking-widest">Kronisk (4v snitt)</p>
                    <p className="text-sm font-bold text-white">{(chronicLoad / 1000).toFixed(1)}t</p>
                </div>
            </div>
        </div>
    );
