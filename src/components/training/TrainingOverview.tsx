...
export function TrainingOverview(
  { exercises, year, periodLabel, isFiltered, onExerciseClick }:
    TrainingOverviewProps,
) {
  const [statFilter, setStatFilter] = React.useState<
    "all" | "run" | "bike" | "strength"
  >("all");
  ...
  const stats = useMemo(() => {
    ...
    const baseExercises = isFiltered
      ? exercises
      : exercises.filter((e) => new Date(e.date).getFullYear() === currentYear);
    ...
    const yearExercises = baseExercises.filter((e) => {
      ...
      if (statFilter === "strength") {
        return t.includes("strength") || t.includes("styrk") ||
          t.includes("gym");
      }
      return true;
    });
    ...
    const monthExercises = exercises.filter((e) => {
      ...
      const matchesType = statFilter === "all" ||
        (statFilter === "run" &&
          (e.type.includes("run") || e.type.includes("löp"))) ||
        (statFilter === "bike" &&
          (e.type.includes("cycl") || e.type.includes("cyk"))) ||
        (statFilter === "strength" &&
          (e.type.includes("strength") || e.type.includes("styrk")));

      return matchesType && d.getMonth() === currentMonth &&
    ...
    const lastMonthExercises = exercises.filter((e) => {
      ...
      return matchesType && d.getMonth() === lastMonth &&
        d.getFullYear() === lastMonthYear;
    });

    const sumDistance = (exs: ExerciseEntry[]) =>
      exs.reduce((sum, e) => sum + (e.distance || 0), 0);
    const sumDuration = (exs: ExerciseEntry[]) =>
      exs.reduce((sum, e) => sum + e.durationMinutes, 0);
    const count = (exs: ExerciseEntry[]) => exs.length;

    const longestRun = yearExercises.filter((e) =>
      (e.type as string) === "running" || (e.type as string) === "löpning" ||
      (e.type as string).includes("cycl")
    ).reduce(
      (max, e) => (e.distance || 0) > (max.distance || 0) ? e : max,
      { distance: 0 } as ExerciseEntry,
    );

    const heavyLiftVolume = yearExercises.filter((e) =>
      (e.type as string) === "strength" || (e.type as string) === "gym" ||
      (e.type as string) === "styrka"
    ).reduce((sum, e) => sum + (e.tonnage || 0), 0);
    const maxStrengthSession = yearExercises.filter((e) =>
      (e.type as string) === "strength" || (e.type as string) === "gym" ||
      (e.type as string) === "styrka"
    ).reduce(
      (max, e) => (max.tonnage || 0) > (e.tonnage || 0) ? max : e,
      {} as ExerciseEntry,
    );

    const fastestPaceSession = yearExercises
      .filter((e) =>
        ((e.type as string) === "running" ||
          (e.type as string) === "löpning") && (e.distance || 0) > 3
      )
      .reduce((best, e) => {
    ...
    const fastestPace = fastestPaceSession.id
      ? (fastestPaceSession.durationMinutes /
        (fastestPaceSession.distance || 1))
      : 999;

    // For Cycling, maybe we want Max Watts?
    const maxWattSession = yearExercises
      .filter(
        (e) => ((e.type as string).includes("cycl") && (e as any).averageWatts),
      )
      .reduce(
        (best, e) =>
          (e as any).averageWatts > ((best as any).averageWatts || 0)
            ? e
            : best,
        {} as ExerciseEntry,
      );

    const maxEnergySession = yearExercises.reduce(
      (max, e) => (e.caloriesBurned || 0) > (max.caloriesBurned || 0) ? e : max,
      {} as ExerciseEntry,
    );
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/20 transition-all">
          <div className="absolute top-0 right-0 p-4 opacity-[0.03] text-[100px] leading-none select-none group-hover:opacity-[0.06] transition-opacity">
            📅
          </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="text-3xl font-black text-white">
                {stats.year.distance.toFixed(1).replace(".", ",")}{" "}
            ...
            <div>
              <div className="text-3xl font-black text-indigo-400">
                {(stats.insights.heavyLiftVolume / 1000).toFixed(1).replace(
                  ".",
                  ",",
                )}{" "}
                <span className="text-sm font-bold text-indigo-500/50">
                  ton
                </span>
              </div>
              <div className="text-[10px] text-slate-400 font-bold uppercase">
            ...
            <div>
              <div className="text-3xl font-black text-emerald-400">
                {Math.floor(stats.year.time / 60).toString().padStart(
                  2,
                  "0",
                )}:{Math.round(stats.year.time % 60).toString().padStart(
                  2,
                  "0",
                )}
                <span className="text-sm font-bold text-emerald-500/50 ml-1">
                  h
                </span>
              </div>
              <div className="text-[10px] text-slate-400 font-bold uppercase">
            ...
            <div>
              <div className="text-3xl font-black text-sky-400">
                {stats.year.count}{" "}
                <span className="text-sm font-bold text-sky-500/50">st</span>
              </div>
              <div className="text-[10px] text-slate-400 font-bold uppercase">
                Antal pass
              </div>
        ...
        <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-sky-500/20 transition-all">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="text-2xl font-black text-white">
                {stats.month.distance.toFixed(1).replace(".", ",")}{" "}
                <span className="text-xs text-slate-500">km</span>
              </div>
              <div className="flex items-center gap-1 mt-1">
                <span
                  className={`text-[10px] font-bold ${
                    stats.month.distance >= stats.lastMonth.distance
                      ? "text-emerald-400"
                      : "text-rose-400"
                  }`}
                >
                  {stats.month.distance >= stats.lastMonth.distance ? "▲" : "▼"}
                  {" "}
                  {(Math.abs(stats.month.distance - stats.lastMonth.distance))
            ...
            <div>
              <div className="text-2xl font-black text-white">
                {Math.floor(stats.month.time / 60).toString().padStart(
                  2,
                  "0",
                )}:{Math.round(stats.month.time % 60).toString().padStart(
                  2,
                  "0",
                )}
              ...
              <div className="flex items-center gap-1 mt-1">
                <span
                  className={`text-[10px] font-bold ${
                    stats.month.time >= stats.lastMonth.time
                      ? "text-emerald-400"
                      : "text-rose-400"
                  }`}
                >
                  {stats.month.time >= stats.lastMonth.time ? "▲" : "▼"}{" "}
                  {Math.round(
                    Math.abs(stats.month.time - stats.lastMonth.time) / 60,
                  )}h
                </span>
              </div>
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-white/5">
            <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">
              Snitt per pass denna månad
            </div>
            <div className="flex gap-4">
              <div>
                <div className="text-lg font-bold text-white">
                  {stats.month.count > 0
                    ? (stats.month.distance / stats.month.count).toFixed(1)
                      .replace(".", ",")
                    : 0} km
                </div>
              </div>
              <div>
                <div className="text-lg font-bold text-white">
                  {stats.month.count > 0
                    ? Math.round(stats.month.time / stats.month.count)
                    : 0} min
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <button
          onClick={() =>
            stats.insights.longestRun.id &&
            onExerciseClick?.(stats.insights.longestRun)}
        ...
        >
          ...
          <span className="text-xl font-black text-white">
            {(stats.insights.longestRun.distance || 0).toFixed(1).replace(
              ".",
              ",",
            )} <span className="text-sm text-slate-500">km</span>
          </span>
          <span className="text-[9px] text-slate-600 mt-1">
            {stats.insights.longestRun.date
              ? new Date(stats.insights.longestRun.date).toLocaleDateString(
                "sv-SE",
        ...
        {statFilter === "bike" && (stats.insights.maxWattSession as any)?.id
          ? (
            <button
              onClick={() =>
                (stats.insights.maxWattSession as any).id &&
                onExerciseClick?.(stats.insights.maxWattSession as any)}
              className="bg-slate-900/30 border border-white/5 p-4 rounded-xl flex flex-col justify-center items-center text-center hover:bg-white/5 transition-all group"
            >
              <span className="text-2xl mb-2 group-hover:scale-110 transition-transform">
                ⚡
              ...
              <span className="text-[9px] text-slate-600 mt-1">
                {(stats.insights.maxWattSession as any).date
                  ? new Date((stats.insights.maxWattSession as any).date)
          ...
          : (
            <button
              onClick={() =>
                stats.insights.fastestPaceSession.id &&
                onExerciseClick?.(stats.insights.fastestPaceSession)}
            ...
            >
              ...
              <span className="text-xl font-black text-emerald-400">
                {stats.insights.fastestPace
                  ? `${Math.floor(stats.insights.fastestPace)}:${
                    Math.round((stats.insights.fastestPace % 1) * 60).toString()
                      .padStart(2, "0")
                  }`
                  : "-"}
                <span className="text-sm text-slate-500">/km</span>
              ...
              <span className="text-[9px] text-slate-600 mt-1">
                {stats.insights.fastestPaceSession.date
                  ? new Date(stats.insights.fastestPaceSession.date)
                    .toLocaleDateString("sv-SE", {
                      day: "numeric",
                      month: "short",
                    })
        ...
        <button
          onClick={() =>
            stats.insights.maxStrengthSession.id &&
            onExerciseClick?.(stats.insights.maxStrengthSession)}
        ...
        >
          ...
          <span className="text-[10px] uppercase font-bold text-slate-500">
            {isFiltered ? "Periodens Volym" : "Årets Volym"}
          </span>
          ...
          <span className="text-[9px] text-slate-600 mt-1">
            {stats.insights.maxStrengthSession.date
              ? new Date(stats.insights.maxStrengthSession.date)
                .toLocaleDateString("sv-SE", { day: "numeric", month: "short" })
              : (isFiltered ? "Totalt i perioden" : "Totalt i år")}
          </span>
        </button>

        <button
          onClick={() =>
            stats.insights.maxEnergySession.id &&
            onExerciseClick?.(stats.insights.maxEnergySession)}
        ...
        >
          ...
          <span className="text-xl font-black text-rose-400">
            {(stats.year.calories / 1000).toFixed(0)}{" "}
            <span className="text-sm text-slate-500">kkcal</span>
          </span>
          <span className="text-[9px] text-slate-600 mt-1">
            {stats.insights.maxEnergySession.date
              ? new Date(stats.insights.maxEnergySession.date)
                .toLocaleDateString("sv-SE", { day: "numeric", month: "short" })
              : (isFiltered ? "Bränt i perioden" : "Bränt i år")}
          </span>
        </button>
      </div>
