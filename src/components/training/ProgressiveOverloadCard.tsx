...
export function ProgressiveOverloadCard(
  { suggestion, compact = false, onSelectWeight }: ProgressiveOverloadCardProps,
) {
  const {
    exerciseName,
    lastWeight,
    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          ...
          <span
            className={`font-bold ${
              isPlateaued ? "text-amber-400" : "text-emerald-400"
            }`}
          >
            {isDistanceBased
              ? `${suggestedDistance}m`
      ...
      <div className="flex items-start gap-3">
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl ${
            isPlateaued ? "bg-amber-500/20" : "bg-emerald-500/20"
          }`}
        >
        ...
        <div className="flex-1">
            ...
            {!isDistanceBased && (
              <div className="flex items-center gap-1 text-[10px]">
                ...
                {progressRate !== 0 && (
                  <span
                    className={`${
                      progressRate > 0 ? "text-emerald-400" : "text-rose-400"
                    }`}
                  >
                    {progressRate > 0 ? "+" : ""}
                    {progressRate}%/pass
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            >
              {isDistanceBased ? <>🏃 {suggestedDistance}m (+2.5%)</> : (
                <>
                  💪 {suggestedWeight}kg × {lastReps}{" "}
 ...
 */
export function PlateauWarningCard(
  { warning, expanded = false }: PlateauWarningCardProps,
) {
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              className={`text-[9px] px-2 py-0.5 ${config.bg} ${config.text} rounded-full font-bold`}
            >
            ...
            <span className="text-[9px] px-2 py-0.5 bg-slate-700 text-slate-300 rounded-full font-bold">
              {getRecommendationIcon()} {recommendation === "deload"
                ? "Deload"
      ...
      {isExpanded && (
        <div className="mt-4 pt-3 border-t border-white/5 space-y-3">
          ...
          {/* Action items */}
          <div>
            <h4 className="text-[9px] font-black text-slate-500 uppercase mb-2">
 ...
 */
export function VolumeRecommendationCard(
  { recommendation }: VolumeRecommendationCardProps,
) {
  const {
    exerciseName,
    currentWeeklyVolume,
