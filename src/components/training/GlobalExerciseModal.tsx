...
export function GlobalExerciseModal(
  { isOpen, onClose, initialType, initialInput, initialDate }:
    GlobalExerciseModalProps,
) {
  ...
  const effectiveDuration = parsed?.duration?.toString() ||
  ...
  const handleSave = () => {
    ...
    addExercise({
      ...
      tonnage: exerciseForm.tonnage
        ? parseFloat(exerciseForm.tonnage)
        : undefined,
      distance: exerciseForm.distance
        ? parseFloat(exerciseForm.distance)
        : undefined,
    });
