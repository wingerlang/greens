...
export function ExerciseDetailModal({
  ...
  const [viewMode, setViewMode] = useState<"history" | "prs" | "annual">(
    "history",
  );
  const [metricMode, setMetricMode] = useState<"absolute" | "relative">(
    "absolute",
  );
  ...
  const exerciseId = `ex-${
    normalizeExerciseName(exerciseName).replace(/\s/g, "-")
  }`;
  ...
  const handleReset = async () => {
    ...
    try {
      const res = await fetch(
        `/api/strength/exercise/${encodeURIComponent(exerciseId)}/reset`,
        {
          method: "DELETE",
  ...
  const exerciseHistory = useMemo(() => {
    ...
    workouts.forEach((w) => {
      ...
      if (exerciseEntries.length > 0) {
        ...
        const bestSetEntry = est1RMs.reduce(
        ...
        const maxTimeFormatted = allSets.find((s) =>
          s.timeSeconds === maxTime
        )?.time || "";
        ...
        const totalCalories = allSets.reduce(
          (sum, s) => sum + (s.calories || 0),
          0,
        );

        const bestTempo = allSets.find((s) =>
          (s.distance === maxDistance && maxDistance > 0) ||
          (s.timeSeconds === maxTime && maxTime > 0)
        )?.tempo;
        ...
        if (set.weight > currentMax) {
          ...
          if (lastPrDate) {
            daysSinceLast = Math.round(
              (currentDate.getTime() - lastPrDate.getTime()) /
                (1000 * 60 * 60 * 24),
            );
          }
  ...
  const annualBestData = useMemo(() => {
    ...
    exerciseHistory.forEach((h) => {
      ...
      if ((h.maxDistance || 0) > byYear[year].maxDistance) {
        byYear[year].maxDistance = h.maxDistance || 0;
      }
      ...
      const val = isWeightedDistanceExercise(exerciseName)
        ? h.maxWeight
        : isDistanceBasedExercise(exerciseName)
        ? (h.maxDistance || 0)
        : h.maxWeight;

      const existingMonth = byYear[year].monthlyMaxes.find((m) =>
        m.date.startsWith(monthKey)
      );
      if (!existingMonth) {
        byYear[year].monthlyMaxes.push({
          value: val,
          date: h.date,
          workout: h.workout,
        });
  ...
  const totalSets = useMemo(
    () => exerciseHistory.reduce((sum, h) => sum + h.sets, 0),
    [exerciseHistory],
  );
  const totalReps = useMemo(
    () => exerciseHistory.reduce((sum, h) => sum + h.reps, 0),
    [exerciseHistory],
  );
  const totalVolume = exerciseHistory.reduce(
    (sum: number, h) => sum + h.volume,
    0,
  );
  ...
  const maxRecord = useMemo(() => {
    ...
    if (hasCals) {
      return exerciseHistory.reduce(
        (prev, curr) =>
          ((curr as any).totalCalories || 0) >
              ((prev as any).totalCalories || 0)
            ? curr
            : prev,
        exerciseHistory[0],
      );
    ...
    if (metricMode === "relative") {
      return exerciseHistory.reduce(
        (
          prev,
          curr,
        ) => ((curr.relativeMaxWeight || 0) > (prev.relativeMaxWeight || 0)
          ? curr
          : prev),
        exerciseHistory[0],
      );
  ...
  const bestRecord = useMemo(() => {
    ...
    if (isDistanceBasedExercise(exerciseName)) {
      return exerciseHistory.reduce(
        (
          prev,
          curr,
        ) => ((curr.maxDistance || 0) > (prev.maxDistance || 0) ? curr : prev),
        exerciseHistory[0],
      );
    }
    if (metricMode === "relative") {
      return exerciseHistory.reduce(
        (
          prev,
          curr,
        ) => ((curr.relativeEst1RM || 0) > (prev.relativeEst1RM || 0)
          ? curr
          : prev),
        exerciseHistory[0],
      );
    ...
    return exerciseHistory.reduce(
      (prev, curr) => (curr.est1RM > prev.est1RM ? curr : prev),
      exerciseHistory[0],
    );
  ...
  const bestValueDisplay = useMemo(() => {
    ...
    if (isDistanceBasedExercise(exerciseName)) {
      const bestCalH = exerciseHistory.reduce(
        (prev, curr) =>
          ((curr as any).totalCalories || 0) >
              ((prev as any).totalCalories || 0)
            ? curr
      ...
      return `${
        Math.round(bestRecord?.totalDistance || bestRecord?.maxDistance || 0)
      }m`;
    }
  ...
  const contextStats = useMemo(() => {
    ...
    const overallBestEst = exerciseHistory.reduce(
      (max, h) => Math.max(max, h.est1RM),
      0,
    );

    // Annual Best for Current Year
    const currentYearHistory = exerciseHistory.filter((h) =>
      h.date.startsWith(currentYear)
    );
    const annualBestEst = currentYearHistory.reduce(
      (max, h) => Math.max(max, h.est1RM),
      0,
    );
    const annualBestAct = currentYearHistory.reduce(
      (max, h) => Math.max(max, h.maxWeight),
      0,
    );

    const bestSession = currentYearHistory.find((h) =>
      h.maxWeight === annualBestAct
    );
    ...
    if (bestDate) {
      const diffTime = Math.abs(
        new Date().getTime() - new Date(bestDate).getTime(),
      );
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      if (diffDays < 30) timeAgo = `${diffDays} dagar sedan`;
      else if (diffDays < 365) {
        timeAgo = `${Math.floor(diffDays / 30)} mån sedan`;
      } else timeAgo = "> 1 år sedan";
    }

    // Is Current Session the Annual Best?
    const isAnnualBest = lastSession.maxWeight >= annualBestAct &&
      annualBestAct > 0;
    ...
    const overallBestAct = exerciseHistory.reduce(
      (max, h) => Math.max(max, h.maxWeight),
      0,
    );
    ...
    const recentHistory = exerciseHistory.filter((h) =>
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-baseline gap-3">
              ...
              <p className="text-slate-500 text-xs font-medium translate-y-[-2px]">
                {exerciseHistory.length} pass med denna övning
              </p>
            </div>
          </div>
        ...
        {showResetConfirm && (
          ...
          >
            <div
              className="bg-slate-800 border border-rose-500/30 rounded-2xl p-6 max-w-sm mx-4 space-y-4"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="text-center">
                <div className="text-4xl mb-2">⚠️</div>
                <h3 className="text-lg font-bold text-white">
                  Återställ "{exerciseName}"?
                </h3>
                <p className="text-sm text-slate-400 mt-2">
                  Detta raderar alla personliga rekord för denna övning.
                  Träningshistoriken behålls.
                </p>
              ...
              <div
                className={`absolute inset-0 ${
                  contextStats.isAllTimeBest
                    ? "bg-amber-500/10"
                    : "bg-yellow-500/5"
                } blur-xl group-hover:opacity-80 transition-all duration-1000`}
              >
              </div>
            )}
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                    Årsbästa {new Date().getFullYear()}
                  </p>
                  {contextStats.isAllTimeBest && (
                    <span className="text-[8px] bg-amber-500 text-slate-950 px-1.5 py-0.5 rounded font-black tracking-tighter animate-pulse">
                      ALL TIME REKORD
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {metricMode === "relative" &&
                            contextStats.bestSessionWorkout?.bodyWeight
                          ? (contextStats.annualBestAct /
                            contextStats.bestSessionWorkout.bodyWeight).toFixed(
                              2,
                            ) + "x"
                    ...
                    <div className="flex gap-2 text-[10px] text-slate-400 font-medium mt-1">
                      ...
                      <span>
                        e1RM: {Math.round(contextStats.annualBestEst)}kg
                      </span>
                    </div>
                  </div>
                  {contextStats.bestSessionWorkout && onSelectWorkout && (
                    <button
                      onClick={() =>
                        onSelectWorkout(contextStats.bestSessionWorkout!)}
              ...
              <div className="pl-4 border-l border-white/5">
                ...
                <div
                  className={`text-sm font-bold flex flex-col ${
                    contextStats.trend === "up"
                      ? "text-emerald-400"
                      : contextStats.trend === "down"
                      ? "text-rose-400"
                      : "text-slate-400"
                  }`}
                >
                  <div className="flex items-center gap-1">
                    <span>
                      {contextStats.trend === "up"
                        ...
                        : contextStats.trend === "down"
                        ? "↘"
                        : "→"} {Math.abs(contextStats.trendKgDiff).toFixed(1)}
                      {" "}
                      kg
                    </span>
                    <span className="text-[10px] opacity-70">
                      ({contextStats.trendPercent > 0 ? "+" : ""}
                      {contextStats.trendPercent.toFixed(1)}%)
                    </span>
                  </div>
                  <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                    {contextStats.trendSessionCount} pass under perioden
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div
            className={`bg-emerald-500/10 border border-emerald-500/30 rounded-xl pt-3 pb-1 px-2 text-center relative flex flex-col items-center justify-start ${
              isDistanceBasedExercise(exerciseName)
                ? "col-span-2"
                : "col-span-1"
            }`}
            ...
            >
              {maxRecord && (
                <span className="absolute top-1.5 right-2 text-[9px] text-emerald-500/60 font-mono">
                  {new Date(maxRecord.date).toLocaleDateString("sv-SE", {
                    month: "short",
                    day: "numeric",
                    year: "2-digit",
                  })}
              ...
              <div className="mb-1 mt-1">
                {(() => {
                  const maxSet =
                    !isDistanceBasedExercise(exerciseName) && maxRecord
                      ? maxRecord.workout.exercises.flatMap((e) => e.sets).find(
                        (s) => s.weight === maxRecord.maxWeight,
                      )
                      : null;
                  ...
                  if (isDistanceBasedExercise(exerciseName)) {
                    ...
                    if (h?.totalCalories > 0) {
                      subText = `${h.totalTimeFormatted} • ${
                        Math.round(h.totalDistance)
                      }m`;
                    } else if (h?.bestTempo) {
                    ...
                    mainText = maxSet
                      ? `${maxSet.reps}x${maxSet.weight}kg`
                      : `${maxEver} kg`;
                  }
                  ...
                  return (
                    <div className="flex flex-col items-center leading-none">
                      <div className="flex items-center gap-1 justify-center">
                        ...
                        {metricMode === "relative" &&
                          (maxRecord?.bwDaysDiff || 0) > 60 && (
                          <span
                            title={`Viktdata är ${maxRecord?.bwDaysDiff} dagar gammal`}
                ...
                <p className="text-[9px] text-emerald-500 uppercase font-bold flex items-center justify-center gap-1 mt-0.5">
                  {(maxRecord as any)?.totalCalories > 0
                    ? "Bästa Kaloripass"
                    : isDistanceBasedExercise(exerciseName)
                    ? "Längsta Distans"
                    : "Tyngsta Lyft"}
                  <span className="opacity-0 group-hover/main:opacity-100 transition-opacity">
                    →
                  </span>
                </p>
              </div>
              ...
              return (
                <div className="space-y-0.5 mt-2 border-t border-emerald-500/20 pt-1 w-full px-1">
                  {top3.map((rec, i) => {
                    ...
                    const weightText = recSet
                      ? `${recSet.reps}x${recSet.weight}kg`
                      : `${rec.maxWeight}kg`;

                    let displayText = weightText;
                    if (metricMode === "relative") {
                      displayText = `${
                        (rec.relativeMaxWeight || 0).toFixed(2)
                      }x`;
                    }
          ...
          {!isDistanceBasedExercise(exerciseName) && (
            <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl pt-3 pb-1 px-2 text-center relative flex flex-col items-center justify-start col-span-1">
              <button
                onClick={() =>
                  bestRecord && onSelectWorkout?.(bestRecord.workout)}
              ...
              >
                {bestRecord && (
                  <span className="absolute top-1.5 right-2 text-[9px] text-purple-500/60 font-mono">
                    {new Date(bestRecord.date).toLocaleDateString("sv-SE", {
                      month: "short",
                      day: "numeric",
                      year: "2-digit",
                    })}
                  ...
                  <div className="flex flex-col items-center leading-none">
                    <div className="flex items-center gap-1 justify-center">
                      <span className="text-lg font-black text-purple-400 group-hover/main:text-purple-300 transition-colors text-nowrap truncate px-1">
                        {metricMode === "relative"
                          ? (bestRecord?.relativeEst1RM
                            ? `${bestRecord.relativeEst1RM.toFixed(2)}x`
                            : "N/A")
                          : (bestRecord?.bestSetString || "0 kg")}
                      </span>
                      {metricMode === "relative" &&
                        (bestRecord?.bwDaysDiff || 0) > 60 && (
                        <span
                          title={`Viktdata är ${bestRecord?.bwDaysDiff} dagar gammal`}
                          className="text-[10px] cursor-help"
                        >
                          ⚠️
                        </span>
                ...
                return (
                  <div className="space-y-0.5 mt-2 border-t border-purple-500/20 pt-1 w-full px-1">
                    {top3.map((rec, i) => {
                      ...
                      if (metricMode === "relative") {
                        displayText = `${
                          (rec.relativeEst1RM || 0).toFixed(2)
                        }x`;
          ...
          <div className="bg-slate-800/30 rounded-2xl p-6 border border-white/5">
            {(() => {
              if (viewMode === "annual") {
                if (annualBestData.length < 2) {
                  return (
                    <p className="text-center text-slate-500 py-8">
                      Inte nog med data för att visa årsbästa.
                    </p>
                  );
                }
                ...
                const maxValRaw = Math.max(
                  ...annualBestData.map((d) =>
                    isDist ? d.maxDistance : d.maxEst1RM
                  ),
                );
                ...
                const maxVal = Math.max(
                  ...chartData.map((d) => Math.max(d.displayEst, d.displayAct)),
                );
                const height = 200;
                const width = 500;

                const getX = (i: number) =>
                  (i / (annualBestData.length - 1)) * width;
                const getY = (val: number) =>
                  height - (val / (maxVal * 1.1)) * height;

                const lineEst = annualBestData.map((d, i) =>
                  `${getX(i)},${getY(isDist ? d.maxDistance : d.maxEst1RM)}`
                ).join(" ");
                const lineAct = annualBestData.map((d, i) =>
                  `${getX(i)},${getY(d.maxWeight)}`
                ).join(" ");
                  ...
                  <div className="w-full h-[250px] relative">
                    ...
                    >
                      ...
                      {chartData.map((d, i) => (
                        <g key={`monthlies-${i}`}>
                          {d.monthlyMaxes.map((m, mIdx) => (
                            ...
                            >
                              <title>
                                {m.date}: {metricMode === "relative"
                                  ? m.displayVal.toFixed(2) + "x"
                                  : m.value + (isDist ? "m" : "kg")}
                              </title>
                            </circle>
                          ))}
                        ...
                        <>
                          ...
                          {chartData.map((d, i) => (
                            <circle
                              onClick={() =>
                                d.maxWeightWorkout &&
                                onSelectWorkout?.(d.maxWeightWorkout)}
                              key={`act-${i}`}
                              cx={getX(i)}
                      ...
                      {chartData.map((d, i) => {
                        ...
                        const actSet = !isDist && d.maxWeightWorkout
                          ? d.maxWeightWorkout.exercises.flatMap((e) => e.sets)
                            .find((s) => s.weight === rawAct)
                          : null;
                        const actString = actSet
                          ? `${actSet.reps}x${actSet.weight}kg`
                          : `${rawAct}kg`;
                        ...
                        return (
                          <g
                            ...
                            onClick={() =>
                              d.maxEst1RMWorkout &&
                              onSelectWorkout?.(d.maxEst1RMWorkout)}
                          >
                            <circle
                              cx={getX(i)}
                          ...
                          <div key={d.year} className="text-center">
                            ...
                            {!isDist && (
                              <div className="flex flex-col items-center">
                                ...
                                <p className="text-[8px] text-amber-500/60 font-mono scale-90">
                                  e1RM: {Math.round(d.maxEst1RM)}
                                </p>
                              </div>
                            )}
                          </div>
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className={`w-3 h-1 rounded-full ${
                            isDist ? "bg-blue-500" : "bg-amber-500"
                          }`}
                        >
                        </span>
                        <span className="text-slate-400">
                          {isDist ? "Max Distans" : "Estimerat 1RM"}
                        </span>
                      </div>
                      {!isDist && (
                        <div className="flex items-center gap-1">
                          <span className="w-3 h-1 rounded-full bg-emerald-500/50 border-b border-emerald-500 border-dashed">
                          </span>
              ...
              if (activeData.length < (isHistory ? 2 : 1)) {
                return (
                  <p className="text-center text-slate-500 py-8">
                    Inte nog med data för att visa progression.
                  </p>
                );
              }
              ...
              const getMetric = (h: any) => {
                if (isDistanceBasedExercise(exerciseName)) {return h
                    .maxDistance || 0;}
                if (isWeightedDistanceExercise(exerciseName)) {return h
                    .maxWeight || 0; // Use maxWeight for sled push
                }
              ...
              const cMin = activeData.length === 1
                ? minVal * 0.9
                : minVal === maxVal
                ? minVal * 0.9
                : minVal * 0.95;
              ...
              return (
                <>
                  <div className="relative h-40 mb-2 mt-8">
                      ...
                      >
                        <span
                          className={`w-2 h-2 rounded-full animate-pulse ${
                            isHistory ? "bg-amber-400" : "bg-emerald-400"
                          }`}
                        >
                        </span>
                    ...
                    <div className="flex items-end gap-1.5 h-full pl-2">
                      {activeData.map((h, i) => {
                        ...
                        if (isNaN(heightPercent) || heightPercent < 15) {
                          heightPercent = 15;
                        }
                          ...
                          >
                            ...
                            >
                              <div className="flex flex-col gap-0.5">
                                ...
                                {!isDistanceBasedExercise(exerciseName) && (
                                  <p className="text-[9px] text-slate-300 font-medium">
                                    ...
                                    {isHistory && (h as any).bestSetString
                                      ...
                                      : `${(h as any).maxWeightReps || "-"}x${
                                        (h as any).maxWeight || 0
                                      }kg`}{" "}
                                ...
                                <p className="text-[9px] text-slate-500 mt-1 pt-1 border-t border-white/5">
                                  {h.date}
                                </p>
                            ...
                            <div
                              className={`w-full relative transition-all duration-300 group-hover:brightness-110 ${
                                isBest
                                  ? (isHistory
                  ...
                  <div className="flex justify-between text-[10px] text-slate-500 mt-4 font-mono font-medium pl-2">
                    ...
                    {activeData.length > 2 && (
                      <span>
                        {activeData[Math.floor(activeData.length / 2)]?.date}
                      </span>
                    )}
            ...
            ? (
              <div className="text-center p-4 bg-slate-800/30 rounded-xl border border-white/5">
                <p className="text-xs text-slate-400">
                  Grafern visar ditt starkaste estimerade 1RM (orange) och ditt
                  tyngsta faktiska lyft (grön) för respektive år.
                </p>
              </div>
            )
            : viewMode === "history"
                      ...
                      <tbody className="divide-y divide-white/5">
                        {exerciseHistory.slice().reverse().map((h, idx) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right text-slate-400 group-hover:text-white transition-colors">
                              {h.sets}
                            </td>
                            ...
                            {isDistanceBasedExercise(exerciseName)
                              ...
                              : (
                                <>
                                  ...
                                  <td className="px-4 py-3 text-right text-slate-400 text-xs">
                                    {Math.round(h.volume).toLocaleString()}{" "}
                                    {isWeightedDistanceExercise(exerciseName)
                                      ? "kg"
                                      : "kg"}
                                  </td>
            ...
            : (
              <div>
                <h3 className="text-sm font-bold text-amber-500 uppercase mb-3 px-1 flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full border border-amber-500/20">
                    {prProgression.length} st
                  </span>
                </h3>
                <div className="space-y-3">
                          ...
                          {groups.reverse().map((prs, groupIdx) => {
                            ...
                            const sessionHeader = (
                              <div className="px-3 py-1.5 flex justify-between items-center bg-slate-900/40">
                                <button
                                  onClick={() =>
                                    onSelectWorkout?.(sortedPrs[0].workout)}
                            ...
                            return (
                              <div
                                ...
                                className={`${
                                  isMulti
                                    ? "bg-slate-800/30 border border-white/5 rounded-2xl overflow-hidden"
                                    : "bg-slate-800/20 border-l-2 border-emerald-500 rounded-r-xl overflow-hidden"
                                }`}
                              >
                                ...
                                >
                                  {!isMulti && (
                                    <div className="flex items-center gap-3 flex-1">
                                      ...
                                      <div>
                                        <p className="text-[10px] text-slate-500 font-mono leading-none mb-1">
                                          {prs[0].date}
                                        </p>
                                  ...
                                  {isMulti && (() => {
                                    const mappedSets = sortedPrs.map(
                                      (s: any) => {
                                        const isBW = s.isBodyweight ||
                                          s.weight === 0;
                                        const calcWeight = isBW
                                          ? (s.extraWeight || 0)
                                          : s.weight;
                                    ...
                                    const maxEst1RM = Math.max(
                                      ...mappedSets.map((s) => s.est1RM),
                                    );

                                    return mappedSets.map((pr: any, idx) => {
                                      const isBest1eRM = maxEst1RM > 0 &&
                                        pr.est1RM === maxEst1RM;
                                      return (
                                        <div
                                          key={`${pr.date}-${pr.weight}-${idx}`}
                                          className="bg-slate-800/50 hover:bg-slate-800 rounded-xl px-3 py-2 flex items-center justify-between group transition-all"
                                        >
                                          <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                              <div className="flex items-baseline gap-2">
                                                <p className="text-lg font-black text-white group-hover:text-emerald-400 transition-colors leading-none">
                                                  {pr.weight} kg
                                                </p>
                                          ...
                                          <p className="text-[8px] text-slate-500 font-mono">
                                            1eRM: {(() => {
                                              ...
                                              const calcWeight = isBW
                                                ? (prs[0].extraWeight || 0)
                                                : (prs[0].weight || 0);
                                              return calculateEstimated1RM(
                                                calcWeight,
                                                prs[0].reps || 0,
                                              );
                                            })()}kg
                                          </p>
                                          {((prs[0].percentIncrease !==
                                              undefined &&
                                            prs[0].percentIncrease > 0) ||
                                            prs[0].daysSinceLast !==
                                              undefined) && (
                  ...
                  {prProgression.length === 0 && (
                    <p className="text-center text-slate-500 py-12 bg-slate-800/30 rounded-2xl border border-dashed border-white/10">
                      Inga PR-data hittades.
                    </p>
                  )}
                </div>
              </div>
