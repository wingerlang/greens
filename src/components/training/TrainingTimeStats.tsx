...
import { PersonalBest, StrengthWorkout } from "../../models/strengthTypes.ts";
...
export function TrainingTimeStats(
  { workouts, days, personalBests = [], dateRangeLabel }:
    TrainingTimeStatsProps,
) {
  ...
  const stats = useMemo(() => {
    ...
    const actualDays = Math.max(
      7,
      Math.ceil(
        (newestWorkout.getTime() - oldestWorkout.getTime()) /
          (24 * 60 * 60 * 1000),
      ),
    );
    ...
    const totalExercises = filteredWorkouts.reduce(
      (sum, w) => sum + (w.uniqueExercises || w.exercises?.length || 0),
      0,
    );
    ...
    personalBests.filter((pb) => new Date(pb.date) >= oldestWorkout).forEach(
      (pb) => {
    ...
    filteredWorkouts.forEach((w) => {
      const day = new Date(w.date).getDay();
      const adjustedDay = day === 0 ? 6 : day - 1;
      exercisesByDay[adjustedDay] += w.uniqueExercises || w.exercises?.length ||
        0;
    });
  ...
  const getChartData = () => {
    switch (viewMode) {
      case "sessions":
        return stats.sessionsByDay;
      case "volume":
        return stats.volumeByDay;
      case "pbs":
        return stats.pbsByDay;
      case "exercises":
        return stats.exercisesByDay;
      case "sets":
        return stats.setsByDay;
      default:
        return stats.sessionsByDay;
    }
  };
    ...
    <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-4 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-white font-black uppercase text-[10px] tracking-wider">
              Träningsrytm
            </h3>
            <p className="text-[8px] text-slate-400 font-bold">
              {dateRangeLabel || `Senaste ${days} dagar`} ·{" "}
              {stats.totalWorkouts} pass · {stats.totalHours}h totalt
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-end justify-around gap-1">
          {["Mån", "Tis", "Ons", "Tor", "Fre", "Lör", "Sön"].map((day, i) => {
            ...
            const barHeight = isZero
              ? 2
              : Math.max(6, Math.round((value / maxValue) * maxBarHeight));
