...
function getExerciseMuscle(exerciseName: string): string {
  ...
  // Fallback heuristics
  if (
    normalized.includes("bench") || normalized.includes("push up") ||
    normalized.includes("push-up") || normalized.includes("chest")
  ) return "Chest";
  if (
    normalized.includes("row") || normalized.includes("pull up") ||
    normalized.includes("pull-up") || normalized.includes("lat")
  ) return "Back";
  if (
    normalized.includes("squat") || normalized.includes("leg press") ||
  ...
  if (normalized.includes("curl") && normalized.includes("hammer")) {
    return "Biceps";
  }
  if (normalized.includes("curl")) return "Biceps";
  if (
    normalized.includes("tricep") || normalized.includes("extension") ||
  ...
  if (normalized.includes("shrug") || normalized.includes("trap")) {
    return "Traps";
  }
  if (
    normalized.includes("crunch") || normalized.includes("plank") ||
    normalized.includes("ab")
  ) return "Core";
  if (normalized.includes("calf") || normalized.includes("calves")) {
    return "Calves";
  }
  if (normalized.includes("glute") || normalized.includes("hip thrust")) {
    return "Glutes";
  }
  if (normalized.includes("hamstring") || normalized.includes("leg curl")) {
    return "Hamstrings";
  }
...
export function MuscleVolumeChart({ workouts }: MuscleVolumeChartProps) {
  ...
  const muscleData = useMemo(() => {
    ...
    const muscles = new Set([
      ...Object.keys(currentWeekVolume),
      ...Object.keys(last4WeeksVolume),
    ]);
  ...
  const maxVolume = Math.max(
    ...muscleData.map((d) => Math.max(d.currentWeek, d.avg4Week)),
    1,
  );
