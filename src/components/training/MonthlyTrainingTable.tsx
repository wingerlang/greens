...
export function MonthlyTrainingTable(
  { exercises, year }: MonthlyTrainingTableProps,
) {
  ...
  const data = useMemo(() => {
    ...
    exercises.forEach((e) => {
      ...
      if (activeTab === "running") {
        matchesTab = type === "running" || type === "löpning";
      } else if (activeTab === "strength") {
        matchesTab = type === "strength" || type === "styrka" || type === "gym";
      } else if (activeTab === "cycling") {
        matchesTab = type === "cycling" || type === "cykling";
      } else if (activeTab === "swimming") {
        matchesTab = type === "swimming" || type === "simning";
  ...
  const fmtTon = (ton: number) =>
    ton > 0 ? (ton / 1000).toFixed(1).replace(".", ",") + " ton" : "-";
  ...
  const totals = useMemo(() => {
    return data.reduce((acc, curr) => ({
      ...
      categories: {
        cardio: {
          distance: acc.categories.cardio.distance +
            curr.categories.cardio.distance,
          duration: acc.categories.cardio.duration +
            curr.categories.cardio.duration,
          count: acc.categories.cardio.count + curr.categories.cardio.count,
        },
        strength: {
          tonnage: acc.categories.strength.tonnage +
            curr.categories.strength.tonnage,
          duration: acc.categories.strength.duration +
            curr.categories.strength.duration,
          count: acc.categories.strength.count + curr.categories.strength.count,
        },
        other: {
          duration: acc.categories.other.duration +
            curr.categories.other.duration,
    ...
    <div className="bg-slate-900 border border-white/10 rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="grid grid-cols-[100px_1fr] bg-slate-900/80 text-xs uppercase font-bold text-slate-500 border-b border-white/10">
        ...
        <div className="grid grid-cols-[1.6fr_0.4fr] divide-x divide-white/5">
          ...
          <div className="text-center p-2 text-slate-500 text-[9px] bg-slate-900/30 flex items-center justify-center">
            TOTALT
          </div>
        </div>
      </div>
              ...
              ? (
                <>
                  <div className="p-2 text-right border-r border-white/5 text-emerald-500/70">
                    Distans <span className="text-slate-600">•</span> Tid{" "}
                    <span className="text-slate-600">•</span> Pass
                  </div>
                  <div className="p-2 text-right border-r border-white/5 text-indigo-500/70">
                    Volym <span className="text-slate-600">•</span> Tid{" "}
                    <span className="text-slate-600">•</span> Pass
                  </div>
      ...
      <div className="divide-y divide-white/5">
              ...
              if (emptyStart !== null) {
                ...
                rows.push(
                  <div
                    ...
                    className="text-xs text-slate-600 bg-black/20 p-3 italic text-center"
                  >
                  ...
                  <div className="grid grid-cols-[1.6fr_0.4fr] divide-x divide-white/5 pointer-events-none">
                    ...
                    >
                      {activeTab === "all"
                          ...
                          <>
                            <div className="p-3 text-right font-mono text-[11px] border-r border-white/5 whitespace-nowrap overflow-hidden text-ellipsis">
                              <span className="text-emerald-400 font-bold">
                                {row.categories.cardio.distance > 0
                                  ? row.categories.cardio.distance.toFixed(1)
                                    .replace(".", ",")
                                  : "-"} km
                              </span>
                              <span className="text-slate-600 mx-1.5">•</span>
                              <span className="text-slate-300">
                                {row.categories.cardio.duration > 0
                                  ? fmtDur(row.categories.cardio.duration)
                                  : "-"}
                              </span>
                              <span className="text-slate-600 mx-1.5">•</span>
                              <span className="text-slate-500">
                                {row.categories.cardio.count || "-"} st
                              </span>
                            </div>
                            <div className="p-3 text-right font-mono text-[11px] border-r border-white/5 whitespace-nowrap overflow-hidden text-ellipsis">
                              <span className="text-indigo-400 font-bold">
                                {row.categories.strength.tonnage > 0
                                  ? (row.categories.strength.tonnage / 1000)
                                    .toFixed(1).replace(".", ",")
                                  : "-"} ton
                              </span>
                              <span className="text-slate-600 mx-1.5">•</span>
                              <span className="text-slate-300">
                                {row.categories.strength.duration > 0
                                  ? fmtDur(row.categories.strength.duration)
                                  : "-"}
                              </span>
                              <span className="text-slate-600 mx-1.5">•</span>
                              <span className="text-slate-500">
                                {row.categories.strength.count || "-"} st
                              </span>
                            </div>
                            ...
                            <div className="p-3 text-right text-sky-500/50 font-mono text-[10px] font-bold">
                              {totals.total.count > 0
                                ? (row.total.count / totals.total.count * 100)
                                  .toFixed(0) + "%"
                                : "-"}
                            </div>
                          </>
                        ...
                        ? (
                          <>
                            <div className="p-3 text-right text-indigo-300 font-mono">
                              {fmtTon(row.selected.tonnage)}
                            </div>
                            <div className="p-3 text-right font-mono">
                              {fmtDur(row.selected.duration)}
                            </div>
                            <div className="p-3 text-right font-mono">
                              {row.selected.count || "-"}{" "}
                            ...
                            <div className="p-3 text-right text-slate-400 font-mono">
                              {row.selected.count > 0
                                ? ((row.selected.tonnage / 1000) /
                                  row.selected.count).toFixed(1) + " t"
                                : "-"}
                            </div>
                          </>
                        )
                        : (
                          <>
                            <div className="p-3 text-right text-emerald-300 font-mono">
                              {fmtDist(row.selected.distance)}
                            </div>
                            <div className="p-3 text-right font-mono">
                              {fmtDur(row.selected.duration)}
                            </div>
                            ...
                            <div className="p-3 text-right text-slate-400 font-mono">
                              {row.selected.count > 0
                                ? (row.selected.distance / row.selected.count)
                                  .toFixed(1) + " km"
                                : "-"}
                            </div>
                          </>
                    ...
                    <div className="grid grid-cols-2 bg-slate-900/30">
                      <div className="p-3 text-right font-mono text-slate-300">
                        {row.total.count || "-"}{" "}
                        <span className="text-xs text-slate-600">st</span>
                      </div>
                      <div className="p-3 text-right font-mono text-slate-300">
        ...
        <div className="grid grid-cols-[100px_1fr] text-sm font-bold bg-white/5 border-t border-white/10">
          ...
          <div className="grid grid-cols-[1.6fr_0.4fr] divide-x divide-white/5">
                    ...
                    <div className="p-3 text-right font-mono text-xs border-r border-white/5">
                      <span className="text-emerald-400">
                        {totals.categories.cardio.distance > 0
                          ? totals.categories.cardio.distance.toFixed(1)
                            .replace(".", ",")
                          : "-"} km
                      </span>
                      <span className="text-slate-600 mx-1.5">•</span>
                      <span className="text-slate-300">
                        {totals.categories.cardio.duration > 0
                          ? fmtDur(totals.categories.cardio.duration)
                          : "-"}
                      </span>
                      <span className="text-slate-600 mx-1.5">•</span>
                      <span className="text-slate-500">
                        {totals.categories.cardio.count || "-"} st
                      </span>
                    </div>
                    <div className="p-3 text-right font-mono text-xs border-r border-white/5">
                      <span className="text-indigo-400">
                        {totals.categories.strength.tonnage > 0
                          ? (totals.categories.strength.tonnage / 1000).toFixed(
                            1,
                          ).replace(".", ",")
                          : "-"} ton
                      </span>
                      <span className="text-slate-600 mx-1.5">•</span>
                      <span className="text-slate-300">
                        {totals.categories.strength.duration > 0
                          ? fmtDur(totals.categories.strength.duration)
                          : "-"}
                      </span>
                      <span className="text-slate-600 mx-1.5">•</span>
                      <span className="text-slate-500">
                        {totals.categories.strength.count || "-"} st
                      </span>
                    </div>
                    <div className="p-3 text-right text-slate-500">
                      {totals.categories.other.duration > 0
                        ? fmtDur(totals.categories.other.duration)
                        : "-"}
                    </div>
                ...
                ? (
                  <>
                    <div className="p-3 text-right text-indigo-400">
                      {fmtTon(totals.selected.tonnage)}
                    </div>
                    <div className="p-3 text-right text-white">
                      {fmtDur(totals.selected.duration)}
                    </div>
                    <div className="p-3 text-right text-white">
                      {totals.selected.count} st
                    </div>
                    <div className="p-3 text-right text-slate-400">
                      {totals.selected.count > 0
                        ? ((totals.selected.tonnage / 1000) /
                          totals.selected.count).toFixed(1) + " t"
                        : "-"}
                    </div>
                  </>
                )
                : (
                  <>
                    <div className="p-3 text-right text-emerald-400">
                      {fmtDist(totals.selected.distance)}
                    </div>
                    <div className="p-3 text-right text-white">
                      {fmtDur(totals.selected.duration)}
                    </div>
                    <div className="p-3 text-right text-slate-400">
                      {fmtPace(
                        totals.selected.distance,
                        totals.selected.duration,
                      )}
                    </div>
                    <div className="p-3 text-right text-white">
                      {totals.selected.count} st
                    ...
                    <div className="p-3 text-right text-slate-400">
                      {totals.selected.count > 0
                        ? (totals.selected.distance / totals.selected.count)
                          .toFixed(1) + " km"
                        : "-"}
                    </div>
                  </>
            ...
            <div className="grid grid-cols-2 bg-slate-900/30">
              <div className="p-3 text-right text-white">
                {totals.total.count} st
              </div>
              <div className="p-3 text-right text-white">
                {fmtDur(totals.total.duration)}
              </div>
            </div>
          </div>
        </div>
