...
export const PeriodWizard: React.FC<PeriodWizardProps> = (
  { isOpen, onClose, onSave },
) => {
  const { addTrainingPeriod, addGoal, getLatestWeight, getLatestWaist } =
    useData();
  ...
  const [configuredGoals, setConfiguredGoals] = useState<
    Omit<PerformanceGoal, "id" | "createdAt" | "periodId">[]
  >([]);
  const [advancedEditIndex, setAdvancedEditIndex] = useState<number | null>(
    null,
  );
  ...
  const handleSubmit = async () => {
    ...
    const period = await addTrainingPeriod({
      ...
      userId: "current",
      description: `Träningsperiod med fokus på ${
        focusList.map((f) => FOCUS_LABELS[f]).join(", ")
      }`,
    });
              ...
              <div className="space-y-6">
                <div className="text-center mb-6">
                  ...
                  <p className="text-white/50 text-sm">
                    Baserat på{" "}
                    {Array.from(foci).map((f) => FOCUS_LABELS[f]).join(" + ")}:
                  </p>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-6">
                <div className="text-center mb-6">
                  ...
                  <p className="text-white/50 text-sm">
                    Justera målvärdena för din period.
                  </p>
                </div>
