...
interface GoalTemplateRowProps {
  ...
  onChange: (
    updated: Omit<PerformanceGoal, "id" | "createdAt" | "periodId">,
  ) => void;
}

export const GoalTemplateRow: React.FC<GoalTemplateRowProps> = (
  { goal, onChange },
) => {
  ...
  const getPrimaryValue = () => {
    ...
    if (goal.type === "nutrition") {
      return goal.targets[0]?.value || goal.nutritionMacros?.calories || 0;
    }
    if (goal.targets && goal.targets.length > 0) {
      return goal.targets[0].value || goal.targets[0].count || 0;
    }
    return 0;
  };
  ...
  const getUnit = () => {
    ...
    if (goal.type === "streak") {
      return goal.category === "nutrition" ? "dagar" : "pass";
    }
    if (goal.targets && goal.targets.length > 0) {
      return goal.targets[0].unit || (goal.targets[0].count ? "pass" : "");
    }
    return "";
  };
  ...
  const handleChange = (val: string) => {
    ...
    if (goal.type === "weight") {
      ...
      if (!updated.targets[0]) {
        updated.targets[0] = { nutritionType: "calories" };
      }
      updated.targets[0].value = safeVal;
      updated.nutritionMacros = {
        ...updated.nutritionMacros,
        calories: safeVal,
      };
    } else {
