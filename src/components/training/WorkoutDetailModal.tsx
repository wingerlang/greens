...
export function WorkoutDetailModal(
    ...
    const fetchDailyActivities = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/activities?start=${workout.date}&end=${workout.date}`,
          {
  ...
  const stravaActivity = useMemo(() => {
    ...
    const getDetails = (a: UniversalActivity) => {
      ...
      const type = a.plan?.activityType || a.performance?.activityType ||
        "other";
      return { title, type };
    };
  ...
  const uniqueExercises = useMemo(() => {
    ...
    workout.exercises.forEach((ex) => {
      ...
      if (!aggregated[exId]) {
        ...
        // Merge volume
        current.totalVolume = (current.totalVolume || 0) +
          (ex.totalVolume || 0);

        // Update top set if better
        if (
          ex.topSet &&
          (!current.topSet || ex.topSet.weight > current.topSet.weight)
        ) {
    ...
    return Object.values(aggregated).map((exercise, i) => {
      const isPB = pbs.some((pb) =>
        pb.workoutId === workout.id && pb.exerciseId === exercise.exerciseId
      );
      return {
        ...exercise,
        isPB,
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {sessionNumber && (
                <span className="bg-white/10 text-white/60 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded border border-white/5">
                  Pass #{sessionNumber}
                  {sessionTotal ? `/${sessionTotal}` : ""} {sessionYear}
                </span>
              )}
            ...
            onClick={() =>
              window.open(
                `https://www.strava.com/activities/${
                  stravaActivity.performance?.source?.externalId || ""
                }`,
                "_blank",
              )}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white font-bold text-xs">
                  {stravaActivity.performance.notes ||
                    stravaActivity.plan?.title || "Strava Activity"}
                </p>
                <p className="text-[9px] text-orange-200/60 mt-0.5">
                  {stravaActivity.plan?.title}
                </p>
              </div>
            </div>
            ...
            <div className="flex box-border gap-4 text-right">
                  ...
                  <p className="text-sm font-black text-white flex items-center gap-1 justify-end">
                    ...
                    {stravaActivity.performance.maxHeartRate && (
                      <span className="text-[10px] text-slate-400 ml-1">
                        ({Math.round(stravaActivity.performance.maxHeartRate)})
                      </span>
                    )}
              ...
              {stravaActivity.performance.kudosCount !== undefined &&
                stravaActivity.performance.kudosCount > 0 && (
                <div>
                  <p className="text-[8px] uppercase font-bold text-slate-500">
                    Kudos
                  </p>
        ...
        {activeTab === "ANALYSIS"
          ? (
            <WorkoutAnalysisTab
              ...
              totalSessionVolume={grandTotalVolume}
            />
          )
          : (
            <>
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                {uniqueExercises.map((exercise, idx) => {
                  const isWeightedDist = isWeightedDistanceExercise(
                    exercise.exerciseName,
                  );
                  // Cast to boolean to ensure type safety
                  const isDist = !!isDistanceBasedExercise(
                    exercise.exerciseName,
                  );
                  const volumePct = grandTotalVolume > 0
                    ? ((exercise.totalVolume || 0) / grandTotalVolume * 100)
                    : 0;

                  return (
                    <div
                      key={idx}
                      className="bg-slate-800/30 rounded-xl overflow-hidden border border-white/5"
                    >
                      <div className="px-3 py-2 bg-slate-900/30 border-b border-white/5 flex justify-between items-center">
                        <div>
                          <h3
                            ...
                            onClick={() =>
                              onSelectExercise?.(exercise.exerciseName)}
                          >
                            {exercise.exerciseName}
                            {grandTotalVolume > 0 &&
                              (exercise.totalVolume || 0) > 0 && (
                              <span className="text-slate-500 text-[10px] font-normal ml-2">
                                ({volumePct.toFixed(0)}% t)
                              </span>
                            )}
                            {onSelectExercise && (
                              <span className="opacity-0 group-hover:opacity-100 text-slate-500 ml-2 text-xs">
                                →
                              </span>
                            )}
                          </h3>
                          <p className="text-[10px] text-slate-500">
                            ...
                            {Math.round(exercise.totalVolume || 0)} kg
                          </p>
                        </div>
                        {exercise.isPB && (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                          <thead className="bg-white/5 text-[9px] text-slate-400 uppercase font-black">
                            <tr>
                              <th className="px-3 py-1.5 text-left w-12">
                                Set
                              </th>
                              {isWeightedDist
                                ? (
                                  <>
                                    ...
                                    <th className="px-3 py-1.5 text-right text-slate-500">
                                      Not
                                    </th>
                                  </>
                                )
                          ...
                          <tbody className="divide-y divide-white/5">
                            {(() => {
                              ...
                              const maxEst1RM = Math.max(
                                ...setStats.map((s) => s.est1RM),
                              );

                              return setStats.map((set, sIdx) => {
                                const weightDisplay = set.isBW
                                  ? (set.extraWeight
                                    ? `+${set.extraWeight}kg`
                                    : "KV")
                                  : `${set.weight}kg`;
                                const isBest1RM = set.est1RM === maxEst1RM &&
                                  maxEst1RM > 0 && !isDist && !isWeightedDist;
                                const isPB = exercise.isPB &&
                                  set.weight === exercise.topSet?.weight;
              ...
              <div className="px-4 py-2 border-t border-white/5 bg-slate-900/50">
                <div
                  onClick={() =>
                    updateStrengthSession(workout.id, {
                      excludeFromStats: !workout.excludeFromStats,
                    })}
                  className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                    workout.excludeFromStats
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-lg ${
                        workout.excludeFromStats ? "opacity-100" : "opacity-40"
                      }`}
                    >
                      🚫
                    </span>
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500">
                        Passet räknas inte med i statistik och poäng
                      </p>
                    </div>
                  </div>
                  <div
                    className={`w-10 h-6 rounded-full relative transition-all ${
                      workout.excludeFromStats ? "bg-rose-500" : "bg-slate-700"
                    }`}
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${
                        workout.excludeFromStats ? "left-5" : "left-1"
                      }`}
                    />
                  </div>
                </div>
            ...
            >
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400 mt-2">
                  Detta tar bort passet permanent. Personliga rekord påverkas
                  inte.
                </p>
              </div>
