...
export function CompressedActivityList(
  { activities }: CompressedActivityListProps,
) {
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-white/5 bg-slate-950/40">
            <th className="px-2 py-2 text-[9px] font-black text-slate-500 uppercase tracking-widest">
              Datum
            </th>
            <th className="px-2 py-2 text-[9px] font-black text-slate-500 uppercase tracking-widest">
              Typ & Titel
            </th>
            <th className="px-2 py-2 text-[9px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">
              Dist (KM)
            </th>
            <th className="px-2 py-2 text-[9px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">
              Tid / Volym
            ...
            <React.Fragment key={activity.id}>
              <tr
                className={`group cursor-pointer transition-colors hover:bg-white/[0.02] ${
                  expandedId === activity.id ? "bg-indigo-500/5" : ""
                }`}
                onClick={() =>
                  setExpandedId(
                  ...
                  {(activity as any).tonnage
                    ...
                    : (
                      <span className="text-xs font-bold text-slate-300">
                        {formatSecondsToTime(
                          (activity.durationMinutes || 0) * 60,
                        )}
                        {activity.startTime && (
                          <div className="text-[9px] text-slate-500 font-normal">
                            kl {activity.startTime}
                          </div>
                        )}
                      </span>
                    )}
              ...
              {expandedId === activity.id && (
                <tr className="bg-slate-950/60">
                  <td
                    colSpan={5}
                    className="px-6 py-4 animate-in slide-in-from-top-2 duration-300"
                  >
                    ...
                    {activity.structure.mainSet &&
                      activity.structure.mainSet.length > 0 &&
                      (activity.structure.mainSet.length > 1 ||
                        (activity.structure.mainSet[0]?.reps || 1) > 1) &&
                      (
                        <div className="mt-4 pt-4 border-t border-white/5 flex flex-wrap gap-4">
                          {activity.structure.mainSet.map((s, i) => (
                            <div
                              key={i}
                              className="text-[10px] bg-slate-900 px-3 py-1.5 rounded-lg border border-white/5"
                            >
                              <span className="text-slate-500 font-black mr-2 uppercase tracking-tighter">
                                Set {i + 1}:
                              </span>
                              <span className="text-white font-black">
                                {s.reps}x {s.distKm}km @ {s.pace}
                              </span>
                            </div>
                          ))}
                        </div>
