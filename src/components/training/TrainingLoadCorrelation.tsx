...
export function TrainingLoadCorrelation({ exercises, weightEntries }: TrainingLoadCorrelationProps) {
    const chartData = useMemo(() => {
        // Group by week aggregate for clarity
        const sortedExercises = [...exercises].sort((a, b) => a.date.localeCompare(b.date));
        ...
        // Align to Monday
        firstDate.setDate(firstDate.getDate() - (firstDate.getDay() === 0 ? 6 : firstDate.getDay() - 1));
        ...
        while (current <= lastDate) {
            ...
            const weekWeight = weekWeightEntries.length > 0
                ? weekWeightEntries.reduce((sum, w) => sum + w.weight, 0) / weekWeightEntries.length
                : null;

            const totalLoad = weekExercises.reduce((sum, ex) => sum + calculateTrainingLoad(ex), 0);
        ...
        <div className="correlation-dashboard mt-6 bg-slate-900/30 rounded-3xl p-6 border border-white/5">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">Korrelation mellan intensitet och fysik</p>
                </div>
                <div className="flex gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Vikt</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-4">
                <div className="flex-1 bg-white/5 rounded-2xl p-3 border border-white/5">
                    <span className="text-[9px] font-black text-indigo-400 uppercase block mb-1">Load Analys</span>
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                        Dina senaste 4 veckor visar en {chartData.length >= 4 && chartData[chartData.length - 1].load > chartData[chartData.length - 4].load ? 'ökande' : 'stabil'} trend i träningsstress.
                        Perfekt för att bygga långsiktig tålighet.
                    </p>
                </div>
                <div className="flex-1 bg-white/5 rounded-2xl p-3 border border-white/5">
                    <span className="text-[9px] font-black text-emerald-400 uppercase block mb-1">Fysisk Respons</span>
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                        Sikta på en jämn viktkurva (-0.2kg/vecka) vid hög load för att optimera kroppssammansättningen (fettförlust vs muskelmassa).
                    </p>
                </div>
            </div>
        </div>
