...
export function MonthlyCalendarModal(
  { monthIndex, year, exercises, onClose }: MonthlyCalendarModalProps,
) {
  const monthName = new Date(year, monthIndex).toLocaleString("sv-SE", {
    month: "long",
  });
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      const dateStr =
        new Date(year, monthIndex, i, 12).toISOString().split("T")[0];
        ...
        <div className="md:w-96 bg-slate-950/50 border-r border-white/5 p-8 flex flex-col gap-6 overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-800/40 p-4 rounded-xl border border-white/5">
              ...
              <p className="text-3xl font-black text-white">
                {Math.floor(stats.duration / 60)}h{" "}
                {Math.round(stats.duration % 60)}m
              </p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="bg-emerald-950/20 p-4 rounded-xl border border-emerald-500/10">
                <p className="text-[10px] text-emerald-500/80 uppercase font-black tracking-widest mb-1">
                  Distans
                </p>
              ...
              <div className="bg-indigo-950/20 p-4 rounded-xl border border-indigo-500/10">
                <p className="text-[10px] text-indigo-500/80 uppercase font-black tracking-widest mb-1">
                  Volym
                </p>
                <p className="text-2xl font-black text-indigo-400">
                  {(stats.tonnage / 1000).toFixed(1)}{" "}
            ...
            <div className="bg-slate-800/40 p-4 rounded-xl border border-white/5">
              ...
              <div className="space-y-3">
                {Object.entries(stats.timeDist).sort((a, b) => b[1] - a[1]).map(
                  ([type, mins]) => {
                      ...
                      <div key={type} className="text-sm">
                        <div className="flex justify-between mb-1">
                          <span className="text-slate-300 capitalize flex items-center gap-2 text-xs font-bold">
                            {type.includes("running") || type.includes("run")
                              ? "🏃 Löpning"
                              : type === "strength"
                              ? "🏋️ Styrka"
                              : type === "cycling"
        ...
        <div className="flex-1 p-8 bg-gradient-to-br from-slate-900 to-slate-800/50 overflow-y-auto">
          <div className="grid grid-cols-7 gap-1 mb-2">
            {["Mån", "Tis", "Ons", "Tor", "Fre", "Lör", "Sön"].map((d) => (
              <div
                key={d}
                className="text-center text-[10px] uppercase font-bold text-slate-600 tracking-wider"
              >
                {d}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 gap-2 auto-rows-[100px]">
            {calendarDays.map((date, i) => {
              if (!date) {
                return <div key={`empty-${i}`} className="bg-transparent" />;
              }

              const hasExercise = date.exercises.length > 0;
              const isHeavyDay = date.exercises.some((e) =>
                e.type === "strength" && (e.tonnage || 0) > 5000
              );
              const isLongRun = date.exercises.some(
                (e) => (e.type === "running" && (e.distance || 0) > 5),
              );
              ...
              return (
                <div
                  ...
                  className={`
                                    ...
                                    ${
                    isRace
                      ? "bg-amber-500/10 border-amber-500/50 hover:bg-amber-500/20 shadow-amber-500/20"
                      : hasExercise
                ...
                >
                  ...
                  <div className="flex flex-wrap content-end gap-1 mt-2">
                    {date.exercises.map((ex) => {
                      ...
                      if (
                        type.includes("strength") || type.includes("styrka")
                      ) {
                        icon = "🏋️";
                        color = "text-indigo-400";
                      }
                      if (type.includes("cycl") || type.includes("cyk")) {
                        icon = "🚴";
                        color = "text-sky-400";
                      }
                      if (type.includes("walk") || type.includes("prom")) {
                        icon = "🚶";
                        color = "text-amber-400";
                      }
                      if (type.includes("swim") || type.includes("sim")) {
                        icon = "🏊";
                        color = "text-cyan-400";
                      }
                      ...
                      return (
                        <div
                          key={ex.id}
                          title={`${ex.type} - ${ex.durationMinutes}m`}
                  ...
                  {hasExercise && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-slate-950 border border-white/10 rounded-lg p-3 shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 hidden md:block z-[60]">
                      <div className="text-[10px] text-slate-500 font-bold mb-1 border-b border-white/5 pb-1 flex justify-between">
                        ...
                        {isRace && (
                          <span className="text-amber-400">TÄVLING</span>
                        )}
                      </div>
                      {date.exercises.map((e, idx) => {
                        const isRun = e.type.includes("running") ||
                          e.type.includes("löp");
                        const isBike = e.type.includes("cycl") ||
                          e.type.includes("cyk");

                        return (
                          <div
                            key={idx}
                            className="flex justify-between items-center text-xs mb-1 last:mb-0"
                          >
                            <span
