...
export function RecipeSelectionModal({
  ...
  const [activeCategory, setActiveCategory] = useState<
    "alla" | "favoriter" | "snabba" | "vegetariskt"
  >("alla");
  ...
  const filteredRecipes = useMemo(() => {
    ...
    if (activeCategory === "snabba") {
      ...
      filtered = filtered.filter((r) => {
        ...
        return !t.includes("kyckling") && !t.includes("nötfärs") &&
          !t.includes("bacon");
      });
    }
    return filtered;
  ...
  const recipeEstimate = useMemo(() => {
    ...
    return calculateRecipeEstimate(
  ...
  const handleSavePayload = () => {
    ...
    (onSave as any)(
      selectedRecipeId,
      Object.keys(activeSwaps).length > 0 ? activeSwaps : undefined,
    );
    setModalSearch("");
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-5xl h-[85vh] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700 bg-slate-800">
          <div>
            ...
            <span className="text-sm text-slate-400 font-medium">
              {WEEKDAY_LABELS[editingSlot.day]} -{" "}
              {MEAL_TYPE_LABELS[editingSlot.meal]}
            </span>
          </div>
          <button
        ...
        <div className="flex gap-2 p-3 overflow-x-auto bg-slate-900/50 border-b border-slate-800 scrollbar-hide">
          {(["alla", "favoriter", "snabba", "vegetariskt"] as const).map(
            (cat) => (
              <button
                key={cat}
          ...
          <div className="overflow-y-auto p-4 flex flex-col gap-6 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-700">
            ...
            <div className="flex flex-col gap-3">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider px-1">
                {modalSearch ? "Sökresultat" : "Alla Recept"}{" "}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <div
                        className={`w-2 h-2 rounded-full ${
                          (MEAL_TYPE_COLORS[recipe.mealType || "dinner"] ||
                            "text-slate-400 bg-slate-500/10").split(" ")[0]
                            .replace("text-", "bg-")
                        }`}
                      />
                    </div>
                      ...
                      {smartSwaps.length > 0 && (
                        <div className="mx-6 mb-4 p-4 bg-emerald-950/30 border border-emerald-500/20 rounded-xl">
                          ...
                          <div className="flex flex-wrap gap-2 mb-4">
                            <button
                              onClick={() => {
                                ...
                                smartSwaps.forEach((s) => {
                                  if (
                                    s.reason === "price" || s.reason === "both"
                                  ) {
                                    priceSwaps[s.originalItem.id] =
                                      s.suggestion.id;
                                  }
                                });
                                setActiveSwaps((prev) => ({
                            ...
                            <button
                              onClick={() => {
                                ...
                                smartSwaps.forEach((s) => {
                                  if (
                                    s.reason === "co2" || s.reason === "both"
                                  ) {
                                    co2Swaps[s.originalItem.id] =
                                      s.suggestion.id;
                                  }
                                });
                                setActiveSwaps((prev) => ({
                          ...
                          <div className="space-y-3">
                            {smartSwaps.map(
                              (swap: SwapSuggestion, idx: number) => {
                                const originalId = swap.originalItem.id; // Use real ID
                                const isApplied = activeSwaps[originalId] ===
                                  swap.suggestion.id;
                        ...
                        <div className="grid grid-cols-2 gap-px bg-slate-800/50 border-y border-slate-800">
                          <div className="bg-slate-900/50 p-4 flex flex-col items-center justify-center text-center">
                            <span className="text-xl font-bold text-white block">
                              {Math.round(
                                recipeEstimate.calories / recipe.servings,
                              )}
                            </span>
                          ...
                          <div className="bg-slate-900/50 p-4 flex flex-col items-center justify-center text-center">
                            ...
                            >
                              {Math.round(
                                recipeEstimate.price / recipe.servings,
                              )} kr
                            </span>
                            <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">
                              Pris/port
              ...
              : (
                <div className="flex flex-col items-center justify-center h-full text-slate-600 p-8 text-center">
                  ...
                  <p className="text-sm font-medium">
                    Välj ett recept för att se detaljer
                  </p>
                </div>
              )}
          </div>
