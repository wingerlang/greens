...
export const CompactGoalCard: React.FC<CompactGoalCardProps> = (
  ...
  const friendlyTargetText = (() => {
    ...
    if (goal.type === "weight") {
      if (goal.targetWeight === undefined || goal.goalDirection === "stable") {
        return "Mål: Stabil vikt";
      }
      const isLoss = goal.goalDirection === "down";
      return isLoss
        ? `Gå ner till ${goal.targetWeight} kg`
        : `Gå upp till ${goal.targetWeight} kg`;
    }
    if (goal.type === "measurement") {
      ...
      const isLoss = goal.goalDirection === "down";
      return isLoss
        ? `Minska ${mt} till ${goal.targetMeasurement}${unit}`
        : `Öka ${mt} till ${goal.targetMeasurement}${unit}`;
    }
  ...
  const friendlyProgressText = (() => {
    ...
    return `${formatValue(Math.round(progress?.current || 0))} / ${
      formatValue(goal.targets[0]?.value || 0)
    } ${goal.targets[0]?.unit}`;
  })();
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        <div className="flex justify-between items-start">
          <h4 className="font-bold text-white text-base truncate pr-8">
            {goal.name}
          </h4>
        ...
        <div className="flex flex-col gap-0.5 mt-1">
          ...
          <span
            className={`text-xs font-bold ${
              percentage >= 100 ? "text-emerald-400" : "text-slate-300"
            }`}
          >
