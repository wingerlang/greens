...
export function GoalsOverviewWidget() {
    const { performanceGoals: goals, universalActivities, weightEntries } = useData();
    ...
    return (
        <div className={`col-span-12 md:col-span-6 lg:col-span-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm ${density === 'compact' ? 'p-2' : 'p-6'}`}>
            <div className="flex items-center justify-between mb-4">
                ...
                <a href="/goals" className="text-[10px] font-bold text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                    Visa alla →
                </a>
            </div>

            <div className={`flex flex-col ${density === 'compact' ? 'gap-1' : 'gap-3'}`}>
                {activeGoals.slice(0, 3).map(goal => (
                    <div
                        key={goal.id}
                        onClick={() => setSelectedGoal(goal)}
                        className={`group relative overflow-hidden bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer rounded-2xl border border-slate-200/50 dark:border-slate-700/30 ${density === 'compact' ? 'p-2' : 'p-3'}`}
                    >
                        <div className="flex items-center gap-3 relative z-10">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between">
                                    <h4 className={`${density === 'compact' ? 'text-xs' : 'text-sm'} font-bold text-slate-900 dark:text-white truncate`}>
                                        {goal.name}
                                    </h4>
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-slate-500 font-medium truncate">
                                    <span>
                                        {goal.type === 'speed' ? 'Snabbt' : 'Mål'}: {goal.progress.target} {goal.targets[0]?.unit}
                                    </span>
                                    {goal.endDate && (
                                        <>
                                            <span className="opacity-30">•</span>
                                            <span className={goal.progress.daysRemaining && goal.progress.daysRemaining < 7 ? 'text-amber-500' : ''}>
