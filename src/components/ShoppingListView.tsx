...
import { MEAL_TYPE_LABELS, WEEKDAY_LABELS } from "../models/types.ts";
...
export function ShoppingListView({
  ...
  const [quantityInput, setQuantityInput] = useState<
    { itemName: string; unit: string } | null
  >(null);
  const [inputValue, setInputValue] = useState("");
  const [selectedItem, setSelectedItem] = useState<ShoppingItem | null>(null);

  if (
    totalItems === 0 && Object.values(shoppingList).every((l) => l.length === 0)
  ) {
    return null;
  }
  ...
  const getCopyButtonClass = () => {
    const base =
      "flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border transition-all active:scale-95";
    if (copyStatus === "copied") {
    ...
    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 mb-8 shadow-xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center text-xl shadow-inner shadow-emerald-500/5">
            🛒
          </div>
          <div>
            ...
            <div className="text-sm text-slate-400 font-medium flex items-center gap-2">
              ...
              {pantryItems.size > 0 && (
                <span>{pantryItems.size} har du hemma</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
          ...
          return (
            <div key={storageType} className="flex flex-col gap-3">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1">
                {storageLabels[storageType] || storageType}
              </h3>
                ...
                {filteredItems.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span className="text-sm text-slate-200 font-medium pl-1 truncate hover:text-cyan-400 transition-colors">
                          {item.name}
                        </span>
        ...
        {(() => {
          ...
          const ownedItemsWithType: Array<
            { name: string; storageType: string }
          > = [];
                  ...
                  >
                    <span className="text-sm text-slate-500 pl-1 flex items-center gap-2">
                      <span className="opacity-50 text-xs">
                        {storageIcons[item.storageType]}
                      </span>
                      <span className="line-through">{item.name}</span>
                    </span>
                    <span className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-700/50 text-slate-400 group-hover:bg-slate-700 group-hover:text-slate-200 transition-all">
                      ↩
                    </span>
                  </div>
                ))}
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-slate-400">
                        📅 {WEEKDAY_LABELS[usage.day]} •{" "}
                        {MEAL_TYPE_LABELS[usage.meal]}
                      </div>
                    </div>
