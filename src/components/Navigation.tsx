...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    icon: "🏠",
    section: "food",
    description: "Hantera ingredienser",
  },
  {
    path: "/recipes",
    label: "Recept",
...
export const Navigation: React.FC<NavigationProps> = ({ onOpenOmnibox }) => {
  ...
  const getLinkClasses = (itemPath: string) => {
    ...
    return `flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
      active
        ? "text-emerald-400 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.15)] ring-1 ring-emerald-500/20"
        : "text-slate-400 hover:text-slate-100 hover:bg-white/5"
    }`;
  ...
  const getGroupClasses = (section: NavSection) => {
    const isInSection = visibleItems
      ...
      .some((i) => {
        ...
        if (i.path === "/planera" && location.pathname === "/planera/traning") {
          return false;
        }
        ...
        if (
          i.path === "/planera" &&
          location.pathname.startsWith("/planera/traning")
        ) return false;

        return location.pathname.startsWith(i.path);
      });

    return `flex items-center gap-1 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
      isInSection
        ? "text-emerald-400 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.15)] ring-1 ring-emerald-500/20"
        : "text-slate-400 hover:text-slate-100 hover:bg-white/5"
    }`;
  ...
  const getMobileLinkClasses = (itemPath: string) => {
    const active = isPathActive(itemPath);
    return `relative flex items-center gap-4 px-3 py-3 rounded-2xl transition-all duration-200 ${
      active
        ? "bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/25 shadow-lg shadow-emerald-500/10"
        : "text-slate-400 hover:bg-white/5 hover:text-slate-100"
    }`;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center gap-1">
            ...
            {targetCalories > 0 && (
              <div
                className="hidden xl:flex flex-col justify-center mr-4 w-24 gap-1"
                title={`${
                  Math.round(dailyCaloriesConsumed)
                } / ${targetCalories} kcal`}
              >
                <div className="flex justify-between text-[9px] font-bold text-slate-500 leading-none">
                  <span>KCAL</span>
                  <span
                    className={dailyCaloriesConsumed > targetCalories
                      ? "text-amber-500"
                      : "text-slate-500"}
                  >
                    {Math.round(
                      (dailyCaloriesConsumed / targetCalories) * 100,
                    )}%
                  </span>
                </div>
                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      dailyCaloriesConsumed > targetCalories
                        ? "bg-amber-500"
                        : "bg-emerald-500"
                    }`}
                    style={{
              ...
              return (
                <div key={section} className="relative group">
                  <button className={getGroupClasses(section)}>
                    ...
                    <ChevronDown
                      size={10}
                      className="opacity-50 ml-1 group-hover:rotate-180 transition-transform"
                    />
                  </button>
            ...
            <div className="relative group">
              <Link
                to="/profile"
                className={`${
                  linkClasses({ isActive: location.pathname === "/profile" })
                } flex items-center gap-2 !px-3 !py-2 bg-slate-800/50 hover:bg-slate-700/50 rounded-xl border border-white/5`}
              >
              ...
              <div className="absolute top-full right-0 mt-1 w-48 bg-slate-900 border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200 z-[100] p-1.5 backdrop-blur-xl">
                <NavLink
                  to="/settings"
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-slate-400 hover:text-white hover:bg-white/5 transition-all w-full text-left"
                >
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <button
              ...
              className="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-900 border border-slate-800 text-slate-400"
            >
              🔍
            </button>
            <button
          ...
          <div className="hidden lg:flex items-center gap-2">
            <button
              onClick={onOpenOmnibox}
              className="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-white transition-colors"
            >
              🔍
            </button>
          </div>
        ...
        <div className="lg:hidden absolute top-16 left-0 right-0 bottom-[-100vh] h-[calc(100vh-64px)] bg-slate-950/95 backdrop-blur-3xl overflow-y-auto pb-safe">
          <div className="p-4 space-y-6">
                ...
                {pinnedItems.map((item) => (
                  <div key={item.path} className="relative group">
                    <NavLink
                      ...
                      onClick={() =>
                        !isEditMode && setIsMenuOpen(false)}
                    >
            ...
            {([
              ...
              return (
                <div key={section} className="space-y-2">
                  ...
                  {items.map((item) => {
                    const isPinned = (settings.pinnedPaths || []).includes(
                      item.path,
                    );
                    ...
                    return (
                      <div key={item.path} className="relative">
                        ...
                        >
                          <span className="text-xl opacity-70 group-hover:opacity-100">
                            {item.icon}
                          </span>
                          <div className="flex flex-col">
            ...
            <div className="pt-6 border-t border-white/5 space-y-2">
              <div className="text-[10px] text-slate-500 uppercase tracking-widest font-bold px-3">
                Konto
              </div>
              <NavLink
                to="/profile"
              ...
              <button
                onClick={logout}
                className="flex items-center gap-4 px-3 py-3 rounded-2xl w-full text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 transition-colors"
              >
