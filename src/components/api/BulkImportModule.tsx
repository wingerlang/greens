...
export const BulkImportModule: React.FC = () => {
  ...
  const analyzeImports = (items: any[]) => {
    ...
    items.forEach((importedRaw) => {
      ...
      const imported: FoodItem = {
        id: importedRaw.id ||
          `food-${importedRaw.name.toLowerCase().replace(/\s+/g, "-")}`,
      ...
      const existing = foodItems.find((f) =>
        f.id === imported.id ||
        f.name.toLowerCase() === imported.name.toLowerCase()
      );

      if (!existing) {
        newConflicts.push({ type: "new", imported, selected: true });
      } else {
        const isSame = existing.calories === imported.calories &&
  ...
  return (
    <div className="space-y-6">
      <div
        className="p-8 border-2 border-dashed border-slate-700 rounded-2xl text-center bg-slate-800/20 hover:border-blue-500 transition-colors cursor-pointer"
      ...
      >
        ...
        <h3 className="font-bold">
          {fileName || "Klicka för att ladda upp JSON"}
        </h3>
        <p className="text-sm text-gray-400">
          Dra och släpp filer här eller välj från din dator
        </p>
      </div>
        ...
        <div className="bg-slate-800/40 rounded-2xl p-6 border border-slate-700 animate-in fade-in slide-in-from-bottom-2">
          <h4 className="font-bold mb-4 flex items-center justify-between">
            ...
            <span className="text-sm font-normal text-gray-400">
              {conflicts.length} objekt hittades
            </span>
          </h4>
            ...
            {conflicts.map((c, i) => (
              <div
                ...
                className={`p-3 rounded-lg flex items-center justify-between border cursor-pointer transition-all ${
                  !c.selected
                    ? "opacity-40 grayscale-[0.5] border-slate-700"
                    : c.type === "new"
                ...
                {c.type === "modified" && (
                  <div className="text-[10px] text-amber-200/50 text-right">
                    <div className="line-through opacity-50">
                      {c.existing?.calories} kcal
                    </div>
                    <div className="font-bold text-amber-400">
                      {c.imported.calories} kcal
                    </div>
                  </div>
                )}
              </div>
