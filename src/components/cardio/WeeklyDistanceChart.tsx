...
export function WeeklyDistanceChart({ data, setStartDate, setEndDate }: WeeklyDistanceChartProps) {
    ...
    const weeklyData = useMemo(() => {
        ...
        while (current <= maxDate) {
            weeks[getDateKey(current)] = { week: getDateKey(current), distance: 0, duration: 0, calories: 0, count: 0 };
        ...
        return filledData.map((d, idx) => {
            ...
            const avg = prev4.reduce((sum, item) => sum + item.distance, 0) / prev4.length;
        ...
        <div className="space-y-6">
            <div className="flex gap-2 mb-2 overflow-x-auto pb-2 custom-scrollbar">
                {[
                    ...
                    <button
                        ...
                        className={`flex-shrink-0 text-[10px] font-black uppercase px-3 py-1.5 rounded-lg border transition-all ${range === p.id ? 'bg-sky-600 border-sky-500 text-white' : 'bg-slate-950 border-white/5 text-slate-500 hover:border-white/10'}`}
                    >
                        {p.label}
                    </button>
                ))}
                ...
                {(() => {
                    ...
                    const renderGap = (indices: number[], yearMarkers: { index: number, year: number }[]) => {
                        if (indices.length === 0) {
                            ...
                            yearMarkers.forEach(ym => {
                                items.push(
                                    <div key={`year-${ym.year}`} className="flex-shrink-0 w-[1px] h-full bg-sky-500/20 relative mx-0.5">
                                        <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-sky-500/50 px-1 bg-sky-500/10 rounded">{ym.year}</span>
                                    </div>
                                );
                            });
                            return;
                        ...
                        if (indices.length >= 4) {
                            items.push(
                                <div key={`break-${indices[0]}`} className="flex-shrink-0 flex flex-col items-center justify-center min-w-[32px] md:min-w-[40px] h-full border-x border-white/5 bg-slate-800/10 rounded-sm relative group/break">
                                    ...
                                    {yearMarkers.map(ym => {
                                        ...
                                        return (
                                            <div key={`year-inner-${ym.year}`} className="absolute top-0 bottom-0 w-[1px] bg-sky-500/20 z-0" style={{ left: `${leftPos}%` }}>
                                                <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-sky-500/30 px-1 bg-sky-500/5 rounded">{ym.year}</span>
                                            </div>
                                        );
                                    })}
                            ...
                            indices.forEach((idx, iInGap) => {
                                ...
                                if (marker) {
                                    items.push(
                                        <div key={`year-${marker.year}`} className="flex-shrink-0 w-[1px] h-full bg-sky-500/20 relative mx-0.5">
                                            <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-sky-500/50 px-1 bg-sky-500/10 rounded">{marker.year}</span>
                                        </div>
                                    );
                                }
                                    ...
                                    <div key={`cross-${idx}`} className="flex-1 min-w-[2px] max-w-[40px] flex flex-col items-center h-full justify-end group/cross relative">
                                        ...
                                        >
                                            <div className="opacity-0 group-hover/trend:opacity-100 group-hover/cross:hidden transition-opacity absolute bottom-full mb-4 bg-slate-900 border border-sky-500/30 p-2 rounded-lg shadow-2xl z-50 pointer-events-none whitespace-nowrap">
                                                <p className="text-[10px] font-black text-sky-400">Trend: {(d.rollingAvg).toLocaleString('sv-SE', { maximumFractionDigits: 1 })} km</p>
                    ...
                    weeklyData.forEach(({ week, distance, rollingAvg }, i) => {
                        ...
                        if (distance === 0 && !isCurrentWeek) {
                            ...
                            // Render Bar
                            const heightPercentage = Math.min((distance / maxDistance) * 100, 100);
                            ...
                            items.push(
                                <div key={`bar-${i}`} className="flex-1 min-w-[4px] max-w-[40px] flex flex-col items-center h-full justify-end group relative z-10">
                                    ...
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute bottom-[calc(100%+8px)] left-1/2 -translate-x-1/2 bg-slate-900 border border-white/10 p-2.5 rounded-xl shadow-2xl z-50 pointer-events-none whitespace-nowrap text-center">
                                        <p className="font-black text-white text-lg leading-none">{distance.toFixed(1)} km</p>
                                        <div className="flex items-center justify-center gap-2 mt-1 mb-1">
                                            <p className="text-[10px] text-slate-400">{Math.round(d.duration / 60)}h {d.duration % 60}m</p>
                                            <span className="w-0.5 h-2 bg-white/10"></span>
                                            <p className="text-[10px] text-slate-400">{d.count} pass</p>
                                        </div>
                                        <p className="text-[9px] text-sky-400 font-bold uppercase">{weekLabel}</p>
                                    </div>
                                    ...
                                    >
                                        <div className="opacity-0 group-hover/trend:opacity-100 transition-opacity absolute bottom-full mb-4 bg-slate-900 border border-sky-500/30 p-2 rounded-lg shadow-2xl z-50 pointer-events-none whitespace-nowrap">
                                            <p className="text-[10px] font-black text-sky-400">Trend: {(rollingAvg).toLocaleString('sv-SE', { maximumFractionDigits: 1 })} km</p>
                                            <p className="text-[8px] text-slate-500 font-bold">{weekLabel}</p>
                                        </div>
            ...
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase tracking-widest pt-4 border-t border-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <span className="flex items-center gap-1.5">
                        <span className="w-4 h-0.5 bg-sky-500/50 border-t border-dashed border-sky-400"></span>
                        Snitt (4v)
                    </span>
                </div>
                <div>
