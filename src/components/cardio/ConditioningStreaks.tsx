...
export function ConditioningStreaks({ dates }: StreaksProps) {
  const { current, longest } = useMemo(() => {
    ...
    const sorted = Array.from(new Set(dates)).sort((a, b) =>
      a.localeCompare(b)
    );
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="bg-gradient-to-br from-orange-500/10 to-orange-600/5 border border-orange-500/20 rounded-2xl p-4 relative overflow-hidden group">
        <div className="relative z-10">
          ...
          <div className="text-[10px] text-orange-500/60 font-bold mt-1">
            Dagar i rad
          </div>
        </div>
        <div className="absolute -bottom-4 -right-4 text-8xl opacity-5 pointer-events-none group-hover:scale-110 transition-transform">
          🔥
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20 rounded-2xl p-4 relative overflow-hidden group">
        <div className="relative z-10">
          ...
          <div className="text-[10px] text-blue-500/60 font-bold mt-1">
            Dagar rekord
          </div>
        </div>
        <div className="absolute -bottom-4 -right-4 text-8xl opacity-5 pointer-events-none group-hover:scale-110 transition-transform">
