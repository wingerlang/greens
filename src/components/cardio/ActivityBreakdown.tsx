      ...
      <div className="space-y-3">
        {breakdown.map((act, i) => (
          <div key={act.type} className="flex items-center gap-3 group">
            <span className="text-lg font-black text-slate-600 w-6 group-hover:text-slate-400 transition-colors">
              {i + 1}
            </span>
            <div className="flex-1">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-xs text-slate-500 font-mono">
                  {Math.floor(act.duration / 60)}h{" "}
                  {Math.round(act.duration % 60)}m
                </span>
              </div>
              <div className="flex gap-4 text-xs text-slate-500 mt-0.5">
                ...
                <span className="text-sky-400">
                  {act.duration > 0 && act.distance > 0
                    ? (act.duration / act.distance).toFixed(1) + " min/km"
                    : ""}
                </span>
              </div>
              ...
              <div className="h-0.5 w-full bg-slate-800 rounded-full mt-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(act.duration / breakdown[0].duration) * 100}%`,
                  }}
                />
              </div>
            </div>
