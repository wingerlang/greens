        ...
        const entries = bulkInput.split('\n')
            ...
            .map(line => {
                ...
                if (parts.length >= 2) {
                    ...
                    const chest = parts.length > 3 ? parseFloat(parts[3].replace(',', '.')) : undefined;
            ...
            .filter((e): e is { date: string, weight: number, waist: number | undefined, chest: number | undefined } => e !== null);
                    ...
                    <div className="p-8">
                        ...
                        {showBulkImport ? (
                            <div className="space-y-4">
                                ...
                                />
                                <p className="text-[10px] text-slate-400 px-1">Format: YYYY-MM-DD Vikt(kg) [Midja(cm)] [Bröst(cm)]</p>
                                <button
                            ...
                            <form onSubmit={handleSave} className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Bröst (cm)</label>
                                        <input
                                            type="number"
                                            step="0.1"
                                            value={tempChest}
                                            ...
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                ...
                                                {entry.waist && (
                                                    <>
                                                        ...
                                                        <span className="text-sm font-bold text-emerald-500">{entry.waist} <span className="text-[10px] font-normal opacity-70">cm (midja)</span></span>
                                                    </>
                                                )}
                                                {entry.chest && (
                                                    <>
                                                        <span className="text-slate-300 dark:text-slate-700 mx-1">|</span>
                                                        <span className="text-sm font-bold text-indigo-500">{entry.chest} <span className="text-[10px] font-normal opacity-70">cm (bröst)</span></span>
                                                    </>
                                                )}
                                            </div>
                                        </div>
