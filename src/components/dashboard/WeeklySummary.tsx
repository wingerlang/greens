...
export function WeeklySummary(
  { selectedDate, activities, history }: WeeklySummaryProps,
) {
  ...
  const weekNum = 1 +
    Math.round(
      (((targetDate.getTime() - jan4.getTime()) / 86400000) - 3 +
        ((jan4.getDay() + 6) % 7)) / 7,
    );
  ...
  const weekVolume =
    weekActivities.reduce((sum, a) => sum + (a.tonnage || 0), 0) / 1000;
  ...
  const currentWeekNum = (() => {
    ...
    return 1 +
      Math.round(
        (((mon.getTime() - jan4.getTime()) / 86400000) - 3 +
          ((jan4.getDay() + 6) % 7)) / 7,
      );
  })();
  ...
  const getDiff = (type: "weight" | "waist" | "chest") => {
    ...
    if (beforeWeek.length > 0) {
      ...
      return {
        diff: latestInWeek.value - baseline.value,
        current: latestInWeek.value,
      };
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-wrap justify-center gap-4 w-full">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl px-4 md:px-8 py-5 flex flex-col items-center justify-center shadow-sm flex-1 min-w-0 hover:shadow-md transition-shadow">
          ...
          <div className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-1">
            Löpning
          </div>
          <div className="text-lg font-bold text-emerald-500">
            {weekActivities.filter((a) =>
              a.type === "running" || a.type === "cycling" ||
              a.type === "walking"
            ).length} pass <span className="text-slate-300 mx-1">|</span>{" "}
            {Math.round(weekDistance)}km
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl px-4 md:px-8 py-5 flex flex-col items-center justify-center shadow-sm flex-1 min-w-0 hover:shadow-md transition-shadow">
          ...
          <div className="text-lg font-bold text-indigo-500">
            {weekActivities.filter((a) => a.type === "strength").length} pass
            {" "}
            <span className="text-slate-300 mx-1">|</span>{" "}
            {weekVolume.toFixed(1)} ton
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl px-4 md:px-8 py-5 flex flex-col items-center justify-center shadow-sm flex-1 min-w-0 hover:shadow-md transition-shadow">
          <div className="text-xl mb-1">⚖️</div>
          <div className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-2">
            Framsteg
          </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-xs font-black ${
                  wDiff && wDiff.diff !== 0
                    ? (wDiff.diff < 0 ? "text-emerald-500" : "text-rose-500")
                    : "text-slate-300"
                }`}
              >
                {wDiff
                  ? (wDiff.diff > 0
                    ? `+${wDiff.diff.toFixed(1)}`
                    : wDiff.diff.toFixed(1))
                  : "-"}
              </span>
            </div>
