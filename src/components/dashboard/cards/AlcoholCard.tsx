...
export const AlcoholCard = ({
  ...
  const isAlcWarning = alcoholLimit !== undefined && !isAlcHigh &&
    currentAlcohol > 0 && currentAlcohol === alcoholLimit;
  ...
  return (
    <div
      ...
      } border shadow-sm hover:scale-[1.01] transition-all cursor-pointer relative overflow-hidden ${
        isAlcHigh
    ...
    >
      ...
      >
        ...
        {density !== "compact" && alcoholLimit !== undefined &&
          alcoholLimit > 0 && (
            ...
            <>
              ...
              <div
                className={`flex gap-0.5 mt-2 ${
                  density === "compact" ? "h-1" : "h-2"
                }`}
              >
                {Array.from({ length: Math.max(currentAlcohol, 4) }).map((
                  _,
                  i,
                ) => (
                  <div
                    ...
                    className={`flex-1 rounded-full cursor-pointer transition-colors ${
                      i < currentAlcohol
                        ? (isAlcHigh ? "bg-rose-500" : "bg-amber-400 shadow-sm")
