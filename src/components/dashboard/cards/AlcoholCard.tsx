...
import type { AlcoholCardProps } from '../dashboard.types.ts';

/**
 * Alcohol intake tracking card.
 * Shows current alcohol units with visual indicator bars.
 */
export const AlcoholCard = ({
    ...
    const isAlcHigh = alcoholLimit !== undefined && currentAlcohol > alcoholLimit;
    const isAlcWarning = alcoholLimit !== undefined && !isAlcHigh && currentAlcohol > 0 && currentAlcohol === alcoholLimit;
                ...
                {isEditing ? (
                    ...
                    <>
                        <div className="flex items-baseline gap-1">
                            <span className={`${density === 'compact' ? 'text-xl' : 'text-3xl'} font-bold ${isAlcHigh ? 'text-rose-600' : isAlcWarning ? 'text-amber-600' : 'text-slate-900 dark:text-white'}`}>{currentAlcohol}</span>
                            <span className="text-[9px] font-bold text-slate-400 uppercase">E</span>
                        </div>
                        <div className={`flex gap-0.5 mt-2 ${density === 'compact' ? 'h-1' : 'h-2'}`}>
                            {Array.from({ length: Math.max(currentAlcohol, 4) }).map((_, i) => (
                                <div
                                    key={i}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        const newVal = (currentAlcohol === i + 1) ? i : i + 1;
                                        onQuickClick(newVal);
                                    }}
                                    className={`flex-1 rounded-full cursor-pointer transition-colors ${i < currentAlcohol ? (isAlcHigh ? 'bg-rose-500' : 'bg-amber-400 shadow-sm') : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200'} `}
                                />
                            ))}
                        </div>
                    </>
