...
export const SleepCard = ({
    ...
    const sleepClasses = currentSleep > 0 ? (sleepColorMap[sleepInfo.color] || sleepColorMap.slate) : sleepColorMap.slate;
    const sleepColorClass = sleepClasses.text;

    const getBackgroundClass = () => {
        if (currentSleep > 0 && currentSleep < 5) return 'bg-rose-50 dark:bg-rose-900/10';
        if (currentSleep >= 5 && currentSleep < 7) return 'bg-amber-50 dark:bg-amber-900/10';
        if (currentSleep >= 7 && currentSleep <= 10) return 'bg-emerald-50 dark:bg-emerald-900/20';
        return 'bg-white dark:bg-slate-900';
    };

    const getIconClass = () => {
        if (currentSleep > 0 && currentSleep < 5) return 'bg-rose-100 text-rose-600';
        if (currentSleep >= 5 && currentSleep < 7) return 'bg-amber-100 text-amber-600';
    ...
    return (
        <div
            onClick={onCardClick}
            className={`${density === 'compact' ? 'p-2.5 rounded-2xl' : 'p-4 rounded-3xl'} shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col hover:scale-[1.01] transition-transform cursor-pointer group relative overflow-hidden h-full ${getBackgroundClass()}`}
        >
                ...
                {isEditing ? (
                    ...
                    <>
                        <div className="flex items-baseline gap-1">
                            <span className={`${density === 'compact' ? 'text-xl' : 'text-3xl'} font-bold ${sleepColorClass}`}>{currentSleep}</span>
