...
export function HealthScoreCard(
  { exercises, meals, userSettings }: HealthScoreCardProps,
) {
  const health = useMemo(
    () => calculateHealthScore(exercises, meals, userSettings),
    [exercises, meals, userSettings],
  );
    ...
    <div className="content-card relative overflow-hidden">
        ...
        <div className="relative w-20 h-20 flex items-center justify-center">
          <svg className="w-full h-full transform -rotate-90">
            ...
            <circle
              ...
              r={radius}
              className={`${
                getRingColor(health.totalScore)
          ...
          <div className="absolute inset-0 flex items-center justify-center flex-col">
            <span
              className={`text-2xl font-black ${
                getScoreColor(health.totalScore)
              }`}
            >
              {health.totalScore}
            </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <div
              className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
                health.training.tsb > 0
                  ? "text-emerald-400 bg-emerald-400/10"
                  : "text-slate-400 bg-slate-800"
              }`}
            >
              TSB {health.training.tsb > 0 ? "+" : ""}
              {health.training.tsb}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl p-3 border border-white/5 bg-white/[0.02] hover:bg-white/5 transition-colors">
          ...
          <div className="flex justify-between items-center">
            <div className="text-[9px] text-slate-400">
              {Math.abs(health.nutrition.caloriesDiff)} kcal{" "}
              {health.nutrition.status === "Surplus" ? "plus" : "minus"}
            </div>
            <div
              className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
                health.nutrition.proteinMet
                  ? "text-emerald-400 bg-emerald-400/10"
        ...
        <div className="rounded-xl p-3 border border-white/5 bg-white/[0.02] hover:bg-white/5 transition-colors">
          ...
          <div className="flex justify-between items-center">
            <div className="text-[9px] text-slate-400">
              {health.consistency.streak} dagar i rad
            </div>
            <div
              className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
                health.consistency.streak >= 4
      ...
      <div className="mt-4 pt-3 border-t border-white/5 flex items-start gap-2 opacity-70">
        ...
        <p className="text-[10px] text-slate-400 italic">
          "Med tanke på din höga träningsbelastning bör du öka kaloriintaget
          något idag för att maximera återhämtningen och behålla din streak."
        </p>
      </div>
    </div>
  );
