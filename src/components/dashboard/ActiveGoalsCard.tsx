  ...
  const extraStats = useMemo(() => {
    ...
    if (goal.type === "weight" || goal.type === "measurement") {
      ...
      const relevantEntries = goal.type === "weight"
        ? weightEntries.filter((w) =>
          new Date(w.date).getTime() >= timeStats.start
        )
        : bodyMeasurements.filter((m) =>
          new Date(m.date).getTime() >= timeStats.start
        );
      ...
      // Forecast
      const targetVal = goal.type === "weight"
        ? (goal.targetWeight || 0)
        : (goal.targetMeasurement || 0);
      const remainingDiff = progress
        ? (targetVal - (progress.actualCurrentValue || 0))
        : 0;
      // Handle direction
      const isLoss = progress && (progress.actualCurrentValue || 0) > targetVal;
      const absoluteRemaining = Math.abs(remainingDiff);

      const weeksToGoal = ratePerWeek > 0
        ? absoluteRemaining / ratePerWeek
        : 999;
    ...
    const isRunGoal = target?.exerciseType === "running" ||
      goal.type === "milestone"; // Simplified check

    if (
      isRunGoal &&
      (goal.type === "distance" || goal.type === "frequency" ||
        goal.type === "speed")
    ) {
      // Filter activities
      const acts = unifiedActivities.filter((a) => {
        const d = new Date(a.date).getTime();
        return d >= timeStats.start && d <= timeStats.end &&
          a.type === "running";
      });
        ...
        {extraStats && (
          <div className="hidden sm:flex items-center gap-2 text-[10px] tabular-nums border-r border-slate-700/50 pr-3 mr-1">
            {(extraStats.type === "weight" ||
              extraStats.type === "measurement") && (
              <>
                <div className="flex flex-col items-end leading-none gap-0.5">
                  <span
                    className={`font-bold ${
                      extraStats.rate && extraStats.rate >= 0
                        ? "text-emerald-400"
                        : "text-rose-400"
                    }`}
                  >
                    {extraStats.rate && extraStats.rate > 0 ? "+" : ""}
                    {extraStats.rate?.toFixed(1)} {extraStats.unit}/v
                  </span>
                ...
                <div className="flex flex-col items-end leading-none gap-0.5 ml-1">
                  ...
                  <span className="text-slate-500">
                    {extraStats.type === "weight" ? "vägn." : "mått"}
                  </span>
                </div>
              </>
            )}
            {extraStats.type === "activity" &&
              (extraStats.totalDist || 0) > 0 && (
              <div className="flex flex-col items-end leading-none gap-0.5">
                <span className="text-blue-400 font-bold">
                  {extraStats.totalDist?.toFixed(1)} km
                </span>
        ...
        {timeStats && (
          <div className="hidden xs:flex flex-col items-end leading-none gap-0.5 min-w-[40px]">
            <div className="flex items-baseline gap-1 text-[10px]">
              ...
              <span className="text-slate-300 font-bold">
                {timeStats.daysElapsed}/{timeStats.daysTotal}
              </span>
            </div>
            {/* Tiny Time Bar */}
            <div className="w-full h-0.5 bg-slate-700 rounded-full mt-0.5">
              <div
                ...
                style={{ width: `${timeStats.percentTime}%` }}
              />
            </div>
          </div>
        )}
...
export const ActiveGoalsCard: React.FC = () => {
  ...
  // Filter active goals
  const activeGoals = (performanceGoals || []).filter((g) =>
    g.status === "active"
  );
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 relative z-10 px-1">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-0.5">
              Pågående Mål
            </h3>
            <div className="text-white text-xs font-bold leading-none">
              {activeGoals.length} aktiva fokus
            </div>
          </div>
        </div>
