...
import { PlannedActivity, getISODate } from '../../models/types.ts';
...
export function PlannerCalendar({ activities, weekStartDate }: PlannerCalendarProps) {
    const unassignedActivities = activities.filter(a => !a.date || a.date === 'UNASSIGNED');
    ...
    const weekDates = DAYS.map((dayName, index) => {
        ...
        return {
            ...
            dayLabel: d.toLocaleDateString('sv-SE', { weekday: 'short', day: 'numeric' })
        };
    });
    ...
    return (
        <div className="flex flex-col h-full gap-6">
            {/* Unassigned Pool */}
            <DropZone id="UNASSIGNED" className="min-h-[120px] bg-slate-900/50 border-2 border-dashed border-slate-700 rounded-3xl p-4 flex gap-4 overflow-x-auto items-center">
            ...
            <div className="grid grid-cols-7 gap-2 h-full">
                {weekDates.map((day) => {
                    const dayActivities = activities.filter(a => a.date === day.dateStr);
                    const totalDist = dayActivities.reduce((sum, a) => sum + (a.estimatedDistance || 0), 0);
                    const hasStrength = dayActivities.some(a => a.category === 'STRENGTH');
                    ...
                    return (
                        ...
                        >
                            <div className="text-center mb-3 pb-2 border-b border-white/5">
                                <span className="block text-[10px] font-black uppercase text-slate-500">{day.dayName.substring(0, 3)}</span>
                                <span className="block text-xs font-bold text-white">{day.dateStr.slice(-2)}</span>
                            </div>

                            <div className="space-y-2 flex-1">
                                {dayActivities.map(activity => (
                                    <DraggableActivityCard key={activity.id} activity={activity} compact />
                                ))}
                            </div>
...
function DropZone({ id, children, className }: { id: string; children: React.ReactNode, className?: string }) {
    ...
    return (
        <div
            ref={setNodeRef}
            className={`${className} ${isOver ? 'ring-2 ring-indigo-500 bg-indigo-500/10' : ''}`}
        >
            {children}
        </div>
    );
