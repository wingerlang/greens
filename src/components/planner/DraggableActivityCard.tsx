...
import { Dumbbell, Flame, Footprints, GripHorizontal, Zap } from "lucide-react";
...
export function DraggableActivityCard(
  { activity, compact = false }: DraggableActivityCardProps,
) {
  ...
  const getIcon = () => {
    ...
    if (activity.category === "INTERVALS" || activity.category === "TEMPO") {
  ...
  const getColors = () => {
    ...
    if (activity.category === "LONG_RUN") {
      return "bg-amber-500/10 text-amber-400 border-amber-500/20";
    }
    if (["INTERVALS", "TEMPO"].includes(activity.category)) {
    ...
    >
      ...
      {!compact && (
        <div className="flex justify-between items-end text-[10px] text-slate-400 font-medium">
          ...
          {activity.targetPace !== "-" && <span>@{activity.targetPace}</span>}
        </div>
      )}
    </div>
  );
