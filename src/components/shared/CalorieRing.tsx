...
export function CalorieRing({
  ...
  const strokeDashoffset = circumference -
    (Math.min(calories, calorieGoal * 1.5) / calorieGoal) * circumference;
  ...
  const innerStrokeDashoffset = innerCircumference -
    (Math.min(protein, proteinGoal) / proteinGoal) * innerCircumference;
    ...
    <div className="relative flex items-center justify-center">
      ...
      >
        ...
        <circle
          stroke="currentColor"
          className={`${
            isProteinMet ? "text-emerald-500" : "text-orange-400"
          } transition-colors duration-500`}
          strokeWidth={is}
      ...
      <div className="absolute flex flex-col items-center justify-center text-center">
        <div
          className={`${sizes.text} font-bold leading-none ${
            isOver ? "text-rose-500" : "text-slate-900 dark:text-white"
          }`}
        >
          {Math.round(calories)}
          ...
          <div className="flex items-center gap-1.5 text-xs font-bold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700">
            <div
              className={`w-2 h-2 rounded-full ${
                isProteinMet ? "bg-emerald-500" : "bg-orange-400"
              }`}
            />
