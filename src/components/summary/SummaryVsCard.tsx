...
import { formatDuration, formatSwedishDate } from "../../utils/dateUtils.ts";
import {
  Activity,
  ArrowDownRight,
  ArrowUpRight,
...
export const SummaryVsCard: React.FC<SummaryVsCardProps> = ({
  ...
  const ComparisonRow = ({
    ...
    const currentDisplay = isTime
      ? formatTimeValue(current)
      : current.toLocaleString();
    const previousDisplay = isTime
      ? formatTimeValue(previous)
      ...
      <div className="grid grid-cols-3 items-center py-1.5 border-b border-white/5 last:border-0">
        ...
        <div className="text-right pr-4">
          ...
          >
            {currentDisplay}
            {unit && !isTime && (
              <span className={`text-[9px] ml-1 text-${colorClass}-500/60`}>
                {unit}
              </span>
            )}
          </span>
          ...
          {!tie && (
            <span
              className={`flex items-center gap-0.5 text-[9px] font-black ${
                currentWins ? "text-emerald-400" : "text-red-400"
              }`}
            >
              {currentWins
        ...
        <div className="text-left pl-4">
          ...
          >
            {previousDisplay}
            {unit && !isTime && (
              <span className="text-[8px] ml-1 opacity-40">{unit}</span>
            )}
          </span>
        </div>
      </div>
  ...
  const prevRunningStats = prevStats.types.find((t) => t.name === "running") ||
    { dist: 0, time: 0, count: 0 };
  const strengthStats = stats.types.find((t) => t.name === "strength") ||
    { dist: 0, time: 0, count: 0 };
  const prevStrengthStats =
    prevStats.types.find((t) => t.name === "strength") ||
    { dist: 0, time: 0, count: 0 };
  ...
  const formatShortDate = (d: string) => {
    ...
    return `${date.getDate()} ${
      date.toLocaleDateString("sv-SE", { month: "short" }).replace(".", "")
    }`;
  };
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center mb-2 pb-1.5 border-b border-white/10">
        <div className="text-lg font-black text-white uppercase tracking-tight">
          {formatShortDate(startDate).toUpperCase()} —{" "}
          {formatShortDate(endDate).toUpperCase()}{" "}
      ...
      <div className="relative z-10 flex-1 space-y-1">
        ...
        <div className="pt-2">
          ...
          <ComparisonRow
            label="Tid"
            current={Math.round(stats.totalTime)}
            previous={Math.round(prevStats.totalTime)}
            isTime
            icon={Clock}
            colorClass="emerald"
          />
          <ComparisonRow
            ...
            previous={prevStats.totalSessions}
            unit="st"
            icon={Zap}
            colorClass="amber"
          />
        ...
        <div className="pt-2">
          ...
          <ComparisonRow
            label="km"
            current={Math.round(runningStats.dist)}
            previous={Math.round(prevRunningStats.dist)}
            unit="km"
            icon={Activity}
            colorClass="emerald"
          />
          <ComparisonRow
            label="Tid"
            current={Math.round(runningStats.time)}
            previous={Math.round(prevRunningStats.time)}
            isTime
            icon={Clock}
            colorClass="emerald"
          />
          <ComparisonRow
            label="Pass"
            current={runningStats.count}
            previous={prevRunningStats.count}
            unit="st"
            icon={TrendingUp}
            colorClass="emerald"
          />
        ...
        <div className="pt-2">
          ...
          <ComparisonRow
            label="Tonnage"
            current={Number((stats.totalTonnage / 1000).toFixed(1))}
            previous={Number((prevStats.totalTonnage / 1000).toFixed(1))}
            unit="t"
            icon={Layers}
            colorClass="indigo"
          />
          <ComparisonRow
            label="Tid"
            current={Math.round(strengthStats.time)}
            previous={Math.round(prevStrengthStats.time)}
            isTime
            icon={Clock}
            colorClass="indigo"
          />
          <ComparisonRow
            label="Pass"
            current={strengthStats.count}
            previous={prevStrengthStats.count}
            unit="st"
            icon={Dumbbell}
            colorClass="indigo"
          />
