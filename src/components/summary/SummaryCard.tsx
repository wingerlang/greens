            ...
            <div className="relative z-10 grid grid-cols-2 gap-4 mb-8">
                ...
                <div className="bg-slate-900/60 backdrop-blur-md p-4 rounded-2xl border border-white/5 shadow-lg group hover:border-emerald-500/20 transition-all">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Aktiva Dagar</p>
                    </div>
                    <div className="flex items-baseline gap-2">
                        <p className="text-3xl font-black text-white">{stats.activeDays}</p>
                        <span className="text-sm font-bold text-slate-500">/ {stats.totalDays}</span>
                    </div>
                    {/* Visual Progress Bar */}
                    <div className="w-full h-1 bg-slate-800 rounded-full mt-3 overflow-hidden">
                        <div
                            className="h-full bg-gradient-to-r from-orange-400 to-red-400 rounded-full"
                            style={{ width: `${Math.min(100, (stats.activeDays / stats.totalDays) * 100)}%` }}
                        />
                    </div>
                </div>
                ...
                <div className="bg-slate-900/60 backdrop-blur-md p-4 rounded-2xl border border-white/5 shadow-lg group hover:border-emerald-500/20 transition-all">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Träningstid</p>
                    </div>
                    <div className="flex items-baseline gap-1">
                        <p className="text-3xl font-black text-white">{Math.floor(stats.totalTime / 60)}</p>
                        <span className="text-sm font-bold text-emerald-500/80">h</span>
                        <p className="text-xl font-black text-white ml-1">{Math.round(stats.totalTime % 60)}</p>
                        <span className="text-xs font-bold text-emerald-500/60">min</span>
                    </div>
                    <p className="text-[10px] text-slate-500 font-bold mt-2">~{stats.totalSessions > 0 ? Math.round(stats.totalTime / stats.totalSessions) : 0} min/pass</p>
                </div>
            </div>
                        ...
                        {stats.fastestRuns[0] && (
                            <Link to={`/activity/${stats.fastestRuns[0].id}`} className="block w-full">
                                <div className="bg-slate-900/50 hover:bg-emerald-900/20 border border-white/5 hover:border-emerald-500/30 p-3 rounded-xl transition-all group/item cursor-pointer flex items-center justify-between w-full">
                                    ...
                                    <div className="text-right">
                                        <p className="text-[10px] text-slate-400 font-bold">{stats.fastestRuns[0].performance?.distanceKm?.toFixed(1)} km</p>
                                    </div>
                                </div>
                            </Link>
                        )}
                        ...
                        {stats.longestRuns[0] && (
                            <Link to={`/activity/${stats.longestRuns[0].id}`} className="block w-full">
                                <div className="bg-slate-900/50 hover:bg-emerald-900/20 border border-white/5 hover:border-emerald-500/30 p-3 rounded-xl transition-all group/item cursor-pointer flex items-center justify-between w-full">
                                    ...
                                    <div className="text-right">
                                        <p className="text-[10px] text-slate-400 font-bold">{formatDuration((stats.longestRuns[0].performance?.durationMinutes || 0) * 60)}</p>
                                    </div>
                                </div>
                            </Link>
                        )}
                ...
                <div className="relative z-10 mt-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4">
                    <div className="flex justify-between items-center mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-amber-400 text-sm font-black uppercase tracking-wide">Prestationer & Lopp</p>
                        </div>
                        {stats.raceCount > 0 && <span className="text-xs font-bold bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full">{stats.raceCount} Lopp</span>}
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-2 text-center">
                        <div className="bg-slate-900/40 p-2 rounded-lg border border-amber-500/10">
                            ...
                            <p className="text-[8px] font-bold text-slate-500 uppercase">Totalt</p>
                        </div>
                        <div className="bg-slate-900/40 p-2 rounded-lg border border-amber-500/10 col-span-3 flex flex-col justify-center">
                            <div className="flex justify-around">
                                <div>
                                    <span className="block font-black text-emerald-400">{stats.runningPRs}</span>
                                    <span className="text-[8px] text-slate-500">Löpning</span>
                                </div>
                                <div className="w-[1px] bg-white/10"></div>
                                <div>
                                    <span className="block font-black text-purple-400">{stats.strengthPRs}</span>
                                    <span className="text-[8px] text-slate-500">Styrka</span>
                                </div>
                            </div>
                        </div>
                        <div className="bg-slate-900/40 p-2 rounded-lg border border-amber-500/10">
                            <p className="text-2xl font-black text-cyan-400">{stats.activeDays}</p>
                            <p className="text-[8px] font-bold text-slate-500 uppercase">Dagar</p>
                        </div>
                    </div>
                </div>
            )}
