...
export const calculateCalorieTarget = (
  ...
  return {
    tdee,
    target,
    explanation: `Baserat på BMR (${bmr}) × aktivitetsnivå (${
      multipliers[activityLevel]
    }) ${adjustments[goal] >= 0 ? "+" : ""}${adjustments[goal]} kcal`,
  };
};
...
export const calculateVolumeStats = (
  ...
  const relevant = history.filter((e) => e.type === type).sort((a, b) =>
