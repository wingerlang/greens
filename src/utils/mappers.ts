...
export function mapUniversalToLegacyEntry(
  a: UniversalActivity,
): ExerciseEntry | null {
  ...
  return {
    id: a.id,
    date: a.date,
    type: (a.performance?.activityType || a.plan?.activityType ||
    ...
    subType: a.performance.subType ||
      (a.plan?.activityCategory === "INTERVALS" ? "interval" : undefined),
    ...
    title: a.plan?.title || a.performance?.notes ||
      (a.performance?.source?.externalId ? "Strava Activity" : undefined), // Use notes as title if plan title missing
    ...
    startTime: a.performance.startTimeLocal
      ? a.performance.startTimeLocal.split("T")[1]?.substring(0, 5)
      : undefined,
    notes: a.performance.notes !== a.plan?.title
      ? a.performance.notes
      : undefined,
