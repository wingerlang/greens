...
export function calculateAcuteLoad(
  ...
  sortedWorkouts.forEach((workout) => {
    ...
    workout.exercises.forEach((exercise) => {
      const targetParts = mapExerciseToBodyParts(
...
export function analyzeInjuryRisk(
  ...
  Object.entries(statsMap).forEach(([key, stats]) => {
    ...
    const activeInjury = injuryLogs.find((l) =>
      l.bodyPart === part && l.status === "active"
    );
    ...
    const recovering = injuryLogs.find((l) =>
      l.bodyPart === part && l.status === "recovering"
    );
