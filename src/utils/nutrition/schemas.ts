...
export const ParsedNutritionSchema = z.object({
  name: z.string().optional().describe(
    "Product name extracted from the source",
  ),
  brand: z.string().optional().describe("Brand name extracted or inferred"),
  packageWeight: z.number().optional().describe(
    "Total weight of the package in grams",
  ),
  calories: z.number().optional().describe(
    "Energy content in kcal per 100g/unit",
  ),
  protein: z.number().optional().describe(
    "Protein content in grams per 100g/unit",
  ),
  carbs: z.number().optional().describe(
    "Carbohydrate content in grams per 100g/unit",
  ),
  ...
  ingredients: z.string().optional().describe(
    "Comma-separated list of ingredients",
  ),
  defaultPortionGrams: z.number().optional().describe(
    "Suggested portion size in grams",
  ),
});
 ...
 */
export const ParseTextSchema = z.string().describe(
  "The raw text containing nutrition information",
);

/**
 * Input schema for JSON-LD extraction.
 */
export const ParseJsonLdSchema = z.array(z.any()).describe(
  "Array of JSON-LD objects extracted from HTML",
);
...
export const ExtractBrandSchema = z.object({
  ...
  knownBrands: z.array(z.string()).optional().default([]).describe(
    "List of known brands to fuzzy match against",
  ),
});
