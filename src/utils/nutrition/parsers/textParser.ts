...
export const parseNutritionText = (inputText: string): ParsedNutrition => {
  ...
  const findValue = (
    ...
    for (const kw of keywords) {
      ...
      const patternImm2 = new RegExp(
        `(\\d+(?:\\.\\d+)?)\\s*(?:g|gram|kcal|kj)?\\s*${escapedKw}`,
        "i",
      );
      ...
      if (matchFlex1) {
        ...
        const fragment = normalized.substring(
          matchFlex1.index!,
          matchFlex1.index! + matchFlex1[0].length,
        );
      ...
      if (matchFlex2 && matchFlex2[1]) {
        ...
        const fragment = normalized.substring(
          matchFlex2.index!,
          matchFlex2.index! + matchFlex2[0].length,
        );
        if (!excludeKeywords.some((ex) => fragment.includes(ex))) {
          return val;
        }
  ...
  // 4. Fat
  result.fat = findValue(["fett", "fat", "lipids"], [
    "mättat",
    "saturated",
    "enkelomättat",
    "fleromättat",
