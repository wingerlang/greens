...
export function calculateDailyScore(
  ...
  if (kcalAccuracy > 0.9 && proteinAccuracy > 0.9) {
    ...
    if (proteinAccuracy < 0.7) {
      messages.push(
        `Öka proteinintaget (+${
          Math.round(proteinGoal - nutrition.protein)
        }g). 🍳`,
      );
    }
    if (kcalAccuracy < 0.8) {
  ...
  if (vitals.water < waterGoal) {
    messages.push(
      `Drick 💧 ${waterGoal - vitals.water} glas till för full pott.`,
    );
  }
  if (vitals.sleep < sleepGoal) {
    messages.push(`Sikta på 😴 ${sleepGoal - vitals.sleep}h mer sömn inatt.`);
  }
  if (vitals.water >= waterGoal && vitals.sleep >= sleepGoal) {
    messages.push("Vitals är på topp! 💎");
  }
  ...
  const b12Coverage = Math.min(
    1.5,
    (nutrition.vitaminB12 || 0) / RDAs.vitaminB12,
  );
  const calciumCoverage = Math.min(
    1.5,
    (nutrition.calcium || 0) / RDAs.calcium,
  );
  const zincCoverage = Math.min(1.5, (nutrition.zinc || 0) / RDAs.zinc);

  const avgVitality =
    (ironCoverage + b12Coverage + calciumCoverage + zincCoverage) / 4;
  ...
  const total = Math.round(
    (nutritionScore * 0.4) + (activityScore * 1.0) + (vitalsScore * 1.0) +
      vitalityScore,
  );
  ...
  const normalizedTotal = Math.min(
    100,
    Math.round(
      (nutritionScore + activityScore + vitalsScore + vitalityScore) / 1.2,
    ),
  );
