...
export function parseIngredientLine(line: string): ParsedIngredient | null {
  ...
  if (match) {
    ...
    const unit = unitStr
      ? (UNIT_PATTERNS[unitStr.toLowerCase()] || unitStr)
      : "pcs";
 ...
 */
export function matchToFoodItem(
...
export function calculateRecipeEstimate(
  ...
  const isComplete = categories.includes("soy_quinoa") ||
  ...
  parsed.forEach((ing) => {
    ...
    if (item) {
      ...
      if (name.includes("svartpeppar") || name.includes("peppar")) {
        hasBlackPepper = true;
      }
      if (
        ["broccoli", "grönkål", "blomkål", "brysselkål", "svartkål"].some((c) =>
          name.includes(c)
        )
      ) hasCruciferous = true;
      if (
        ["senap", "rädisa", "pepparrot", "rucola"].some((m) => name.includes(m))
      ) hasMyrosinase = true;
      if (["vitlök", "lök", "purjolök"].some((a) => name.includes(a))) {
        hasAlliums = true;
      }
    }
  });
  ...
  if (totalVitA > 500 && totalFat > 10) {
    synergies.push({
      ...
      name: "Närings-Unlock",
      description:
        "Hälsosamma fetter behövs för att ta upp fettlösligt Vitamin A (betakaroten).",
      icon: "🥑+🥕",
      impact: "unlock",
    });
  }
  ...
  if (hasTurmeric && hasBlackPepper) {
    synergies.push({
      ...
      name: "Bio-Boost",
      description:
        "Piperin i svartpeppar ökar upptaget av kurkumin med upp till 2000%.",
      icon: "🧂+🌿",
      impact: "efficiency",
    });
  }
  ...
  if (hasCruciferous && hasMyrosinase) {
    synergies.push({
      ...
      description:
        "Senapsfrö eller rucola hjälper till att aktivera det cancerförebyggande sulforafanet i kål.",
      icon: "🥦+🧂",
      impact: "efficiency",
    });
  }
  ...
  if (hasAlliums && (totalIron > 3 || totalZinc > 3)) { // Zinc/Iron focused
    synergies.push({
      ...
      name: "Mineral-Brygga",
      description:
        "Svavelföreningar i lök och vitlök ökar biotillgängligheten hos järn och zink.",
      icon: "🧄+🍛",
      impact: "absorption",
    });
  }
