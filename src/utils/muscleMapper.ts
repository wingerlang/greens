...
export function mapMuscleGroupToBodyParts(group: MuscleGroup): BodyPart[] {
  switch (group) {
    ...
    case "back":
      return ["upper_back", "lats" as any]; // lats isn't a BodyPart, map to upper_back
    case "lats":
      return ["upper_back"];
    case "traps":
      return ["upper_back", "neck"];
    case "shoulders":
      return ["shoulders"];
    case "biceps":
      return ["biceps"];
 ...
 */
export function mapExerciseToBodyParts(
  exerciseName: string,
  ...
  if (name.includes("squat") || name.includes("knäböj")) {
    return ["quads", "glutes", "lower_back", "adductors"];
  }
  if (name.includes("deadlift") || name.includes("marklyft")) {
    return ["hamstrings", "glutes", "lower_back", "upper_back", "forearms"];
  }
  if (name.includes("bench press") || name.includes("bänkpress")) {
    return ["chest", "triceps", "shoulders"];
  }
  if (name.includes("overhead") || name.includes("militärpress")) {
    return ["shoulders", "triceps", "upper_back"];
  }
  if (name.includes("pull up") || name.includes("chins")) {
    return ["upper_back", "biceps", "forearms"];
  }
  if (name.includes("row") || name.includes("rodd")) {
    return ["upper_back", "biceps", "lower_back"];
  }
  ...
  if (name.includes("extension") && primaryMuscle === "triceps") {
    return ["triceps"];
  }
  if (name.includes("extension") && primaryMuscle === "quads") {
    return ["quads", "knees"];
  }
  if (name.includes("raise") && primaryMuscle === "shoulders") {
    return ["shoulders"];
  }
