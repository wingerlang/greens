...
import { ROWING_STANDARDS, SKIERG_STANDARDS, ErgStandard } from '../pages/tools/data/ergStandards.ts';
...
export const getErgLevel = (
    ...
    const standards = type === 'row' ? ROWING_STANDARDS[gender] : SKIERG_STANDARDS[gender];
...
export const analyzeErgPerformance = (
    ...
    const cardioActivities = exerciseEntries.filter(e => {
        ...
        const isTypeMatch = type === 'row'
            ...
            : (entryType === 'cross_country_skiing' && (title.includes('erg') || notes.includes('erg'))); // tough for ski erg, relies on title

        const isKeywordMatch = keywords.some(k => title.includes(k) || notes.includes(k));
    ...
    strengthSessions.forEach(session => {
        ...
        relevantExercises.forEach(ex => {
            ex.sets.forEach((set, setIndex) => {
                ...
                const setSeconds = set.timeSeconds || (set.time ? ErgMath.parseTime(set.time) : 0);
