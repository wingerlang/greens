...
export function getTopSet(
  exercise: StrengthWorkoutExercise,
): StrengthSet | null {
...
function analyzeProgressTrend(
  ...
  const recentAvg =
    history.slice(0, mid).reduce((sum, h) => sum + h.estimated1RM, 0) / mid;
  const olderAvg =
    history.slice(mid).reduce((sum, h) => sum + h.estimated1RM, 0) /
    (history.length - mid);
...
export function getProgressionSuggestion(
  ...
  const suggestedVolume = Math.round(
    suggestedWeight * suggestedReps * lastSession.exercise.sets.length,
  );

  // Calculate progress rate
  const progressRate = history.length >= 2
    ? ((history[0].estimated1RM - history[history.length - 1].estimated1RM) /
      history[history.length - 1].estimated1RM) * 100 / history.length
    : 0;
  ...
  const primaryMessage = isDistance
    ? `${
      formatRelativeDate(lastSession.workout.date)
    }: ${lastDistance}m. Sikte på ${suggestedDistance}m (+2.5%)`
    : `${
      formatRelativeDate(lastSession.workout.date)
    }: ${weight}kg × ${reps}. Prova ${suggestedWeight}kg × ${suggestedReps} idag!`;
  ...
  return {
    ...
    plateauMessage: sessionsSinceProgress >= config.plateauSessionThreshold
      ? `Ingen ökning på ${sessionsSinceProgress} pass. ${
        sessionsSinceProgress >= config.deloadThreshold
          ? "Dags för deload!"
          : "Prova att ändra något."
      }`
      : undefined,
    tips,
  };
...
function countSessionsSinceProgress(
  ...
  for (let i = 0; i < history.length - 1; i++) {
    ...
    const hasProgress = current.weight > previous.weight ||
      ...
      ((current.distance || 0) > 0 &&
        (current.distance || 0) > (previous.distance || 0)); // Distance improvement
...
export function getPlateauWarnings(
  ...
  for (const name of exerciseNames) {
    ...
    if (sessionsSince >= config.plateauSessionThreshold) {
      ...
      const averageWeight = history.reduce((sum, h) => sum + h.weight, 0) /
        history.length;
      ...
      if (sessionsSince >= config.changeExerciseThreshold) {
        recommendation = "change_exercise";
      } else if (sessionsSince >= config.deloadThreshold) {
        recommendation = "deload";
      } else if (sessionsSince >= config.plateauSessionThreshold + 1) {
      ...
      const averageDistance = isDistance
        ? history.reduce((sum, h) => sum + h.distance, 0) / history.length
        : 0;
      const peakDistance = isDistance
        ? Math.max(...history.map((h) => h.distance))
        : 0;
...
export function getWeeklyVolumeRecommendations(
  ...
  const baselinePeriodStart = new Date(
    now.getTime() - weeksToAnalyze * 7 * 24 * 60 * 60 * 1000,
  );
  ...
  for (const [name, data] of exerciseData) {
    ...
    const change = ((recentWeeklyAvg - baselineWeeklyAvg) / baselineWeeklyAvg) *
      100;

    let recommendation: WeeklyVolumeRecommendation["recommendation"] =
      "maintain";
    let message = "";
    ...
    if (change < -30) {
      ...
      message = `Volymen har sjunkit ${
        Math.abs(Math.round(change))
      }% senaste 2v - sikta på ~${targetVolume}kg/vecka`;
    } else if (change > 40) {
      recommendation = "decrease";
      targetVolume = Math.round(baselineWeeklyAvg * 1.15);
      message = `Volymen har ökat ${
        Math.round(change)
      }% - ev. risk för överträning`;
    } else {
      message = `Stabil volym (${change > 0 ? "+" : ""}${
        Math.round(change)
      }% från snittet)`;
    }
...
export function getUnderperformers(
  ...
  for (const [name, stats] of exerciseStats) {
    ...
    if (lastPBDate) {
      daysSinceLastPB = Math.floor(
        (now.getTime() - new Date(lastPBDate).getTime()) /
          (1000 * 60 * 60 * 24),
      );
    ...
    if (setsSinceLastPB >= 50) {
      message =
        `${setsSinceLastPB} set sedan senaste rekord – överväg ny teknik eller variation`;
    } else if (daysSinceLastPB && daysSinceLastPB >= 180) {
      message = `6+ månader sedan senaste rekord trots ${setsSinceLastPB} set`;
    } else {
      message = `${setsSinceLastPB} set utan nytt rekord (${
        Math.round(daysSinceLastPB || 0)
      } dagar)`;
    }
  ...
  return underperformers.sort((a, b) =>
    (b.daysSinceLastPB || 0) - (a.daysSinceLastPB || 0)
  );
}
