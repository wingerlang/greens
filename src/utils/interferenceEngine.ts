...
export type InterferenceRiskLevel =
  | "NONE"
  | "LOW"
  | "MODERATE"
  | "HIGH"
...
export function classifyActivity(
  activity: UniversalActivity | PlannedActivity | ExerciseEntry,
): SignalCategory {
  // Normalization to handle different shapes (Universal vs Planned vs Entry)
  const rawType = (activity as any).type || (activity as any).activityType ||
    (activity as any).performance?.activityType || "";
  const type = rawType.toUpperCase(); // Normalize to uppercase for comparison
  const category =
    ((activity as any).category || (activity as any).plan?.activityCategory ||
      "").toUpperCase();
  const intensity = (activity as any).intensity; // 'low' | 'moderate' | 'high'
  const title = ((activity as any).title || (activity as any).name || "")
    .toUpperCase();
  ...
  if (
    type === "STRENGTH" || category === "STRENGTH" ||
    title.includes("STYRKA") || title.includes("GYM") ||
  ...
  const isCardio = cardioTypes.includes(type) ||
    ["RUN", "BIKE", "EASY", "LONG_RUN", "INTERVALS", "TEMPO", "RECOVERY"]
      .includes(category);
  ...
  if (isCardio) {
    ...
    if (
      ...
      title.includes("INTERVALL") || title.includes("TÄVLING") ||
  ...
  // 4. Neutral
  if (
    type === "REST" || category === "REST" || type === "YOGA" ||
    type === "STRETCHING"
  ) {
    return "NEUTRAL";
  }
...
export function analyzeInterference(activities: any[]): ConflictWarning[] {
  ...
  for (const [date, dailyActs] of Object.entries(activitiesByDate)) {
    ...
    const ampkHighActs = signals.filter((s) =>
      s.type === "AMPK_HIGH" || s.type === "HYBRID"
    );
    ...
    if (mtorActs.length > 0 && ampkHighActs.length > 0) {
      ...
      if (strengthWithTime.length > 0 && cardioWithTime.length > 0) {
        ...
        if (cardioTime < strengthTime) {
          ...
          sequenceWarning =
            " ⚠️ Felaktig ordning upptäckt: Kondition före Styrka minskar styrkeeffekten avsevärt.";
        }
      }
      ...
      warnings.push({
        ...
        scientificExplanation:
          "Att blanda mTOR-signaler (styrka) med höga AMPK-nivåer (intervaller/tempo/långpass) kan hämma muskeltillväxten. AMPK stänger av proteinsyntesen." +
          sequenceWarning,
        involvedActivityIds: [
          ...mtorActs.map((s) => s.id),
          ...ampkHighActs.map((s) => s.id),
        ],
        suggestion:
          "Separera passen med minst 6 timmar. Styrka på morgonen, kondition på kvällen. Om du måste köra nära: Styrka FÖRST.",
      });
    }

    // === Check 2: mTOR + AMPK_LOW (Moderate Risk) ===
    if (
      mtorActs.length > 0 && ampkLowActs.length > 0 && ampkHighActs.length === 0
    ) {
      warnings.push({
        ...
        scientificExplanation:
          "Även lågintensiv kondition aktiverar AMPK i viss mån. Med kort tid mellan passen kan återhämtningen påverkas.",
        involvedActivityIds: [
          ...mtorActs.map((s) => s.id),
          ...ampkLowActs.map((s) => s.id),
        ],
        suggestion:
          "Försök att ha minst 4-6 timmar mellan passen. Håll löpningen kort och lätt.",
      });
    }
    ...
    if (mtorActs.length >= 2) {
      ...
      const sameMuscles = muscleGroups1.some((m: string) =>
        muscleGroups2.includes(m)
      );
      ...
      warnings.push({
        ...
        message: sameMuscles
          ? "Dubbla styrkepass (samma muskler!)"
          : "Dubbla styrkepass",
    ...
    if (ampkHighActs.length >= 2) {
      warnings.push({
        ...
        scientificExplanation:
          "Två högintensiva konditionspass samma dag (intervaller, tempo, långpass) leder snabbt till överträning, stressfrakturer och hormonstörningar.",
        involvedActivityIds: ampkHighActs.map((s) => s.id),
        suggestion:
          "Kör endast ETT hårt pass per dag. Om du dubbeltränar kondition: Morgon HÅRT, Kväll LUGNT.",
      });
    }
    ...
    if (dailyActs.length >= 3) {
      warnings.push({
        ...
        scientificExplanation:
          "Tre träningspass samma dag leder till konstant förhöjda kortisolnivåer, sänkt testosteron och nedsatt immunförsvar. Endast för elitidrottare under mycket specifika förhållanden.",
        involvedActivityIds: dailyActs.map((a) => a.id),
        suggestion:
          "Undvik trippelpass. Risken för nedbrytning är större än chansen till uppbyggnad för de flesta atleter.",
      });
    }
    ...
    if (hybridActs.length > 0 && mtorActs.length > 0) {
      warnings.push({
        ...
        scientificExplanation:
          "Hyrox aktiverar extrema nivåer av både mTOR (styrkedelen) och AMPK (löpningen). Att lägga till ytterligare styrka samma dag överbelastar kroppen totalt.",
        involvedActivityIds: [
          ...hybridActs.map((s) => s.id),
          ...mtorActs.map((s) => s.id),
        ],
        suggestion:
          "Prioritera återhämtning dagen efter Hyrox. Om du måste kombinera, håll styrkan mycket lätt.",
      });
    }
  }
  ...
  for (let i = 0; i < sortedDates.length - 1; i++) {
    ...
    const dayDiff = (tomorrowDate.getTime() - todayDate.getTime()) /
      (1000 * 60 * 60 * 24);
    ...
    const todaySignals = todayActs.map((act) => ({
      type: classifyActivity(act),
      act,
      id: act.id,
    }));
    const tomorrowSignals = tomorrowActs.map((act) => ({
      type: classifyActivity(act),
      act,
      id: act.id,
    }));
    ...
    const todayLegStrength = todaySignals.filter((s) =>
      s.type === "MTOR" &&
      (s.act.muscleGroups?.includes("legs") ||
        s.act.title?.toLowerCase().includes("ben"))
    );
    ...
    if (todayLegStrength.length > 0 && tomorrowMorningCardio.length > 0) {
      warnings.push({
        ...
        scientificExplanation:
          "Efter tung benträning behöver musklerna sova för återhämtning. Tidig morgonlöpning dagen efter avbryter reparationsprocessen och ökar skaderisken.",
        involvedActivityIds: [
          ...todayLegStrength.map((s) => s.id),
          ...tomorrowMorningCardio.map((s) => s.id),
        ],
        suggestion:
          "Flytta löpningen till eftermiddag/kväll, eller gör den mycket lätt (återhämtningsjogg max 30 min).",
      });
    }
  }
