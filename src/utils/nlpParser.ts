...
export type OmniboxIntent =
  ...
  | {
    type: "vitals";
    data: {
      vitalType: "sleep" | "water" | "coffee" | "nocco" | "energy" | "steps";
      amount: number;
      caffeine?: number;
    };
    date?: string;
  ...
  | {
    type: "measurement";
    data: { measurementType?: BodyMeasurementType; value?: number };
    date?: string;
  }
 ...
 */
export function parseTrainingString(
  input: string,
):
  | { type: ExerciseType; duration: number; intensity: ExerciseIntensity }
  | null {
...
export function parseOmniboxInput(input: string): OmniboxIntent {
  ...
  if (
    lower.startsWith("gå till") || lower.startsWith("navigera") ||
    lower.startsWith("/")
  ) {
    let path = "/";
    ...
    } else if (target.includes("vikt") || target.includes("weight")) {
      path = "/health/body";
    } else if (target.includes("sömn") || target.includes("sleep")) {
      path = "/health/body";
    } else if (
      target.includes("mått") || target.includes("body") ||
      target.includes("measurements")
    ) path = "/health/body";
  ...
  const weightMatch = lower.match(
    /(?:vikt\s*[:\s]*|)(\d+(?:[.,]\d+)?)\s*(?:kg|kilo)?$/,
  );
  if (
    weightMatch &&
    (lower.startsWith("vikt") || lower.endsWith("kg") || lower.endsWith("kilo"))
  ) {
...
function parseDate(input: string): { date?: string; remaining: string } {
  ...
  if (input.includes("idag") || input.includes("today")) {
    return {
      ...
      remaining: input.replace(/idag|today/g, "").trim(),
    };
  }
  ...
  if (input.includes("igår") || input.includes("yesterday")) {
    ...
    return {
      date: getISODate(d),
      remaining: input.replace(/igår|yesterday/g, "").trim(),
    };
  }
  ...
  if (input.includes("förrgår") || input.includes("day before yesterday")) {
    ...
    return {
      date: getISODate(d),
      remaining: input.replace(/(i\s*)?förrgår|day before yesterday/g, "")
        .trim(),
    };
  }
  ...
  if (dateMatch) {
    ...
    return {
      date: `${year}-${month}-${day}`,
      remaining: input.replace(dateMatch[0], "").trim(),
    };
  }
...
export function parseCycleString(input: string): SmartCycle | null {
  ...
  else if (lower.includes("balans") || lower.includes("maintain")) {
    goal = "neutral";
  }
...
function parseVitals(input: string): OmniboxIntent | null {
  ...
  const sleepMatch = lower.match(
    /(?:sömn|sova|sov|natt)\s*(\d+(?:[.,]\d+)?)\s*(?:h|t|timmar|tim)?/i,
  ) ||
    lower.match(
      /(\d+(?:[.,]\d+)?)\s*(?:h|t|timmar|tim)?\s*(?:sömn|sova|sov|natt)/i,
    );
  if (sleepMatch) {
    return {
      type: "vitals",
      data: {
        vitalType: "sleep",
        amount: parseFloat(sleepMatch[1].replace(",", ".")),
      },
    };
  }
  ...
  const standaloneSleepMatch = lower.match(
    /^(\d+(?:[.,]\d+)?)\s*(?:h|t|timmar|tim)$/i,
  );
  ...
  const stepsMatch = lower.match(/(\d+)\s*steg/i) ||
    lower.match(/steg\s*(\d+)/i);
  if (stepsMatch) {
    return {
      type: "vitals",
      data: { vitalType: "steps", amount: parseInt(stepsMatch[1]) },
    };
  }

  // Caffeine with custom mg: "105 caf", "200mg koffein", "koffein 150"
  const caffeineMatch =
    lower.match(/(\d+)\s*(?:mg)?\s*(?:caf|koffein|caffeine)/i) ||
    lower.match(/(?:koffein|caffeine)\s*(\d+)\s*(?:mg)?/i);
  if (caffeineMatch) {
    const caffeineMg = parseInt(caffeineMatch[1]);
    return {
      type: "vitals",
      data: { vitalType: "coffee", amount: 1, caffeine: caffeineMg },
    };
  }
  ...
  if (coffeeMatch) {
    ...
    return {
      type: "vitals",
      data: { vitalType: "coffee", amount, caffeine: caffeine * amount },
    };
  }

  // Energy drinks: "nocco" (~180mg), "energidryck" (~80mg)
  const energyMatch = lower.match(
    /(\d+)?\s*(nocco|energydryck|energy|energidryck)/i,
  );
  if (energyMatch) {
    ...
    return {
      type: "vitals",
      data: { vitalType: type, amount, caffeine: caffeine * amount },
    };
  }
...
function parseExercise(input: string): OmniboxIntent | null {
  ...
  const sortedKeywords = Object.keys(typeKeywords).sort((a, b) =>
    b.length - a.length
  );
  ...
  if (
    !type &&
    (input.includes("träning") || input.includes("tränat") ||
      input.includes("pass"))
  ) {
  ...
  if (
    input.includes("lätt") || input.includes("låg") ||
    input.includes("lugnt") || input.includes("lugn") ||
    input.includes("slow") || input.includes("easy")
  ) intensity = "low";
  else if (
    input.includes("hög") || input.includes("hårt") || input.includes("hard") ||
    input.includes("tuff") || input.includes("snabb") ||
    input.includes("snabbt") || input.includes("fast")
  ) intensity = "high";
  else if (input.includes("max") || input.includes("ultra")) {
    intensity = "ultra";
  }
  ...
  if (tonnageMatch) {
    ...
    if (setRepWeightMatch) {
      ...
      if (
        multiTonnageMatch && (type === "strength" || input.includes("lyft"))
      ) {
  ...
  if (hrRangeMatch) {
    ...
    const hrMatch = input.match(/(?:puls|hr|bpm)\s*(\d+)/i) ||
      input.match(/(\d+)\s*bpm/i);
    if (hrMatch) {
  ...
  if (!type) {
    if (
      input.includes("intervall") || input.includes("långpass") ||
      input.includes("lopp") || input.includes("tävling") ||
      input.includes("ultra")
    ) {
      type = "running";
    } else if (tonnage) {
...
function parseFood(input: string): OmniboxIntent | null {
  ...
  const sortedKeywords = Object.keys(mealKeywords).sort((a, b) =>
    b.length - a.length
  );
  ...
  for (const pattern of quantityPatterns) {
    ...
    if (match) {
      ...
      if (rawUnit.startsWith("kg") || rawUnit.startsWith("kilo")) {
        ...
        rawUnit.startsWith("st") || rawUnit.startsWith("pcs") ||
        rawUnit.startsWith("stk")
      ) {
        quantity = rawNum;
        unit = "st";
