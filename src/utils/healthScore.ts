...
export function calculateHealthScore(
  ...
  const validExercises = exercises.filter((e) => new Date(e.date) <= date).sort(
  ...
  const calculateAvgLoad = (days: number) => {
    ...
    const relevantEx = validExercises.filter((e) =>
  ...
  const activeBurn = todaysExercise.reduce(
  ...
  for (let i = 0; i < 7; i++) {
    const d =
      new Date(date.getTime() - (i * 86400000)).toISOString().split("T")[0];
  ...
  const total = Math.round(
    (trainScore * 0.4) + (nutScore * 0.4) + (consScore * 0.2),
  );
