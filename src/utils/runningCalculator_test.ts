...
Deno.test("getPaceZones - VDOT 50", () => {
    ...
    if (zones.easy < zones.threshold) throw new Error("Easy pace should be slower (higher min/km) than Threshold");
    if (zones.threshold < zones.interval) throw new Error("Threshold pace should be slower than Interval");
});
