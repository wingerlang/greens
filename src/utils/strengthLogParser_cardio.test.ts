...
Deno.test("Parses StrengthLog Cardio formats correctly", () => {
    ...
    const row = workout.exercises.find(e => e.exerciseName === "Rowing (Machine)");
    ...
    const cross = workout.exercises.find(e => e.exerciseName === "Cross Trainer (Elliptical)");
    ...
    assertEquals(crossSet?.distance, 4500, "Cross Trainer distance should be 4500m");
    assertEquals(crossSet?.calories, 283, "Cross Trainer calories should be 283");
    assertEquals(crossSet?.time, "00:20:00", "Cross Trainer time should be 00:20:00");

    // 3. Assault Air Bike: 7 sets with km x kcal x time format
    const bike = workout.exercises.find(e => e.exerciseName === "Assault Air Bike");
    ...
    assertEquals(bikeSet1?.time, "00:01:31", "Bike set 1 time should be 00:01:31");

    // 4. Stair Climber: 2.5 x 31 x 00:04:00 (floors/10 x kcal x time)
    const stairs = workout.exercises.find(e => e.exerciseName === "Stair Climber");
    ...
    assertEquals(stairsSet?.time, "00:04:00", "Stair Climber time should be 00:04:00");

    // 5. Machine Chest Fly: 15 x 50, 12 x 57.5, 10 x 65, 4 x 74.5 (reps x weight)
    const fly = workout.exercises.find(e => e.exerciseName === "Machine Chest Fly");
