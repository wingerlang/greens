...
export const CARB_TARGET_PRESETS = [
  ...
  { id: "high", label: "90 g/h", value: 90, desc: "Tränad mage, långt lopp" },
  {
    ...
    label: "120 g/h",
    value: 120,
    desc: "Elite, dubbla transportörer",
  },
] as const;
 ...
 */
export function calculateCaffeineRemaining(
  initialMg: number,
  elapsedMinutes: number,
): number {
  const elapsedHours = elapsedMinutes / 60;
  // Half-life formula: remaining = initial * (0.5)^(t/halfLife)
  const remaining = initialMg *
    Math.pow(0.5, elapsedHours / CAFFEINE_HALF_LIFE_HOURS);
...
export function simulateCaffeine(
  ...
  let currentCaffeine = calculateCaffeineRemaining(
    preRaceMg,
    preRaceMinutesBefore,
  );
  ...
  for (let t = 0; t <= totalRaceMinutes; t += stepMinutes) {
    // Apply decay
    if (t > 0) {
      currentCaffeine = calculateCaffeineRemaining(
        currentCaffeine,
        stepMinutes,
      );
    }
 ...
 */
export function calculateWeatherPenaltyFactor(
  tempC: number,
  humidity: number,
): number {
 ...
 */
export function estimateKcalBurnRate(
  weightKg: number,
  speedKph: number,
): number {
...
export function simulateRace(
  ...
  const sortedEvents = [...intakeEvents].sort((a, b) =>
  ...
  while (currentDist < profile.distanceKm) {
    ...
    while (
      ...
      sortedEvents[eventIndex].distanceKm <= currentDist
    ) {
...
export function calculateDropbagLogistics(
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    const segmentEvents = intakeEvents.filter((e) =>
      e.distanceKm >= startKm && e.distanceKm < endKm
    );
...
export function generateIntakeEvents(options: GenerateIntakeOptions): {
  ...
  const drinkCount = drink.carbsG > 0
    ? (drinkPercent > 0 ? Math.ceil(drinkCarbs / drink.carbsG) : 0)
    : 0;
  ...
  for (let i = 0; i < totalIntakes; i++) {
    ...
    const useGel = gelCount > 0 &&
      (drinkCount === 0 ||
        (gelIndex / gelCount) <= (drinkIndex / (drinkCount || 1)));
    ...
    if (useGel && gelIndex < gelCount) {
      ...
      const useCafGel = includeCaffeine &&
        (!caffeineInLastThird || km >= caffeineStartKm);
  ...
  // Calculate summary
  const totalCarbsG = events.reduce(
    (sum, e) => sum + (e.product?.carbsG || 0) * e.amount,
    0,
  );
  const totalLiquidMl = events.reduce(
    (sum, e) => sum + (e.product?.liquidMl || 0) * e.amount,
    0,
  );
  const totalCaffeineMg = events.reduce(
    (sum, e) => sum + (e.product?.caffeineMg || 0) * e.amount,
    0,
  );
  const intervalMinutes = totalIntakes > 1
    ? targetTimeMinutes / (totalIntakes - 1)
    : 0;
 ...
 */
export function suggestCarbSourceDistribution(
  tempC: number,
): { gelPct: number; drinkPct: number; reason: string } {
  if (tempC >= 25) {
    return {
      gelPct: 30,
      drinkPct: 70,
      reason: "Varmt väder – prioritera vätskerik energi",
    };
  }
  if (tempC >= 20) {
    return {
      gelPct: 50,
      drinkPct: 50,
      reason: "Uppvärmt – balanserad mix rekommenderas",
    };
  }
  if (tempC >= 10) {
    return {
      gelPct: 70,
      drinkPct: 30,
      reason: "Behagligt – gel-fokuserat fungerar bra",
    };
  }
  return {
    gelPct: 80,
    drinkPct: 20,
    reason: "Kallt väder – minimal dryck krävs",
  };
}
...
export function calculateEnergyBreakdown(
  ...
  const carbsIntakeG = intakeEvents.reduce(
    (sum, e) => sum + (e.product?.carbsG || 0) * e.amount,
    0,
  );
