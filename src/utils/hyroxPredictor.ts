import { HyroxActivityStats, HyroxStation } from "../models/types.ts";
...
export const HYROX_STANDARDS: Record<HyroxClass, ClassStandards> = {
  ...
  "DOUBLES_MIXED": {
    ...
    wallBall: 6,
    baseTimeFactor: 0.62,
  }, // Usually Men's weights? No, Sleds are Men's weights, Lunges/Wallballs depend? (Actually Mixed uses Open Men weights mostly but let's approximate)
  "RELAY": {
    sledPush: 152,
    sledPull: 103,
    lunge: 20,
...
export function predictHyroxTime(
  ...
  simulation: {
    runGlobalImprovement?: number;
    stationGlobalImprovement?: number;
  } = {},
): HyroxPrediction {
  ...
  const ratios: { station: HyroxStation; ratio: number }[] =
    (Object.keys(estimatedSplits) as HyroxStation[])
      .filter((k) => k !== "run_1km")
      .map((station) => ({
        station,
