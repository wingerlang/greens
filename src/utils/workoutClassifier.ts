 ...
 */
export function getWorkoutCategoryFromTitle(
  title: string,
...
export function classifyWorkout(
  ...
  for (const exercise of workout.exercises) {
    const muscle = getExerciseMuscleGroup(
      exercise.exerciseName,
      customMappings,
    );
    const volume = exercise.totalVolume ||
      exercise.sets.reduce((sum, s) => sum + s.reps * s.weight, 0);
...
export function classifyWorkouts(
  ...
  return workouts.map((workout) => ({
    ...
    workoutCategory: workout.workoutCategory ||
      classifyWorkout(workout, customMappings),
  }));
}
