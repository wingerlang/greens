import { ExerciseEntry, UserSettings } from "../models/types.ts";
...
export function calculateAdaptiveGoals(
  ...
  const baseCalories = calorieTargetOverride || settings.dailyCalorieGoal ||
    2000;
  ...
  const totalBurned = exercises
    .filter((ex) =>
      !(ex.excludeFromStats || (ex as any).performance?.excludeFromStats)
    )
...
export function calculateTrainingLoad(exercise: ExerciseEntry): number {
  ...
  // 60 * 1.0 * 1.0 * 0.83 = ~50
  return Math.round(
    exercise.durationMinutes * intensityFactor * subTypeFactor * 0.83,
  );
}
...
export function getPerformanceBreakdown(
  ...
  const isRunning = ["running", "run", "walking", "walk", "hiking", "trail"]
  ...
  const isCurrentExcluded = activity.excludeFromStats ||
    activity.performance?.excludeFromStats;
  ...
  if (isRunning) {
    ...
    if (dist === 0 || dur === 0) {
      return {
        ...
        type: "cardio",
        components: [],
        summary: "Ingen data för beräkning.",
    ...
    if (hr === 0) {
      ...
      components.push({
        label: "Tempo (GAP)",
        value: `${Math.floor(gapSec / 60)}:${
          Math.round(gapSec % 60).toString().padStart(2, "0")
        }/km`,
        score: totalScore,
        max: 100,
    ...
    if (dur >= 60) {
      totalScore += 10;
      components.push({
        label: "Uthållighets-boost",
        value: "+10",
        score: 100,
        max: 100,
        description: "Bonus för pass över 60 minuter.",
        icon: "⏱️",
        color: "text-blue-400",
      });
    } else if (dur >= 30) {
      ...
      components.push({
        ...
        score: 50,
        max: 100,
        description: "Bonus för pass över 30 minuter.",
        icon: "⏱️",
        color: "text-blue-400",
      });
    }
      ...
      if (runningHistory.length > 0) {
        ...
        if (dist > maxDist && dist > 2) {
          ...
          components.push({
            ...
            score: 100,
            max: 100,
            description: "Ditt längsta löppass hittills!",
            icon: "🗺️",
        ...
        const similarDistHistory = runningHistory.filter((h) =>
          (h.distance || 0) >= dist * 0.8 && (h.distance || 0) <= dist * 1.2
        );
        if (similarDistHistory.length > 0) {
          const fastestPace = Math.min(
            ...similarDistHistory.map((h) =>
              (h.durationMinutes * 60) / (h.distance || 1)
            ),
          );
          if (paceSec < fastestPace * 0.98) {
            ...
            components.push({
              ...
              score: 100,
              max: 100,
              description: "Ditt snabbaste tempo på denna distans!",
              icon: "💨",
              color: "text-yellow-400",
              isPersonalBest: true,
            });
    ...
    if (isCurrentExcluded) {
      components.push({
        ...
        max: 100,
        description:
          "Detta pass är markerat som felaktigt och räknas ej i statistik/PBs.",
        icon: "⚠️",
        color: "text-red-400",
      });
    }
    ...
    summary = roundedScore > 85
      ? "Exceptionell prestation!"
      : (roundedScore > 65
        ? "Riktigt bra driv i passet."
        : "En stabil insats i banken.");
  ...
  if (isStrength) {
    ...
    if (tonnage === 0 || dur === 0) {
      return {
        ...
        type: "strength",
        components: [],
        summary: "Ingen tonnage-data tillgänglig.",
        isPersonalBest: false,
      };
    }
    ...
    if (dur >= 60) {
      ...
      components.push({
        ...
        score: 100,
        max: 100,
        description: "Bonus för rejäl passlängd.",
        icon: "⏱️",
        color: "text-indigo-400",
      });
    }
    ...
    if (historyBefore.length > 0 && !isCurrentExcluded) {
      const strengthHistory = historyBefore.filter((h) => {
        const t = (h.type || h.activityType || "").toLowerCase();
        return ["strength", "weightlifting", "gym", "styrka"].some((tag) =>
          t.includes(tag)
        );
      });
      ...
      if (strengthHistory.length > 0) {
        const maxTonnage = Math.max(
          ...strengthHistory.map((h) => h.tonnage || 0),
        );
        if (tonnage > maxTonnage && tonnage > 1000) {
          ...
          components.push({
            ...
            score: 100,
            max: 100,
            description: "Ditt tyngsta styrkepass någonsin!",
            icon: "💪",
            color: "text-yellow-400",
            isPersonalBest: true,
          });
        ...
        const maxWorkRate = Math.max(
          ...strengthHistory.map((h) =>
            (h.tonnage || 0) / (h.durationMinutes || 1)
          ),
        );
        if (workRate > maxWorkRate && workRate > 50) {
          ...
          components.push({
            ...
            score: 100,
            max: 100,
            description: "Ditt högsta arbetstempo hittills!",
            icon: "⚡",
            color: "text-yellow-400",
            isPersonalBest: true,
          });
    ...
    const roundedScore = Math.min(100, Math.round(totalScore));
    summary = roundedScore > 85
      ? "Massivt pass! Grym volym."
      : (roundedScore > 65
        ? "Stabilt och intensivt pass."
        : "Bra tempo genom passet.");
