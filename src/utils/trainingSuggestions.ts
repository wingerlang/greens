...
export function getTrainingSuggestions(
  ...
  const getDaysSinceHardRun = () => {
    ...
    return lastHardRun
      ? Math.floor(
        (dateObj.getTime() - new Date(lastHardRun.date).getTime()) / 86400000,
      )
      : 999;
  };
  ...
  if (weeklyStats) {
    const kmGoal = goals.find((g) =>
      g.status === "active" && g.targets?.some((t) => t.unit === "km")
    );
    if (kmGoal) {
      ...
      if (forecastKm > targetKm * 1.1) {
        ...
        suggestions.push({
          ...
          reason: `Du ligger redan 10% över veckomålet (${
            forecastKm.toFixed(1)
          } km vs ${targetKm} km).`,
        ...
        if (remaining > 0 && isLateWeek) {
          ...
          suggestions.push({
            ...
            reason: `Du saknar ${
              remaining.toFixed(1)
            } km för att nå veckomålet (prognos: ${
              forecastKm.toFixed(1)
            }/${targetKm})`,
        ...
        if (
          daysSinceHardRun > 5 &&
          !suggestions.some((s) => s.intensity === "high")
        ) {
          suggestions.push({
            id: generateId(),
            type: "RUN",
            ...
            reason:
              `Du ligger under målet och har inte kört hårt på ${daysSinceHardRun} dagar.`,
            distance: 8,
            duration: 45,
            intensity: "high",
          });
  ...
  const strengthGoal = goals.find((g) =>
    g.status === "active" && g.name.toLowerCase().includes("styrka")
  );
  if (strengthGoal && weeklyStats) {
    const targetSessions = strengthGoal.targets?.find((t) =>
      t.unit?.includes("pass")
    )?.value || 3;
    ...
    if (
      remainingSessions > 0 && remainingSessions >= daysLeft &&
  ...
  if (sameWeekdayHistory.length > 0) {
    ...
    const relevantEntries = sameWeekdayHistory.filter((e) =>
      e.type === mostCommonType
    );
    const avgDuration = Math.round(
      relevantEntries.reduce((sum, e) => sum + e.durationMinutes, 0) /
        relevantEntries.length / 5,
    ) * 5;
    const avgDist = relevantEntries.some((e) => e.distance)
      ? Math.round(
        relevantEntries.reduce((sum, e) => sum + (e.distance || 0), 0) /
          relevantEntries.length * 10,
      ) / 10
      : undefined;

    if (
      mostCommonType === "running" && !suggestions.some((s) => s.type === "RUN")
    ) {
      suggestions.push({
        id: generateId(),
        type: "RUN",
  ...
  if (recentRuns.length >= 3) {
    ...
    for (let i = 0; i < Math.min(2, sortedBuckets.length); i++) {
      ...
      if (
        count >= 2 &&
        !suggestions.some((s) => Math.abs((s.distance || 0) - distVal) < 1)
      ) {
        suggestions.push({
          id: generateId(),
          type: "RUN",
          ...
          reason:
            `Du har sprungit ca ${distVal}km ${count} gånger senaste månaden`,
          distance: distVal,
  ...
  const score = (s: TrainingSuggestion) => {
    ...
    if (s.label.includes("Måljakt") || s.label.includes("Styrkemål")) {
      return 100;
    }
