 ...
 */
export function parseStrengthLogCSV(
  csvContent: string,
  userId: string,
  source: "strengthlog" | "hevy" = "strengthlog",
): ParsedCSV {
  ...
  const isHevyFormat = source === "hevy" ||
    (firstLine.includes('"title"') && firstLine.includes('"start_time"')) ||
    (firstLine.includes("title,") && firstLine.includes("start_time,"));

  // Detect text format: "MM-DD-YYYY HH:mm - HH:mm"
  // Regex: \d{2}-\d{2}-\d{4} \d{2}:\d{2} - \d{2}:\d{2}
  const isTextFormat = /^\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2}\s+-\s+\d{2}:\d{2}/
    .test(lines[0]);
...
function parseStrengthLogTextExport(
  lines: string[],
  userId: string,
): ParsedCSV {
  ...
  for (let i = startIdx; i < lines.length; i++) {
    ...
    if (
      line.startsWith("Sets:") || line.startsWith("Reps:") ||
      line.startsWith("Volume:") || line.startsWith("Average weight:") ||
      line.startsWith("#")
    ) continue;
    ...
    diffSets.forEach((s, idx) => {
      ...
      const isCardioPattern = parts.length >= 3 &&
        parts[parts.length - 1].includes(":");

      if (
        isCardioPattern ||
        (exerciseName.toLowerCase().includes("rowing") && parts.length >= 2)
      ) {
        // Cardio Parsing Mode
        ...
        if (
          lowerName.includes("stair climber") ||
          lowerName.includes("trappmaskin")
        ) {
...
function parseNewStrengthLogFormat(lines: string[], userId: string): ParsedCSV {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      let workoutExercise = workout.exercises.find((we) =>
        we.exerciseId === exerciseDef.id
      );
      if (!workoutExercise) {
        workoutExercise = {
          exerciseId: exerciseDef.id,
...
function parseLine(line: string, ctx: ParserContext) {
  ...
  if (
    values[0]?.startsWith("Exercise,") || values[0]?.startsWith('"Exercise,')
  ) {
...
function parseExerciseSet(values: string[], ctx: ParserContext) {
  ...
  const existingExercise = ctx.currentWorkout.exercises.find((ex) =>
...
function finalizeWorkout(ctx: ParserContext) {
  ...
  for (const exercise of workout.exercises) {
    ...
    for (const set of exercise.sets) {
      ...
      if (
        set.weight > topWeight ||
        (set.weight === topWeight && set.reps > topReps)
      ) {
...
function trackPersonalBest(
  ...
  if (isWeightedDistanceExercise(exercise.name)) {
    ...
    if (!existing) {
      ...
      set.weight === existing.value &&
      (set.distance || 0) > (existing.distance || 0)
    ) {
      isNewPB = true;
    }
  ...
  if (
    isDistanceBasedExercise(exercise.name) && set.calories && set.calories > 0
  ) {
  ...
  if (
    isDistanceBasedExercise(exercise.name) && set.distance && set.distance > 0
  ) {
...
function parseHevyLogFormat(lines: string[], userId: string): ParsedCSV {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      const workoutName = val(idx.title) || val(idx.description) ||
        "Unknown Workout";
      ...
      let workoutExercise = workout.exercises.length > 0
      ...
      workoutExercise = workout.exercises.find((we) =>
        we.exerciseId === exerciseDef.id
      );
      if (!workoutExercise) {
        workoutExercise = {
...
function guessExerciseCategory(name: string): ExerciseCategory {
  ...
  if (
    lower.includes("barbell") || lower.includes("deadlift") ||
  ...
  if (
    lower.includes("machine") || lower.includes("leg press") ||
  ...
  if (
    lower.includes("cable") || lower.includes("pushdown") ||
    lower.includes("fly")
  ) return "cable";
  if (lower.includes("kettlebell")) return "kettlebell";
  if (
    lower.includes("push-up") || lower.includes("pull-up") ||
    lower.includes("dip") ||
  ...
  if (
    lower.includes("rowing") || lower.includes("bike") || lower.includes("run")
  ) return "cardio";
