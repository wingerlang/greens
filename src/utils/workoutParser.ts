import { SegmentType, WorkoutSegment, ParsedWorkout, PaceTarget, RecoveryDefinition } from '../models/analysisTypes.ts';
...
function classifyLine(line: string): SegmentType {
    if (line.match(/(uppv|warm|upp|start|jogg(?!ing)|öppna)/) && !line.includes('vila')) return 'WARMUP';
    // Exclusion: 'ner till' (progressive pace) should not be cooldown
    if (line.match(/(ner|cool|slut|avjogg)/) && !line.includes('ner till')) return 'COOLDOWN';
...
function parsePace(text: string): PaceTarget | undefined {
    ...
    const progMatch = text.match(/(\d{1,2}[.:]\d{2})\s*(?:->|till|to|-|ner till)\s*(\d{1,2}[.:]\d{2})/);
    if (progMatch) {
...
function parseRecovery(line: string, prevSegmentReps: number = 1): RecoveryDefinition | undefined {
    ...
    const listMatch = line.match(/(?:vila|rest|jogg)?\s*:?\s*((?:\d+\s*,?\s*){2,})/);
    if (listMatch) {
        const nums = listMatch[1].split(/[,\s]+/).map(n => parseInt(n.trim())).filter(n => !isNaN(n));
        if (nums.length > 1) {
...
export function parseWorkout(title: string, description: string): ParsedWorkout {
    const raw = `${title}\n${description}`;
    const cleanLines = raw.split('\n').map(normalizeText).filter(l => l.length > 0);
        ...
        if ((dist || time) && !recovery && i + 1 < cleanLines.length) {
            ...
            if (classifyLine(nextLine) === 'REST') {
                ...
                if (nextRec) {
                    ...
                    if (nextRec.type === 'variable_list' && nextRec.variableValues?.length === currentReps) {
                        // Perfect match
                    }
                }
            }
    ...
    segments.forEach(seg => {
        if (seg.reps > 1 && seg.recovery?.type === 'variable_list' && seg.recovery.variableValues?.length === seg.reps) {
            // Expand!
    ...
    if (lowerRaw.includes('långpass') || lowerRaw.includes('long run') || lowerRaw.includes('longrun')) {
    ...
    } else if (lowerRaw.includes('tävling') || lowerRaw.includes('race') || lowerRaw.includes('lopp')) {
