...
export function calculateSpeedProgress(
  ...
  return bestTimeSeconds === Infinity
    ? { value: 0 }
    : { value: bestTimeSeconds, activityId: bestActivityId };
}
...
export function calculateStreak(
  ...
  if (period === "daily") {
    ...
    for (let i = 0; i < activeDates.length - 1; i++) {
      ...
      const diffDays = (curr.getTime() - next.getTime()) /
        (1000 * 60 * 60 * 24);
    ...
    if (
      ...
      (activeDates[0] === today || activeDates[0] === yesterdayStr)
    ) {
      currentStreak = tempStreak;
    }
    bestStreak = Math.max(bestStreak, tempStreak);
    ...
    const thisWeek = `${new Date().getFullYear()}-W${
    ...
    const isActive = activeWeeks[0] === thisWeek ||
      activeWeeks[0] === lastWeekStr;
        ...
        if (item.type === "foodItem") {
          ...
          if (food) {
            ...
            switch (nutritionType) {
              case "calories":
                total += food.calories * multiplier;
                break;
              case "protein":
                total += food.protein * multiplier;
                break;
              case "carbs":
                total += food.carbs * multiplier;
                break;
              case "fat":
          ...
          if (recipe) {
            ...
            recipe.ingredients.forEach((ing) => {
              ...
              if (f) {
                ...
                switch (nutritionType) {
                  case "calories":
                    recipeTotal += f.calories * mult;
                    break;
                  case "protein":
                    recipeTotal += f.protein * mult;
                    break;
                  case "carbs":
                    recipeTotal += f.carbs * mult;
                    break;
                  case "fat":
                    recipeTotal += f.fat * mult;
                    break;
                }
              }
            });
            const perServing = recipe.servings > 0
              ? recipeTotal / recipe.servings
              : recipeTotal;
...
export function assessGoalDifficulty(
  ...
  if (goal.type === "frequency") {
    ...
    const targetPerWeek = goal.period === "weekly"
      ? targetValue
      : goal.period === "monthly"
      ? targetValue / 4
      : targetValue / 12;
    ...
    const totalTonnage = recentEntries.reduce((sum, e) =>
      sum + (e.tonnage || 0), 0) / 1000;
...
export function isGoalOnTrack(
  ...
  if ((goal.period === "once" || goal.type === "weight") && !goal.endDate) {
...
export function calculateGoalProgress(
  ...
  switch (goal.type) {
    ...
    case "streak":
      ...
      break;
    case "nutrition":
      current = calculateNutritionProgress(
        goal,
        mealEntries,
        foodItems,
        recipes,
      ...
      targetValue =
        goal.nutritionMacros?.[target?.nutritionType || "calories"] ||
        target?.value || 1;
      break;
    case "weight":
  ...
  const percentage = (targetValue > 0 && !isNaN(targetValue) && !isNaN(current))
  ...
  return {
    ...
    daysRemaining,
    estimatedCompletionDate: !isComplete
      ? estimateCompletionDate(goal, current, targetValue)
      : undefined,
    periodStart: getGoalPeriodDates(goal).start,
    periodEnd: getGoalPeriodDates(goal).end,
    actualCurrentValue: goal.type === "weight"
      ? latestWeightVal
