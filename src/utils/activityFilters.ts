...
export function parseSmartQuery(
  query: string,
): { filters: SmartFilter[]; remainingText: string } {
  ...
  const distRegex =
    /(~)?\s*(?:([<>])\s*)?(\d+(?:[.,]\d+)?)(?:\s*-\s*(\d+(?:[.,]\d+)?))?\s*(?:km|k|m)?(?:\s*\+-\s*(\d+(?:[.,]\d+)?)\s*(km|k|m)?)?/gi;
  remainingText = remainingText.replace(
    distRegex,
    (match, approx, op, v1, v2, tolerance, tolUnit) => {
      ...
      if (approx || tolerance) {
        ...
        if (tolerance) {
          ...
          if (
            tolUnit?.toLowerCase() === "m" ||
            (tolerance.length >= 3 && !tolUnit && parseInt(tolerance) >= 100)
          ) {
            tolVal = tolVal / 1000;
          }
        }
        ...
        filters.push({
          ...
          value: val1 - tolVal,
          value2: val1 + tolVal,
          label: approx && !tolerance
            ? `~ ${val1} km`
            : `${val1} ± ${
              tolVal < 1 ? Math.round(tolVal * 1000) + "m" : tolVal + "km"
            }`,
          originalQuery: match.trim(),
        });
      } else if (v2) {
  ...
  const paceRegex =
    /(?:([<>])\s*)?(\d{1,2}:\d{2})(?:\s*-\s*(\d{1,2}:\d{2}))?\s*(?:min\/km|\/km|pace|tempo)/gi;
  ...
  const durationRegex =
    /(?:([<>])\s*)?(\d+(?:[.,]\d+)?|(\d+:\d{2}))\s*(h|min|m|tim|t)/gi;
  const parseDuration = (d: string, unit: string) => {
    ...
    if (unit.toLowerCase().startsWith("h") || unit.toLowerCase() === "tim") {
      return val * 60;
    }
    return val;
  };
  ...
  remainingText = remainingText.replace(
    ...
    (match, op, d1, d2, unit) => {
      ...
      if (
        unit === "t" && !d1.includes(":") && parseFloat(d1) > 24
      ) return match;
...
export function applySmartFilters<T extends ExerciseEntry>(
  activities: T[],
  filters: SmartFilter[],
): T[] {
    ...
    return filters.every((f) => {
      switch (f.type) {
        case "distance":
          ...
          if (f.operator === "range") {
            return activity.distance >= f.value &&
              activity.distance <= (f.value2 || f.value);
          }
        ...
        case "tonnage":
          ...
          if (f.operator === "range") {
            return ton >= f.value && ton <= (f.value2 || f.value);
          }
        ...
        case "duration":
          ...
          if (f.operator === "range") {
            return dur >= f.value && dur <= (f.value2 || f.value);
          }
