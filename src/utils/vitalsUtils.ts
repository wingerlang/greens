 ...
 */
export function getRecentVitals(dailyVitals: Record<string, DailyVitals>, days: number = 7): VitalsDay[] {
 ...
 */
export function analyzeSleep(hours: number): { color: string; status: string; description: string } {
    if (hours < 5) return {
        color: 'rose',
        status: 'Bristfällig',
        description: 'För lite sömn kan sänka din förbränning och öka hungerhormoner.'
    };
    if (hours < 8) return {
        color: 'amber',
        status: 'Rimlig',
