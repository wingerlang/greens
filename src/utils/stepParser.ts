...
const COOKING_VERBS = [
    'stek', 'koka', 'grädda', 'ugnsrosta', 'rosta', 'fräs', 'bryn',
    'låt', 'vila', 'sjud', 'puttra', 'micra', 'mikra', 'vispa',
    'knåda', 'baka', 'ångkoka', 'blanchera', 'fritera', 'woka',
    'eftergrädda', 'jäs', 'jäsa', 'mixa', 'koka'
];
...
function extractTimerLabel(text: string, matchIndex: number): string | null {
    ...
    // 2. Tokenize
    const words = currentSentence.split(/\s+/).map(w => w.replace(/[^a-zA-ZåäöÅÄÖ]/g, ''));
    ...
    if (foundVerb) {
        ...
        if (foundVerb.toLowerCase().startsWith('låt')) {
            ...
            for (let j = foundVerbIndex + 1; j < words.length; j++) {
                ...
                const subMatch = COOKING_VERBS.find(v => subWord === v || subWord.startsWith(v));
                if (subMatch) {
        ...
        if (words[foundVerbIndex + 1]) {
            ...
            const stoppers = ['i', 'på', 'med', 'och', 'vid', 'ca', 'tills', 'under', 'mitt', 'slätt', 'fint'];
 ...
 */
export function extractTimer(text: string): { minutes: number; label: string; isManual?: boolean } | null {
    for (const pattern of TIMER_PATTERNS) {
        ...
        if (match && match[1]) {
            ...
            if (minutes > 0 && minutes <= 180) { // Sanity check: max 3 hours
                ...
                const contextLabel = extractTimerLabel(text, match.index || 0) || `${minutes} min`;
...
export function parseIngredientLine(line: string): MatchedIngredient | null {
    ...
    const match = trimmed.match(/^([\d,./]+)?\s*(dl|l|msk|tsk|g|kg|st|ml|cl|port|portion(?:er)?)?\s*(.+)$/i);
