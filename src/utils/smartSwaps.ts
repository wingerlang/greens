...
export function findSmartSwaps(
  ...
  ingredients.forEach((ing) => {
    ...
    // Skip logic...
    if (
      ["spices", "condiments", "other", "baking"].includes(
    ...
    candidates.forEach((candidate) => {
      ...
      const priceDiff = (originalItem.pricePerUnit || 0) -
        (candidate.pricePerUnit || 0);
      const priceSavingPct =
        (originalItem.pricePerUnit && originalItem.pricePerUnit > 0)
          ? priceDiff / originalItem.pricePerUnit
          : 0;

      const co2Diff = (originalItem.co2PerUnit || 0) -
        (candidate.co2PerUnit || 0);
      const co2SavingPct =
        (originalItem.co2PerUnit && originalItem.co2PerUnit > 0)
    ...
    if (bestCandidate) {
      suggestions.push({
        ...
        reason,
        savingsFn: (amount) => {
          const saved = ((originalItem.pricePerUnit || 0) -
            (bestCandidate!.pricePerUnit || 0)) * (amount / 1000);
          return `${Math.round(saved)} kr`;
        },
        impactFn: (amount) => {
          const saved = ((originalItem.co2PerUnit || 0) -
            (bestCandidate!.co2PerUnit || 0)) * (amount / 1000);
