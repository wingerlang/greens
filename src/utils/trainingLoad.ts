import { UniversalActivity, TrainingLoadData } from '../models/types.ts';
...
export function calculateTSS(activity: UniversalActivity): number {
    ...
    if (perf.avgHeartRate) {
        ...
        return (perf.durationMinutes / 60) * (intensityFactor * intensityFactor) * 100;
    }
 ...
 */
export function calculateTrainingLoad(activities: UniversalActivity[], days = 90): TrainingLoadData[] {
    // 1. Sort activities chronologically
    const sorted = [...activities].sort((a, b) => a.date.localeCompare(b.date));

    // 2. Map to daily load
    const dailyMap = new Map<string, { tss: number, distance: number, duration: number }>();

    sorted.forEach(a => {
        if (!a.performance) return;
        const current = dailyMap.get(a.date) || { tss: 0, distance: 0, duration: 0 };
    ...
    for (let i = 0; i <= days; i++) {
        ...
        const dayData = dailyMap.get(dateStr) || { tss: 0, distance: 0, duration: 0 };
