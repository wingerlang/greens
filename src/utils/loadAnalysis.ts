...
import {
  ...
  SetAnalysis,
  WeeklyLoadData,
} from "../models/loadAnalysisTypes.ts";
...
export function calculateEffectiveLoad(
  ...
  const sortedSessions = [...sessions].sort((a, b) =>
    new Date(a.date).getTime() - b.date.getTime()
  );
  ...
  const getReferenceMax = (
    ...
    if (lastActivity) {
      const daysSinceLast = (currentDate - lastActivity.date) /
        (1000 * 60 * 60 * 24);
  ...
  for (const session of sortedSessions) {
    ...
    const weekKey = getWeekStartDate(new Date(session.date)) +
      ` (v.${getWeekNumber(new Date(session.date))})`;
    ...
    for (const exercise of session.exercises) {
      ...
      for (const set of exercise.sets) {
        ...
        const isInRange =
          (!filters.startDate || session.date >= filters.startDate) &&
  ...
  const results: WeeklyLoadData[] = Array.from(weeklyMap.entries()).map(
    ([dateStr, data]) => {
      ...
      return {
        ...
        averageE1RM: data.e1RMCount > 0
          ? Math.round((data.e1RMSum / data.e1RMCount) * 10) / 10
          : 0,
...
export function generateLoadInsights(data: WeeklyLoadData[]): LoadInsight {
  ...
  // Averages
  const avgEffectiveSets = recent.reduce((sum, w) => sum + w.effectiveSets, 0) /
    recent.length;
  const avgStrength = recent.reduce((sum, w) => sum + w.maxE1RM, 0) /
    recent.length; // Use Peak Strength as proxy for capability? Or Avg? Spec says "Trend"
  ...
  if (strengthTrend > 1.0) { // Tolerance
    return {
      ...
      title: "Bra jobbat!",
      message:
        "Din nuvarande volym ger resultat. Du blir starkare vecka för vecka.",
  ...
  if (avgEffectiveSets > 20 && strengthTrend <= 1.0) {
    return {
      ...
      message:
        "Du tränar mycket men styrkan ökar inte. Överväg att sänka volymen för mer återhämtning.",
  ...
  if (avgEffectiveSets < 3) {
    return {
      ...
      message:
        "Din volym är låg. Öka antalet tuffa set för att driva utveckling.",
      color: "text-blue-400",
    };
  }
  ...
  if (avgEffectiveSets > 15 && strengthTrend <= 1.0) {
    return {
      ...
      message:
        "Du gör många set, men styrkan står stilla. Försök höja vikten och göra färre set.",
  ...
  return {
    ...
    message:
      "Du håller igång träningen. Fortsätt utmana dig själv för att se tydligare resultat.",
