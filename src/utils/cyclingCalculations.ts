...
import { StrengthSet, StrengthWorkout } from "../models/strengthTypes.ts";
import {
  ASSAULT_BIKE_STANDARDS,
  CYCLING_POWER_PROFILE,
} from "../pages/tools/data/cyclingStandards.ts";

// ============================================
// Constants & Types
// ============================================

export type AssaultInterval =
  | "10s"
  | "30s"
  | "1m"
  | "90s"
...
export const ASSAULT_BIKE_INTERVALS: {
  label: string;
  key: AssaultInterval;
  type: "time" | "distance";
  target: number;
...
export const analyzeAssaultBikePerformance = (
  ...
  cardioActivities.forEach((activity) => {
    ...
    ASSAULT_BIKE_INTERVALS.filter((i) => i.type === "time").forEach(
      (interval) => {
  ...
  strengthSessions.forEach((session) => {
    ...
    relevantExercises.forEach((ex) => {
      ex.sets.forEach((set, setIndex) => {
        ...
        if (setDistMeters > 0) {
          ASSAULT_BIKE_INTERVALS.filter((i) => i.type === "distance").forEach(
            (interval) => {
              ...
              if (Math.abs(setDistMeters - interval.target) < 10) {
                ...
                if (setSeconds > 0) {
                  ...
                  const isFaster = !currentBest ||
                    (setSeconds < (currentBest.durationMinutes * 60));
                  ...
                  if (isFaster) {
                    results[interval.key] = {
                      ...
                      sourceId: session.id,
                      description:
                        `${session.name} (Set ${set.setNumber}) - ${setDistMeters}m @ ${
 ...
 */
export const extractFtpFromHistory = (
  ...
  const cyclingActivities = eligibleEntries.filter((e) =>
    (e.type === "cycling" || e.title?.toLowerCase().includes("zwift") ||
      e.title?.toLowerCase().includes("wattbike")) &&
    e.averageWatts &&
...
export const getBest20MinPower = (
  entries: ExerciseEntry[],
): { watts: number; date: string } | null => {
